...
| F3-COUNT | Klarner BFS-tree lattice-animal count: count(n) <= C * K^n for d=4 | CONDITIONAL_BRIDGE | Recursive deletion / full word decoder still incomplete; v2.54 proves the Mathlib-backed unrooted non-cut deletion step, and v2.55 proves the root-avoiding safe-deletion base case for `k = 2`, but the global root-avoiding safe-deletion theorem needed by `PlaquetteGraphAnchoredSafeDeletionExists` is still open for `k >= 3` | LatticeAnimalCount.lean v2.48.0 parent selector, v2.50.0 first-deletion/residual primitive, v2.51.0 conditional recursive-deletion handoff (commit `d76b672`), v2.52.0 degree-one leaf deletion subcase (commit `343bfd8`), v2.53.0 safe-deletion hypothesis/driver, v2.54.0 unrooted non-cut deletion, and v2.55.0 card-two safe deletion (commit `027bf7d`): `plaquetteGraphPreconnectedSubsetsAnchoredCard_exists_erase_mem_of_card_two`, `physicalPlaquetteGraphPreconnectedSubsetsAnchoredCard_exists_erase_mem_of_card_two` | Continue `CLAY-F3-COUNT-RECURSIVE-001`: prove the root-avoiding non-cut/safe-deletion strengthening for `k >= 3`, package it as `PlaquetteGraphAnchoredSafeDeletionExists`, then iterate into the full anchored word decoder |
