...
| F3-COUNT | Klarner BFS-tree lattice-animal count: count(n) <= C * K^n for d=4 | CONDITIONAL_BRIDGE | Recursive deletion / full word decoder still incomplete; existence of a deletion preserving residual preconnectedness is not yet proved | LatticeAnimalCount.lean v2.48.0 parent selector, v2.50.0 first-deletion/residual primitive, and v2.51.0 conditional recursive-deletion handoff (commit `d76b672`): `plaquetteGraphPreconnectedSubsetsAnchoredCard_erase_mem_of_preconnected` and `physicalPlaquetteGraphPreconnectedSubsetsAnchoredCard_firstDeleteResidual1296_mem_of_preconnected` | Continue `CLAY-F3-COUNT-RECURSIVE-001`: prove a leaf/deletion-order theorem that supplies the preconnected residual hypothesis, then iterate into full anchored word decoder |
