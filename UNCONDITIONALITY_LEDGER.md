...
| F3-COUNT | Klarner BFS-tree lattice-animal count: count(n) <= C * K^n for d=4 | CONDITIONAL_BRIDGE | Recursive deletion / full word decoder still incomplete; v2.54 proves the Mathlib-backed unrooted non-cut deletion step, v2.55 proves the root-avoiding safe-deletion base case for `k = 2`, and v2.56 proves safe deletion is equivalent to the exact non-root non-cut formulation; the global non-root non-cut theorem remains open for `k >= 3` | LatticeAnimalCount.lean v2.48.0 parent selector, v2.50.0 first-deletion/residual primitive, v2.51.0 conditional recursive-deletion handoff (commit `d76b672`), v2.52.0 degree-one leaf deletion subcase (commit `343bfd8`), v2.53.0 safe-deletion hypothesis/driver, v2.54.0 unrooted non-cut deletion, v2.55.0 card-two safe deletion (implementation commit `2ea7a2a`), and v2.56.0 bridge: `PlaquetteGraphAnchoredNonRootNonCutExists`, `plaquetteGraphAnchoredSafeDeletionExists_iff_nonRootNonCutExists`, physical specialization | Continue `CLAY-F3-COUNT-RECURSIVE-001`: prove `PlaquetteGraphAnchoredNonRootNonCutExists` for `k >= 3`, derive `PlaquetteGraphAnchoredSafeDeletionExists` via v2.56, then iterate into the full anchored word decoder |
