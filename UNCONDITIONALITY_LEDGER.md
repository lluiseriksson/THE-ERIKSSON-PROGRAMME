...
| F3-COUNT | Klarner BFS-tree lattice-animal count: count(n) <= C * K^n for d=4 | CONDITIONAL_BRIDGE | Recursive deletion / full word decoder still incomplete; v2.54 proves the Mathlib-backed unrooted non-cut deletion step, v2.55 proves the root-avoiding safe-deletion base case for `k = 2`, v2.56 proves safe deletion is equivalent to the exact non-root non-cut formulation, v2.57 proves a global two-non-cut theorem would imply the non-root non-cut/safe-deletion gap, v2.58 proves the root-avoiding safe-deletion base case for `k = 3`, v2.59 packages the already-proved base zone `2 ≤ k ≤ 3`, and v2.60 splits the remaining target to `4 ≤ k` via `PlaquetteGraphAnchoredHighCardTwoNonCutExists`; the high-cardinality two-non-cut or direct non-root non-cut theorem remains open | LatticeAnimalCount.lean v2.48.0 parent selector, v2.50.0 first-deletion/residual primitive, v2.51.0 conditional recursive-deletion handoff (commit `d76b672`), v2.52.0 degree-one leaf deletion subcase (commit `343bfd8`), v2.53.0 safe-deletion hypothesis/driver, v2.54.0 unrooted non-cut deletion, v2.55.0 card-two safe deletion (implementation commit `2ea7a2a`), v2.56.0 bridge (commit `3a90ebc`), v2.57.0 bridge (commit `0d2ebc9`), v2.58.0 card-three safe deletion (commit `2233f40`), v2.59.0 base-zone packaging (commit `e17f316`), and v2.60.0 high-cardinality bridge (commit `<TBD-after-commit>`): `plaquetteGraphAnchoredSafeDeletionExists_of_highCardTwoNonCutExists`, physical specialization | Continue `CLAY-F3-COUNT-RECURSIVE-001`: prove `PlaquetteGraphAnchoredHighCardTwoNonCutExists` for `4 ≤ k` (or prove the matching high-card non-root non-cut statement directly), derive `PlaquetteGraphAnchoredSafeDeletionExists`, then iterate into the full anchored word decoder |
