...
| F3-COUNT | Klarner BFS-tree lattice-animal count: count(n) <= C * K^n for d=4 | CONDITIONAL_BRIDGE | Recursive deletion / full word decoder still incomplete; v2.54 proves the Mathlib-backed unrooted non-cut deletion step, v2.55 proves the root-avoiding safe-deletion base case for `k = 2`, v2.56 proves safe deletion is equivalent to the exact non-root non-cut formulation, and v2.57 proves a global two-non-cut theorem would imply the non-root non-cut/safe-deletion gap; the global two-non-cut or non-root non-cut theorem remains open | LatticeAnimalCount.lean v2.48.0 parent selector, v2.50.0 first-deletion/residual primitive, v2.51.0 conditional recursive-deletion handoff (commit `d76b672`), v2.52.0 degree-one leaf deletion subcase (commit `343bfd8`), v2.53.0 safe-deletion hypothesis/driver, v2.54.0 unrooted non-cut deletion, v2.55.0 card-two safe deletion (implementation commit `2ea7a2a`), v2.56.0 bridge (commit `3a90ebc`), and v2.57.0 bridge (commit `<TBD-after-commit>`): `PlaquetteGraphAnchoredTwoNonCutExists`, `plaquetteGraphAnchoredNonRootNonCutExists_of_twoNonCutExists`, physical specialization | Continue `CLAY-F3-COUNT-RECURSIVE-001`: prove `PlaquetteGraphAnchoredTwoNonCutExists` globally (or prove `PlaquetteGraphAnchoredNonRootNonCutExists` directly), derive `PlaquetteGraphAnchoredSafeDeletionExists` via v2.56/v2.57, then iterate into the full anchored word decoder |
