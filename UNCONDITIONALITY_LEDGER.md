...
| F3-COUNT | Klarner BFS-tree lattice-animal count: count(n) <= C * K^n for d=4 | CONDITIONAL_BRIDGE | Recursive deletion / full word decoder still incomplete | LatticeAnimalCount.lean v2.48.0 parent selector plus v2.50.0 first-deletion primitive: `physicalPlaquetteGraphPreconnectedSubsetsAnchoredCard_firstDeleteCode1296_spec`, `...firstDelete1296_mem_erase_root` | Continue `CLAY-F3-COUNT-RECURSIVE-001`: iterate first-delete/root-shell parent selectors into residual deletion and full anchored word decoder |
