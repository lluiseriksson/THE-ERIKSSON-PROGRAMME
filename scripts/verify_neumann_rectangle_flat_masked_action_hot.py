...
ROOT = '/content/hrpoly-neumann-directional-masks-promoted-cold-v1'
RUNNER_HASH = '63c74f2d84af465e3017ed7ac2d1d3e1ac56b29cb5de62528153f787f26ead19'
...
def verify(files):
    ...
    require(data['scope'] == "literal flat rectangular Laplacian with directional full-period masks; not integer boundary identification, physical carrier choice, reflection covariance, regional inverse, uniform B0 or window15", 'PHYSICAL_SCOPE')
    ...
    require(review['cold']['production_outputs']['NeumannRectangleDirectionalMasks.olean'] == runner.PARENT_OLEAN_HASH, 'PARENT_OUTPUT_REVIEW')
    ...
    commands['physical_prerequisites'] = ["lake","build","YangMills.RG.NeumannRectangleDirectionalMasks","YangMills.RG.NeumannFlatInternalBondAction"]
    for s in ('clean_before', 'clean_after'):
        commands[s] = ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'lean-toolchain', 'lake-manifest.json']
    for s, name in [('physical_draft', 'NeumannRectangleFlatMaskedActionDraft.lean')]:
    ...
    for s, path in [('physical_draft', 'tmp/NeumannRectangleFlatMaskedActionDraft.lean')]:
