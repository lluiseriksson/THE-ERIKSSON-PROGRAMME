"""Pre-registered four-core/eight-annulus endpoint probe at delta=0.008."""
...
def main():
    ...
    print("R4 008 ENDPOINT SPLIT PROBE", "core_boxes", CORE_BOXES,
    ...
    for split in PHYSICAL_SPLITS:
        ...
        print("SPLIT", split, "TWO-ENDPOINT-PASS" if passed else "FAIL",
              flush=True)
        if passed:
            print("R4 008 ENDPOINT SPLIT DESIGN PASS", split,
                  "EXHAUSTIVE COVER REQUIRED", flush=True)
            return 0
    print("R4 008 ENDPOINT SPLIT DESIGN FAIL", flush=True)
    return 1
