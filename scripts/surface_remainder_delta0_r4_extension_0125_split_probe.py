...
CORE_BOXES = (
    ...
    (Fraction(11, 1000), Fraction(3, 250)),
    (Fraction(3, 250), DELTA_MAX),
)
ANNULUS_BOXES = tuple(
    (Fraction(j, 1000), Fraction(j+1, 1000)) for j in range(12)
) + ((Fraction(3, 250), DELTA_MAX),)
PHYSICAL_SPLITS = (
...
def main():
    ...
    for split in splits:
        if run_split(split, boxes):
            ...
            return 0
    print("R4 0125 SPLIT DESIGN FAIL", flush=True)
    return 1
