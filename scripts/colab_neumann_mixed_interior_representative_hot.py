#!/usr/bin/env python3
"""Bounded interior representative diagnostic; not an operator/image interchange."""
...
BASE = '3391796fbcd65fa8f4453f5e482b36bc19f455c6'
ROOT = Path('/content/hrpoly-neumann-physical-readout-integral-promoted-cold-v1')
OUT = Path('/content/neumann-mixed-interior-representative-hot-v1')
...
def main():
    ...
    if not Path('/content/physical-readout-integral-cold-evidence-preserved.json').is_file():
    ...
    ack = json.loads(Path('/content/physical-readout-integral-cold-evidence-preserved.json').read_text())
