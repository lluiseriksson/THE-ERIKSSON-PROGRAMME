#!/usr/bin/env python3
"""Exact five-name mixed-coordinate cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
SOURCE = 'fc95d9151248b428474028e2bd4a603a959f2bc9'
REV = 'neumann-mixed-coordinate-promoted-cold-v1'
...
BLOBS = {
    "YangMills/RG/NeumannMixedCoordinateCoverage.lean": "ebbb0a2966b5aa419b9ed12db5665ec469ab7a76594b9889488a47749013a7b1",
    "YangMills/RG/NeumannMixedCoordinateCoverageAudit.lean": "0eb6152428855c391317bca43ffdfcbc85e878a74db0a1485787075e313f0a7d"
}
NAMES = {
    "mixed_audit": [
        "YangMills.RG.neumannMixedCoordinateImage_bijective",
        "YangMills.RG.neumannMixedCoordinateEquiv",
        "YangMills.RG.neumannMixedCoordinateFamilyEquiv",
        "YangMills.RG.neumannMixedCoordinateFamilyEquiv_apply",
        "YangMills.RG.neumannMixedCoordinateFamily_bijective"
    ]
}
...
COMMANDS = {
    ...
    "mixed_audit": [
        ...
        "env",
        "lean",
        "YangMills/RG/NeumannMixedCoordinateCoverageAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-mixed-coordinate-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
            parent_diagnostics=[{"source":"4d6cd7ea7880feafe5b1161fa3ab2d1822334323","archive_sha256":"f57f887f3c6cae43fc9f4ddb97de97c9b9718d8a904ddece1d7eb5298caa45bd"}],
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="mixed coordinate image bijection with single FULL branch and PROPER reflection pair; not physical carrier classification, physical inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-mixed-coordinate-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannMixedCoordinateCoverage.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-mixed-coordinate-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"4d6cd7ea7880feafe5b1161fa3ab2d1822334323","archive_sha256":"f57f887f3c6cae43fc9f4ddb97de97c9b9718d8a904ddece1d7eb5298caa45bd"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="mixed coordinate image bijection with single FULL branch and PROPER reflection pair; not physical carrier classification, physical inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    for name in ["NeumannMixedCoordinateCoverage.olean"]:
    ...
    print('NEUMANN_MIXED_COORDINATE_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_MIXED_COORDINATE_COLD_EVIDENCE_VERIFIED')
