...
def validate(path: Path, lo: Fraction, hi: Fraction) -> int:
    ...
    # The extension wrapper records its own dependency before the contract
    # header; accept that explicit provenance preamble while requiring the
    # terminal header to be present exactly once.
    assert lines.count("SCALED BULK SIGN ROW MID TERMINAL ORDER24") == 1
