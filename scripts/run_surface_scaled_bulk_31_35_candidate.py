"""Run or replay the four frozen scaled-bulk [31,35] units."""
...
import certify_surface_scaled_bulk_31_35_candidate as cert
...
def run(unit: str, suffix: str = "") -> None:
    ...
    result = subprocess.run(
        [sys.executable, str(ROOT / "scripts" /
         "certify_surface_scaled_bulk_31_35_candidate.py"),
...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = [pool.submit(run, unit, suffix) for unit in cert.SEGMENTS]
        for future in as_completed(futures):
            future.result()
    print("SCALED BULK [31,35]", "REPLAY" if args.replay else "PRODUCTION",
          "COMPLETE", len(cert.SEGMENTS))
    return 0
