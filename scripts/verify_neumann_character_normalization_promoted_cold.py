#!/usr/bin/env python3
"""Exact five-name character normalization cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
REV = 'neumann-character-normalization-promoted-cold-v1'
...
COMMANDS = {
    "haar_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannTorusCharacterIntegral"
    ],
    "haar_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannTorusCharacterIntegralAudit.lean"
    ],
    "alias_prerequisites": [
        "lake",
        "build",
    ...
    "alias_audit": [
        ...
        "env",
        "lean",
        "YangMills/RG/NeumannCenteredAliasCharacterAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-character-normalization-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="normalized Haar character integral and literal centered-alias character identities; not physical source equation, regional inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-character-normalization-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannTorusCharacterIntegral.olean", "NeumannCenteredAliasCharacter.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-character-normalization-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"a5811d5ddd78faf6fe638aa00db30276af826baf","archive_sha256":"d80bf1b479adfafb568d43335c229c98b060aab77b7d26cadfbabbed5c9e83de","source_blobs":{"tmp/NeumannTorusCharacterIntegralRepro.lean":"78e060fc4b296ca2aa14b79281850e6f0921fd17df183f24b809d18754c64317","tmp/NeumannCenteredAliasCharacterDraft.lean":"33e22d4852cbdf17f2d44445528fef1afbd2106f9b63c74d6ca10c867a926b53"}}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="normalized Haar character integral and literal centered-alias character identities; not physical source equation, regional inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    ...
    print('NEUMANN_CHARACTER_NORMALIZATION_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_CHARACTER_NORMALIZATION_COLD_EVIDENCE_VERIFIED')
