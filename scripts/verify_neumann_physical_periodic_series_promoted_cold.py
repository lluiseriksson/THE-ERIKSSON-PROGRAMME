#!/usr/bin/env python3
"""Exact five-name physical periodic-series cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
REV = 'neumann-physical-periodic-series-promoted-cold-v1'
...
COMMANDS = {
    ...
    "series_audit": [
        ...
        "env",
        "lean",
        "YangMills/RG/NeumannPhysicalPeriodicSeriesAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-physical-periodic-series-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="literal single-branch periodic source series and convergence under existing physical decay windows; not mixed coverage, uniform sum budget, regional inverse, B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-physical-periodic-series-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannPhysicalPeriodicSeries.olean", "NeumannPhysicalPeriodicSummability.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-physical-periodic-series-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"0bdb056bad23b77adda9d6b13a4917386836e174","archive_sha256":"7bf052a0e1117bb525e4633dc0fe07eee918ed1a660f208b22c2ef313e33209e"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="literal single-branch periodic source series and convergence under existing physical decay windows; not mixed coverage, uniform sum budget, regional inverse, B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    ...
    print('NEUMANN_PHYSICAL_PERIODIC_SERIES_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_PHYSICAL_PERIODIC_SERIES_COLD_EVIDENCE_VERIFIED')
