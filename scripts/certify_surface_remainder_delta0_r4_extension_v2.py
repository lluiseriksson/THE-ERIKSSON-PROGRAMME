"""Authoritative corrected-outer regular-K2 cover on delta [0,1/200]."""
...
import surface_remainder_delta0_r4_extension_v2_probe as probe


ROOT = Path(__file__).resolve().parents[1]
DEPENDENCIES = (
    "scripts/certify_surface_remainder_delta0_r4_extension_v2.py",
    "scripts/surface_remainder_delta0_r4_extension_v2_probe.py",
    "scripts/surface_remainder_delta0_r4_extension_v2_cover.py",
...
GRID_RANGES = ((0, 149, 96), (150, 157, 192))
...
def grid_for(index):
    ...
    if len(matches) != 1:
        raise ValueError("v2 production grid map is not a partition")
    return matches[0]
...
def main():
    ...
    print("CONFIG delta_max 1/200 outer_domain v2 start",
          args.start_index, "stop", args.stop_index,
          "grid_ranges", GRID_RANGES, flush=True)
    worst = None
    for index in range(args.start_index, args.stop_index):
        lo, hi = boxes[index]
        grid = grid_for(index)
        radius, flat, retained, c4, value, margin = probe.judge(
        ...
        if radius != 14 or not lower > 0:
            print("CERTIFICATE FAIL index", index, flush=True)
            return 1
        if worst is None or lower < worst[0]:
            worst = (lower, index)
    print("CERTIFIED SEGMENT regular K2 r4 v2 delta [0,1/200] start",
