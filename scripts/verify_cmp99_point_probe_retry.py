...
PASS proves the two point-probe draft checks only, never a promoted physical cold seal.
...
def verify(files, spec, old, gate):
    ...
    contract = json.loads(files['point-probe-retry-contract.json'])
    for field, value in dict(source=spec.SOURCE, revision=spec.REV,
        ...
        prior_fail_archive='cea5571fea96a7ce138184cffddbca13dc724a0a1441de06f5f242ab3b79e01a').items():
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'preflight.json',
                      'point-probe-retry-contract.json'}
    indexed = {}
    ...
    if status == 'PASS' or axioms:
        require('physical_point_probe_draft' in indexed and
            indexed['physical_point_probe_draft']['exit'] == 0, 'AXIOM_STAGE')
        actual = gate.exact_axioms(files['physical_point_probe_draft.log'].decode(), spec.NAMES)
...
def self_test(spec, old, gate):
    ...
    for stage in BOOT + list(spec.COMMANDS):
        ...
        if stage == 'physical_point_probe_draft':
    ...
    contract = dict(source=spec.SOURCE,revision=spec.REV,cold_seal=False,
        ...
        actual_axioms=gate.exact_axioms(files['physical_point_probe_draft.log'].decode(),spec.NAMES),
    ...
    files['point-probe-retry-contract.json'] = json.dumps(contract).encode()
    ...
    for word in ('sorryAx','ofReduceBool','Other.axiom'):
        ...
        s='physical_point_probe_draft'; bad[s+'.log']=bad[s+'.log'].replace(b'Quot.sound',word.encode())
    ...
    keep={'evidence.json','preflight.json','gate-contract.json','point-probe-retry-contract.json'}
    for r in d['records']:
    ...
    fail['point-probe-retry-contract.json']=json.dumps(c).encode()
    verify(fail,spec,old,gate)
    print('POINT_PROBE_RETRY_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=7 fail_prefix=1')
...
def main():
    ...
    spec=load(args.helpers,'colab_cmp99_point_probe_retry.py')
    ...
    print('POINT_PROBE_RETRY_PACKAGE_VERIFIED COLD_SEAL=0')
