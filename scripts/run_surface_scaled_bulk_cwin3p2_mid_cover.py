"""Production/replay driver for the fixed mid-order beta cover.

The rational partition and Taylor settings are frozen in the paired
preregistration document.  A failed unit stops the cover and no manifest is
...
CWIN = Fraction(3, 2)
PREREG = "docs/SURFACE-G2-CWIN3P2-MID-COVER-48P25-69-PREREG-20260722.md"
...
def cover_rows(box, t_lo: Fraction, t_hi: Fraction):
    ...
    while stack:
        ...
        if upper < 0:
            ...
            continue
        if x2 - x1 <= MIN_DT:
            raise RuntimeError(f"mid-order cover failure near t={float(x1)}")
        mid = (x1 + x2) / 2
...
def transcript(index, lo, hi, rows, cache_entries):
    ...
    lines = [
        "SCALED BULK SIGN ROW UNIT CWIN3P2 MID COVER",
    ...
    lines.extend([
        ...
        "SCOPE quarantined mid-order cover unit only; no G2/G6 promotion",
    ])
    return label, "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    for index, lo, hi in units():
        ...
        box = scaled.bulk.BetaTaylorBox(
            lo, hi, prec=PREC, order=ORDER, t_order=T_ORDER
        )
        t_hi = scaled.bulk.PI_UP - CWIN / hi
        rows = cover_rows(box, Fraction(3, 5), t_hi)
        name, text = transcript(index, lo, hi, rows,
                               cache.cache_info().currsize)
        suffix = "_rerun" if replay else ""
        (ROOT / "scripts" / f"surface_scaled_bulk_{name}{suffix}.txt").write_text(
            text, encoding="utf-8"
        )
        print("PASS", name, "rows", len(rows), flush=True)
    print("CWIN3P2 MID COVER", "REPLAY" if replay else "PRODUCTION", "PASS")
    return 0
