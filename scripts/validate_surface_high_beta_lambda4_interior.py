"""Validate the committed lambda-four interior production/replay pair."""
...
import certify_surface_high_beta_lambda4_interior as cert
...
PRODUCTION = (
    ROOT / "scripts"
    / "surface_high_beta_lambda4_interior_production_20260728.txt"
)
REPLAY = (
    ROOT / "scripts"
    / "surface_high_beta_lambda4_interior_replay_20260728.txt"
)
SOURCE_HEAD = "56bab787526e2b3a0d2b6c9b1ce059465bf1c8b0"
...
def validate() -> dict[str, object]:
    ...
    if "HIGH-BETA LAMBDA4 INTERIOR ABSOLUTE-MOMENT PASS" not in lines:
    ...
    if not arb(rho.upper()) < arb(1) / 100:
        raise AssertionError("rho upper endpoint is not below 1/100")
    if not arb(adverse.upper()) < arb(3) / 4:
        raise AssertionError("adverse upper endpoint is not below 3/4")
...
def main() -> int:
    result = validate()
    print("HIGH-BETA LAMBDA4 INTERIOR REPLAY VALIDATION PASS")
