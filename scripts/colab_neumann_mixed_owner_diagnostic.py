#!/usr/bin/env python3
"""Fresh Colab diagnostic for seven same-index owner declarations.

No restoration of project build outputs. Stop on the first real child error.
This is not a production cold seal, regional B0, window15 or terminal hRpoly.
...
def main():
    ...
    runner = base.runner
    runner.RUNNER_REV = 'neumann-mixed-owner-diagnostic-v1'
    ...
    runner.QUEUE = [
        ("sealed_prerequisite", ["lake","build","YangMills.RG.NeumannMixedBranchPackaging"], None),
        ("owner_diagnostic", ["lake","env","lean","tmp/NeumannMixedOwnerDraft.lean"], audit_names["owner_diagnostic"]),
    ]
    ...
    def make_evidence(status, opened):
        ...
        (runner.EVIDENCE / 'neumann-mixed-owner-diagnostic-contract.json').write_text(json.dumps({
            'source_sha': SOURCE,
            ...
            'scope': 'DIAGNOSTIC ONLY: same-index owner transport; saved olean is the sealed prerequisite, not a production owner module; no physical inverse, uniform B0 or window15',
        }, sort_keys=True) + '\n', encoding='utf-8')
        outputs = {}
