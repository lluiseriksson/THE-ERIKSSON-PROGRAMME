"""Validate both authoritative corrected-outer [0,1/200] segments."""
...
import certify_surface_remainder_delta0_r4_extension_v2 as cert
import surface_remainder_delta0_extension_probe as regular


ROOT = Path(__file__).resolve().parents[1]
TRANSCRIPTS = (
    ROOT/"scripts"/"certify_surface_remainder_delta0_r4_extension_v2_part1.txt",
    ROOT/"scripts"/"certify_surface_remainder_delta0_r4_extension_v2_part2.txt",
)
...
def validate():
    ...
    for path in TRANSCRIPTS:
        ...
        for line in lines:
            ...
            if row["band_radius"] != 14:
                raise AssertionError(f"wrong band radius at {index}")
            if not arb(row["margin_lower"]) > 0:
                raise AssertionError(f"nonpositive margin at {index}")
            rows[index] = row
    if segments != [(0, 150), (150, 158)]:
    ...
    print("K2 exact-r4 v2 transcripts OK: 158 boxes, band radius 14, "
