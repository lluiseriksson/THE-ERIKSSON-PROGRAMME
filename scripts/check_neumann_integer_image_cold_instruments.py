...
runner = ast.parse(Path('scripts/colab_neumann_integer_image_promoted_cold.py').read_text())
verifier = ast.parse(Path('scripts/verify_neumann_integer_image_promoted_cold.py').read_text())
...
assert sum(map(len, names.values())) == 7
assert list(commands) == ['reflection_focal', 'reflection_audit']
...
print('PINNED_SOURCE_BLOBS_NAMES_QUEUE=PASS files=2 names=7')
launcher = ast.parse(Path('scripts/launch_neumann_integer_image_promoted_cold.py').read_text())
preserver = ast.parse(Path('scripts/preserve_verify_neumann_integer_image_promoted_cold.py').read_text())
nb = json.loads(Path('scripts/colab_neumann_integer_image_promoted_cold.ipynb').read_text())
...
check = runpy.run_path('scripts/preserve_verify_neumann_integer_image_promoted_cold.py', run_name='instrument_test')
...
script = 'scripts/verify_neumann_integer_image_promoted_cold.py'
