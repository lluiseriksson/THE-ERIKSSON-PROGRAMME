"""Exact first delta coefficient of the regularized S2 carrier.
...
It does not bound the next Taylor remainder.
"""
...
def trunc1(expression):
    return sp.series(expression, d, 0, 2).removeO().expand()
...
w_over = trunc1(P+Q-d*P*Q/c**2)
w = d*w_over
root = trunc1(sp.sqrt(1-w))
phase = trunc1(-4*c*w_over/(1+root))
phase0 = sp.simplify(phase.subs(d, 0))
phase1 = sp.simplify(phase.coeff(d, 1))
assert sp.simplify(phase0+c*(s2+t2)/2) == 0
exp_correction = 1+d*phase1
inv_z = trunc1(d/(4*c*root))

# Fifth-order companions are needed only through their first coefficient here.
a_relative = trunc1(1-sp.Rational(3, 8)*inv_z)
b_relative = trunc1(1-sp.Rational(15, 8)*inv_z)
d_weight = trunc1(2*(1-d*(P+Q)))
cc = 2*c**2-1
f_over = trunc1(-4*P*(
    -2*cc*P*d-cc*Q*d+2*cc+2*P*Q*d**2-P*d-2*Q*d+1
))

k_regular = trunc1(root**sp.Rational(-3, 2)*a_relative*exp_correction)
h_regular = trunc1(root**sp.Rational(-5, 2)*b_relative*exp_correction)

integrands = {
    "KD": trunc1(k_regular*d_weight),
    "KF1": trunc1(k_regular*f_over),
    "HDD2": trunc1(h_regular*d_weight**2),
    "HDF3": trunc1(h_regular*d_weight*f_over),
}
...
kd0, kd1 = moments["KD"]
kf0, kf1 = moments["KF1"]
hd0, hd1 = moments["HDD2"]
hf0, hf1 = moments["HDF3"]
