...
SOURCE = '971bc5f2dc4065785cc21b300c10c66f21cf85d0'
BASE = '7f7455b286da5809a0ee64a1511684f887cf8a88'
REV = 'neumann-generated-mass-complete-offsets-hot-v2'
ROOT = '/content/hrpoly-neumann-complete-fibre-promoted-cold-v1'
RUNNER_HASH = '78daf60627f0c0ab6ffdbda43ff82202cbf86db4dbcfa11e25633677b9f4bcf1'
...
def verify(files):
    ...
    require(data['scope'] == "actual generated source-weighted/counting mass finite offsets and common integer image; not regional inverse, uniform B0 or window15", 'PHYSICAL_SCOPE')
    ...
    require(review['status'] == 'PASS_AFTER_READER_CORRECTION' and review['source'] == BASE, 'PARENT_REVIEW_STATUS')
    require(review['original_launch_status'] == 'FAIL' and review['original_outer_sha256'] == runner.PARENT_OUTER_SHA, 'ORIGINAL_FAIL_PRESERVED')
    ...
    commands['mathlib_repro'] = ['lake', 'env', 'lean', '-o', '/content/' + REV + '-evidence/mathlib_repro.olean',
        'tmp/' + REV + '/NeumannGeneratedMassCompleteOffsetsRepro.lean']
    commands['physical_prerequisites'] = ["lake","build","YangMills.RG.NeumannGeneratedCompleteFibre","YangMills.RG.NeumannGeneratedAverageFieldAction","YangMills.RG.BalabanCMP99SourceFlatGeneratedQprimeDirectOwnerKernel"]
    for s in ('clean_before', 'clean_after'):
        commands[s] = ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'lean-toolchain', 'lake-manifest.json']
    for s, name in [('physical_draft', 'NeumannGeneratedMassCompleteOffsetsDraft.lean')]:
    ...
    for s, path in [('physical_draft', 'tmp/NeumannGeneratedMassCompleteOffsetsDraft.lean')]:
