...
DEPS = (
    ...
    "scripts/certify_surface_remainder_k4_centered_00275_00280.py",
...
def parse(path: Path) -> list[str]:
    ...
    assert f"CONFIG unit {UNIT} delta 11/400:7/250 " in lines[5]
    ...
    # The carrier change in 9bcc7f55 only preserves TJet inputs; these
    # transcripts used Arb inputs and were sealed before that hash changed.
    ...
    assert len(cells) <= 9216
    return lines


def main() -> None:
    production = parse(ROOT / "scripts" / f"surface_remainder_k4_{UNIT}.txt")
    replay = parse(ROOT / "scripts" / f"surface_remainder_k4_{UNIT}_rerun.txt")
    assert production == replay
    print("K4 CENTERED BAND VALIDATION PASS", UNIT)
