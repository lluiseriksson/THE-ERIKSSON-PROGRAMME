...
RUNNER_HASH = 'af90f74a1c11f1a467bfaddfcd03a99ebd377602cdb161c323d2ea44e51b71c8'
...
def verify(files, spec, gate, prior_hash):
    ...
    if data['status'] == 'PASS':
        ...
        require(json.loads(files['prior-launch-status.json']) == dict(status='PASS',source=spec.BASE,
            revision='cmp99-physical-real-slice-retry-v2',cold_seal=False), 'PRIOR_STATUS')
...
def self_test(spec, gate):
    ...
    try:
        ...
        files['prior-launch-status.json'] = json.dumps(dict(status='PASS',source=spec.BASE,
            revision='cmp99-physical-real-slice-retry-v2',cold_seal=False)).encode()
        records, axioms = [], {}
        ...
        for kind in ('head','exit','command','output','missing','sorryAx','ofReduceBool','Other.axiom'):
            ...
            elif kind == 'missing': d['axioms'].pop('point_probe')
            else:
                name = 'point_probe.log'
                fs[name] = fs[name].replace(b'propext',kind.encode(),1)
                next(r for r in d['records'] if r['stage']=='point_probe')['sha256'] = sha(fs[name])
            pack(fs,d)
            try: verify(fs,spec,gate,prior)
            except ValueError: rejected += 1
            else: raise AssertionError('CORRUPTION_ACCEPTED='+kind)
        fs,d = copy.deepcopy(files),copy.deepcopy(data)
        d.update(status='FAIL',error='FIRST_ERROR=owner_cast_repro',records=d['records'][:7],
        ...
        print('OWNER_POINT_VERIFIER_SELF_TEST=PASS synthetic=1 rejected='+str(rejected)+' fail_prefix=1')
    finally:
        spec.BLOBS = original
...
def main():
    ...
    for attr in ('ROOT','PRIOR','HELPERS','LOGS'):
        setattr(spec,attr,PurePosixPath(str(getattr(spec,attr)).replace('\\','/')))
    ...
    print('OWNER_POINT_HOT_PACKAGE_VERIFIED COLD_SEAL=0')
