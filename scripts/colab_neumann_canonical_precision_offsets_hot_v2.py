"""Bounded canonical-Neumann finite-action HOT v2; parent cold independently sealed.
...
def main():
    ...
    previous = Path('/content/neumann-canonical-precision-offsets-hot-v1-evidence')
    previous_result = json.loads((previous / 'result.json').read_text())
    assert sha(Path(str(previous) + '.tar.gz').read_bytes()) == 'e7da0c8d4d9172a0de6e3ab74ceccf7177712e3806f75bdeb940389593f85acd', 'PREVIOUS_FAILURE_ARCHIVE'
    assert previous_result['status'] == 'FAIL' and previous_result['source'] == SOURCE, 'PREVIOUS_FAILURE_SOURCE'
    assert previous_result['records'][-1]['stage'] == 'physical_prerequisites' and previous_result['records'][-1]['timed_out'] is True and previous_result['records'][-1]['exit'] == -9, 'PREVIOUS_TIMEOUT'
