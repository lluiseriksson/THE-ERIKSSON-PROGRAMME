...
import verify_neumann_mixed_branch_packaging_hot_v2 as v
...
runner='924eef5767ccf38c662f25b0c204d5f6da0a61fe:scripts/colab_neumann_mixed_branch_packaging_hot_v2.py'
...
for s,c in cmds.items():
 ...
 if s=='parent_verify':text='{}\nNEUMANN_MIXED_FIXED_SOURCE_COLD_EVIDENCE_VERIFIED\n'
 if s in v.NAMES:
 ...
 files[s+'.log']=text.encode();records.append(dict(stage=s,command=c,cwd=v.ROOT,exit=0,timed_out=False,seconds=.1,sha256=v.sha(text.encode())))
...
for label in ['cold_upgrade','wrong_cwd','forbidden_axiom']:
 bad=dict(files)
 data=json.loads(bad['result.json'])
 if label=='cold_upgrade':
  data['cold_seal']=True
 elif label=='wrong_cwd':
  data['records'][3]['cwd']='/wrong'
 else:
  bad['mixed.log']=bad['mixed.log'].replace(b'Quot.sound',b'sorryAx')
  for r in data['records']:
   if r['stage']=='mixed':r['sha256']=v.sha(bad['mixed.log'])
 bad['result.json']=json.dumps(data).encode()
 bad['manifest.json']=json.dumps({n:v.sha(b) for n,b in bad.items() if n!='manifest.json'}).encode()
 try:v.verify(bad,{},'parenthash',gate)
 except Exception:rejected+=1
 else:raise AssertionError(label)
print('MIXED_FIXED_SOURCE_READER_SELF_TEST_PASS synthetic=1 rejected='+str(rejected))
