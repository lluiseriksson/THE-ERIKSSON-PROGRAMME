# OS-R-6 pass 32 -- FRESH-CLONE VERIFICATION (the SHA below is the
# commit that registers the pass-15 wiring numbers, Addendum 624).
# Brand-new clone in /content/osr6fresh (never reuses /content/osr1).
...
SHA = '813658de4a1ac5f4365611c444ebf1785ed679cc'
EXPECT_CORE = 8481
EXPECT_REPORTS = 3055
REPO = pathlib.Path('/content/osr6fresh')
...
for name in ['os_reconstruction_uniform_gap', 'os_reconstruction_measure_uniform',
             ...
             'groundKernel_reversible', 'norm_dressVac_le',
             'os_reconstruction_ground_state_clustering',
             'window_nonempty_interacting', 'tanh_tenth_le',
...
for f in ['YangMills/OS/OSReconstructionUniform.lean', 'YangMillsCore.lean',
          ...
          'scripts/osr6_pass32_freshclone.py']:
    p = REPO / f
    if p.exists():
        h = hashlib.sha256(p.read_bytes()).hexdigest()
        print(f'{h}  {p.stat().st_size:>8}  {f}', flush=True)
print('OSR6_FRESHCLONE_DONE', flush=True)
