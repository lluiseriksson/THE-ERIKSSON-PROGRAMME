...
import launch_neumann_canonical_spacing_promoted_cold as launch
import preserve_neumann_canonical_spacing_cold as preserve
import verify_neumann_canonical_spacing_promoted_cold as reader

assert launch.SOURCE == preserve.SOURCE == reader.SOURCE == 'dd9354a87d2e9d60dac69aa79f287b53e45ba207'
assert launch.REV == preserve.REV == reader.REV == 'neumann-canonical-spacing-promoted-cold-v1'
...
nb=json.loads(Path('scripts/colab_neumann_canonical_spacing_promoted_cold.ipynb').read_text())
...
ref='339d5a2e7a353a221c7d6a203d6ff70ef0e3c1ea'
name='launch_neumann_canonical_spacing_promoted_cold.py'
blob=subprocess.check_output(['git','cat-file','blob',ref+':scripts/'+name],timeout=5)
...
runpy.run_path('scripts/test_neumann_canonical_spacing_cold_preparation.py',run_name='__main__')
print('CANONICAL_SPACING_COLD_PACKAGE=PASS LAUNCHER_HASH=1 TRANSPORTS=4 CODE_CELLS=1')
