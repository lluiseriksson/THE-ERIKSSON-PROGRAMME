...
import subprocess
import types
import verify_neumann_actual_scalar_diagnostic as v

def gitblob(ref,path):
    return subprocess.check_output(['git','cat-file','blob',ref+':'+path],timeout=5)
runner=gitblob('7343544cd9e72d14fb73fc5b740ec03bdeca86fa','scripts/colab_neumann_actual_scalar_diagnostic.py')
...
queue=eval(compile(ast.Expression(q),'queue','eval'),{'lib':'.lake/build/lib/lean','NAMES':{s:frozenset(ns) for s,ns in v.NAMES.items()}})
...
outputs={n:b'SYNTHETIC NOT AN OLEAN' for n in ['NeumannActualFullSolutionScalarDraft.olean']}
f.update(outputs)
...
f['scalar-contract.json']=json.dumps(contract).encode()
...
for key,value in [('exit',7),('timed_out',True),('command',['lake','build','Wrong']),('seconds',float('nan'))]:
    g=copy.deepcopy(f);d=copy.deepcopy(data);d['records'][-2][key]=value;g['scalar_physical.json']=json.dumps(d['records'][-2]).encode();g['evidence.json']=json.dumps(d).encode();reject(g)
for replacement in ('sorryAx','ofReduceBool','Unknown.axiom'):
    g=copy.deepcopy(f);d=copy.deepcopy(data);g['scalar_physical.log']=g['scalar_physical.log'].replace(b'Quot.sound',replacement.encode());d['records'][-2]['output_sha256']=v.sha(g['scalar_physical.log']);g['scalar_physical.json']=json.dumps(d['records'][-2]).encode();g['evidence.json']=json.dumps(d).encode();reject(g)
