...
SOURCE = 'ca988d2a270e89f38a29b89949212c5ccbfeee1c'
BASE = '108f30f954e3d807f16c4264fac56f4814c65ea4'
PIN = '0cce4bd0f0f50cd6810fc973e85c07939c8a5e2db8a06d54d85554e4603bf2ad'
RUNNER = '14d59004d9c4c90d06891eff0c8a1b534187ae8aa9a3dbb55d64f6061b22324b'
REV = 'neumann-physical-transfer-hot-v1'
ROOT = '/content/hrpoly-neumann-boundary-orbit-diagnostic-v2'
NAMES = {'YangMills.RG.neumannPhysicalGreen_blockBoundaryTransfer_massUniform'}
COMMANDS = {
    ...
    'physical_prerequisite': ['lake', 'build', 'YangMills.RG.NeumannPhysicalHalfCellReflection'],
    'orbit': ['lake', 'env', 'lean', '-o', '/content/'+REV+'/orbit.olean',
        'tmp/'+REV+'/NeumannPhysicalBoundaryTransferDraft.lean'],
...
def verify(files, parent, parent_hash, gate):
    ...
    require(sha(files['NeumannPhysicalBoundaryTransferDraft.lean']) == PIN, 'SOURCE_HASH')
    ...
    expected_files = {'runner.py', 'result.json', 'parent-review.json',
        'NeumannPhysicalBoundaryTransferDraft.lean', 'orbit.olean'}
    for r in records:
        stage = r['stage']
        require(r['command'] == COMMANDS[stage] and r['cwd'] == ROOT, 'COMMAND='+stage)
        require(r['exit'] == 0 and r['timed_out'] is False and
            r['timeout_seconds'] == (3600 if stage == 'physical_prerequisite' else 120), 'EXIT_TIMEOUT='+stage)
...
def main():
    ...
    print('PHYSICAL_TRANSFER_HOT_EVIDENCE_VERIFIED')
