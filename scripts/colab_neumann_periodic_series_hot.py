...
OUT=Path('/content/neumann-periodic-series-hot-v1')
PINS={
...
def main():
 ...
 try:
  ...
  run('prerequisite',['lake','build','YangMills.RG.NeumannActualFullGreenReflectionSummability'])
  names={
  ...
  for stage,file in [('series','NeumannPhysicalPeriodicSeriesDraft.lean'),('summability','NeumannPhysicalPeriodicSummabilityDraft.lean')]:
   text=run(stage,['lake','env','lean','-o',str(OUT/(stage+'.olean')),str(OUT/file)])
