"""300-bit rescue sign-row driver for one finite-beta candidate unit."""
...
def run(unit, lo, hi):
    ...
    lines = [
        "SCALED BULK SIGN ROW UNIT CWIN3P2 RESCUE300",
    ...
    lines.extend((f"t_rows {len(rows)}",
                  ...
                  "SCOPE candidate rescue unit only; no G2/G6 promotion"))
    return "\n".join(lines) + "\n"
