...
SOURCE = '8acbb1277f07810708f56036afbec787177aef0a'
BASE = '8570618f20c62c5724555589f910b84fdf803c33'
REV = 'neumann-generated-complete-fibre-hot-v2'
ROOT = '/content/hrpoly-neumann-average-field-cohort-cold-v1'
RUNNER_HASH = '2a0943480850c17df830ab2cbc56c56304de67035f5c36f91afaf639658ae9fa'
...
def verify(files):
    ...
    commands['mathlib_repro'] = ['lake', 'env', 'lean', '-o', '/content/' + REV + '-evidence/mathlib_repro.olean',
        'tmp/' + REV + '/NeumannGeneratedCompleteFibreSumRepro.lean']
    commands['physical_prerequisites'] = ['lake', 'build', 'YangMills.RG.BalabanCMP99SourceFlatGeneratedTerminalBlockCollapse', 'YangMills.RG.BalabanCMP99SourceFlatQprimeBlockOffsetEquiv', 'YangMills.RG.NeumannGeneratedIntegerCountingKernel', 'YangMills.RG.NeumannIntegerBlockImageAverage']
    for s in ('clean_before', 'clean_after'):
        commands[s] = ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'lean-toolchain', 'lake-manifest.json']
    for s, name in [('physical_draft', 'NeumannGeneratedCompleteFibreDraft.lean')]:
    ...
    expected_files = {'runner.py', 'records.json', 'result.json', 'lean-sha256.txt', 'lake-sha256.txt',
        *(Path(p).name for p in runner.PINS), *(s + '.log' for s in stages), *data['outputs']}
    require(set(files) == expected_files, 'FILE_SET')
    for s, path in [('physical_draft', 'tmp/NeumannGeneratedCompleteFibreDraft.lean')]:
