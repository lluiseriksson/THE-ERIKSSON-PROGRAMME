#!/usr/bin/env python3
"""Exact seven-name owner diagnostic reader; NOT a production seal. Independent contract and output checks. No Lean, Git or subprocess calls.
...
SOURCE = 'bab8c90d09f7dfeaf7965ce3b0381a96bfabd19e'
REV = 'neumann-mixed-owner-diagnostic-v1'
...
BLOBS = {
    "tmp/NeumannMixedOwnerDraft.lean": "4d37d5ae52d3b8b3323e6b460b145b0a655ea829acc8afee09387a9a24fc8621"
}
NAMES = {
    "owner_diagnostic": [
...
COMMANDS = {
    ...
    "owner_diagnostic": [
        ...
        "env",
        "lean",
        "tmp/NeumannMixedOwnerDraft.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-mixed-owner-diagnostic-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
            parent_diagnostics=[{"source":"c9818c183643a19b0cb88d614ea7b10ad3e8eda6","archive_sha256":"b1f33671a8ac555162b88469f4f92a90e69ca3035af137668eb91ab929d9b310"}],
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="DIAGNOSTIC ONLY: same-index owner transport; saved olean is the sealed prerequisite, not a production owner module; no physical inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-mixed-owner-diagnostic-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    return dict(status='PASS', cold_seal=False, source_sha=SOURCE, runner_rev=REV,
...
def self_test(gate, old):
    ...
    files['neumann-mixed-owner-diagnostic-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"c9818c183643a19b0cb88d614ea7b10ad3e8eda6","archive_sha256":"b1f33671a8ac555162b88469f4f92a90e69ca3035af137668eb91ab929d9b310"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="DIAGNOSTIC ONLY: same-index owner transport; saved olean is the sealed prerequisite, not a production owner module; no physical inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    ...
    print('NEUMANN_MIXED_OWNER_DIAGNOSTIC_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_MIXED_OWNER_DIAGNOSTIC_EVIDENCE_VERIFIED')
