#!/usr/bin/env python3
"""Bounded literal physical character/readout diagnostics; no full source equation seal."""
...
OUT = Path('/content/neumann-physical-character-readout-hot-v1')
...
def main():
    ...
    mask = Path('/content/neumann-masked-difference-hot-v1/result.json')
    if not Path('/content/masked-difference-hot-evidence-preserved.ok').is_file():
        raise RuntimeError('MASK_EVIDENCE_NOT_PRESERVED')
    if json.loads(mask.read_text()).get('status') != 'PASS':
        raise RuntimeError('MASK_DIAGNOSTIC_NOT_PASS')
    import psutil
