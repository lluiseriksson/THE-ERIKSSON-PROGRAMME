...
OVERLAY_SOURCE = 'f6e9516367e77fac1f70e2f3eb1fe855850ceea2'

BASE = '1885a6c0886e595f0d3013c6ac2b9a71f3c93036'
ROOT = Path('/content/hrpoly-neumann-selector-opposite-promoted-cold-v1')
OUT = Path('/content/neumann-normalized-integral-congruence-hot-v1')
GATE_HASH = '016ca4daf0cd06c8016ece106334cc10a4c332c0a58f7f383f03c6f6b3e287c2'
DRAFTS = {
    "tmp/NeumannIntegralRewriteRepro.lean": "06c79ca74c0a56c18149e223b973452e9def6af8834e970aca3f803fb64b444a"
}
NAMES = {
    "type_probe": [
...
def main():
    ...
    parent = Path('/content/neumann-alias-physical-integral-hot-v3.tar.gz')
    if sha(parent.read_bytes()) != '0bcd3dff7f0f48317c122ea4396afc88c56fbdaf6e264bcaee871095d5e43e88':
        raise RuntimeError('PARENT_FAILURE_HASH')
    if not Path('/content/physical-integral-hot-v3-preserved.ok').is_file():
    ...
    for path, digest in DRAFTS.items():
        ...
        if target.exists() and target.read_bytes() != blob:
            if sha(target.read_bytes()) != {"tmp/NeumannIntegralRewriteRepro.lean":"2f1fe353cce9fcb15a90f347b006b718e5857c73918a866eeb2402525000dd92","tmp/NeumannAliasPhysicalIntegralDraft.lean":"aa15c6c1d655aaa02b2a50fe6c85482fafe8d349358997afd976d02ebca42cd6"}.get(path):
