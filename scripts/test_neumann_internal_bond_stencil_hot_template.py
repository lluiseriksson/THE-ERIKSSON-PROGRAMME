...
rt = Path('tmp/colab_neumann_internal_bond_stencil_hot_template.py').read_text()
vt = Path('tmp/verify_neumann_internal_bond_stencil_hot_template.py').read_text()
...
parent = dict(status='VERIFIED_COLD_PASS', source=r0.BASE, cold_seal=True,
    ...
    production_outputs={'NeumannCanonicalSpacingNormalization.olean': '3'*64}))
...
draft = files['NeumannInternalBondStencilDraft.lean'].decode()
...
commands = {
    ...
    'mathlib_repro': ['lake', 'env', 'lean', '-o', (r.OUT / 'mathlib_repro.olean').as_posix(),
        'tmp/' + r.REV + '/NeumannInternalBondStencilRepro.lean'],
    'physical_prerequisites': ["lake","build","YangMills.RG.BalabanCMP89SourceNeumannRegionalPrecision","YangMills.RG.BalabanCMP99ActiveRegionSourceCovariantAdjointStencil"],
    'physical_draft': ['lake', 'env', 'lean', '-o', (r.OUT / 'physical_draft.olean').as_posix(),
        'tmp/' + r.REV + '/NeumannInternalBondStencilDraft.lean'],
...
print('INTERNAL_BOND_STENCIL_TEMPLATE_SYNTHETIC_SELF_TEST=PASS valid=1 rejected=10 preserved_failure=1')
