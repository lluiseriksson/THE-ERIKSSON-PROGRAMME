...
REV = 'neumann-physical-readout-integral-promoted-cold-v1'
...
COMMANDS = {
    "integral_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannPhysicalReadoutIntegral"
    ],
    "integral_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannPhysicalReadoutIntegralAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-physical-readout-integral-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="actual fine-endpoint alias precision readout integrated using produced mass-uniform domain; not physical operator interchange, counting inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-physical-readout-integral-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannPhysicalReadoutIntegral.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-physical-readout-integral-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"f308441f2cf19b2294189cab14d1a1d6d1217330","archive_sha256":"3a612b299e0aea5b689979bf8a39095f676a129903e24060e56237463fb878d5","source_blobs":{"tmp/NeumannPhysicalReadoutIntegralDraft.lean":"cd0a8e40a5003eb4665326b53c355a62d661a62539baff4507e3398187590fd7"}}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="actual fine-endpoint alias precision readout integrated using produced mass-uniform domain; not physical operator interchange, counting inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
