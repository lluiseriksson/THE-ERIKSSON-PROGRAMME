...
ROOT = '/content/hrpoly-neumann-wrap-flat-promoted-cold-v1'
RUNNER_HASH = '6806ed2f34cdc71519611f6199493839f3f45d1dc262a9bf1dcf9fa348c90af1'
...
def verify(files):
    ...
    require(data['scope'] == "actual directional torus rectangle bond masks including full-period sides; not physical carrier choice, reflection covariance, regional inverse, uniform B0 or window15", 'PHYSICAL_SCOPE')
    ...
    require(review['cold']['production_outputs']['NeumannRectangleWrapProbe.olean'] == runner.PARENT_OLEAN_HASH, 'PARENT_OUTPUT_REVIEW')
    ...
    commands['mathlib_repro'] = ['lake', 'env', 'lean', '-o', '/content/' + REV + '-evidence/mathlib_repro.olean',
        'tmp/' + REV + '/NeumannRectangleDirectionalMasksRepro.lean']
    commands['physical_prerequisites'] = ["lake","build","YangMills.RG.BalabanCMP89NeumannRectangleActiveRegion"]
    for s in ('clean_before', 'clean_after'):
        commands[s] = ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'lean-toolchain', 'lake-manifest.json']
    for s, name in [('physical_draft', 'NeumannRectangleDirectionalMasksDraft.lean')]:
    ...
    for s, path in [('mathlib_repro', 'tmp/NeumannRectangleDirectionalMasksRepro.lean'), ('physical_draft', 'tmp/NeumannRectangleDirectionalMasksDraft.lean')]:
