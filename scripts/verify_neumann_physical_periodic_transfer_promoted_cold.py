#!/usr/bin/env python3
"""Exact two-name physical periodic-transfer cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
SOURCE = '24dc691e451ab9b6684f950a3fdd5a78e913997f'
REV = 'neumann-physical-periodic-transfer-promoted-cold-v1'
...
BLOBS = {
    "YangMills/RG/NeumannPhysicalPeriodicTransfer.lean": "005510b16c71cf676749ff56dd198e07ec1f711ea7f16a50d5dd0befa8634375",
    "YangMills/RG/NeumannPhysicalPeriodicTransferAudit.lean": "ef88ac98fd4cd7131248f0937e29ec09a477b2db72059ef2788eb11ad3b7e453"
}
NAMES = {
    "periodic_audit": [
        "YangMills.RG.neumannPhysicalGreen_blockShiftTransfer",
...
COMMANDS = {
    "periodic_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannPhysicalPeriodicTransfer"
    ],
    "periodic_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannPhysicalPeriodicTransferAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-physical-periodic-transfer-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
            parent_diagnostics=[{"source":"3b0576aa1329025f815ebee0edd15666d6e432bd","archive_sha256":"dad847bcbea6053b72643932b8d7a2dc61906d7faa9583ef0c61183737a92c3d"}],
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="literal common-block physical endpoint transfer and integer-period image transfer; not periodic-series convergence, mixed coverage, regional inverse, B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-physical-periodic-transfer-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannPhysicalPeriodicTransfer.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-physical-periodic-transfer-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"3b0576aa1329025f815ebee0edd15666d6e432bd","archive_sha256":"dad847bcbea6053b72643932b8d7a2dc61906d7faa9583ef0c61183737a92c3d"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="literal common-block physical endpoint transfer and integer-period image transfer; not periodic-series convergence, mixed coverage, regional inverse, B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    for name in ["NeumannPhysicalPeriodicTransfer.olean"]:
    ...
    print('NEUMANN_PHYSICAL_PERIODIC_TRANSFER_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_PHYSICAL_PERIODIC_TRANSFER_COLD_EVIDENCE_VERIFIED')
