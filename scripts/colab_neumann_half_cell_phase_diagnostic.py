"""Fresh, bounded phase diagnostic: Mathlib repro before physical compilation.
...
NAMES = {
    'phase_repro': frozenset({'NeumannHalfCellPhaseRepro.finite_exp_reverse'}),
    'phase_physical': frozenset({'YangMills.RG.neumannEntireAverageFactor_halfCellPhase'}),
}
...
def main():
    ...
    r.QUEUE = [
        ...
        ('phase_repro', ['lake', 'env', 'lean', '-o', lib + '/NeumannHalfCellPhaseRepro.olean', 'tmp/NeumannHalfCellPhaseRepro.lean'], NAMES['phase_repro']),
        ('phase_prerequisites', ['lake', 'build', 'YangMills.RG.BalabanCMP89Eq251EntireAverageAmplitude'], None),
        ('phase_physical', ['lake', 'env', 'lean', '-o', lib + '/NeumannEntireAverageHalfCellPhaseDraft.olean', 'tmp/NeumannEntireAverageHalfCellPhaseDraft.lean'], NAMES['phase_physical']),
        ('clean_after', ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'tmp/NeumannHalfCellPhaseRepro.lean', 'tmp/NeumannEntireAverageHalfCellPhaseDraft.lean', 'lean-toolchain', 'lake-manifest.json'], None),
    ]
    ...
    def preserve(status, opened):
        ...
        outputs = {}
        for name in ('NeumannHalfCellPhaseRepro', 'NeumannEntireAverageHalfCellPhaseDraft'):
        ...
        (r.EVIDENCE / 'phase-contract.json').write_text(json.dumps(dict(
