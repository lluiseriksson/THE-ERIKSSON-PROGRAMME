#!/usr/bin/env python3
"""Exact six-name FULL periodic-interval cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
REV = 'neumann-periodic-interval-promoted-cold-v1'
...
BLOBS = {
    "YangMills/RG/NeumannPeriodicIntervalCoverage.lean": "685dcfd85483492daec1507f52aac80a3351b16261cc074b0782823cf7a0614a",
    "YangMills/RG/NeumannPeriodicIntervalCoverageAudit.lean": "8cbc054743422e700e10e28c621e4ef7cb5d177e688d2ef34095b067d3b32a80"
}
NAMES = {
    "interval_audit": [
        "YangMills.RG.neumannPeriodicInterval_periodQuotient",
        "YangMills.RG.neumannPeriodicInterval_periodRemainder",
        "YangMills.RG.neumannPeriodicIntervalFamily_injective",
        "YangMills.RG.neumannPeriodicIntervalFamily_surjective",
        "YangMills.RG.neumannPeriodicIntervalFamily_bijective",
        "YangMills.RG.neumannIntegerPeriodicOwner"
    ]
}
...
COMMANDS = {
    "interval_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannPeriodicIntervalCoverage"
    ],
    "interval_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannPeriodicIntervalCoverageAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-periodic-interval-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
            parent_diagnostics=[{"source":"1b222f64aa3034861a3eee17e02c56b5d452eb44","archive_sha256":"497f93438c55d69118cb4cf1f6141e411a8b8ded1a26b95762350c9472c0839b"}],
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="FULL half-open integer interval bijection and periodic owner transport; not mixed coverage, physical inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-periodic-interval-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannPeriodicIntervalCoverage.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-periodic-interval-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"1b222f64aa3034861a3eee17e02c56b5d452eb44","archive_sha256":"497f93438c55d69118cb4cf1f6141e411a8b8ded1a26b95762350c9472c0839b"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="FULL half-open integer interval bijection and periodic owner transport; not mixed coverage, physical inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    for name in ["NeumannPeriodicIntervalCoverage.olean"]:
    ...
    print('NEUMANN_PERIODIC_INTERVAL_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_PERIODIC_INTERVAL_COLD_EVIDENCE_VERIFIED')
