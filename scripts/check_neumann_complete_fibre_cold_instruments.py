...
runner = ast.parse(Path('scripts/colab_neumann_complete_fibre_promoted_cold.py').read_text())
verifier = ast.parse(Path('scripts/verify_neumann_complete_fibre_promoted_cold.py').read_text())
...
assert len(literal_assignment(verifier, 'BLOBS')) == 2
assert sum(map(len, names.values())) == 6
assert list(commands) == ['fibre_focal', 'fibre_audit']
...
print('PINNED_SOURCE_BLOBS_NAMES_QUEUE=PASS files=2 names=6')
launcher = ast.parse(Path('scripts/launch_neumann_complete_fibre_promoted_cold.py').read_text())
preserver = ast.parse(Path('scripts/preserve_verify_neumann_complete_fibre_promoted_cold.py').read_text())
nb = json.loads(Path('scripts/colab_neumann_complete_fibre_promoted_cold.ipynb').read_text())
...
check = runpy.run_path('scripts/preserve_verify_neumann_complete_fibre_promoted_cold.py', run_name='instrument_test')
scopes = [ast.literal_eval(v) for node in ast.walk(runner) if isinstance(node, ast.Dict)
    for k, v in zip(node.keys, node.values) if isinstance(k, ast.Constant) and k.value == 'scope']
assert scopes == [check['SCOPE']], 'RUNNER_PRESERVER_SCOPE'
contract_name = 'neumann-complete-fibre-promoted-cold-contract.json'
check['verify_scope']({contract_name: json.dumps({'scope': check['SCOPE']}).encode()})
for bad in ('rectangle image-family coverage', 'uniform physical B0 proved', None):
    try:
        check['verify_scope']({contract_name: json.dumps({'scope': bad}).encode()})
    except ValueError as error:
        assert str(error) == 'PHYSICAL_SCOPE'
    else:
        raise AssertionError('WRONG_SCOPE_ACCEPTED')
print('PHYSICAL_SCOPE_ACCEPTED=1 REJECTED=3')
...
script = 'scripts/verify_neumann_complete_fibre_promoted_cold.py'
