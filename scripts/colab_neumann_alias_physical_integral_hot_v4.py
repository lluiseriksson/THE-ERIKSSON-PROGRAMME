...
OVERLAY_SOURCE = '858bbc063675f71f5cc60bf935d957e63e924147'

BASE = '1885a6c0886e595f0d3013c6ac2b9a71f3c93036'
ROOT = Path('/content/hrpoly-neumann-selector-opposite-promoted-cold-v1')
OUT = Path('/content/neumann-alias-physical-integral-hot-v4')
GATE_HASH = '016ca4daf0cd06c8016ece106334cc10a4c332c0a58f7f383f03c6f6b3e287c2'
DRAFTS = {
    "tmp/NeumannAliasPhysicalIntegralDraft.lean": "1e681883e56f138def4374eb0c03d727cf6166e7b9565368855670188b185f76"
}
NAMES = {
    "physical_integral": [
...
def main():
    ...
    for path, digest in DRAFTS.items():
        ...
        if target.exists() and target.read_bytes() != blob:
            if sha(target.read_bytes()) != {"tmp/NeumannIntegralRewriteRepro.lean":"6063bf14dd808ec07d41a89107ee7144d9afd1d07581c3d2b3be725a60fc2df5","tmp/NeumannAliasPhysicalIntegralDraft.lean":"ac631ace0f5b6348dc0b944c2faf6aa17702748334ac043098baebaa7f3bd8c7"}.get(path):
