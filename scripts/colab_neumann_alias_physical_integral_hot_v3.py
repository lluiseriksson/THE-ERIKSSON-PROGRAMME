...
OUT = Path('/content/neumann-alias-physical-integral-hot-v3')
...
def main():
    ...
    parent = Path('/content/neumann-alias-physical-integral-hot-v2.tar.gz')
    if sha(parent.read_bytes()) != '1e058bca0a18922fa760b17f179ab69d11cc931de155137001835bf3ee3ddd62':
        raise RuntimeError('PARENT_FAILURE_HASH')
    if not Path('/content/physical-integral-hot-v2-preserved.ok').is_file():
    ...
    for path, digest in DRAFTS.items():
        ...
        if target.exists() and target.read_bytes() != blob:
            if sha(target.read_bytes()) != {"tmp/NeumannIntegralRewriteRepro.lean":"6063bf14dd808ec07d41a89107ee7144d9afd1d07581c3d2b3be725a60fc2df5","tmp/NeumannAliasPhysicalIntegralDraft.lean":"aa15c6c1d655aaa02b2a50fe6c85482fafe8d349358997afd976d02ebca42cd6"}.get(path):
