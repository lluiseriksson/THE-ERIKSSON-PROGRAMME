...
import preserve_neumann_wrap_flat_cold as p
...
cold = p.load(helpers['verify_neumann_wrap_flat_promoted_cold.py'],
...
launcher = blob('f60fd00d462d6959c519e3cbe7ec88dbfded6102', p.LAUNCHER)
...
commands = [
    ...
    ('physical_cold_graph', [py, work + 'colab_neumann_wrap_flat_promoted_cold.py']),
...
def verify_outer(files):
    ...
    with patch.object(argparse.ArgumentParser, 'parse_args', return_value=args):
        with contextlib.redirect_stdout(output):
            p.main()
    assert 'NEUMANN_WRAP_FLAT_COLD_PRESERVED COMPILER_SEAL=1' in output.getvalue()
...
changed['NeumannFlatInternalBondAction.olean'] += b'corrupt'
...
print('WRAP_FLAT_COLD_PRESERVATION_SELF_TEST=PASS valid=1 rejected=' + str(len(bads)))
