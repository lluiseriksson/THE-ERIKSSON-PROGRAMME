...
from verify_neumann_image_rectangle_hot_v2 import verify
from verify_neumann_counting_reflection_diagnostic_v2 import unpack, sha

p = Path('validation-evidence/neumann-image-rectangle-hot-v2-20260906/neumann-image-rectangle-hot-v2-evidence.tar.gz')
b = p.read_bytes()
assert sha(b) == 'aae64e9f3eb9966b7bfe1c90398e9460608082c2d3388fa4e2a67d2639f01f2e'
...
for n in ('runner.py', 'NeumannImageRectangleCoverageDraft.lean', 'mathlib-repro.lean',
          'physical_draft.log', 'physical_draft.olean'):
    f = dict(good)
    f[n] += b'corruption'
    bad.append(f)
for key, value in [('support_source', '0'*40), ('parent_outer_sha256', '0'*64), ('source', '0'*40), ('cold_seal', True), ('status', 'FAIL')]:
