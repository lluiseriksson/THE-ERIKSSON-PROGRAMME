#!/usr/bin/env python3
"""Bounded opposite-difference algebra; no physical operator identification."""
...
OUT = Path('/content/neumann-opposite-difference-hot-v1')
...
def main():
    ...
    if not Path('/content/alias-selector-hot-evidence-preserved.ok').is_file():
        raise RuntimeError('SELECTOR_EVIDENCE_NOT_PRESERVED')
    selector = json.loads(Path('/content/neumann-alias-exponential-selector-hot-v1/result.json').read_text())
    if selector.get('status') != 'PASS' or selector.get('source_sha') != BASE:
        raise RuntimeError('SELECTOR_NOT_PASS')
    import psutil
