...
import launch_neumann_wrap_flat_promoted_cold as launch
import preserve_neumann_wrap_flat_cold as preserve
import verify_neumann_wrap_flat_promoted_cold as reader

assert launch.SOURCE == preserve.SOURCE == reader.SOURCE == '15e776db59c051571f79e47974acc2fbf597f715'
assert launch.REV == preserve.REV == reader.REV == 'neumann-wrap-flat-promoted-cold-v1'
...
nb=json.loads(Path('scripts/colab_neumann_wrap_flat_promoted_cold.ipynb').read_text())
...
ref='f60fd00d462d6959c519e3cbe7ec88dbfded6102'
name='launch_neumann_wrap_flat_promoted_cold.py'
blob=subprocess.check_output(['git','cat-file','blob',ref+':scripts/'+name],timeout=5)
...
runpy.run_path('scripts/test_neumann_wrap_flat_cold_preparation.py',run_name='__main__')
print('WRAP_FLAT_COLD_PACKAGE=PASS LAUNCHER_HASH=1 TRANSPORTS=4 CODE_CELLS=1')
