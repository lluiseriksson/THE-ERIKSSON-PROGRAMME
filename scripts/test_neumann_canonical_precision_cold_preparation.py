"""Actual producer AST vs independent reader and two source audit names.
...
import verify_neumann_canonical_precision_promoted_cold as v
...
tree = ast.parse(Path('scripts/colab_neumann_canonical_precision_promoted_cold.py').read_text())
...
audit = blob('YangMills/RG/NeumannCanonicalPrecisionOffsetActionAudit.lean').decode()
...
v.verify = original
files = captured[0]
key = 'neumann-canonical-precision-promoted-cold-contract.json'
...
print('CANONICAL_PRECISION_COLD_PREPARATION=PASS ACTUAL_CONTRACT=1 SOURCE_BLOBS=2 AUDIT_NAMES=2')
