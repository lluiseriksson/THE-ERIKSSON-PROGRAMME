...
ROOT = '/content/hrpoly-neumann-mass-offsets-promoted-cold-v1'
RUNNER_HASH = '3f9f6b809259872b8ad3ebcd12ddeeddcf4fb05d28849a795197fac974f3282c'  # Git blob f5e55dd1e, not worktree bytes.
...
def verify(files):
    ...
    require(data['scope'] == "literal canonical Neumann precision finite-offset action only; not Fourier normalization, regional inverse, uniform B0 or window15", 'PHYSICAL_SCOPE')
    ...
    require(review['cold']['production_outputs'] == {'NeumannGeneratedMassCompleteOffsets.olean': runner.PARENT_OLEAN_HASH}, 'PARENT_OUTPUT_REVIEW')
    ...
    commands['mathlib_repro'] = ['lake', 'env', 'lean', '-o', '/content/' + REV + '-evidence/mathlib_repro.olean',
        'tmp/' + REV + '/NeumannCanonicalPrecisionOffsetActionRepro.lean']
    commands['physical_prerequisites'] = ["lake","build","YangMills.RG.BalabanCMP89SourceFlatGeneratedFiniteDepthCanonicalNeumannPrecision","YangMills.RG.BalabanCMP89NeumannPrecisionThreeSpecies","YangMills.RG.NeumannRetainedCountingMassDictionary","YangMills.RG.NeumannGeneratedMassCompleteOffsets"]
    for s in ('clean_before', 'clean_after'):
        commands[s] = ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'lean-toolchain', 'lake-manifest.json']
    for s, name in [('physical_draft', 'NeumannCanonicalPrecisionOffsetActionDraft.lean')]:
    ...
    require(set(data['outputs']) == {'physical_draft.olean', 'mathlib_repro.olean'}, 'OUTPUT_SET')
    ...
    for s, path in [('physical_draft', 'tmp/NeumannCanonicalPrecisionOffsetActionDraft.lean')]:
