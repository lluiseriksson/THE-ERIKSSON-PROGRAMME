...
import subprocess
import types
import verify_neumann_coordinate_alias_reflection_hot as v

def blob(ref, path):
    return subprocess.check_output(['git', 'cat-file', 'blob', ref + ':' + path], timeout=5)

rb = blob('4de83287ad5a0ea7515946fe3b376440a5a354a4', 'scripts/colab_neumann_coordinate_alias_reflection_hot.py')
...
commands = {
    ...
    'physical_prerequisites': ['lake', 'build', 'YangMills.RG.BalabanCMP99SourceAliasReflectionInvolutive', 'YangMills.RG.BalabanCMP99SourceAliasReflectionStabilizedSolution'],
    'physical_draft': ['lake', 'env', 'lean', '-o', '/content/' + r.REV + '-evidence/physical_draft.olean', 'tmp/' + r.REV + '/NeumannCoordinateAliasReflectionDraft.lean'],
...
d = dict(status='PASS', source=r.SOURCE, base_source=r.BASE, cold_seal=False,
    ...
    scope='one-coordinate actual alias permutation and central preservation only; not precision or Green covariance, inverse, B0 or window15',
...
for bad in ('sorryAx', 'ofReduceBool', 'Unknown.axiom'):
    g = copy.deepcopy(f)
    dd = copy.deepcopy(d)
    g['physical_draft.log'] = g['physical_draft.log'].replace(b'propext', bad.encode(), 1)
    dd['records'][6]['log_sha256'] = v.sha(g['physical_draft.log'])
    g['records.json'] = json.dumps(dd['records']).encode()
    g['result.json'] = json.dumps(dd).encode()
    try: v.verify(g)
    except (ValueError, KeyError): rejected += 1
    else: raise AssertionError('FORBIDDEN_AXIOM_ACCEPTED')
for altered in ('extra_file', 'missing_output', 'missing_axiom'):
    g = copy.deepcopy(f)
    dd = copy.deepcopy(d)
    if altered == 'extra_file': g['unexpected.txt'] = b'extra'
    if altered == 'missing_output': del g['physical_draft.olean']
    if altered == 'missing_axiom':
        g['physical_draft.log'] = b''
        dd['records'][6]['log_sha256'] = v.sha(b'')
        g['records.json'] = json.dumps(dd['records']).encode()
        g['result.json'] = json.dumps(dd).encode()
    try: v.verify(g)
    except (ValueError, KeyError): rejected += 1
    else: raise AssertionError('INCOMPLETE_ACCEPTED')
