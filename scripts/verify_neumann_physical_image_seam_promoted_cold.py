#!/usr/bin/env python3
"""Exact eleven-name physical image-seam cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
REV = 'neumann-physical-image-seam-promoted-cold-v1'
...
COMMANDS = {
    "seam_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannPhysicalImageSeam"
    ],
    "seam_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannPhysicalImageSeamAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-physical-image-seam-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="literal physical all-reflecting source-image summability and lower/upper ghost seam with fine/block boundary dictionary; not mixed FULL directions, regional point-source equation, inverse, B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-physical-image-seam-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannBoundaryOrbitPermutation.olean","NeumannBoundaryImagePermutation.olean","NeumannBoundaryImageSeriesReindex.olean","NeumannPhysicalBoundaryTransfer.olean","NeumannPhysicalImageSeam.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-physical-image-seam-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"970356bfc58ec2ca9ba94163be952d382c0689c9","archive_sha256":"cd7a66477087916e59c54bf0f6d995810ab838d10eedb7168ec3a7fc04e12977"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="literal physical all-reflecting source-image summability and lower/upper ghost seam with fine/block boundary dictionary; not mixed FULL directions, regional point-source equation, inverse, B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    ...
    print('NEUMANN_PHYSICAL_IMAGE_SEAM_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_PHYSICAL_IMAGE_SEAM_COLD_EVIDENCE_VERIFIED')
