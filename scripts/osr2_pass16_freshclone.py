# OS-R-2 pass 16 -- FRESH-CLONE VERIFICATION (the SHA below is the
# commit that registers the pass-15 wiring numbers, Addendum 624).
# Brand-new clone in /content/osr2fresh (never reuses /content/osr1).
...
SHA = '70276c78f3f33215c0075200faa43e33143dc57e'
EXPECT_CORE = 8481
EXPECT_REPORTS = 3028
REPO = pathlib.Path('/content/osr2fresh')
...
for f in ['YangMills/OS/OSReconstructionUniform.lean', 'YangMillsCore.lean',
          ...
          'scripts/osr2_pass16_freshclone.py']:
    p = REPO / f
    if p.exists():
        h = hashlib.sha256(p.read_bytes()).hexdigest()
        print(f'{h}  {p.stat().st_size:>8}  {f}', flush=True)
print('OSR2_FRESHCLONE_DONE', flush=True)
