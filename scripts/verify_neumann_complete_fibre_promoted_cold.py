#!/usr/bin/env python3
"""Read-only constructed complete-fibre cold-graph evidence verifier. No Lean, Git or subprocess calls.
...
def verify(files, gate, old):
    ...
    for field, value in dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="constructed complete active fibre, offsets and integer-coordinate sum dictionary; not regional inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9}).items():
