...
RUNNER_HASH = 'c2109ca7a216a616b09e4355e19048f912f53289a6bb0a2694b49b9d2b3ad892'
...
def verify(files):
    ...
    commands['mathlib_repro'] = ['lake', 'env', 'lean', '-o', '/content/' + REV + '-evidence/mathlib_repro.olean',
        'tmp/' + REV + '/NeumannGeneratedAverageFieldActionIteRepro.lean']
    commands['physical_prerequisites'] = ['lake', 'build', 'YangMills.RG.BalabanCMP99SourceFlatGeneratedQprimeTerminalOwner']
    for s in ('clean_before', 'clean_after'):
        commands[s] = ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'lean-toolchain', 'lake-manifest.json']
    for s, name in [('physical_draft', 'NeumannGeneratedAverageFieldActionDraft.lean')]:
    ...
    for s, path in [('physical_draft', 'tmp/NeumannGeneratedAverageFieldActionDraft.lean')]:
