# OS-R-5 pass 27 -- FRESH-CLONE VERIFICATION (the SHA below is the
# commit that registers the pass-15 wiring numbers, Addendum 624).
# Brand-new clone in /content/osr5fresh (never reuses /content/osr1).
...
SHA = 'fc274db76f7d7f0abf8eb3f55d5ca022014bd9a3'
EXPECT_CORE = 8481
EXPECT_REPORTS = 3050
REPO = pathlib.Path('/content/osr5fresh')
...
for f in ['YangMills/OS/OSReconstructionUniform.lean', 'YangMillsCore.lean',
          ...
          'scripts/osr5_pass27_freshclone.py']:
    p = REPO / f
    if p.exists():
        h = hashlib.sha256(p.read_bytes()).hexdigest()
        print(f'{h}  {p.stat().st_size:>8}  {f}', flush=True)
print('OSR5_FRESHCLONE_DONE', flush=True)
