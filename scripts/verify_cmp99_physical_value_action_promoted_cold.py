#!/usr/bin/env python3
"""Read-only promoted physical value-action cold-graph evidence verifier. No Lean, Git or subprocess calls.
...
COMMANDS = {
    "value_action_focal": [
        "lake",
        "build",
        "YangMills.RG.BalabanCMP99SourceFlowPhysicalValueAction"
    ],
    "value_action_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/BalabanCMP99SourceFlowPhysicalValueActionAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['f5-physical-value-action-cold-contract.json'])
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'f5-physical-value-action-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["BalabanCMP99SourceFlowPhysicalValueAction.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['f5-physical-value-action-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostic_source='7bdc0fa1e76b3e2d3d64d1822bb30459d6697979',
        parent_diagnostic_archive_sha256='1f5f0f549a9a6f46f82096221b37f81e4c3f68591abaf469598d3fecd89609a9',
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
    ...
    print('F5_PHYSICAL_VALUE_ACTION_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('F5_PHYSICAL_VALUE_ACTION_COLD_EVIDENCE_VERIFIED')
