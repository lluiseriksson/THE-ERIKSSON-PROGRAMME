#!/usr/bin/env python3
"""Fresh cold gate for seven promoted integer image-owner declarations.
...
def main():
    ...
    runner = base.runner
    runner.RUNNER_REV = 'neumann-integer-image-promoted-cold-v1'
    ...
    runner.QUEUE = [
        ("reflection_focal", ["lake","build","YangMills.RG.NeumannIntegerImageCountingKernel"], None),
        ("reflection_audit", ["lake","env","lean","YangMills/RG/NeumannIntegerImageCountingKernelAudit.lean"], audit_names["reflection_audit"]),
    ]
    ...
    def make_evidence(status, opened):
        ...
        (runner.EVIDENCE / 'neumann-integer-image-promoted-cold-contract.json').write_text(json.dumps({
            'source_sha': SOURCE,
            ...
            'scope': 'integer image-owner-indicator only; not physical Q intertwining, regional inverse, B0 or window15',
        }, sort_keys=True) + '\n', encoding='utf-8')
        outputs = {}
