"""Authoritative regular K2 cover through delta=0.007."""
...
DEPENDENCIES = (
    "scripts/certify_surface_remainder_delta0_r4_extension_007.py",
    "scripts/surface_remainder_delta0_r4_extension_007_split_cover.py",
    "scripts/surface_remainder_delta0_outer_domain_v4.py",
    "scripts/surface_remainder_delta0_outer_domain_v3.py",
    "scripts/surface_remainder_delta0_outer_domain_v2.py",
    "scripts/surface_remainder_delta0_r4_extension_007_probe.py",
...
GRID_RANGES = ((0, 118, 96), (119, 157, 192))
...
def grid_for(index):
    ...
    if len(matches) != 1:
        raise ValueError("0.007 production grid map is not a partition")
    return matches[0]
...
def main():
    ...
    print("CONFIG delta_max 7/1000 physical_inner 23/20 band_radius 137/10 "
          "core_boxes 0:1/200,1/200:3/500,3/500:7/1000 "
          "annulus_births 0:7 start", args.start_index, "stop",
    ...
    for index in range(args.start_index, args.stop_index):
        ...
        if radius != cover.Fraction(137, 10) or not lower > 0:
            print("CERTIFICATE FAIL index", index, flush=True)
            return 1
        if worst is None or lower < worst[0]:
            worst = (lower, index)
    print("CERTIFIED SEGMENT regular K2 r4 delta [0,7/1000] start",
