...
rt = Path('tmp/colab_neumann_rectangle_directional_masks_hot_template.py').read_text()
...
parent = dict(status='VERIFIED_COLD_PASS', source=r0.BASE, cold_seal=True,
    ...
    production_outputs={'NeumannRectangleWrapProbe.olean': '3'*64}))
...
draft = files['NeumannRectangleDirectionalMasksDraft.lean'].decode()
...
commands = {
    ...
    'mathlib_repro': ['lake', 'env', 'lean', '-o', (r.OUT / 'mathlib_repro.olean').as_posix(),
        'tmp/' + r.REV + '/NeumannRectangleDirectionalMasksRepro.lean'],
    ...
    'physical_draft': ['lake', 'env', 'lean', '-o', (r.OUT / 'physical_draft.olean').as_posix(),
        'tmp/' + r.REV + '/NeumannRectangleDirectionalMasksDraft.lean'],
...
assert literal == {s:commands[s] for s in (
    'base_head','mathlib_pin','clean_before','mathlib_repro','physical_prerequisites','clean_after')}
...
assert len(loops) == 1
assert ast.literal_eval(loops[0].iter) == [('physical_draft','NeumannRectangleDirectionalMasksDraft.lean')]
...
env.update(op=PurePosixPath(r.OUT.as_posix())/'physical_draft.olean',
    path='NeumannRectangleDirectionalMasksDraft.lean')
assert eval(compile(ast.Expression(draft_call.args[1]),'<draft command>','eval'),env) == commands['physical_draft']
repro = files['NeumannRectangleDirectionalMasksRepro.lean'].decode()
assert re.findall(r'^#print axioms (\w+)\s*$',repro,re.M) == r.NAMES['mathlib_repro']
arithmetic = repro[repro.index('theorem rectangleForwardMask_nat'):repro.index('#print axioms')].strip()
assert arithmetic in draft, 'REPRO_ARITHMETIC_DRIFT'
print('ACTUAL_RUNNER_AST_CONTRACT=PASS calls=8 expanded_stages=9 EXACT_REPRO=1')
...
logs['mathlib_repro'] = '\n'.join("'YangMills.RG." + n + "' depends on axioms: [propext, Classical.choice, Quot.sound]" for n in r.NAMES['mathlib_repro']).encode()
records = []
...
bad.append(pack(f, d))
f, d = copy.deepcopy(files), copy.deepcopy(data)
f['mathlib_repro.log'] = f['mathlib_repro.log'].replace(b'Quot.sound', b'Other.axiom')
d['records'][5]['log_sha256'] = sha(f['mathlib_repro.log'])
bad.append(pack(f, d))
...
print('DIRECTIONAL_MASKS_TEMPLATE_SYNTHETIC_SELF_TEST=PASS valid=1 rejected=11 preserved_failure=1')
