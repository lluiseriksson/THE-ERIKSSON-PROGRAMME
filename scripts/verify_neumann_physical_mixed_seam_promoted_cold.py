#!/usr/bin/env python3
"""Exact three-name physical mixed seam cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
REV = 'neumann-physical-mixed-seam-promoted-cold-v1'
...
COMMANDS = {
    ...
    "physical_seam_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannPhysicalMixedSeam"
    ],
    "physical_seam_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannPhysicalMixedSeamAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-physical-mixed-seam-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="literal physical complex Green mixed series with actual FULL flag: summability and periodic/PROPER boundary identities; not source equation, finite operator interchange, inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-physical-mixed-seam-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannPhysicalMixedSeam.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-physical-mixed-seam-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"0d63d2b0f234d3780b60d6ce1242c6aa8e3f67ae","archive_sha256":"aed81d7fb9f7f55b197e0a21c71c4912591b45d3a6f8ea53b0f3218d0de5230c","overlay_sha256":"8b6a3b21158c3d047e76695c0da179511ae7d353a4be8c95a4da26726e73b155"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="literal physical complex Green mixed series with actual FULL flag: summability and periodic/PROPER boundary identities; not source equation, finite operator interchange, inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    ...
    print('NEUMANN_PHYSICAL_MIXED_SEAM_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_PHYSICAL_MIXED_SEAM_COLD_EVIDENCE_VERIFIED')
