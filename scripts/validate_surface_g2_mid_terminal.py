"""Validator for one fresh terminal order-22 mid-gap unit."""
...
def validate(path: Path, lo: Fraction, hi: Fraction) -> int:
    ...
    assert lines[0] == "SCALED BULK SIGN ROW MID TERMINAL ORDER22"
    assert "SCOPE terminal direct-sign mid gap; no H_tail promotion" in lines
...
def main() -> int:
    ...
    print("MID TERMINAL VALIDATION PASS", args.lo, args.hi,
