#!/usr/bin/env python3
"""Exact six-name mass-offset cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-mass-offsets-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="actual generated source-weighted/counting mass finite offsets and common integer image; not regional inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-mass-offsets-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannGeneratedMassCompleteOffsets.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-mass-offsets-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostic_source='6d02bcfd4091fb549dd9856f6fa95bb10e854643',
        parent_diagnostic_archive_sha256='515f57a9898c8ef1460d353e9b85c6547cd9403514ad9521b39b73d30452382b',
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        ...
        scope="actual generated source-weighted/counting mass finite offsets and common integer image; not regional inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    ...
    print('NEUMANN_MASS_OFFSETS_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_MASS_OFFSETS_COLD_EVIDENCE_VERIFIED')
