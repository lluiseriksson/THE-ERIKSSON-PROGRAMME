# OS-R-3 pass 20 -- FRESH-CLONE VERIFICATION (the SHA below is the
# commit that registers the pass-15 wiring numbers, Addendum 624).
# Brand-new clone in /content/osr3fresh (never reuses /content/osr1).
...
SHA = '639296c4d43219b0966e4d273eebd4222ef9396c'
EXPECT_CORE = 8481
EXPECT_REPORTS = 3035
REPO = pathlib.Path('/content/osr3fresh')
...
for f in ['YangMills/OS/OSReconstructionUniform.lean', 'YangMillsCore.lean',
          ...
          'scripts/osr3_pass20_freshclone.py']:
    p = REPO / f
    if p.exists():
        h = hashlib.sha256(p.read_bytes()).hexdigest()
        print(f'{h}  {p.stat().st_size:>8}  {f}', flush=True)
print('OSR3_FRESHCLONE_DONE', flush=True)
