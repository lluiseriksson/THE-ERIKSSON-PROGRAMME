#!/usr/bin/env python3
"""Fail-closed nominal quarantine for measured pytest debt.
...
with a versioned nominal manifest and with the exact comparison-base result.
"""
...
SCHEMA = "pytest-nominal-scientific-debt/v1"
REPORT_SCHEMA = "pytest-failure-inventory/v1"
DECISION_SCHEMA = "pytest-nominal-scientific-debt-decision/v1"
...
def pytest_sessionfinish(session: Any, exitstatus: int) -> None:
    report_path = os.environ.get("PYTEST_NOMINAL_REPORT")
    if not report_path:
        return
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(failures, list) or len(failures) != 9:
        raise RuntimeError("manifest must contain exactly nine nominal failures")
    if not isinstance(refs, list) or len(refs) != 6:
        raise RuntimeError("manifest must contain exactly six measured refs")
    nodeids: set[str] = set()
    for item in failures:
        if not isinstance(item, dict):
            raise RuntimeError("manifest failure entry is not an object")
        required = ("nodeid", "phase", "first_cause", "cause_sha256")
        if any(not isinstance(item.get(key), str) or not item[key] for key in required):
            raise RuntimeError("manifest failure entry is incomplete")
        if item["nodeid"] in nodeids:
            raise RuntimeError("manifest contains a duplicate nodeid")
        nodeids.add(item["nodeid"])
        expected = hashlib.sha256(
            (item["phase"] + "\0" + item["first_cause"]).encode("utf-8")
        ).hexdigest()
        if expected != item["cause_sha256"]:
            raise RuntimeError(f"manifest cause hash mismatch for {item['nodeid']}")
    for ref in refs:
...
def evaluate(
    ...
    known = {_key(item): item for item in manifest["known_failures"]}
    known_nodeids = [item["nodeid"] for item in manifest["known_failures"]]
    first_cause = ""
    ...
    for label, report in (("comparison base", base), ("current", current)):
        ...
        if not isinstance(collected, list):
            ...
            regressions.append({"kind": "invalid inventory", "where": label})
            break
        missing = [nodeid for nodeid in known_nodeids if nodeid not in collected]
        if missing:
            first_cause = f"{label} did not execute nominal nodeid {missing[0]}"
    ...
    for item in base_failures:
        if _key(item) not in known:
            regressions.append({"kind": "comparison-base drift", **item})
    active = set(base_keys)
    for item in current_failures:
        key = _key(item)
        if key not in known:
            regressions.append({"kind": "new failure", **item})
        elif key not in active:
            regressions.append({"kind": "reintroduced repaired failure", **item})
    if regressions:
        cause = regressions[0].get("first_cause") or regressions[0]["kind"]
        return _decision("FAIL", str(cause), base, current, [], [], regressions), 1

    current_set = set(current_keys)
    debt = [known[key] for key in base_keys if key in current_set]
    improvements = [known[key] for key in base_keys if key not in current_set]
    return _decision("PASS", "nominal debt only", base, current, debt, improvements, []), 0
...
def run_pytest(repo: Path, nodeids: list[str], timeout_seconds: int) -> dict[str, Any]:
    ...
    env["PYTEST_NOMINAL_REPORT"] = str(report_path)
    command = [sys.executable, "-m", "pytest", "-q", "-p", "pytest_nominal_debt_guard", *nodeids]
    print("PYTEST_NOMINAL_COMMAND=" + " ".join(command), flush=True)
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="pytest-nominal-base-") as worktree_name:
            ...
            try:
                nodeids = [item["nodeid"] for item in manifest["known_failures"]]
        ...
        decision = _decision("FAIL", str(exc), {}, {}, [], [], [{"kind": "guard error", "detail": str(exc)}])
