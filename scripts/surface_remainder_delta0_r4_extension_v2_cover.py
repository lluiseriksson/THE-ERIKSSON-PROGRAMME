"""Exhaustive corrected-outer design cover on delta in [0,1/200]."""
...
def main():
    ...
    worst = None
    print("R4 V2 COVER DESIGN delta_max 1/200 boxes", len(boxes),
          "grid_ladder", GRID_LADDER, flush=True)
    for index, (lo, hi) in enumerate(boxes):
        passed = None
        for grid in GRID_LADDER:
            try:
                radius, flat, head, c4, value, margin = probe.judge(
            ...
            print("TRY index", index, "t", lo, hi, "grid", grid,
                  "band_radius", radius, "band_e", flat,
            ...
            if lower > 0:
                ...
                break
        if passed is None:
            print("R4 V2 COVER DESIGN FAIL index", index, flush=True)
            return 1
    ...
    print("R4 V2 COVER DESIGN PASS rows", len(boxes),
