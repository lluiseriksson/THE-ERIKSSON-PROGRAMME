#!/usr/bin/env python3
"""Hot retained-runtime diagnostic for the generated two-scale gate endpoint."""

from __future__ import annotations

import re
from pathlib import Path
import subprocess
import time


RUNNER_REV = "cmp89-neumann-generated-two-scale-gate-reachability-hot-v1"
SOURCE_SHA = "8f03fc36022487e7fff32a2989055ad36b628de2"
ROOT = Path("/content/hrpoly-cmp89-neumann-physical-gate-reachability-cold")
EXPECTED_BLOBS = {
    "YangMills/RG/BalabanCMP89SourceNeumannGeneratedTwoScaleGateReachability.lean":
        "ed44169490bf32a8ccdfa80ef730954dda684b10",
    "YangMills/RG/BalabanCMP89SourceNeumannGeneratedTwoScaleGateReachabilityAudit.lean":
        "f062aa65b6422a6fd3e56194ec253cf19521a279",
}
EXPECTED_DECLARATION = (
    "YangMills.RG."
    "exists_pos_cmp89SourceNeumann_generatedTwoScale_physical_absorption_radius"
)
ALLOWED_AXIOMS = {"propext", "Classical.choice", "Quot.sound"}


def run(stage: str, command: list[str]) -> str:
    print("STAGE=" + stage + " CMD=" + repr(command), flush=True)
    started = time.perf_counter()
    child = subprocess.run(
        command,
        cwd=ROOT,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
    )
    elapsed = time.perf_counter() - started
    print(child.stdout, end="" if child.stdout.endswith("\n") else "\n", flush=True)
    print(
        "STAGE=" + stage + " EXIT=" + str(child.returncode)
        + " SECONDS=" + f"{elapsed:.3f}",
        flush=True,
    )
    if child.returncode != 0:
        raise RuntimeError("STAGE_FAILED=" + stage)
    return child.stdout


def main() -> int:
    print("RUNNER_REV=" + RUNNER_REV, flush=True)
    if not ROOT.is_dir():
        raise RuntimeError("RETAINED_ROOT_MISSING=" + str(ROOT))
    run("fetch", ["git", "fetch", "--no-tags", "origin", SOURCE_SHA])
    run("checkout", ["git", "checkout", "--detach", SOURCE_SHA])
    head = run("head", ["git", "rev-parse", "HEAD"]).strip()
    if head != SOURCE_SHA:
        raise RuntimeError("HEAD_MISMATCH=" + head)
    for relative, expected in EXPECTED_BLOBS.items():
        actual = run("blob_" + Path(relative).stem, ["git", "hash-object", relative]).strip()
        print("SOURCE_BLOB=" + relative + " OID=" + actual, flush=True)
        if actual != expected:
            raise RuntimeError("SOURCE_BLOB_MISMATCH=" + relative)
    manifest = Path("/content/cmp89-neumann-generated-two-scale-gate-hot-paths.txt")
    manifest.write_text("\n".join(EXPECTED_BLOBS) + "\n", encoding="utf-8")
    run("overlay_text_guard", [
        "python3", "scripts/check_lean_overlay_text.py",
        "--paths-from", str(manifest), "--require-prevalidation",
    ])
    run("import_prefix_guard", [
        "python3", "scripts/check_lean_import_prefix.py", *EXPECTED_BLOBS,
    ])
    run("focal", [
        "lake", "build",
        "YangMills.RG.BalabanCMP89SourceNeumannGeneratedTwoScaleGateReachability",
    ])
    audit = run("audit", [
        "lake", "env", "lean",
        "YangMills/RG/BalabanCMP89SourceNeumannGeneratedTwoScaleGateReachabilityAudit.lean",
    ])
    compact = re.sub(r"\s+", "", audit)
    for forbidden in ("sorryAx", "ofReduceBool", "Lean.ofReduceBool"):
        if forbidden in compact:
            raise RuntimeError("FORBIDDEN_AXIOM=" + forbidden)
    blocks = re.findall(r"'([^']+)'dependsonaxioms:\[([^\]]*)\]", compact)
    if len(blocks) != 1 or blocks[0][0] != EXPECTED_DECLARATION:
        raise RuntimeError("AXIOM_DECLARATION_MISMATCH=" + repr(blocks))
    axioms = {item for item in blocks[0][1].split(",") if item}
    if not axioms.issubset(ALLOWED_AXIOMS):
        raise RuntimeError("AXIOM_SET=" + repr(sorted(axioms)))
    print(
        "AXIOM_GATE=" + EXPECTED_DECLARATION
        + " AXIOMS=" + ",".join(sorted(axioms)),
        flush=True,
    )
    print("HOT_FINAL_STATUS=PASS", flush=True)
    return 0


if __name__ == "__main__":
    try:
        raise SystemExit(main())
    except Exception as error:
        print("ERROR=" + repr(error), flush=True)
        print("HOT_FINAL_STATUS=FAIL", flush=True)
        raise
