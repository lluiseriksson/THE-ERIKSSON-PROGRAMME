"""Actual producer AST vs independent reader and four source audit names.
...
import verify_neumann_directional_masks_promoted_cold as v
...
tree = ast.parse(Path('scripts/colab_neumann_directional_masks_promoted_cold.py').read_text())
...
audit = blob('YangMills/RG/NeumannRectangleDirectionalMasksAudit.lean').decode()
...
v.verify = original
files = captured[0]
key = 'neumann-directional-masks-promoted-cold-contract.json'
...
for stage in v.NAMES:
    for replacement in (b'', b"'Other.name' depends on axioms: [propext]"):
        ...
        try:
            v.verify(bad, gate, old)
        except (ValueError, RuntimeError):
            continue
        raise AssertionError('BAD_AUDIT_ACCEPTED=' + stage)
print('PER_AUDIT_MISSING_WRONG_NAME_REJECTIONS=2')
print('DIRECTIONAL_MASKS_COLD_PREPARATION=PASS ACTUAL_CONTRACT=1 SOURCE_BLOBS=2 AUDIT_NAMES=4')
