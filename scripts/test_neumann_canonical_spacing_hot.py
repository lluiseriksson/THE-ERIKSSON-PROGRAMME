...
rt = subprocess.check_output(['git', 'cat-file', 'blob',
    '4cf13d2824b78c44779e15da9bd4f861e967fbd2:scripts/colab_neumann_canonical_spacing_hot.py'], timeout=5).decode()
vt = Path('scripts/verify_neumann_canonical_spacing_hot.py').read_text()
r0 = types.ModuleType('actual_runner')
exec(compile(rt, 'actual_runner', 'exec'), r0.__dict__)
assert sha(rt.encode()) == '581705fc93decb6a98b004baf435b9f6059fae3b73d2e8a94e615337d16f0d75'
parent_blob = subprocess.check_output(['git', 'cat-file', 'blob',
    r0.REVIEW_REF + ':' + r0.REVIEW_PATH], timeout=5)
...
assert parent['cold']['production_outputs'] == {
    'NeumannCanonicalPrecisionOffsetAction.olean': r0.PARENT_OLEAN_HASH}
rb = rt.encode()
...
draft = files['NeumannCanonicalSpacingNormalizationDraft.lean'].decode()
...
commands = {
    ...
    'mathlib_repro': ['lake', 'env', 'lean', '-o', (r.OUT / 'mathlib_repro.olean').as_posix(),
        'tmp/' + r.REV + '/NeumannCanonicalSpacingNormalizationRepro.lean'],
    'physical_prerequisites': ["lake","build","YangMills.RG.NeumannCanonicalPrecisionOffsetAction","YangMills.RG.BalabanCMP99SourceFlatGeneratedTerminalBlockCollapse"],
    'physical_draft': ['lake', 'env', 'lean', '-o', (r.OUT / 'physical_draft.olean').as_posix(),
        'tmp/' + r.REV + '/NeumannCanonicalSpacingNormalizationDraft.lean'],
...
data = dict(status='PASS', source=r.SOURCE, base_source=r.BASE, cold_seal=False,
    ...
    scope='canonical fine-to-terminal spacing and counting coefficient normalization only; not boundary inverse, uniform B0 or window15')
...
print('CANONICAL_SPACING_ACTUAL_RUNNER_SYNTHETIC_SELF_TEST=PASS valid=1 rejected=10 preserved_failure=1')
