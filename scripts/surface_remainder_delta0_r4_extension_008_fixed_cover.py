"""Frozen-map exhaustive design cover for the delta<=0.008 candidate."""
...
def grid_for(index):
    ...
    if len(matches) != 1:
        raise ValueError("0.008 design grid map is not a partition")
    return matches[0]
...
def main():
    ...
    print("R4 008 FIXED COVER DESIGN", "segment", args.start_index,
          args.stop_index, "grid_ranges", GRID_RANGES,
    ...
    for index in range(args.start_index, args.stop_index):
        ...
        print("ROW", index, "t", lo, hi, "grid", grid, "radius", radius,
              ...
              flush=True)
        if radius != cover.Fraction(66, 5) or not lower > 0:
            print("ROW DESIGN FAIL", index, grid, "NONPOSITIVE", flush=True)
            return 1
        histogram[grid] += 1
        if worst is None or lower < worst[0]:
            worst = (lower, index)
    print("R4 008 FIXED COVER DESIGN SEGMENT PASS", args.start_index,
          args.stop_index, "rows", args.stop_index-args.start_index,
