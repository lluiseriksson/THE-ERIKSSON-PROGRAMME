"""Authoritative unit driver for the preregistered high-beta lambda-four strip."""
...
UNITS = tuple((start, start + 10) for start in range(100, 200, 10))
DEPENDENCIES = (
    ...
    "docs/SURFACE-HIGH-BETA-G5-LAMBDA4-PREREG-20260728.md",
)
SIDE = arb(5) / 2
...
def near_bounds_lambda4(delta_max: arb):
    """Finite-tail near charge with the registered exp(4) factor."""
...
def budgets_lambda4(delta_max: arb):
    near_u, near_b = near_bounds_lambda4(delta_max)
...
def verify_geometry() -> None:
    eta_max = Fraction(9, 1000) * Fraction(4, 1) / 2
    assert eta_max == Fraction(18, 1000)
...
def main() -> int:
    ...
    print("HIGH-BETA G5 LAMBDA4 UNIT", args.unit, flush=True)
    ...
    print(
        ...
        "lambda_max 4 near_exp exp(4) low_argument_z_ge_4 "
    ...
    for lambda_index in range(*unit):
        for delta_index in range(9):
            ...
            rows += 1
    print(
        "CERTIFIED HIGH-BETA G5 LAMBDA4 UNIT",
        args.unit,
        rows,
        "rows",
        "worst_delta_index",
