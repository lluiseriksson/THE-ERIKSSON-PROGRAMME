#!/usr/bin/env python3
"""Exact three-name canonical Neumann precision cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
SOURCE = 'dd9354a87d2e9d60dac69aa79f287b53e45ba207'
REV = 'neumann-canonical-spacing-promoted-cold-v1'
...
BLOBS = {
    "YangMills/RG/NeumannCanonicalSpacingNormalization.lean": "6126e07692dabf5b3fd55d571af7ad9a5f722550094ae3be1c317de2157aae9a",
    "YangMills/RG/NeumannCanonicalSpacingNormalizationAudit.lean": "c86b482c2b931dd7e77967f90a03a017f1ec87790d2f9d6bd32448d588147fc6"
}
NAMES = {
    "canonical_spacing_audit": [
        "YangMills.RG.neumannCanonicalFourierSpacing_terminal_eq_one",
        "YangMills.RG.neumannCanonicalFourierSpacing_countingCoefficient",
        "YangMills.RG.neumannCanonicalFourierSpacing_countingMassCoefficient"
    ]
}
NAMES = {s: set(names) for s, names in NAMES.items()}
COMMANDS = {
    "canonical_spacing_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannCanonicalSpacingNormalization"
    ],
    "canonical_spacing_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannCanonicalSpacingNormalizationAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-canonical-spacing-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
            parent_diagnostic_source='852d3b82ed5d0ffd2e5c38eb4eeb96c433858bbc',
        parent_diagnostic_archive_sha256='b72c538bc5b8205284396969b9a8b1f8bc45466ad8e2439474e85e92eb4ce733',
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="canonical fine-to-terminal spacing and counting coefficient normalization only; not boundary inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-canonical-spacing-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannCanonicalSpacingNormalization.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-canonical-spacing-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostic_source='852d3b82ed5d0ffd2e5c38eb4eeb96c433858bbc',
        parent_diagnostic_archive_sha256='b72c538bc5b8205284396969b9a8b1f8bc45466ad8e2439474e85e92eb4ce733',
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        ...
        scope="canonical fine-to-terminal spacing and counting coefficient normalization only; not boundary inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    for name in ["NeumannCanonicalSpacingNormalization.olean"]:
    ...
    print('NEUMANN_CANONICAL_SPACING_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_CANONICAL_SPACING_COLD_EVIDENCE_VERIFIED')
