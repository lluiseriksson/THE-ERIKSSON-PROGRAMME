...
RUNNER_HASH = '6334e8d3c6946b8313f24cd47b793e03bad6c8770fc4b4193ac73e7432b10f73'
...
def verify(files):
    ...
    commands = {
        ...
        'physical_prerequisites': ['lake', 'build', 'YangMills.RG.BalabanCMP99SourceFlatGeneratedQprimeDirectOwnerKernel'],
    }
    for s in ('clean_before', 'clean_after'):
        commands[s] = ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'lean-toolchain', 'lake-manifest.json']
    for s, name in [('mathlib_repro', 'NeumannFiniteIntegerCoordinatesMathlibRepro.lean'),
    ...
    for r in data['records']:
        ...
        require(r['timeout_seconds'] == (3600 if s == 'physical_prerequisites' else 120), 'TIMEOUT_BUDGET')
    ...
    for s, path in [('mathlib_repro', 'tmp/NeumannFiniteIntegerCoordinatesMathlibRepro.lean'),
