#!/usr/bin/env python3
"""Exact physical-flag diagnostic reader (not a production seal). Independent contract and output checks. No Lean, Git or subprocess calls.
...
REV = 'neumann-physical-full-flag-diagnostic-v1'
...
COMMANDS = {
    "flag_repro": [
        ...
        "env",
        "lean",
        "tmp/NeumannFullFlagCastRepro.lean"
    ],
    "flag_prerequisite": [
        "lake",
        "build",
    ...
    "flag_diagnostic": [
        ...
        "env",
        "lean",
        "tmp/NeumannPhysicalFullFlagDraft.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-physical-full-flag-diagnostic-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="DIAGNOSTIC ONLY: computed FULL flag and actual directional masks; saved olean is the sealed prerequisite, not a production flag module; no mixed Green, physical inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-physical-full-flag-diagnostic-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannRectangleDirectionalMasks.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-physical-full-flag-diagnostic-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"4f2f812dc06d82e5f0b7d93ad6d91d87ad07fe67","archive_sha256":"9918e9db4d228ca104841936a34d4252df1f1d3925f1ba4ceb7299298b19c574"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="DIAGNOSTIC ONLY: computed FULL flag and actual directional masks; saved olean is the sealed prerequisite, not a production flag module; no mixed Green, physical inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    ...
    print('NEUMANN_PHYSICAL_FULL_FLAG_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_PHYSICAL_FULL_FLAG_COLD_EVIDENCE_VERIFIED')
