...
import subprocess
import types
import verify_neumann_half_cell_phase_diagnostic as v

def gitblob(ref,path):
    return subprocess.check_output(['git','cat-file','blob',ref+':'+path],timeout=5)
runner=gitblob('0505a1567cd48a5a9838e50103be80108324d171','scripts/colab_neumann_half_cell_phase_diagnostic.py')
...
outputs={n:b'SYNTHETIC NOT AN OLEAN' for n in ['NeumannHalfCellPhaseRepro.olean','NeumannEntireAverageHalfCellPhaseDraft.olean']}
f.update(outputs)
...
f['phase-contract.json']=json.dumps(contract).encode()
...
for key in ('phase_repro.log','NeumannEntireAverageHalfCellPhaseDraft.olean'):
...
print('PHASE_DIAGNOSTIC_READER_SYNTHETIC=PASS valid=1 rejected='+str(rejected)+' PRODUCER_QUEUE_AST=PASS COMPILER_CHECKED=0')
