#!/usr/bin/env python3
"""Fresh-checkout cold queue for CMP89 (2.46) integrand periodicity and seam."""
...
spec = importlib.util.spec_from_file_location("cmp89_integrand_seam_base", BASE_RUNNER)
...
EXPECTED = {
    314: {
        "YangMills.RG.cmp89Eq246PhysicalFineToFineGreenIntegrand_periodShift",
    },
    315: {
        "YangMills.RG.cmp89Eq246PhysicalFineToFineGreenIntegrand_boundarySeam",
    },
}
...
runner.RUNNER_REV = "cmp89-eq246-integrand-periodicity-seam-cold-v1"
runner.SOURCE_SHA = "dec1cb163d7d0d2dd5d79270f6591db68a17ec5f"
runner.MIN_RAM_GIB = 11.0
runner.ALLOW_GPU_RUNTIME = False
runner.ROOT = Path("/content/hrpoly-cmp89-eq246-integrand-periodicity-seam-cold-v1")
runner.EVIDENCE = Path("/content/hrpoly-cmp89-eq246-integrand-periodicity-seam-cold-v1-evidence")
runner.ARCHIVE = Path("/content/hrpoly-cmp89-eq246-integrand-periodicity-seam-cold-v1-evidence.tar.gz")
runner.PATH_MANIFEST = Path("/content/hrpoly-cmp89-eq246-integrand-periodicity-seam-cold-v1-paths.txt")
runner.SOURCE_BLOBS = {
...
runner.QUEUE = [
    (
        ...
        ["lake", "build", "YangMills.RG.BalabanCMP89Eq246FineToFineGreenIntegrandPeriodicity"],
        None,
    ),
    (
        "integrand_periodicity_audit",
        ["lake", "env", "lean", "YangMills/RG/BalabanCMP89Eq246FineToFineGreenIntegrandPeriodicityAudit.lean"],
        314,
    ),
    (
        "boundary_seam_focal",
        ["lake", "build", "YangMills.RG.BalabanCMP89Eq246FineToFineGreenBoundarySeam"],
        None,
    ),
    (
        "boundary_seam_audit",
        ["lake", "env", "lean", "YangMills/RG/BalabanCMP89Eq246FineToFineGreenBoundarySeamAudit.lean"],
        315,
    ),
]
