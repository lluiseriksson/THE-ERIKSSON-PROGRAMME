...
ROOT = '/content/hrpoly-neumann-actual-common-translation-diagnostic-v1'
RUNNER_HASH = '55ccd9d7db084238a5b164eb1d53efcd65f03f32078b437b3325d79a3a7218cb'
...
def verify(files):
    ...
    require(data['scope'] == "one-coordinate actual alias permutation and central preservation only; not precision or Green covariance, inverse, B0 or window15", 'PHYSICAL_SCOPE')
    ...
    require(review['outputs']['NeumannActualCommonBlockTranslationDraft.olean'] == runner.PARENT_OLEAN_HASH, 'PARENT_OUTPUT_REVIEW')
    ...
    commands['physical_prerequisites'] = ["lake","build","YangMills.RG.BalabanCMP99SourceAliasReflectionInvolutive","YangMills.RG.BalabanCMP99SourceAliasReflectionStabilizedSolution"]
    for s in ('clean_before', 'clean_after'):
        commands[s] = ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'lean-toolchain', 'lake-manifest.json']
    for s, name in [('physical_draft', 'NeumannCoordinateAliasReflectionDraft.lean')]:
    ...
    for s, path in [('physical_draft', 'tmp/NeumannCoordinateAliasReflectionDraft.lean')]:
