#!/usr/bin/env python3
"""Fresh cold gate for five promoted counting reflection/dictionary declarations.
...
def main():
    ...
    runner = base.runner
    runner.RUNNER_REV = 'neumann-counting-promoted-cold-v1'
    ...
    runner.QUEUE = [
        ("reflection_focal", ["lake","build","YangMills.RG.NeumannGeneratedCountingMassReflection"], None),
        ("reflection_audit", ["lake","env","lean","YangMills/RG/NeumannGeneratedCountingMassReflectionAudit.lean"], audit_names["reflection_audit"]),
        ("dictionary_focal", ["lake","build","YangMills.RG.NeumannRetainedCountingMassDictionary"], None),
        ("dictionary_audit", ["lake","env","lean","YangMills/RG/NeumannRetainedCountingMassDictionaryAudit.lean"], audit_names["dictionary_audit"]),
    ]
    ...
    def make_evidence(status, opened):
        ...
        (runner.EVIDENCE / 'neumann-counting-promoted-cold-contract.json').write_text(json.dumps({
            'source_sha': SOURCE,
            ...
            'scope': 'full-carrier counting reflection and retained terminal mass dictionary only; not arbitrary regional invariance, Neumann inverse, derivative B0 or window15',
        }, sort_keys=True) + '\n', encoding='utf-8')
        outputs = {}
