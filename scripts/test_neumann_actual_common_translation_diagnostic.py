...
import subprocess
import types
import verify_neumann_actual_common_translation_diagnostic as v

def gitblob(ref,path):
    return subprocess.check_output(['git','cat-file','blob',ref+':'+path],timeout=5)
runner=gitblob('6a6a6b2252421b57d3de0a18814098598094ba64','scripts/colab_neumann_actual_common_translation_diagnostic.py')
...
outputs={n:b'SYNTHETIC NOT AN OLEAN' for n in ['NeumannActualFullSolutionScalarDraft.olean','NeumannCommonBlockEndpointPhaseDraft.olean','NeumannActualCommonBlockTranslationDraft.olean']}
f.update(outputs)
...
f['translation-contract.json']=json.dumps(contract).encode()
...
for key,value in [('exit',7),('timed_out',True),('command',['lake','build','Wrong']),('seconds',float('nan'))]:
    g=copy.deepcopy(f);d=copy.deepcopy(data);d['records'][-2][key]=value;g['translation_physical.json']=json.dumps(d['records'][-2]).encode();g['evidence.json']=json.dumps(d).encode();reject(g)
for replacement in ('sorryAx','ofReduceBool','Unknown.axiom'):
    g=copy.deepcopy(f);d=copy.deepcopy(data);g['translation_physical.log']=g['translation_physical.log'].replace(b'Quot.sound',replacement.encode());d['records'][-2]['output_sha256']=v.sha(g['translation_physical.log']);g['translation_physical.json']=json.dumps(d['records'][-2]).encode();g['evidence.json']=json.dumps(d).encode();reject(g)
...
g=copy.deepcopy(f);c=copy.deepcopy(contract);c['cold_seal']=True;g['translation-contract.json']=json.dumps(c).encode();reject(g)
...
print('ACTUAL_TRANSLATION_DIAGNOSTIC_READER_SYNTHETIC=PASS valid=1 rejected='+str(rejected)+' PRODUCER_QUEUE_AST=PASS COMPILER_CHECKED=0')
