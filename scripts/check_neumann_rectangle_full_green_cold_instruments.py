...
runner = ast.parse(Path('scripts/colab_neumann_rectangle_full_green_cohort_cold.py').read_text())
verifier = ast.parse(Path('scripts/verify_neumann_rectangle_full_green_cohort_cold.py').read_text())
...
assert sum(map(len, names.values())) == 8
assert list(commands) == ['rectangle_focal', 'rectangle_audit', 'full_green_focal', 'full_green_audit']
...
print('PINNED_SOURCE_BLOBS_NAMES_QUEUE=PASS files=4 names=8')
launcher = ast.parse(Path('scripts/launch_neumann_rectangle_full_green_cohort_cold.py').read_text())
preserver = ast.parse(Path('scripts/preserve_verify_neumann_rectangle_full_green_cohort_cold.py').read_text())
nb = json.loads(Path('scripts/colab_neumann_rectangle_full_green_cohort_cold.ipynb').read_text())
...
check = runpy.run_path('scripts/preserve_verify_neumann_rectangle_full_green_cohort_cold.py', run_name='instrument_test')
...
script = 'scripts/verify_neumann_rectangle_full_green_cohort_cold.py'
