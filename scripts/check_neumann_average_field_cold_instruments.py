...
runner = ast.parse(Path('scripts/colab_neumann_average_field_cohort_cold.py').read_text())
verifier = ast.parse(Path('scripts/verify_neumann_average_field_cohort_cold.py').read_text())
...
assert list(commands) == ['average_focal', 'average_audit', 'field_focal', 'field_audit']
...
print('PINNED_SOURCE_BLOBS_NAMES_QUEUE=PASS files=4 names=6')
launcher = ast.parse(Path('scripts/launch_neumann_average_field_cohort_cold.py').read_text())
preserver = ast.parse(Path('scripts/preserve_verify_neumann_average_field_cohort_cold.py').read_text())
nb = json.loads(Path('scripts/colab_neumann_average_field_cohort_cold.ipynb').read_text())
...
check = runpy.run_path('scripts/preserve_verify_neumann_average_field_cohort_cold.py', run_name='instrument_test')
...
contract_name = 'neumann-average-field-cohort-cold-contract.json'
...
script = 'scripts/verify_neumann_average_field_cohort_cold.py'
