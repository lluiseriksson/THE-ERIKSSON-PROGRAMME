#!/usr/bin/env python3
"""Exact five-name selector/opposite cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
REV = 'neumann-alias-physical-integral-promoted-cold-v1'
...
BLOBS = {
    "YangMills/RG/NeumannAliasPhysicalIntegral.lean": "060762d55ef929b942b3dd8ffc4334312ef59a263ff4a7c755e9f0ccdc96d7b4",
    "YangMills/RG/NeumannAliasPhysicalIntegralAudit.lean": "1e509234054fb85ec03270f0ea1a43dba588c650aeedbf3863ccdda8a8748592"
}
NAMES = {
    "integral_audit": [
...
COMMANDS = {
    "integral_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannAliasPhysicalIntegral"
    ],
    "integral_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannAliasPhysicalIntegralAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-alias-physical-integral-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
            parent_diagnostics=[{"source":"858bbc063675f71f5cc60bf935d957e63e924147","archive_sha256":"a1dc16653121c137d2beb05ff52abcf086dd75353ae06d17de097b319326b44f","source_blobs":{"tmp/NeumannAliasPhysicalIntegralDraft.lean":"1e681883e56f138def4374eb0c03d727cf6166e7b9565368855670188b185f76"}}],
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="literal normalized finite alias integral with explicit N^4; not physical inverse, density/counting transport, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-alias-physical-integral-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannAliasPhysicalIntegral.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-alias-physical-integral-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"858bbc063675f71f5cc60bf935d957e63e924147","archive_sha256":"a1dc16653121c137d2beb05ff52abcf086dd75353ae06d17de097b319326b44f","source_blobs":{"tmp/NeumannAliasPhysicalIntegralDraft.lean":"1e681883e56f138def4374eb0c03d727cf6166e7b9565368855670188b185f76"}}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="literal normalized finite alias integral with explicit N^4; not physical inverse, density/counting transport, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    for name in ["NeumannAliasPhysicalIntegral.olean"]:
    ...
    print('NEUMANN_SELECTOR_OPPOSITE_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_SELECTOR_OPPOSITE_COLD_EVIDENCE_VERIFIED')
