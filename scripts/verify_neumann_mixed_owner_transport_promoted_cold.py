#!/usr/bin/env python3
"""Exact seven-name mixed-owner cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
REV = 'neumann-mixed-owner-transport-promoted-cold-v1'
...
COMMANDS = {
    ...
    "owner_audit": [
        ...
        "env",
        "lean",
        "YangMills/RG/NeumannMixedOwnerTransportAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-mixed-owner-transport-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="exact common mixed-index owner transport and literal counting coefficient; not physical carrier classification, physical inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-mixed-owner-transport-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannMixedOwnerTransport.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-mixed-owner-transport-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"bab8c90d09f7dfeaf7965ce3b0381a96bfabd19e","archive_sha256":"551bda077375b7ecf64560ee0a58537ed22356f1676674e6f6508d063117301c","overlay_sha256":"898441dee7921c9f235e6142a11082f3fc034977c257d4fcee6ab27f0966c6d2"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="exact common mixed-index owner transport and literal counting coefficient; not physical carrier classification, physical inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    ...
    print('NEUMANN_MIXED_OWNER_TRANSPORT_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_MIXED_OWNER_TRANSPORT_COLD_EVIDENCE_VERIFIED')
