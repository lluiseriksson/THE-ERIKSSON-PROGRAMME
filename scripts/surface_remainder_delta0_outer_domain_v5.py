"""Byte-separate outer-domain extension registered through delta=0.008."""
...
def _require_delta(delta_max):
    ...
    if not Fraction(0) < delta_max <= MAX_DELTA:
        raise ValueError("v5 outer-domain contract ends at 1/125")
    return aq(delta_max)
...
def annulus_derivative_bounds_box_to(
        delta_lo, delta_hi, physical_inner=Fraction(23, 20),
        inner=12, outer=32, width=Fraction(1, 2)):
    """V5-local annulus bounds; no older domain cap or cache is reused."""
    ...
    if not Fraction(0) <= delta_lo < delta_hi <= MAX_DELTA:
        raise ValueError("delta subbox is outside the v5 contract")
