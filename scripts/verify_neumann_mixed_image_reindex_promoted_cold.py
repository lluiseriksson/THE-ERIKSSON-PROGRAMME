#!/usr/bin/env python3
"""Exact seven-name mixed image reindex cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
REV = 'neumann-mixed-image-reindex-promoted-cold-v1'
...
COMMANDS = {
    ...
    "reindex_audit": [
        ...
        "env",
        "lean",
        "YangMills/RG/NeumannMixedImageReindexAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-mixed-image-reindex-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="same mixed index FULL translation and PROPER reflection permutations with totalized tsum reindexing; not physical endpoint transfer, inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-mixed-image-reindex-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannMixedImageReindex.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-mixed-image-reindex-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"c22daed8b254ba08bbd500084a79504f597ba48a","archive_sha256":"f81d4abdff21e76951e852204f1299ebf20d2397d3bc389d27f550d2b9580ceb","overlay_sha256":"e1517f8173f02b4dbcc1bd8104b7f73cbb24b968438a0422043799a94dc1deba"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="same mixed index FULL translation and PROPER reflection permutations with totalized tsum reindexing; not physical endpoint transfer, inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    ...
    print('NEUMANN_MIXED_IMAGE_REINDEX_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_MIXED_IMAGE_REINDEX_COLD_EVIDENCE_VERIFIED')
