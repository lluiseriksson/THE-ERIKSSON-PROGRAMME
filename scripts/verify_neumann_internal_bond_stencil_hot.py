...
ROOT = '/content/hrpoly-neumann-canonical-spacing-promoted-cold-v1'
RUNNER_HASH = '6894d5bcfedebfda7427e6596883a7fa43cb45a025ad2347437fb484449428ac'
...
def verify(files):
    ...
    require(data['scope'] == "actual Neumann internal-bond adjoint and finite divergence only; not reflected Green boundary law, regional inverse, uniform B0 or window15", 'PHYSICAL_SCOPE')
    ...
    require(review['cold']['production_outputs'] == {'NeumannCanonicalSpacingNormalization.olean': runner.PARENT_OLEAN_HASH}, 'PARENT_OUTPUT_REVIEW')
    ...
    commands['mathlib_repro'] = ['lake', 'env', 'lean', '-o', '/content/' + REV + '-evidence/mathlib_repro.olean',
        'tmp/' + REV + '/NeumannInternalBondStencilRepro.lean']
    commands['physical_prerequisites'] = ["lake","build","YangMills.RG.BalabanCMP89SourceNeumannRegionalPrecision","YangMills.RG.BalabanCMP99ActiveRegionSourceCovariantAdjointStencil"]
    for s in ('clean_before', 'clean_after'):
        commands[s] = ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'lean-toolchain', 'lake-manifest.json']
    for s, name in [('physical_draft', 'NeumannInternalBondStencilDraft.lean')]:
    ...
    for s, path in [('physical_draft', 'tmp/NeumannInternalBondStencilDraft.lean')]:
