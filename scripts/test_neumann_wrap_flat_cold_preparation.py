"""Actual producer AST vs independent reader and three source audit names.
...
import verify_neumann_wrap_flat_promoted_cold as v
...
tree = ast.parse(Path('scripts/colab_neumann_wrap_flat_promoted_cold.py').read_text())
...
contracts = [n for n in ast.walk(tree) if isinstance(n, ast.Dict) and any(
    isinstance(k, ast.Constant) and k.value == 'parent_diagnostics' for k in n.keys)]
...
audit = '\n'.join(blob(p).decode() for p in v.BLOBS if p.endswith('Audit.lean'))
names = [l.removeprefix('#print axioms ') for l in audit.splitlines()
...
v.verify = original
files = captured[0]
key = 'neumann-wrap-flat-promoted-cold-contract.json'
...
for field,value in [('parent_diagnostics','wrong'),('scope','regional inverse'),
...
for stage in v.NAMES:
    for replacement in (b'', b"'Other.name' depends on axioms: [propext]"):
        bad = copy.deepcopy(files)
        data = json.loads(bad['evidence.json'])
        bad[stage + '.log'] = replacement
        record = next(r for r in data['records'] if r['stage'] == stage)
        record['output_sha256'] = v.sha(replacement)
        bad[stage + '.json'] = json.dumps(record).encode()
        bad['evidence.json'] = json.dumps(data).encode()
        try: v.verify(bad, gate, old)
        except (ValueError, RuntimeError): continue
        raise AssertionError('BAD_AUDIT_ACCEPTED=' + stage)
print('PER_AUDIT_MISSING_WRONG_NAME_REJECTIONS=4')
print('WRAP_FLAT_COLD_PREPARATION=PASS ACTUAL_CONTRACT=1 SOURCE_BLOBS=4 AUDIT_NAMES=3')
