#!/usr/bin/env python3
"""Exact three-name fixed-source cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
SOURCE = '8da9ffc57d7de329501fc5ba5d8084cdcbe5fb4f'
REV = 'neumann-mixed-fixed-source-promoted-cold-v1'
...
BLOBS = {
    "YangMills/RG/NeumannMixedFixedSource.lean": "ba89febbf4e1c2276c232627a59dec8052004c0398265b69e1866aaeb85dfaeb",
    "YangMills/RG/NeumannMixedFixedSourceAudit.lean": "2ac4868a1579ab62584124b7c325621252f57848aa4447a3cf0d297cb2473be3"
}
NAMES = {
    "fixed_audit": [
        "YangMills.RG.neumannMixedInsertSource_injective",
        "YangMills.RG.neumannMixedFixedSourceImage_injective",
        "YangMills.RG.neumannMixedFixedSourceDifference_injective"
    ]
}
...
COMMANDS = {
    "fixed_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannMixedFixedSource"
    ],
    "fixed_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannMixedFixedSourceAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-mixed-fixed-source-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
            parent_diagnostics=[{"source":"ddbdd8913f91ff778594756bbecaad0d804dfb6c","archive_sha256":"85070e6d0aa48435546728425f1261863ce8130205fd1b6e1cba5e3585b1413f"}],
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="fixed-source mixed image and difference injectivity; not full coverage at fixed source, physical carrier classification, physical inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-mixed-fixed-source-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannMixedFixedSource.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-mixed-fixed-source-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"ddbdd8913f91ff778594756bbecaad0d804dfb6c","archive_sha256":"85070e6d0aa48435546728425f1261863ce8130205fd1b6e1cba5e3585b1413f"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="fixed-source mixed image and difference injectivity; not full coverage at fixed source, physical carrier classification, physical inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    for name in ["NeumannMixedFixedSource.olean"]:
    ...
    print('NEUMANN_MIXED_FIXED_SOURCE_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_MIXED_FIXED_SOURCE_COLD_EVIDENCE_VERIFIED')
