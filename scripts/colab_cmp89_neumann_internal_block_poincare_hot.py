#!/usr/bin/env python3
"""Retained-checkout diagnostic for the one-block Neumann Poincare bound."""
...
def main() -> int:
    ...
    run("import_prefix_guard", ["python3", "scripts/check_lean_import_prefix.py",
                                 *SOURCE_BLOBS])
    run("cmp89_neumann_internal_block_poincare_focal", ["lake", "build",
        "YangMills.RG.BalabanCMP89SourceNeumannInternalBlockPoincare"])
    audit = run("cmp89_neumann_internal_block_poincare_audit", ["lake", "env",
        "lean", "YangMills/RG/BalabanCMP89SourceNeumannInternalBlockPoincareAudit.lean"])
    audit_axioms(audit)
