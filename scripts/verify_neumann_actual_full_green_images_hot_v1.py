...
RUNNER_HASH = '3c255db7a510f8ff26e95afa3ca5ab8456d5ae791fb4f69340208efad57caa45'
...
def verify(files):
    ...
    commands['physical_prerequisites'] = ['lake', 'build', 'YangMills.RG.BalabanCMP89NeumannRectangularPhysicalGreenInsertion', 'YangMills.RG.BalabanCMP89Eq246MassUniformCenteredGreenFourierSummability']
    for s in ('clean_before', 'clean_after'):
        commands[s] = ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'lean-toolchain', 'lake-manifest.json']
    for s, name in [('physical_draft', 'NeumannActualFullGreenReflectionSummabilityDraft.lean')]:
    ...
    for r in data['records']:
        ...
        require(r['timeout_seconds'] == (1800 if s == 'physical_prerequisites' else 120), 'TIMEOUT_BUDGET')
    ...
    for s, path in [('physical_draft', 'tmp/NeumannActualFullGreenReflectionSummabilityDraft.lean')]:
