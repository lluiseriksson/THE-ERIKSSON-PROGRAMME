...
| G2 | analytic bulk tail `(H_tail)` | `HEAD_EXACT_TAIL_OPEN` | the regular delta-zero lane independently derives exact `T`, `r2`, `r3`; Bessel companions, phase/prefactor centering, fixed completion and outer value tail are rigorous; still required is the head-subtracted uniform delta remainder (plus ordered coverage through `1/20`) in the literal S2''' or direct joint judge |
