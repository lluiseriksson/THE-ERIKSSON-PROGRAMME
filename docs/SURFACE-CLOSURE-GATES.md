...
| G2 | analytic bulk tail `(H_tail)` | `REGULAR_008_CERTIFIED_POSITIVE_PARTIAL` | the regular lane independently derives exact `T`, `r2`, `r3`, `r4`, `r5`, and `r6`; eight manifested production segments close `[0,1/125] x [0,pi]` on all 158 born `t` boxes using four incremental core boxes, eight annulus births, physical split `1181/1000`, band radius `66/5`, a frozen mixed `384/192/384` grid map, and worst strict lower margin `0.0000111964` at index 50, absorbing positive births 1--7; the earlier unparameterized transcripts remain quarantined; the independent positive stress descendant remains closed; the 42 births `j=8,...,49` remain required through `1/20` |
