...
| G2 | bulk closure above beta 20 | `BLOCKED` | The finite role is now proved: `scripts/audit_surface_finite_role_relay.py` combines Theorem A, exact `4F_B^2E'=W`, positive common scaling, the corrected exact-union bulk subcover, certified G4, and certified G5 to conclude `E'<0` on `20<=beta<=1000/9`, all `0<t<pi`.  The high-beta role remains blocked: K2 must be stated as the direct joint-remainder replacement for the old split-tail route and bound to the unconditional mirror estimate and G5 without a domain gap.  No literal `(H_tail)` promotion is permitted. |
