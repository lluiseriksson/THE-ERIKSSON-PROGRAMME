...
| G2 | analytic bulk tail `(H_tail)` | `BLOCKED` | The exact implication `4F_B^2E'=W` and positive scaling `W^J=e^{-8\beta}W` pass dependency-free algebra checks (`scripts/verify_surface_direct_sign_relay.py`).  The corrected authoritative moving-seam audit (at `beta_hi`) now reports the exact union `[20,1000/9]`, no rational gap, and a complete adjacent canonical subcover.  Its `relay_status` deliberately remains `RELAY_LEMMA_UNPROVED`: a separate role audit must bind this cover to Theorem A and the exact direct-sign relay before G2 or the manuscript can move. |
