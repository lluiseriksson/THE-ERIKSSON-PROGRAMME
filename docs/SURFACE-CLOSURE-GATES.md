...
| G2 | analytic bulk tail `(H_tail)` | `REGULAR_006_CERTIFIED_POSITIVE_PARTIAL` | the regular lane independently derives exact `T`, `r2`, `r3`, `r4`, `r5`, and `r6`; the current production pair closes `[0,3/500] x [0,pi]` on all 158 born `t` boxes using two core boxes, six annulus births, physical split `11/10`, band radius `71/5`, and worst strict lower margin `0.0303077`, absorbing positive births 1--5; the earlier unparameterized transcripts remain quarantined; the independent positive stress descendant remains closed; the 44 births `j=6,...,49` remain required through `1/20` |
