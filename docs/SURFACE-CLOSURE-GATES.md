...
| G5 | moving right edge | `COMPACT_CERTIFIED_HALFLINE_OPEN_REPAIR` | the manifested six-transcript Arb union closes `pi-1.5/beta<=t<pi` for every `3<=beta<=20` (410 contiguous beta boxes, 410 normalized boxes, 6,448 regular boxes); for `beta>=125`, exact divided differences remove the endpoint zeros and reduce the target to five scaled families with `H=P0/(4 B0^2)`; the moving-saddle geometry and uniform tail budgets are audited, and row 63 plus its Fourier oracle regression are green; however, a clean-process rerun of upper mixed row 74 returned indeterminate `P0` and `H`, so the earlier untranscripted 75-row design line is withdrawn under `INC-G5-HALFLINE-NONREPRODUCIBLE-DESIGN`; stronger mixed grids, the frozen production transcript, executable coverage validator, independent execution rerun, and finite overlap `20<beta<125` remain terminal work |
