...
| G2 | analytic bulk tail `(H_tail)` | `HEAD_EXACT_TAIL_OPEN` | the regular delta-zero lane independently derives exact `T`, `r2`, `r3`, `r4`; Bessel companions, phase/prefactor centering, fixed completion and outer value/derivative tails are rigorous; the fully charged `[0,1/1000]` endpoint cover is in production, while the uniform `delta^4` remainder and ordered positive-box coverage through `1/20` remain required in the literal S2''' or direct joint judge |
...
stress cell. A design-only second-order spatial Taylor model now contains independently
computed pure and mixed derivatives and exhibits strong contraction under mesh
refinement. It is not yet an S1''' or S2''' certificate, does not treat the completion,
and does not cross `delta=0`. Its next promotion condition is an adversarially tested
literal weighted-sum driver, not a larger unmanifested numerical run.
