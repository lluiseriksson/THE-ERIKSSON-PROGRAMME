...
| G2 | analytic bulk tail `(H_tail)` | `OPEN_DESIGN` | the Bessel value/ratio source now has a convergent uniform fifth-order integral remainder; still required are the common exponent/geometry remainder, either the literal S2''' judge or the direct joint inequality for repaired `Y=X_main/delta`, the analytic `delta=0` patch, and uniform parameter coverage |
