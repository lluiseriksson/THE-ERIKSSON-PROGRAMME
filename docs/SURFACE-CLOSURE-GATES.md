...
An order-22 repair passed units 32--54 without changing the mesh or stopping
rule, for 4,187 rows with exact production/replay equality.  Its separate
...
An independent chain audit now checks the twenty-three repair units for exact beta
adjacency, per-unit t adjacency, row-count agreement, and strictly negative
upper fields.  It passes with 4,187 rows via
