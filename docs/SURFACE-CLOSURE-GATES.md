...
An order-22 repair passed units 32--48 without changing the mesh or stopping
rule, for 3,021 rows with exact production/replay equality.  Its separate
...
An independent chain audit now checks the seventeen repair units for exact beta
adjacency, per-unit t adjacency, row-count agreement, and strictly negative
upper fields.  It passes with 3,021 rows via
