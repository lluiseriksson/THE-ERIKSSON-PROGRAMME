...
predicate all twenty preregistered rescue units on `[81,86]` now pass
production/replay and validation.  The corrected exact-rational union audit
reports the single component `[20,1000/9]`, no gap, and a complete adjacent
canonical subcover.  Raw archive adjacency remains false only because valid
historical boxes overlap; see
`INCIDENT-G2-AUDIT-OVERLAP-UNION-20260727.md`.  G2 remains blocked pending a
separate role audit.
...
An additional `1/512` beta-width high-split probe on the first residual box
`[3409/32,54545/512]` also failed at
`t=3.0615057356714304` after 590 seconds.  It is recorded in
...
box `[3409/32,1705/16]` with 249 adjacent strict-negative rows, followed by
`[1705/16,853/8]` with 261 rows.  Production and replay are byte-identical;
...
`[429/4,215/2]`, followed by `ladder06` on `[215/2,431/4]`, `ladder07`
on `[431/4,108]`, `ladder08` on `[108,433/4]`, `ladder09` on
`[433/4,217/2]`, `ladder10` on `[217/2,435/4]`, `ladder11` on
`[435/4,109]`, `ladder12` on `[109,437/4]`, `ladder13` on
...
| G2 | bulk closure above beta 20 | `BLOCKED` | The finite role is proved on `20<=beta<=1000/9`, all `0<t<pi`.  On the high-beta half-line, the non-bilinear term satisfies `Q>19/20`, the direct G5 production/replay union now closes `3/2<=lambda<=2`, the far pure-mirror perturbation is below `1e-30`, and the independently assembled third-block rest perturbation is below `4.489e-6`.  The remaining interior obligation is the two principal-square ratio signs in `SURFACE-HIGH-BETA-RATIO-REDUCTION-20260727.md`.  K2 must be stated as the direct joint-remainder replacement for the old split-tail route; no literal `(H_tail)` promotion is permitted. |
