...
| G2 | analytic bulk tail `(H_tail)` | `REGULAR_005_V2_CERTIFIED_POSITIVE_PARTIAL` | the regular lane independently derives exact `T`, `r2`, `r3`, `r4`, `r5`, and `r6`; the corrected v2 production pair closes `[0,1/200] x [0,pi]` on all 158 born `t` boxes with explicit moving-band radius 14 and worst strict lower margin `0.0269577`, absorbing positive births 1--4; the earlier unparameterized `1/250` and `1/200` transcripts remain quarantined; the degree-five/order-four positive driver independently closes the single descendant `[0.0495,0.05] x [2.9,2.92]`; the 45 births `j=5,...,49` remain required through `1/20` |
