...
| G2 | analytic bulk tail `(H_tail)` | `ENDPOINT_001_CERTIFIED_POSITIVE_POINT` | the regular lane independently derives exact `T`, `r2`, `r3`, `r4`, `r5`, and `r6`; the current endpoint certificate closes `[0,1/1000] x [0,pi]`; the attempted `1/250` and exact-`r4` `1/200` extensions are quarantined because their outer annulus and moving-band charge retained the endpoint-only `delta_max=0.001` domain; the degree-five/order-four positive driver independently closes the single descendant `[0.0495,0.05] x [2.9,2.92]`, but no complete positive birth inherits that point result; all 49 positive delta births remain required through `1/20` |
