...
| G2 | analytic bulk tail `(H_tail)` | `REGULAR_007_CERTIFIED_POSITIVE_PARTIAL` | the regular lane independently derives exact `T`, `r2`, `r3`, `r4`, `r5`, and `r6`; the current production pair closes `[0,7/1000] x [0,pi]` on all 158 born `t` boxes using three incremental core boxes, seven annulus births, physical split `23/20`, band radius `137/10`, and worst strict lower margin `0.0190953`, absorbing positive births 1--6; the earlier unparameterized transcripts remain quarantined; the independent positive stress descendant remains closed; the 43 births `j=7,...,49` remain required through `1/20` |
