...
| G2 | analytic bulk tail `(H_tail)` | `BLOCKED` | The exact algebraic implication `4F_B^2E'=W` and positive scaling `W^J=e^{-8\beta}W` now have a dependency-free executable check (`scripts/verify_surface_finite_w_sign_relay.py`).  The current admissibility audit still finds three uncovered beta gaps (`[193/4,69]`, `[81,401/4]`, `[102,1000/9]`), `beta_union_complete=false`, and `relay_status=RELAY_LEMMA_UNPROVED`; the scaled derivative-tail contract and exhaustive finite cover remain unproved, so no G2 promotion is permitted. |
