...
| 12b | **localized covariance compression bridge** | **open source-facing prerequisite**: the generic finite-matrix construction `P_S C P_S` and its positive `CFC.sqrt` root are present in PRE-VALIDATION source, including support, PSD, and norm contraction; still open are the identification of `C` with the coordinate matrix of the literal interacting inverse, the inverse-coercivity bound, and the lower nondegeneracy certificate | tree + absent: CMP99 [14] for the physical inverse/coercivity discharge |
