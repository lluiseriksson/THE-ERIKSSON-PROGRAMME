...
### Step 8b.24/C6c.9: per-depth physical CMP99 (3.42) assembler (SEALED; uniform bridge open; 20/41 unchanged)

Source checkpoint `165d6288ede6ae354d8b82c690a9a7ca564ba62e`
assembles the four sealed per-depth physical components into one literal
`CMP99Eq342SourceLocalizedGreenCertificate`.  Fresh Colab Pro+ revision
`regional-prefix-eq342-certificate-v3` completed the 8,703-job focal and its
audit, which reported exactly `[propext, Classical.choice, Quot.sound]`.
Seal checkpoint `61d0b71323c54a5381a01e2cf8cbe2def80956b9` retires exactly
the two assembler PRE-VALIDATION marks and imports the audit into
`YangMillsCore`.

The theorem uses terminal spacing `L^(depth+1) * spacing` and one common
amplitude, but both its amplitude and rate still depend on `depth`; it is not
the source-uniform `B0`/`delta0` certificate, does not attain window 15 and
does not move the terminal counter.
...
exponential transport internally and preserves the same canonical rate and
the same single `L^(2*(depth+1))` source-scale payment.  It accepts no free
metric, owner-decay bound, regional Green or coercivity certificate.
...
pins that exact checkpoint and exactly those two source blobs.

Both files remain visibly PRE-VALIDATION and have passed only lightweight
textual/import-prefix checks; no Lean or Lake ran on Windows.  Uniform
...
The v1 Colab run stopped at the focal after `4765.143` seconds.  The source
had not imported the existing backward-owner exponential transport theorem,
and its final four-direction scalar normalization left one algebraic goal
after denominator clearing; no audit ran.  The runner recorded evidence
SHA-256
...
A hot rerun of only that failed focal recovered the exact messages in a log
with SHA-256
`8CCBD5F3F697E1FC27824F9F9BF1DBBF459E493C4AA6858C887DECD72EEE5AAB`;
it is diagnostic evidence, not a seal.

Corrected PRE-VALIDATION checkpoint
`5e1cb84f8ca1c7b47b3af77e252afcac63937b71` adds only the missing import,
applies `ring` to every goal generated by `field_simp`, and removes the
unused simplifier argument.  No statement, hypothesis or constant changes.
The corrected source Git-blob SHA-256 is
`BCCD2573B0FD0FFC39FE69A3CE5ABE875C48664FE430FC2580EE698DBF9C9620`.
Runner checkpoint `ea8326f75193ddbff818aae638c8ac499e9cf894`
contains revision `regional-prefix-laplacian-owner-decay-v2`, Git-blob
SHA-256
...
### Step 8b.24/C6c.9h: literal source-flow full complex precision dictionary (COLAB-VERIFIED; 20/41 unchanged)

`BalabanCMP99SourceFlowFlatPhysicalPrecisionComplexDictionary` composes the
two independently sealed summands at each generated active target.  The
canonical complexification of the literal covariant Laplacian plus the
source-flow counting-space `Q'^*Q'` mass is identified with the full-box
complex precision action at bare mass zero and the printed weighted
coefficient `cmp99SourceFlowFlatFullComplexA a M depth`.  Both summands and
the scalar normalization are constructed internally; no full precision,
inverse or Green equality is accepted from the caller.

Source checkpoint `867325d51c978816a58436fcd11675f2a17eea30` fixed the
source/audit blobs at SHA-256
...
instrumental checkpoint `5d7034ce630f04d860529b7c40915e87f32835a2` passed in a
fresh Colab Pro+ CPU/high-RAM checkout with Mathlib pinned at
`07642720480157414db592fa85b626dafb71355b`.  The focal completed 8,648
jobs in `2413.757` seconds and the audit in `10.083` seconds.  Its sole
declaration printed exactly `[propext, Classical.choice, Quot.sound]`; the
...
Seal checkpoint `32d8288fd7cac9963d7b399619cc7a3b72a3a476` retires exactly the two
PRE-VALIDATION marks and imports the audit into `YangMillsCore`.

This is the literal full precision dictionary, not yet its separated-carrier
complex inverse, physical Green identification or a regional
...
### Step 8b.24/C6c.9j: source-separated Step-7b precision dictionary (COLAB-VERIFIED; 20/41 unchanged)

`BalabanCMP99SourceSeparatedSourceFlowFlatPhysicalStep7bPrecisionDictionary`
identifies the canonical complexification of the literal source-separated
ambient precision with the full-box Step-7b precision carrying the printed
coefficient `cmp99SourceFlowFlatFullComplexA a L depth`.  The equality is
first constructed on ambient real fields from the sealed real physical
dictionary and the full-box physical complexification, then extended to every
complex field by the explicit real/imaginary decomposition and complex
linearity.  No complex precision equality or inverse is accepted as input.

PRE-VALIDATION source checkpoint
`27c05dc43db1b9f311018dba11ed2847e4010dc4` fixed source/audit Git-blob
SHA-256 values
...
`source-flow-separated-step7b-precision-v1`, passed in a fresh Colab Pro+
CPU/high-RAM checkout.  The focal completed 8,663 jobs in `2394.835` seconds
and the audit in `19.437` seconds.  Both declarations printed exactly
...
Seal checkpoint `c3c4984abd0c98c0e3b71ef9bafeea26f3b8a339` retires exactly the two
PRE-VALIDATION marks and imports the audit into `YangMillsCore`.  This is the
Step-7b precision action dictionary, not yet a physical Green identification,
...
### Step 8b.24/C6c.9q: literal source-flow point-source owner bound (COLAB-VERIFIED; 20/41 unchanged)

`BalabanCMP99SourceSeparatedSourceFlowFlatPhysicalPointSourceOwnerBound`
combines the sealed C3 equality, the physical zero-residue Green majorant and
the antipodal-safe C4a owner dictionary.  Its coefficient is literally
`cmp99SourceFlowFlatFullComplexA a L depth`; positivity is derived from
`ha : 0 < a`.  The result preserves the physical amplitude, residue factor,
boundary factor, actual coarse owner and source-vector norm.

PRE-VALIDATION checkpoint `934efa2a281824db90d13724eb143f7df2a7c4db`
introduced the source/audit pair.  Runner v1 checkpoint
`f8cc3b8a0ce3188aa8280569c29f310648ba2900` stopped at the first elaboration
error after `2132.877` seconds: the scalar Green call lacked the explicit
`K := Kfine`, leaving `NeZero ?m` stuck.  Diagnostic evidence/archive SHA-256
values are
...
Repair checkpoint `8bb90cd1f666ad94eca099ec26cd81d29239f0b8` changed exactly
that specialization.  Its source/audit Git-blob SHA-256 values are
...
passed in a fresh Colab Pro+ CPU/high-RAM checkout.  The focal completed 8,773
jobs in `2830.365` seconds and the audit in `19.501` seconds.  Both
declarations printed exactly `[propext, Classical.choice, Quot.sound]`;
`FINAL_STATUS=PASS`.  Evidence SHA-256 is
...
the two PRE-VALIDATION marks and imports the audit into `YangMillsCore`.  This
is the Unit-F point-source owner bound, not the CMP99 source-localization
...
The next finite adapter is cold-sealed: a single uniform localized value
bound for the canonical regional Green is sufficient to assemble all four
CMP99 (3.42) actions.  Exact source
`d8db241ce99ef37ceecc14afefc3a19b88ed5f20` compiled `8707/8707` jobs in
`5229.215` seconds in a fresh Colab Pro+ CPU/high-RAM checkout; its exact
audit exited zero in `10.071` seconds and uses only
`{propext, Classical.choice, Quot.sound}`.  The locally verified evidence
archive SHA-256 is
...
The generated two-scale specialization is now cold-sealed at exact source
`3213c3615d9432ee778560093303a834b0c7abe2`.  In a fresh Colab Pro+ CPU/high-
RAM checkout with no restored project `.lake/build`, its focal completed
`8495/8495` jobs in `1129.107` seconds and its exact audit exited zero in
`9.650` seconds with only `{propext, Classical.choice, Quot.sound}`.  Ledger
Addendum 987 records the locally verified archive and evidence hashes.

This closes the literal depth-two adapter: the coarse background is the
first `Ubar` background generated by the same source radius chain.  It does
not turn the two-scale proof into a depth-uniform induction.  A quantitative
Neumann Poincare theorem for the full terminal retained average remains a
separate source-facing frontier; the existing generated Dirichlet Poincare
machinery cannot be reused by definition because its zero-extension
...
| 14 | specialize step 13 to the two within-block endpoint displacements and apply step 12 to its combined affine base | `BalabanCMP99PhysicalFullGreenOwnerResidueBound`, **cold-sealed** at source `ea524400bbf59777d461e8d04790516771258988` (Ledger Addendum 1108), scalar physical owner bound with explicit amplitude, geometric factor and `exp(2*rho)` |
| 15 | compose step 14 with step 10's constructed residue-class certificate | `BalabanCMP99GeneratedFullPointSourceOwnerBound`, **cold-sealed** at source `ea524400bbf59777d461e8d04790516771258988` (Ledger Addendum 1108), retaining literal `K^-4` normalization and `norm(v A)`; not a depth-uniform source-flow or regional bound |
...
Steps 14--15 and their exact audits passed the fresh-checkout v2 gate at
source `ea524400bbf59777d461e8d04790516771258988`: focal 8696 jobs / 1616.682 s,
audit 10.568 s; then focal 8808 jobs / 543.118 s, audit 18.311 s, all exit zero.
The eight exact declarations use the allowed trio. The archive, all eighteen
stage records and downloaded hashes were verified (Ledger Addendum 1108).
Only these four notices are retired; their audits enter the aggregator with
no new root-build claim. The frozen statements retain all analytic windows
and construct the physical reconstruction via step 10. This is a fixed-
coefficient full-G bound, not a uniform physical B0 or window-15 attainment.
...
### Eq. (2.46) complete directed Fourier sum (cold-sealed)
...
Fresh Colab Pro+ CPU/high-RAM runner
`cmp89-eq246-directed-full-solution-sum-cold-v1` checked out exact source
checkpoint `a95ed996f0a60ed7101e7b99dbe8bf998da21c7d`, verified the official
toolchain asset and exact Mathlib pin, restored no project `.lake/build`
graph, and passed the source focal in `1402.362 s` and exact audit in
`11.734 s`.  The runner's exact three-declaration axiom gate passed and the
fail-closed local verifier accepted the source pin, source blobs, two-stage
queue, environment pins, evidence JSON and archive hash.  Both
PRE-VALIDATION notices are retired and the audit enters `YangMillsCore.lean`.
This brick does not yet perform the normalized Brillouin synthesis, identify
the resulting integral with the physical generated Green, prove CMP89 (2.42),
