...
correction in (3.95) into every ordered atom word, and
`cmp99Eq395PhysicalCorrectedCovariance_eq_canonical` identifies its convergent
Neumann sum with the canonical covariance.  What remains is the source
Section-C parser: assign the four printed atom roles to that exhaustive raw
word, prove the closest-anchor grouping and its at-most-three attachment
bound, adjoin the patched-parametrix head, and transport the result through
the rectangular readout above.  Thus the missing alphabet is generated by a
proved parser from exhaustive atoms; it must not be filled by guessing the
examples hidden behind "etc.".
