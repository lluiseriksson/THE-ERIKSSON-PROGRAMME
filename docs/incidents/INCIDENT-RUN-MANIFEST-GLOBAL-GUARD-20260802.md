...
normalizes repository separators, and checks path case explicitly. It still
fails strict validation, now with a platform-stable baseline of 3,814 errors.
...
The stabilized population is 39 strictly valid manifests and 584 manifests
with inherited violations. The versioned baseline contains every protected
...
| Strict rule | Original Ubuntu | Stabilized baseline |
|---|---:|---:|
| `inputs[*].path: file does not exist` | 937 | 683 |
| `script: expected an object` | 563 | 563 |
| `started_utc: expected a non-empty string` | 289 | 289 |
| `finished_utc: expected a non-empty string` | 281 | 281 |
| `working_directory: expected a non-empty string` | 279 | 279 |
| `outputs: expected an array` | 258 | 258 |
| invalid contract `status` | 211 | 211 |
| `supersedes: expected an array of run identifiers` | 193 | 193 |
| `environment: expected an object` | 192 | 192 |
| `inputs: expected an array` | 173 | 173 |
| invalid `command` | 154 | 154 |
| `environment.libraries: expected an object` | 103 | 103 |
| `inputs[*].sha256_lf` mismatch | 94 | 94 |
| `schema_version: expected 1` | 76 | 76 |
| missing `run_id` | 75 | 75 |
| missing `claim_scope` | 74 | 74 |
| missing quarantine reason | 50 | 50 |
| `inputs[*].sha256` raw mismatch | 36 | 0 |
| `outputs[*].sha256` raw mismatch | 32 | 2 |
| `outputs[*].sha256_lf` mismatch | 24 | 24 |
| duplicate output ownership | 10 | 10 |
| non-UTC `finished_utc` | 7 | 7 |
| invalid input SHA-256 syntax | 6 | 6 |
| missing `script.path` | 5 | 5 |
| quarantine reason on non-quarantined run | 3 | 3 |
| superseded run missing backlink | 2 | 2 |
| superseded target not marked superseded | 2 | 2 |
| invalid output SHA-256 syntax | 2 | 2 |
| `script.sha256_lf` mismatch | 1 | 1 |
| unknown superseded run | 1 | 1 |
| `run_id`/filename mismatch | 1 | 1 |
| **Total** | **4,134** | **3,814** |

The stabilized classes group into 2,929 schema/layout errors, 817
artifact-reference/digest errors, and 68 lifecycle/ownership errors. The 320
...
replacement REJECTED; stale PASS INVALIDATED.
