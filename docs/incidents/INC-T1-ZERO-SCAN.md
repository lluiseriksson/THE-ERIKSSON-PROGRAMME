...
`tests/test_surface_monotone_contract.py` always invokes the scanner from `scripts/`,
requires a nonzero scan, and records the current failure as strict `xfail`. Once the live
scripts are repaired, the test becomes a fatal `XPASS` until this quarantine is reviewed
