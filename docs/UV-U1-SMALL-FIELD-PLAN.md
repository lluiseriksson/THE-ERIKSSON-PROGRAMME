...
| **S1'** | **ℓ²(lattice) operator contraction** (genuine, deterministic): `∑_bonds ‖Q A‖² ≤ L^{2-d}·∑_bonds ‖A‖²` — summing S1 over all bonds, each fine bond hit exactly `L` times (blocks tile the lattice + shift bijection). `L^{2-d} < 1` for `d ≥ 3` (physical `d = 4`): the bare averaging operator **is an ℓ²-contraction**.  Explicit ratio (`d ≥ 3`): `∑‖QA‖² ≤ L⁻¹·∑‖A‖²` (`linAvg_l2_contraction`).  `RG/AveragingL2.linAvg_l2_le` (+ `sum_blockOf`, `shift_bijective`, `linAvg_l2_contraction`). | **CLOSED** (ledger Add. 43–44, core 8247) |
