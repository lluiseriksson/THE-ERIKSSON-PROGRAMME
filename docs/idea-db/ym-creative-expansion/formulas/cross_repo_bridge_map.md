# Cross-repo bridge map

This file tells the constructor which repo supplies which kind of fuel.

| Bridge | Source repos | Use | Hard guard |
|---|---|---|---|
| `hRpoly` frontier | THE-ERIKSSON-PROGRAMME + Dimock/Balaban source DB | Main lattice-M3 UV activity target | Does not solve continuum Clay by itself. |
...
1. `RiemannPrimeResolvent.ErrorBudget` -> `YMActivityErrorBudget`: dictionary defects become explicit fields instead of hidden assumptions.
2. `finiteSquaredResolvent_nonneg` + Dimock `G_k` random walk -> finite SPD covariance-root localization toy theorem.
3. `targetPreservingWeightedTreeBound` -> Appendix-F exact-union fiber: extract target metric before rooted overcount.
4. Catalan Prüfer-profile route -> optional second-Ursell/Catalan smallness window.
5. PhysMath moves `preserve_exact_target`, `audit_information_loss`, `normalize_parent_kernel`, `close_exact_generating_function` -> automatic review checklist for every proposed formula.
