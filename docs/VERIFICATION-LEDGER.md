...
## Addendum 434 (2026-06-27, **source citation operational lookup bridge**)

Files touched: `scripts/source_citations.py`,
`tests/test_source_citations.py`, `docs/SOURCE-CITATIONS.md`,
`CURRENT-STATE.md`, and this ledger.

This checkpoint closes a tooling gap in the repository citation system:
`python scripts/source_citations.py show <key>` now loads both
`docs/source-citations/*.json` and `docs/source-db/catalogs/*.json`.  As a
result, operational routing keys such as
`crosswalk.eq231.p-family-source-dictionary-route` and
`proof.eq231.source-package.live-fields.v2` resolve directly from the compact
citation CLI instead of requiring a separate `source_db.py` lookup or manual
JSON inspection.  Compact output now also prints operational `use_for`,
`do_not_use_for`, and `dictionary_links` fields when present.  The local-text
excerpt resolver also accepts repo-local paths such as
`docs/source-db/indices/...` in addition to private source-cache artifacts.
...
Results: the CLI smoke test now resolves the Eq. (2.31) operational crosswalk
and live-field card.  Citation validation passed with 100 citations from 15
sources.  Source DB verification passed with 9 catalog files and the SQLite
build retained hash
`5566d6a528995fcab637921fafabb1916e9832d12d095a713c06fc452664bd55`.
`python -m pytest tests\test_source_citations.py tests\test_source_db.py`
...
axioms; the forbidden-token scan found no matches.  Full `lake build
YangMillsCore` passed at 8364 jobs with only pre-existing warnings.
`lake env lean oracle_check.lean` exited 0 with the existing standard axiom
set `[propext, Classical.choice, Quot.sound]`.

Honest scope: this is source-navigation infrastructure, not a theorem
discharge.  The CMP116 Eq. (2.31) source package remains blocked on the exact
...
### 2026-06-27 - Eq231 corrected Y0cStar carrier source-lock

This checkpoint corrects the active CMP116 Eq. (2.31) carrier source-lock after
the post-`57875f1` audit.  The previous two-premise record
`CMP116Eq231InteriorBoundaryToGapSource` remains a sufficient Lean interface,
but the registered source material keeps `P subset Y0^{c,*}` separate from the
interior/no-`dZ0` clause.  The active source target is now:
...
The source-citation and source-db entries now identify this as the next
source-pending endpoint/base dictionary.  The rebuilt SQLite source index hash
is `f394fbde226fce789c84180310d2364be89a383c7890da67c47966c7a060be5f`.
...
python scripts\source_db.py lean cmp116Eq231_y0cStarInteriorBoundary_to_gapCubes_of_source
python scripts\source_db.py lean CMP116Eq231PositiveTailOwnershipSource.of_y0cStarInteriorBoundary
python scripts\source_db.py lean cmp116Eq231_bond_fst_mem_gapCubes_of_y0cStarInteriorBoundary
python scripts\source_db.py frontier --term eq231 --status lean_linked --limit 10
python -m pytest tests\test_source_citations.py tests\test_source_db.py
python -m py_compile scripts\source_db.py scripts\source_citations.py
lake env lean YangMills\RG\BalabanCMP116Eq231.lean
...
Results: source DB verification passed with 9 catalog files; citation
validation passed with 100 citations from 15 sources; source DB Lean queries
resolve the new corrected source-lock declarations to the CMP116/CMP109
source-pending entries and Eq. (2.31) live-field cards.  The source tooling
pytest suite passed 13 tests, and Python compilation passed.  The focused
Eq. (2.31) Lean file elaborated, the focused olean target built successfully
at 8194 jobs, and `lake build YangMillsCore` passed at 8364 jobs with only
pre-existing warnings.  `git diff --check` and cached diff check passed with
only CRLF conversion warnings, the consistency script reported zero `sorry`
and zero verified-core axioms, the forbidden-token scan found no matches, and
`lake env lean oracle_check.lean` exited 0 with 2780 stdout lines after a
longer timeout budget.

Honest scope: this commit does not prove
`CMP116Eq231Y0cStarInteriorBoundaryToGapSource`, does not prove
`CMP116Eq231PositiveTailOwnershipSource.positive_tail_in_gap`, and does not
prove the Eq. (2.31) membership/admissibility dictionaries, pointwise
P-residual majorization, Eq. (2.37), Eq. (2.29), H#, continuum, or Clay.
...
### 2026-06-27 - Eq231 incidence-filtered P-stage bridge

This checkpoint extends the incidence fallback from a finite carrier/boundary
record to explicit P-stage consumers:
...
Both statements intentionally require the geometry majorant to use
`cmp116Eq231IncidenceGapMass`.  They therefore support a future
incidence-specific source theorem without letting downstream code confuse the
fallback with the original four-direction `M^{-4}|Z0 \ Y0|` mass.

The rebuilt SQLite source index hash is
`0d4dc83d593c8de7f9a6dc36c8bcc5680c0f8c388c54660779785c9cdf2350e5`.
...
python scripts\source_db.py verify
python scripts\source_db.py show proof.eq231.incidence-carrier.fallback
python scripts\source_db.py lean cmp116PStageSourceBound_of_eq231_incidenceFilteredBondSets
python scripts\source_db.py lean CMP116Lemma3PStageSourceScaleBoundary.of_eq231_incidenceFilteredBondSets
lake build +YangMills.RG.BalabanCMP116Lemma3ScaleFamily:olean
python scripts\source_citations.py validate
python -m pytest tests\test_source_citations.py tests\test_source_db.py
python -m py_compile scripts\source_db.py scripts\source_citations.py
...
Results: focused Lean elaboration passed for the residual-stage and scale-family
files; source DB verification passed with 9 catalog files; the incidence card
and both new Lean targets resolve through the source-db CLI; the scale-family
olean target built successfully at 8256 jobs with only pre-existing warnings;
source citation validation passed with 102 citations from 15 sources; the
source-db/source-citation pytest suite passed 13 tests; Python compilation
passed; `git diff --check` and `git diff --cached --check` passed; the
consistency script reported zero `sorry` and zero verified-core axioms; the
forbidden-token scan found no matches; `lake build YangMillsCore` passed at
8364 jobs with only pre-existing warnings; `lake env lean oracle_check.lean`
exited 0 and printed the expected 2780-line oracle dependency report.

Honest scope: this commit still does not prove that CMP116 uses the incidence
carrier, does not prove the source theorem retargeting Eq. (2.31) to
...
### 2026-06-27 - Direct raw-sum scalar UV bridge

This checkpoint extends the UV activity-budget adapter with the direct raw-sum
route to the scalar single-scale consumer:

```lean
YMActivityErrorBudget.singleScaleUVDecay_of_rawYMActivityDecay
YMActivityErrorBudget.RawYMActivityDecomposition.singleScaleUVDecay_of_tsum
```

The first theorem says that a `RawYMActivityDecay` estimate feeds
`SingleScaleUVDecay` when the scalar remainder is exactly the absolutely
summable raw-activity sum and the raw weight has bounded total sum.  The second
theorem composes that bridge with the named five-component
`RawYMActivityDecomposition` record.

The direct-sum identity and summability hypotheses are part of the theorem
surface; nothing here asserts the Appendix-F/H# renormalized activity theorem.
...
### 2026-06-27 - Canonical raw-sum decomposition constructor

This checkpoint adds the canonical exact-sum/profile constructor for the named
raw activity decomposition:

```lean
YMActivityErrorBudget.RawYMActivityDecomposition.of_sum_components_profile
```

The constructor is for the case where the raw activity is definitionally the
sum of the source term plus the covariance, dictionary, support, and Jacobian
defects, and where the raw weight is exactly `B.profile (dist Y)`.  It fills the
scale-nonnegativity field from `∀ k, 0 ≤ g k`, fills the
profile-to-weight comparison by reflexivity, and fills the exact-decomposition
field by `rfl`.  The five component estimates remain explicit hypotheses.
...
Honest scope: this proves only definitional bookkeeping for a canonical raw
...
### 2026-06-27 - Raw-activity summability derived from weight summability

This checkpoint removes a duplicate bookkeeping premise from the direct
raw-activity scalar UV route.  A raw pointwise decay estimate against a
summable raw weight now theorem-feeds absolute summability of the raw
summand:

```lean
YMActivityErrorBudget.summable_abs_of_rawYMActivityDecay
YMActivityErrorBudget.singleScaleUVDecay_of_rawYMActivityDecay_summableWeight
YMActivityErrorBudget.RawYMActivityDecomposition.singleScaleUVDecay_of_tsum_summableWeight
YMActivityErrorBudget.singleScaleUVDecay_of_sum_components_profile_tsum_summableWeight
```

The exact scalar identity `Rsc = tsum Hraw`, raw/profile-weight summability,
and the weight-sum bound remain explicit hypotheses.  What is removed is only
the separately supplied `Summable (fun Y => |Hraw t k Y|)` premise, since it is
already implied by the pointwise decay estimate and summability of the raw
weight.
...
DB/citation pytest suite passed 13 tests.  `git diff --check` passed with only
CRLF conversion warnings on modified working-copy files.  `lake env lean
oracle_check.lean` exited 0 after the new theorem cluster was added to the
oracle file, and printed the expected oracle dependency report.  The
...
Honest scope: this is source-independent summability bookkeeping.  It does not
...
### 2026-06-27 - Activity-budget marginal mass-gap bridge

This checkpoint composes the named five-channel raw Yang--Mills activity
decomposition with the existing marginal-coupling mass-gap assembly:
...
The first theorem consumes a packaged
`YMActivityErrorBudget.RawYMActivityDecomposition`, derives the existing
`SingleScaleUVDecay` predicate with weight summability, and feeds
`lattice_mass_gap_of_singleScaleUVDecay_marginal`.  The second theorem is the
canonical exact-sum/profile specialization where the raw activity is literally
the source term plus covariance, dictionary, support, and Jacobian defects.
...
Results: focused Lean elaboration passed for the touched UV adapter module.
The focused olean build for `YangMills.RG.YMActivityBudgetUV` passed.  The
`YangMillsCore.lean` import check elaborated.  Source DB verification passed
with 9 catalog files; source-citation validation passed with 102 citations from
15 sources; and the source DB/citation pytest suite passed 13 tests.  `git diff
...
oracle_check.lean` exited 0 and printed only the expected standard Lean axiom
dependencies for the new theorems (`propext`, `Classical.choice`,
`Quot.sound`), inherited from their existing summability and mass-gap
ingredients.  The consistency checker reported zero Lean `sorry` and zero
...
Honest scope: this is theorem composition and bookkeeping.  It does not prove
any source-shaped component estimate, covariance/root localization, dictionary
...
### 2026-06-27 - M3 frontier method projects through raw-Hsharp marginal assembly

This checkpoint adds a method-style marginal assembly theorem for a completed
...
The theorem first projects the completed `CMP116RawSourceM3Frontier` to the
named `PhysicalGaugeCMP116RawHsharpFrontier`, then feeds the existing raw-H#
frontier marginal assembly theorem.  This avoids caller-side reconstruction of
the narrower raw-H# frontier and keeps the import direction clean:
`PhysicalGaugeCMP116RawHsharpFrontier.lean` can depend on `RawM3`, while
`RawM3` cannot import the raw-H# frontier module without a cycle.
...
Results: focused Lean elaboration passed for
`YangMills.RG.PhysicalGaugeCMP116RawHsharpFrontier`; the focused olean build
passed with only pre-existing linter warnings in unrelated files.
`YangMillsCore.lean` elaborated.  Source DB verification passed with 9 catalog
files; source-citation validation passed with 102 citations from 15 sources;
and the source DB/citation pytest suite passed 13 tests.  `git diff --check`
passed with only CRLF conversion warnings on modified working-copy files.  The
full `lake build YangMillsCore` passed at 8366 jobs with only pre-existing
linter warnings in unrelated files.  `lake env lean oracle_check.lean` exited 0
and reported only expected Lean standard axiom dependencies for the new theorem
(`propext`, `Classical.choice`, `Quot.sound`).  The consistency checker
...
frontier records.  It proves no source theorem, no Gaussian pushforward, no
covariance-root localization, no Wilson-Hessian identification, no local
activity construction, no raw pointwise decay, no H# estimate, no marginal-flow
theorem, no IR estimate, no Eq. (2.31) carrier dictionary, no `hRpoly`, no
continuum result, and no Clay statement.
