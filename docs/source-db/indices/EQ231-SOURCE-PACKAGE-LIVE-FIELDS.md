...
and
`exists_fullCarrierAdmissibility_without_positiveTailOwnershipSource` prove
that the corrected page-12 admissibility split does not itself imply the
endpoint/base gap dictionary or the final positive-tail ownership consumer. In
the countermodels, the source-admissible predicate and all three bond
predicates are true, while `gapCubes` is empty; therefore the three-premise
source-lock fails unless a separate source theorem identifies the encoded first
coordinate with a cube in `Z0 \ Y0`.
