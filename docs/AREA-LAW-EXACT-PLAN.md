...
| E4b-1b | **Tail substrate:** `tsum_pi_prod_nonneg` (real Pi-Cauchy for nonneg families, by `Complex.ofReal_injective` transport of E1), `tsum_pow_div_factorial` (`= exp x`), `tsum_pow_div_factorial_succ` (`= exp x − 1`, via `Summable.tsum_eq_zero_add`).  Remaining for the tail: the union bound `[#supp m ≥ A] ≤ ∑_{S : #S = A}[∀p∈S, mₚ ≥ 1]` (pointwise into a finite indicator sum), the per-`S` factorized constrained sum (shift-reindex `mₚ = nₚ+1` on `S` via an `Equiv`, then `tsum_pi_prod_nonneg` with the mixed family `x^{k+1}/(k+1)!` on `S`, `x^k/k!` off `S`), giving `∑'_{#supp ≥ A} ∏ x^{mₚ}/mₚ! ≤ C(#P,A)·(e^x−1)^A·e^{x(#P−A)} ≤ 2^{#P}(e^x−1)^A e^{x·#P}`. | **substrate CLOSED** (oracle clean) |
