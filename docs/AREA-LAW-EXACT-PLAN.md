...
| E4b-2 | **Final assembly:** Stage 1 → `tsum_mul_left` to absorb `tr(W_C)` → E2 swap (domination `c m := N·∏(2δN)^{mₚ}/mₚ!`, summable via the norm-form of E1's summability) → `‖∑'‖ ≤ ∑'‖·‖` + the E4b-2a dichotomy (ite-family) → constrained-Pi tail `∑'_{#supp m ≥ A} ∏ (2δN)^{mₚ}/mₚ! ≤ 2^{#P}·(e^{2δN}−1)^A·e^{2δN·#P}` ⇒ `‖∫ tr(W_C)·∏ exp(zₚ)‖ ≤ N_c·2^{#P}·e^{2δN_c·#P}·(e^{2δN_c}−1)^{Area}`.  NO smallness hypothesis needed — the tail holds for all `x ≥ 0`. | open (final brick) |
