...
| **M-log-2b** | **`log(exp X) = X` near `0`** (the local-inverse identity) — composition of the `exp`/`log` series; ties `nearLog`'s linearisation to `exp` and completes (0.8). | BCHD.pdf; standard | `nearLog_expSub_eq` / `expSeries`-composition. | open (next M-log brick; the genuinely hard one) |
