...
| **B3-full (interface)** | **Bałaban's axiomatic group average `M`.** The faithful route to `Ū` (CMP 109: results hold for any `M` with these properties). | CMP 109 (0.5)–(0.9) | `structure GroupAverage G` (`M : Multiset G → G`, inv-equivariance (0.5), bi-equivariance (0.6); perm-inv (0.7) automatic via `Multiset`, closure (0.9) automatic via codomain) + `left/right/conj_equiv` (the **gauge-covariance seed** `M({uU_ju⁻¹}) = u·M·u⁻¹`). | **CLOSED** (ledger Add. 26, core 8242) |
| **B3-full (analytic)** | The linearisation (0.8) `(1/i)log M(exp iA_j) = n⁻¹ΣA_j + O(‖A‖²)` (ties `M` to `Q`=`linAvg`) and Federbush inhabitation (0.10); then the operator `Ū` via CMP 109 (0.12) / CMP 98 (15). | CMP 109 (0.8),(0.10),(0.12); CMP 98 (14)–(15) [clean transcription received] | carried `GroupAverage` obligations + `Ū` def. **NEEDS a near-identity matrix-`log` framework** (matrix `exp`/`log` near `1`). Cross-check: `linAvg` = CMP 95 (1.8) prototype; `Ū`/(0.12) uses the Euclidean-symmetric refined contour `Γ_{c,x}` with `L^{-(d+1)}`. | open (analytic) |
