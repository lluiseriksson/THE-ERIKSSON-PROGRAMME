...
| **M-log-3** | **Conjugation-equivariance of `nearLog`** — `nearLog(u·Y·u⁻¹) = u·(nearLog Y)·u⁻¹` for a unit `u`. The algebraic core of B4-Ū; conjugation is a continuous linear map (`ContinuousLinearMap.mulLeftRight`) passed through the series. NO `log(exp)=id`. | CMP 98 (11); CMP 109 (0.12) | `nearLog_conj`. | **CLOSED** (ledger Add. 35, core 8245) |
