...
| **M-log-2** | **`log(exp X) = X` near `0`** (the local-inverse identity) + the `O(‖X‖²)` BCH remainder bound — unblocks (0.8). | BCHD.pdf; standard | `nearLog_exp`, `norm_nearLog_exp_sub_le`. | open (next M-log brick) |
