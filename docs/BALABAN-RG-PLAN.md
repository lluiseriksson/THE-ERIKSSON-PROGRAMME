...
| **B4** | **Gauge covariance of `Ū`.** Averaging intertwines gauge transformations. | CMP 98 (11) | `avg_gaugeTransform` — combines B4-prep (holonomy law) with `GroupAverage.conj_equiv`/`biequiv` (the averaged contour variable (0.11) is covariant). | open (needs `Ū`, B3-full analytic) |
