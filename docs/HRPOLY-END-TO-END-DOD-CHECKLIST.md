...
| Uniform physical `B0, delta0` | **open** | Addendum 963 seals one uniform literal source-flow flat point-source `B0` and its arbitrary localized-field reduction.  Addendum 972 inserts the literal normalized CMP89 (2.48) full-lattice Green into the complete rectangular Neumann image series with one common amplitude/rate and no rectangle-volume factor.  Addendum 973 cold-seals the exact regional value-bound consumer conditional on the named CMP89 (2.42) representation certificate; it does not construct that certificate.  Addendum 974 cold-seals the algebraic fine-rate to fixed-owner-rate transport without a free owner map or rectangle cardinality.  Addendum 976 cold-seals the actual half-open rectangle embedding, reflection-period floor and canonical CMP99 owner metric; the depth-free `exp(2*rho)` endpoint is hot-green but awaits its own cold seal.  The named CMP89 (2.42) representation producer and one positive pair uniform in depth for the transported physical actions remain open.  Ambient flat, per-depth or per-action constants do not close this row. |
