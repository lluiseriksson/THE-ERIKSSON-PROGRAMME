...
| C6 source/localized retained dependency boundary | **in progress** | All 34 boundary blobs are byte-identical at C6d source `3738ddb6...`, the active Eq. (3.59) cold source `cd6ff656...`, and the current branch.  The active Eq. (3.59) fresh root can therefore supply the cold compiler half without weakening scope.  Seal still requires literal Eq. (3.59) `FINAL_STATUS=PASS`, its independently verified archive, a retained-runtime pass over all 92 C6d axiom readouts, the combined fail-closed archive verifier, and exact 34-file marker retirement. |
...
| Retained-tower/source-region transport of the four physical actions | **in progress** | The complex Eq. (3.59) one-scale average and internally generated two-tower pair are sealed from fresh Colab evidence at source `5c0dc371...` (ledger Addendum 945).  The exact real-slice gate is running in one fresh Colab checkout at source `cd6ff656...`; no real-slice mark is retired before its literal terminal sentinel and archive verification.  Eq. (3.60), the regional resolvent and re-instantiation of the four fixed-depth CMP96 actions on that one Green remain open.  Their existing Green lives on `cmp96SourceSeparatedRegionalCell`, not on the arbitrary selected source region, so direct restriction cannot identify the two Dirichlet inverses.  Equality of independently chosen towers, precisions or Greens remains inadmissible; see `tmp/C6D-POST-EQ337-FOUR-ACTION-BOUNDARY.md`. |
