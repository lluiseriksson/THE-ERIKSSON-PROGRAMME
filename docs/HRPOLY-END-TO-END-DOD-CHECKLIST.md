...
| Uniform physical `B0, delta0` | **open** | Addendum 963 seals one uniform literal source-flow flat point-source `B0` and its arbitrary localized-field reduction.  Addendum 972 inserts the literal normalized CMP89 (2.48) full-lattice Green into the complete rectangular Neumann image series with one common amplitude/rate and no rectangle-volume factor.  Addendum 973 cold-seals the exact regional value-bound consumer conditional on the named CMP89 (2.42) representation certificate; it does not construct that certificate.  Fine-to-owner transport and one positive amplitude/rate pair uniform in depth have not yet been produced for the transported physical actions.  Ambient flat, per-depth or per-action constants do not close this row. |
