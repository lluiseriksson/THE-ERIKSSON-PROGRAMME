...
| Retained-tower/source-region transport of the four physical actions | **in progress** | The complex Eq. (3.59) tower, its exact physical real slice and the 34-file retained source boundary are sealed (Addenda 945--947); C6d.1 seals the literal localized retained precision (Addendum 949), and the baseline real slice, retained complex extension and recursive physical/analytic tower agreement are sealed by Addendum 951 from a fresh `11026`-job root. Next is the terminal two-tower pair plus the physical-radius bridge, followed by a C6d-specific Eq. (3.60) wrapper with separate Laplacian and averaging backgrounds, then the regional resolvent and re-instantiation of the four fixed-depth CMP96 actions on that one Green. The existing Green lives on `cmp96SourceSeparatedRegionalCell`, not on the arbitrary selected source region, so direct restriction cannot identify the two Dirichlet inverses. Equality of independently chosen towers, precisions or Greens remains inadmissible; see `tmp/C6D-POST-EQ337-FOUR-ACTION-BOUNDARY.md`. |
