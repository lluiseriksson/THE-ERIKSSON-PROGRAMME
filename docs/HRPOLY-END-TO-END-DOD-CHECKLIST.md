...
| Uniform physical `B0, delta0` | **open** | Addendum 963 seals one uniform literal source-flow flat point-source `B0` and its arbitrary localized-field reduction.  The canonical regional/Dirichlet bound and one positive amplitude/rate pair uniform in depth have not yet been produced for the transported physical actions.  Ambient flat, per-depth or per-action constants do not close this row. |
