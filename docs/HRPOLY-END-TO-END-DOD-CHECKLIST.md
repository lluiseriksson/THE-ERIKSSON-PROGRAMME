...
| Retained-tower/source-region transport of the four physical actions | **in progress** | The complex Eq. (3.59) tower, its exact physical real slice and the 34-file retained source boundary are sealed (Addenda 945--947); C6d.1 seals the literal localized retained precision (Addendum 949). Addendum 951 seals the baseline real slice, retained complex extension and recursive physical/analytic tower agreement from a fresh `11026`-job root. Addendum 952 then seals the terminal tower pair, physical-radius bridge, C6d-specific Eq. (3.60) route and closed physical precision from a fresh `11040`-job root. The live boundary is now the positive-depth source-generated coercivity/baseline Green, followed by the full-companion ambient precision, explicit active-region carrier reindexing and re-instantiation of the four fixed-depth CMP96 actions on that one Green. The exact `depth = 0` companion/Green branch or an explicit connection to the separately sealed flat base case remains required; positive-depth evidence does not cover it. The existing fixed-depth Green lives on `cmp96SourceSeparatedRegionalCell`, not on the arbitrary selected source region, so direct restriction still cannot identify the two Dirichlet inverses. Equality of independently chosen towers, precisions or Greens remains inadmissible; see `tmp/C6D-SOURCE-DIRICHLET-COMPRESSION-DESIGN.md`. |
