...
| Retained-tower/source-region transport of the four physical actions | **complete at fixed depth; uniform bound open** | The complex Eq. (3.59) tower, its exact physical real slice and the 34-file retained source boundary are sealed (Addenda 945--947); C6d.1 seals the literal localized retained precision (Addendum 949). Addenda 951--965 seal the physical/analytic tower agreement, terminal precision, source coercivity and Green, full-companion compression, source-separated ambient branches, distance rescaling and exact D2 decay. Addendum 966 closes the fixed-depth action/certificate prefix from fresh source `77d9f4b4...`: 55/55 stages, a `11145`-job same-checkout root and all allowed axiom readouts passed; the fail-closed sealer retired exactly 40 notices with manifest `365C7470...`. The remaining live boundary is one positive `B0, delta0` pair uniform in depth for these transported physical actions. Equality of independently chosen towers, precisions or Greens remains inadmissible; see `tmp/C6D-SOURCE-DIRICHLET-COMPRESSION-DESIGN.md`. |
...
| Uniform physical `B0, delta0` | **open** | Addendum 963 seals one uniform literal source-flow flat point-source `B0` and its arbitrary localized-field reduction.  Addendum 972 seals an analytic insertion of the normalized CMP89 (2.48) kernel into a rectangular image series, but Addendum 1006 reclassifies that kernel source-faithfully as `G_j Q_j^*`, with a fine output endpoint and a coarse input endpoint; it is not the full fine-to-fine Green required by CMP89 (2.42).  Addendum 1008 cold-seals the internally constructed solution of the complete arbitrary-source alias system (2.46); the normalized fine delta, fine-to-fine Fourier synthesis, inverse transform and reflection identity (2.42) remain open.  Addendum 973 cold-seals the exact regional value-bound consumer conditional on the named CMP89 (2.42) representation certificate; it does not construct that certificate.  Addendum 974 cold-seals the algebraic fine-rate to fixed-owner-rate transport without a free owner map or rectangle cardinality.  Addendum 976 cold-seals the actual half-open rectangle embedding, reflection-period floor and canonical CMP99 owner metric; the depth-free `exp(2*rho)` endpoint is hot-green but awaits its own cold seal.  The source-faithful route must now specialize (2.46) to the normalized fine point source, synthesize the full fine-to-fine Green, prove (2.42), and only then compose with `Q_j^*`, or prove a direct reflected-`G_j Q_j^*` theorem with its compatibility dictionary.  One positive pair uniform in depth for those transported physical actions also remains open.  Ambient flat, per-depth or per-action constants do not close this row. |
