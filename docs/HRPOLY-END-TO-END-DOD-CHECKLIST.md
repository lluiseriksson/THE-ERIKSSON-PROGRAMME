...
| Physical flowing-radius-chain producer | **open** | The low-level recursion may consume `CMP99SourceComplexUbarRadiusChain`, but the closed source endpoint must mirror the sealed real `CMP99SourceUbarClosedBudget`: generated radii, an explicit one-step growth factor and one initial-scale `terminal_small` inequality producing the finite chain.  Receiving the chain, or a record with `∀ k < depth` smallness fields, merely packages the per-scale strict gates and does not close Eq. (3.37). |
