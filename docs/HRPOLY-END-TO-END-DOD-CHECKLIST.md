...
| Uniform physical `B0, delta0` | **open** | Addendum 963 seals one uniform literal source-flow flat point-source `B0` and its arbitrary localized-field reduction.  Addendum 972 seals an analytic insertion of the normalized CMP89 (2.48) kernel into a rectangular image series, but Addendum 1006 reclassifies that kernel source-faithfully as `G_j Q_j^*`, with a fine output endpoint and a coarse input endpoint; it is not the full fine-to-fine Green required by CMP89 (2.42).  Addendum 973 cold-seals the exact regional value-bound consumer conditional on the named CMP89 (2.42) representation certificate; it does not construct that certificate.  Addendum 974 cold-seals the algebraic fine-rate to fixed-owner-rate transport without a free owner map or rectangle cardinality.  Addendum 976 cold-seals the actual half-open rectangle embedding, reflection-period floor and canonical CMP99 owner metric; the depth-free `exp(2*rho)` endpoint is hot-green but awaits its own cold seal.  The source-faithful route must construct the full fine-to-fine Green from CMP89 (2.46), prove (2.42), and only then compose with `Q_j^*`, or prove a direct reflected-`G_j Q_j^*` theorem with its compatibility dictionary.  One positive pair uniform in depth for those transported physical actions also remains open.  Ambient flat, per-depth or per-action constants do not close this row. |
