...
| **P2b-ii-b-1** | **Holes modified metric definition & skeleton card bound** (`modifiedMetric`, `skeleton_card_le_modifiedMetric_add_one`, and `modifiedMetric_empty_holes`, `RG/ModifiedMetric.lean`): define the modified distance and prove the skeleton cardinality bound. | code — combinatorics | **DONE** (core 8264) |
| **P2b-ii-b-2** | **Modified-metric summability under Gaussian suppression**: prove the full modified-metric summability (eqs 150–151, which needs the holes' Gaussian suppression). | code — HARD (geometry + suppression) | open |
