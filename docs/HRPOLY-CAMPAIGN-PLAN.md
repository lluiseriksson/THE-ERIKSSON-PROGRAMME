...
| **P2b-ii-a** | **Holes multiplicity bounds and multi-hole combinatorics** (`absorbedHole_touches_skeleton_multi` and `touchingHoles_card_le`, `RG/ModifiedMetric.lean`): the multi-hole skeleton-touching theorem and multiplicity bound showing that the number of touching holes is at most $\Delta \cdot |Y|$. | code — combinatorics | **DONE** (core 8164) |
| **P2b-ii-b** | **Holes modified metric definition & summability**: define the modified distance `d_M(·, mod Ω^c)` (Dimock II §3.1.2, eqs 150–151), and the full modified-metric summability (needs the holes' Gaussian suppression). | code — HARD (geometry + suppression) | open |
