...
/-- The connected finite sum is the `tsum` of its cardinality buckets. -/
theorem connectedFiniteSum_eq_cardBucketTsum
    {d L : ℕ} [NeZero d] [NeZero L]
    (K_bound : Finset (ConcretePlaquette d L) → ℝ)
    (p q : ConcretePlaquette d L) :
    (∑ Y ∈ (Finset.univ : Finset (Finset (ConcretePlaquette d L))).filter
...
/-- Bucket-bound consumer using the bucket `tsum` API directly. -/
theorem connectedFiniteSum_le_of_cardBucketBounds_tsum
    {d L : ℕ} [NeZero d] [NeZero L]
    (K_bound : Finset (ConcretePlaquette d L) → ℝ)
    (p q : ConcretePlaquette d L)
    (r : ℝ) (hr_pos : 0 < r) (hr_lt1 : r < 1)
    ...
    (∑ Y ∈ (Finset.univ : Finset (Finset (ConcretePlaquette d L))).filter
      ...
      ∑' n : ℕ, C_conn * (n : ℝ) ^ dim * A₀ *
        r ^ (n + ⌈siteLatticeDist p.site q.site⌉₊) := by
  rw [connectedFiniteSum_eq_cardBucketTsum K_bound p q]
...
/-- Shifted bucket-bound consumer using the bucket `tsum` API directly. -/
theorem connectedFiniteSum_le_of_cardBucketBounds_tsum_shifted
    {d L : ℕ} [NeZero d] [NeZero L]
    (K_bound : Finset (ConcretePlaquette d L) → ℝ)
    (p q : ConcretePlaquette d L)
    (r : ℝ) (hr_pos : 0 < r) (hr_lt1 : r < 1)
...
/-- Total-size exponential finite-connecting-sum consumer using the bucket
`tsum` API directly. -/
theorem finiteConnectingSum_le_of_cardBucketBounds_tsum_total_exp
    {d L : ℕ} [NeZero d] [NeZero L]
    (K_bound : Finset (ConcretePlaquette d L) → ℝ)
    (p q : ConcretePlaquette d L)
    (r K : ℝ) (hr_pos : 0 < r) (hK_pos : 0 < K) (hKr_lt1 : K * r < 1)
    ...
    (h_bucket : ∀ n ∈ Finset.range (Fintype.card (ConcretePlaquette d L) + 1),
      (∑ Y ∈ (Finset.univ : Finset (Finset (ConcretePlaquette d L))).filter
          ...
          if Y.card = n + ⌈siteLatticeDist p.site q.site⌉₊
            then K_bound Y else 0) ≤
        C_conn * K ^ (n + ⌈siteLatticeDist p.site q.site⌉₊) * A₀ *
          r ^ (n + ⌈siteLatticeDist p.site q.site⌉₊)) :
    (∑ Y ∈ (Finset.univ : Finset (Finset (ConcretePlaquette d L))).filter
        (fun Y => p ∈ Y ∧ q ∈ Y), K_bound Y) ≤
      ∑' n : ℕ, C_conn * K ^ (n + ⌈siteLatticeDist p.site q.site⌉₊) * A₀ *
  ...
  rw [finiteConnectingSum_eq_cardBucketTsum K_bound p q h_zero]
  refine Summable.tsum_le_tsum ?_ (cardBucketSum_summable K_bound p q)
    (connecting_cluster_tsum_summable_total_exp r K hr_pos hK_pos hKr_lt1
      C_conn A₀ ⌈siteLatticeDist p.site q.site⌉₊)
  intro n
  by_cases hn : n ∈ Finset.range (Fintype.card (ConcretePlaquette d L) + 1)
  · exact h_bucket n hn
  · rw [cardBucketSum_eq_zero_of_not_mem_range K_bound p q n hn]
    exact connecting_cluster_summand_nonneg_total_exp r K hr_pos hK_pos
      C_conn A₀ hC hA ⌈siteLatticeDist p.site q.site⌉₊ n
...
/-- Total-size exponential variant of
`cardBucketSum_le_of_count_and_pointwise`, using the standard bucket count
`C_conn * K^(n + ceil dist)`. -/
theorem cardBucketSum_le_of_count_and_pointwise_total_exp
    {d L : ℕ} [NeZero d] [NeZero L]
    (K_bound : Finset (ConcretePlaquette d L) → ℝ)
    (p q : ConcretePlaquette d L)
    (r : ℝ) (hr_pos : 0 < r)
  ...
  calc
    ...
    _ = (B.card : ℝ) * term := by
        simp [term, nsmul_eq_mul]
    _ ≤ (C_conn * K ^ (n + ⌈siteLatticeDist p.site q.site⌉₊)) * term := by
        exact mul_le_mul_of_nonneg_right (by simpa [B] using h_count) hterm_nonneg
    _ = C_conn * K ^ (n + ⌈siteLatticeDist p.site q.site⌉₊) * A₀ *
        r ^ (n + ⌈siteLatticeDist p.site q.site⌉₊) := by
        ring
...
#print axioms finiteConnectingSum_le_of_cardBucketBounds_tsum_total_exp
...
#print axioms cardBucketSum_le_of_count_and_pointwise_total_exp
...
/-- Physical `d = 4` F3 endpoint from physical Mayer/activity data and the
physical total-size exponential count frontier.

Unlike `physicalClusterCorrelatorBound_of_physicalMayerData_expCount_ceil`,
this consumes the standard total-cardinality estimate
`C_conn * K^(n + ceil dist)` and returns decay at the effective parameter
`K*r`. -/
theorem physicalClusterCorrelatorBound_of_physicalMayerData_totalExpCount_ceil
    (N_c : ℕ) [NeZero N_c]
    (r K : ℝ) (hr_pos : 0 < r)
    (hK_pos : 0 < K) (hKr_lt1 : K * r < 1)
    (C_conn A₀ : ℝ) (hC : 0 < C_conn) (hA : 0 < A₀)
    ...
    PhysicalClusterCorrelatorBound N_c (K * r)
      (clusterPrefactorExp r K C_conn A₀) := by
  intro L _ β hβ F hF p q hdist
  rw [data.wilsonConnectedCorr_eq_toTruncatedActivities_connectingSum
    β hβ F hF p q hdist]
      ...
      (data.toTruncatedActivities β F p q).connectingBound p q ≤
        ∑' n : ℕ,
          C_conn * K ^ (n + ⌈siteLatticeDist p.site q.site⌉₊) * A₀ *
            r ^ (n + ⌈siteLatticeDist p.site q.site⌉₊) := by
    rw [TruncatedActivities.connectingBound_eq_finset_sum]
    exact finiteConnectingSum_le_of_cardBucketBounds_tsum_total_exp
      (fun Y => (data.toTruncatedActivities β F p q).K_bound Y) p q
      r K hr_pos hK_pos hKr_lt1 C_conn A₀ hC hA
      (fun Y hp hq hnot =>
        data.toTruncatedActivities_K_bound_eq_zero_of_not_connected
          β F p q Y hp hq hnot)
      (fun n hn => cardBucketSum_le_of_count_and_pointwise_total_exp
        (fun Y => (data.toTruncatedActivities β F p q).K_bound Y) p q
        r hr_pos K C_conn A₀ hA n
        (h_count.apply p q n hn hdist)
        (fun Y hY => pointwiseBucketBound_of_card_decay
          ...
          (fun Y =>
            data.toTruncatedActivities_K_bound_le_cardDecay β F p q Y)))
  exact ((data.toTruncatedActivities β F p q).two_point_decay_from_cluster_tsum_total_exp
    p q r K hr_pos hK_pos hKr_lt1 C_conn A₀ hC hA
    ⌈siteLatticeDist p.site q.site⌉₊ h_bound).trans
      (clusterPrefactorExp_effective_rpow_ceil_le_exp
        r K hr_pos hK_pos hKr_lt1 C_conn A₀ hC hA
...
#print axioms physicalClusterCorrelatorBound_of_physicalMayerData_totalExpCount_ceil
...
This packages exactly the two physical F3 obligations: physical Mayer/activity
data and the physical volume-uniform shifted count bound. -/
structure PhysicalOnlyShiftedF3MayerCountPackage
    (N_c : ℕ) [NeZero N_c] (wab : WilsonPolymerActivityBound N_c) where
  mayer : PhysicalShiftedF3MayerPackage N_c wab
  count : PhysicalShiftedF3CountPackage
...
/-- Build the fully physical F3 package from independently-produced physical
Mayer and physical count halves. -/
def ofSubpackages
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (mayer : PhysicalShiftedF3MayerPackage N_c wab)
    (count : PhysicalShiftedF3CountPackage) :
    PhysicalOnlyShiftedF3MayerCountPackage N_c wab where
  mayer := mayer
  count := count

/-- Project the physical Mayer/activity half out of a fully physical package. -/
def mayerPackage
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
...
/-- Project the physical count half out of a fully physical package. -/
def countPackage
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalOnlyShiftedF3MayerCountPackage N_c wab) :
...
/-- Increase only the physical count polynomial profile dimension in the fully
physical F3 package, preserving its Mayer half and count constant. -/
def mono_count_dim
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalOnlyShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    PhysicalOnlyShiftedF3MayerCountPackage N_c wab where
  mayer := pkg.mayer
  count := pkg.count.mono_dim k

@[simp] theorem mono_count_dim_mayer
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalOnlyShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    (pkg.mono_count_dim k).mayer = pkg.mayer := rfl

@[simp] theorem mono_count_dim_count_C_conn
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalOnlyShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    (pkg.mono_count_dim k).count.C_conn = pkg.count.C_conn := rfl

@[simp] theorem mono_count_dim_count_dim
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalOnlyShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
...
/-- The physical finite-volume truncated activities carried by the fully
physical F3 package. -/
noncomputable def toTruncatedActivities
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalOnlyShiftedF3MayerCountPackage N_c wab)
    ...
    (p q : ConcretePlaquette physicalClayDimension L) :
    TruncatedActivities (ConcretePlaquette physicalClayDimension L) :=
  pkg.mayer.data.toTruncatedActivities β F p q
...
theorem toTruncatedActivities_K_bound_le_cardDecay
  ...
  exact pkg.mayer.data.toTruncatedActivities_K_bound_le_cardDecay β F p q Y
...
theorem toTruncatedActivities_K_bound_eq_zero_of_not_connected
    ...
    (hp : p ∈ Y) (hq : q ∈ Y) (h_not_connected : ¬ PolymerConnected Y) :
    (pkg.toTruncatedActivities β F p q).K_bound Y = 0 := by
  exact pkg.mayer.data.toTruncatedActivities_K_bound_eq_zero_of_not_connected
    β F p q Y hp hq h_not_connected
...
theorem wilsonConnectedCorr_eq_toTruncatedActivities_connectingSum
    ...
    wilsonConnectedCorr (sunHaarProb N_c)
      (wilsonPlaquetteEnergy N_c) β F p q =
    (pkg.toTruncatedActivities β F p q).connectingSum p q := by
  exact pkg.mayer.data.wilsonConnectedCorr_eq_toTruncatedActivities_connectingSum
    β hβ F hF p q hdist

/-- Direct application form of the physical count half stored in the fully
physical F3 package. -/
theorem apply_count
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalOnlyShiftedF3MayerCountPackage N_c wab)
...
/-- Direct application form of the physical count half after increasing only
the count polynomial dimension in a fully physical F3 package. -/
theorem mono_count_dim_apply_count
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalOnlyShiftedF3MayerCountPackage N_c wab) (k : ℕ)
    {L : ℕ} [NeZero L]
    (p q : ConcretePlaquette physicalClayDimension L) (n : ℕ)
...
/-- Physical `d = 4` F3 endpoint from the single fully physical
Mayer/count package. -/
theorem physicalClusterCorrelatorBound_of_physicalOnlyShiftedF3MayerCountPackage
    {N_c : ℕ} [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (pkg : PhysicalOnlyShiftedF3MayerCountPackage N_c wab) :
    PhysicalClusterCorrelatorBound N_c wab.r
...
#print axioms PhysicalOnlyShiftedF3MayerCountPackage.ofSubpackages_mayerPackage
#print axioms PhysicalOnlyShiftedF3MayerCountPackage.ofSubpackages_countPackage
#print axioms PhysicalOnlyShiftedF3MayerCountPackage.mayerPackage_A₀
#print axioms PhysicalOnlyShiftedF3MayerCountPackage.mayerPackage_data
#print axioms PhysicalOnlyShiftedF3MayerCountPackage.countPackage_C_conn
#print axioms PhysicalOnlyShiftedF3MayerCountPackage.countPackage_dim
...
#print axioms PhysicalOnlyShiftedF3MayerCountPackage.toTruncatedActivities_K_bound_le_cardDecay
#print axioms PhysicalOnlyShiftedF3MayerCountPackage.toTruncatedActivities_K_bound_eq_zero_of_not_connected
#print axioms PhysicalOnlyShiftedF3MayerCountPackage.wilsonConnectedCorr_eq_toTruncatedActivities_connectingSum
...
/-- Increase only the physical count polynomial profile dimension in the mixed
physical F3 package, preserving its global Mayer half and count constant. -/
def mono_count_dim
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
...
def toPhysicalOnly
  ...
  PhysicalOnlyShiftedF3MayerCountPackage.ofSubpackages
    pkg.mayer.toPhysical pkg.count

@[simp] theorem toPhysicalOnly_mayerPackage
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalShiftedF3MayerCountPackage N_c wab) :
    pkg.toPhysicalOnly.mayerPackage = pkg.mayer.toPhysical := rfl

@[simp] theorem toPhysicalOnly_countPackage
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalShiftedF3MayerCountPackage N_c wab) :
    pkg.toPhysicalOnly.countPackage = pkg.count := rfl

@[simp] theorem toPhysicalOnly_mayer_A₀
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalShiftedF3MayerCountPackage N_c wab) :
    pkg.toPhysicalOnly.mayer.A₀ = pkg.mayer.A₀ := rfl

@[simp] theorem toPhysicalOnly_count_C_conn
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalShiftedF3MayerCountPackage N_c wab) :
    pkg.toPhysicalOnly.count.C_conn = pkg.count.C_conn := rfl
...
/-- Direct application form of the physical count half after increasing only
the count polynomial dimension in a mixed physical F3 package. -/
theorem mono_count_dim_apply_count
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : PhysicalShiftedF3MayerCountPackage N_c wab) (k : ℕ)
    {L : ℕ} [NeZero L]
...
/-- Increase only the count polynomial profile dimension in the single global
F3 package, preserving the Mayer half and the count constant. -/
def mono_count_dim
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
...
@[simp] theorem mono_count_dim_C_conn
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    (pkg.mono_count_dim k).C_conn = pkg.C_conn := rfl

@[simp] theorem mono_count_dim_A₀
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    (pkg.mono_count_dim k).A₀ = pkg.A₀ := rfl

@[simp] theorem mono_count_dim_dim
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    (pkg.mono_count_dim k).dim = pkg.dim + k := rfl

@[simp] theorem mono_count_dim_data
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
...
/-- Direct application form of the count half after increasing only the count
polynomial dimension in the global shifted F3 package. -/
theorem mono_count_dim_apply_count
    {N_c : ℕ} [NeZero N_c] {wab : WilsonPolymerActivityBound N_c}
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ)
    {d L : ℕ} [NeZero d] [NeZero L]
...
/-- The single-package F3 route still yields the authentic mass-gap structure
after increasing only the count polynomial dimension. -/
noncomputable def clayMassGap_of_shiftedF3MayerCountPackage_mono_count_dim
    (N_c : ℕ) [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    ClayYangMillsMassGap N_c :=
  clayMassGap_of_shiftedF3MayerCountPackage N_c wab (pkg.mono_count_dim k)

/-- The single-package F3 route still yields the connected-decay hub after
increasing only the count polynomial dimension. -/
noncomputable def clayConnectedCorrDecay_of_shiftedF3MayerCountPackage_mono_count_dim
    (N_c : ℕ) [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    ClayConnectedCorrDecay N_c :=
  clayConnectedCorrDecay_of_shiftedF3MayerCountPackage N_c wab
    (pkg.mono_count_dim k)

/-- The weak theorem endpoint is unchanged by increasing only the count
polynomial dimension. -/
theorem clay_theorem_of_shiftedF3MayerCountPackage_mono_count_dim
    (N_c : ℕ) [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    ClayYangMillsTheorem :=
  clay_theorem_of_shiftedF3MayerCountPackage N_c wab (pkg.mono_count_dim k)
...
/-- The mono-count single-package mass-gap endpoint keeps decay rate
`kpParameter wab.r`. -/
theorem clayMassGap_of_shiftedF3MayerCountPackage_mono_count_dim_mass_eq
    (N_c : ℕ) [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    (clayMassGap_of_shiftedF3MayerCountPackage_mono_count_dim
      N_c wab pkg k).m = kpParameter wab.r := rfl

/-- The mono-count single-package mass-gap endpoint exposes the shifted
cluster prefactor. -/
theorem clayMassGap_of_shiftedF3MayerCountPackage_mono_count_dim_prefactor_eq
    (N_c : ℕ) [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    (clayMassGap_of_shiftedF3MayerCountPackage_mono_count_dim
      N_c wab pkg k).C =
      clusterPrefactorShifted wab.r pkg.C_conn pkg.A₀ (pkg.dim + k) := rfl

/-- The mono-count single-package connected-decay endpoint keeps decay rate
`kpParameter wab.r`. -/
theorem clayConnectedCorrDecay_of_shiftedF3MayerCountPackage_mono_count_dim_mass_eq
    (N_c : ℕ) [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
...
/-- The mono-count single-package connected-decay endpoint exposes the shifted
cluster prefactor. -/
theorem clayConnectedCorrDecay_of_shiftedF3MayerCountPackage_mono_count_dim_prefactor_eq
    (N_c : ℕ) [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (pkg : ShiftedF3MayerCountPackage N_c wab) (k : ℕ) :
    (clayConnectedCorrDecay_of_shiftedF3MayerCountPackage_mono_count_dim
...
/-- The subpackage mass-gap endpoint has decay rate `kpParameter wab.r`. -/
theorem clayMassGap_of_shiftedF3Subpackages_mass_eq
    (N_c : ℕ) [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (mayer : ShiftedF3MayerPackage N_c wab)
    (count : ShiftedF3CountPackage) :
    (clayMassGap_of_shiftedF3Subpackages N_c wab mayer count).m =
      kpParameter wab.r := rfl

/-- The subpackage mass-gap endpoint has the shifted cluster prefactor. -/
theorem clayMassGap_of_shiftedF3Subpackages_prefactor_eq
    (N_c : ℕ) [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (mayer : ShiftedF3MayerPackage N_c wab)
    (count : ShiftedF3CountPackage) :
    (clayMassGap_of_shiftedF3Subpackages N_c wab mayer count).C =
      clusterPrefactorShifted wab.r count.C_conn mayer.A₀ count.dim := rfl

/-- The subpackage connected-decay endpoint has decay rate
`kpParameter wab.r`. -/
theorem clayConnectedCorrDecay_of_shiftedF3Subpackages_mass_eq
    (N_c : ℕ) [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (mayer : ShiftedF3MayerPackage N_c wab)
    (count : ShiftedF3CountPackage) :
    (clayConnectedCorrDecay_of_shiftedF3Subpackages N_c wab mayer count).m =
      kpParameter wab.r := rfl

/-- The subpackage connected-decay endpoint has the shifted cluster
prefactor. -/
theorem clayConnectedCorrDecay_of_shiftedF3Subpackages_prefactor_eq
    (N_c : ℕ) [NeZero N_c]
    (wab : WilsonPolymerActivityBound N_c)
    (mayer : ShiftedF3MayerPackage N_c wab)
    (count : ShiftedF3CountPackage) :
...
#print axioms ShiftedF3MayerCountPackage.mono_count_dim_C_conn
...
#print axioms clayMassGap_of_shiftedF3MayerCountPackage_mono_count_dim_mass_eq
#print axioms clayMassGap_of_shiftedF3MayerCountPackage_mono_count_dim_prefactor_eq
#print axioms clayConnectedCorrDecay_of_shiftedF3MayerCountPackage_mono_count_dim_mass_eq
#print axioms clayConnectedCorrDecay_of_shiftedF3MayerCountPackage_mono_count_dim_prefactor_eq
#print axioms clayMassGap_of_shiftedF3Subpackages_mass_eq
