  ...
  have hpn : ∀ (n : ℕ), ∫ x, (max (min (u x) (n : ℝ)) (-(n : ℝ)) -
    ...
    have hEtn : E (fun x => max (min (u x) (n : ℝ)) (-(n : ℝ)) - mn) ≤ E u := by
      ...
      · subst hn
        ...
        -- Goal: E (fun x => max (min (u x) 0) 0) ≤ E u
        have htrunc : (fun x : Ω => max (min (u x) (0:ℝ)) (0:ℝ)) = fun _ => 0 := by
          ext x; exact max_eq_right (min_le_right (u x) 0)
        have hE0 : E (fun _ : Ω => (0:ℝ)) = 0 := by
          have h := hE_scale 0 (fun _ : Ω => (1:ℝ)); simpa using h
        rw [htrunc, hE0]; exact hE_base.1 u
