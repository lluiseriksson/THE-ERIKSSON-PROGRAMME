...
set_option maxHeartbeats 12000000 in
/-- Ordered-partition block formula at a contour point independent of the
fixed vertex-interpolation base. -/
theorem
    ftaylorSeries_cmp116FiniteMultiaffineInterpolation_sourcePi4FullRealCovariance_apply_orderedPartitionBlock_at
    {M Q Nc R Δ n : ℕ}
    [NeZero M] [NeZero Q] [NeZero (Nc ^ 2 - 1)]
    (anchor : FinBox 4 Q)
    (K : PhysicalEndomorphism M Q Nc)
  ...
  exact
    ftaylorSeries_cmp116FiniteMultiaffineInterpolation_sourcePi4FullRealCovariance_apply_coordinateBlock_at
      anchor K hc hmass hK hAhead hrho hrate hgeom PatchCert htri
      hrange hΔ hΔ1 vertexBase sigma L hL hcover
      (fun i => coordinates (partition.emb block i))
      (cmp102Eq80SourcePi4FaaDiBrunoBlockCoordinates_fresh
        partition coordinates hinjective block)
      hRweak hvertexBase hsigma hsmall

set_option maxHeartbeats 16000000 in
/-- Exact physical Faà di Bruno expansion at an arbitrary certified contour
point, while the finite vertex polynomial keeps a separate fixed base. -/
theorem
  ...
  rw [
    iteratedFDeriv_cmp102Eq80SourcePi4RealPotentialVertexPolynomial_apply_eq_sum_orderedFinpartition
      anchor K hc hmass hK D D₃ V₀ Δπ J
      vertexBase L sigma A
      (fun i => Pi.single (coordinates i) 1) hV₀]
  apply Finset.sum_congr rfl
  intro partition _hpartition
  congr 1
  funext block
  ...
  exact
    ftaylorSeries_cmp116FiniteMultiaffineInterpolation_sourcePi4FullRealCovariance_apply_orderedPartitionBlock_at
      anchor K hc hmass hK hAhead hrho hrate hgeom PatchCert htri
      hrange hΔ hΔ1 vertexBase sigma L hL hcover coordinates hinjective
      hRweak hvertexBase hsigma hsmall partition block
