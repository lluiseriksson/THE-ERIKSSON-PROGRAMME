...
PRE-VALIDATION: the empty-carrier no-go theorem below is present in source,
...
/-- No centered-conditioned physical term source exists over the empty
localized block carrier.  Its required covariance-lower certificate contains
an actual carrier witness, while the literal physical coordinate dictionary
has no coordinate localized in the empty block region.

This theorem records why a terminal source *family* must request sources only
for genuine, nonempty localization indices rather than for every raw `Z0`. -/
theorem not_nonempty_cmp116Eq226CenteredConditionedPhysicalTermSource_empty_Z0
    {nDelta nY M Q Nc L lieDim : ℕ}
    [NeZero M] [NeZero Q] [NeZero Nc] [NeZero (Nc ^ 2 - 1)]
    [NeZero (2 * Q)] [NeZero (M * (2 * Q))] [NeZero L] [NeZero lieDim]
    (Dict : PhysicalGaugeCMP116Dictionary
    ...
    ¬ Nonempty
      (CMP116Eq226CenteredConditionedPhysicalTermSource
        (nDelta := nDelta) (nY := nY) Dict Y0 P ∅ Z) := by
  intro h
  rcases h with ⟨S⟩
  have hcarrier :=
    S.conditionedCovariance_nondegenerate.carrier_nonempty
