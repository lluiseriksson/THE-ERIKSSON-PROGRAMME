...
precede depth,K,Q,Nc. No outer-space isometry, total-owner cardinality,
proper regional inverse, derivative B0 or window15 attainment is claimed.
Counters20/41,TermSource0 remain unchanged.
-/
...
theorem cmp99SourceFlowPhysicalRealGreen_typedKernel
...
theorem cmp99SourceFlowPhysicalRealGreen_ownerAction_of_budget
  ...
  classical
  have hk := cmp99SourceFlowPhysicalRealGreen_typedKernel
    (L := L) (K := K) (Q := Q) (Nc := Nc)
    hL depth ha hrho hamplitude hradius hdenWindow hpairWindow
...
theorem exists_cmp99SourceFlowPhysicalRealGreen_uniformValueAction
    {a : ℝ} (ha : 0 < a) (hL : 2 ≤ L) :
    ∃ rho C : ℝ, 0 < rho ∧ 0 < C ∧
  ...
  exact cmp99SourceFlowPhysicalRealGreen_ownerAction_of_budget
    (L := L) (K := K) (Q := Q) (Nc := Nc)
