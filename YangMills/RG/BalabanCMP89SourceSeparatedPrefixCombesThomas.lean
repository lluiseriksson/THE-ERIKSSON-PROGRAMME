import YangMills.RG.BalabanCMP96SourceSeparatedRegionalPrefixGreen
...
This file uses P7's literal CMP85 final-prefix precision: zero bare mass and the source counting coefficient generated at prefix `depth`. The precision range, norm budget, coercivity and inverse laws are all constructed from the same physical tower. P5's ambient Green and P8's local Dirichlet Green then inherit exponential kernel bounds.
...
noncomputable def cmp89SourceSeparatedPrefixCountingCoefficient
  ...
  let T := cmp89SourceSeparatedPrefixTower
...
noncomputable def cmp89SourceSeparatedPrefixPrecisionUpperBound
    (hL : 2 ≤ L) (depth : ℕ) (spacing epsilon a : ℝ)
    (background : GaugeConfig 4
      (cmp99RegionalLatticeSize L (2 * (K * Q)) (depth + 1)) (SUN Nc))
    (budget : CMP99SourceUbarClosedBudget 4 L Nc (depth + 1) epsilon)
    (fineSmall : ∀ e : ConcreteEdge 4
      ...
      ‖(background e : Matrix (Fin Nc) (Fin Nc) ℂ) - 1‖ ≤ epsilon) : ℝ :=
  16 / spacing ^ 2 +
    |cmp89SourceSeparatedPrefixCountingCoefficient hL depth
      spacing epsilon a background budget fineSmall|

theorem cmp89SourceSeparatedPrefixPrecisionUpperBound_pos
    (hL : 2 ≤ L) (depth : ℕ) {spacing epsilon a : ℝ}
    (hspacing : 0 < spacing)
    (background : GaugeConfig 4
      (cmp99RegionalLatticeSize L (2 * (K * Q)) (depth + 1)) (SUN Nc))
    ...
    0 < cmp89SourceSeparatedPrefixPrecisionUpperBound hL depth
      spacing epsilon a background budget fineSmall := by
  unfold cmp89SourceSeparatedPrefixPrecisionUpperBound
...
theorem norm_cmp89SourceSeparatedFinalPrefix_Qprime_le_one
  ...
  have hterminal : T.towerAt (Fin.last (depth + 1)) =
    ...
    simpa only [T, regions, Omega,
      cmp89SourceSeparatedPrefixTower,
...
private noncomputable def cmp89SourceSeparatedFinalPrefixQprimeMass
    (hL : 2 ≤ L) (depth : ℕ) (spacing epsilon : ℝ)
    (background : GaugeConfig 4
      (cmp99RegionalLatticeSize L (2 * (K * Q)) (depth + 1)) (SUN Nc))
    (budget : CMP99SourceUbarClosedBudget 4 L Nc (depth + 1) epsilon)
  ...
  let T := cmp89SourceSeparatedPrefixTower
    ...
    (spacing := spacing) (epsilon := epsilon) hL depth background budget
    fineSmall
  let r := cmp85LastPositivePrefix (depth + 1) (Nat.succ_pos depth)
  (T.towerAt r.1).Qprime.adjoint.comp (T.towerAt r.1).Qprime

/-- The exact final-prefix `Q'†Q'` has the printed terminal-block radius. -/
theorem cmp89SourceSeparatedFinalPrefix_QprimeMass_finiteRange
    (hL : 2 ≤ L) (depth : ℕ) (spacing epsilon : ℝ)
    (background : GaugeConfig 4
      (cmp99RegionalLatticeSize L (2 * (K * Q)) (depth + 1)) (SUN Nc))
    (budget : CMP99SourceUbarClosedBudget 4 L Nc (depth + 1) epsilon)
    ...
    FinitePiLpFiniteRange
      (cmp89SourceSeparatedFinalPrefixQprimeMass
        (L := L) (K := K) (Q := Q) (Nc := Nc)
...
theorem cmp89SourceSeparatedFinePrefixPrecision_finiteRange
    (hL : 2 ≤ L) (depth : ℕ) (spacing epsilon a : ℝ)
    (background : GaugeConfig 4
      (cmp99RegionalLatticeSize L (2 * (K * Q)) (depth + 1)) (SUN Nc))
    (budget : CMP99SourceUbarClosedBudget 4 L Nc (depth + 1) epsilon)
  ...
  have hmass :=
    cmp89SourceSeparatedFinalPrefix_QprimeMass_finiteRange
      hL depth spacing epsilon background budget fineSmall
        source target v hmassFar
  rw [cmp89SourceSeparatedFinePrefixPrecision,
    cmp85SourceGeneratedPrefixPrecision,
    cmp85BareMassPrecision, cmp99SourceGaugePrecision]
  ...
  change
    ((cmp99ActiveRegionSourceCovariantLaplacian
      ...
      (_ : ℝ) • ((cmp89SourceSeparatedFinalPrefixQprimeMass
...
theorem norm_cmp89SourceSeparatedFinePrefixPrecision_le
    (hL : 2 ≤ L) (depth : ℕ) {spacing epsilon a : ℝ}
    (hspacing : 0 < spacing)
    (background : GaugeConfig 4
      (cmp99RegionalLatticeSize L (2 * (K * Q)) (depth + 1)) (SUN Nc))
    ...
    (fineSmall : ∀ e : ConcreteEdge 4
      (cmp99RegionalLatticeSize L (2 * (K * Q)) (depth + 1)),
      ‖(background e : Matrix (Fin Nc) (Fin Nc) ℂ) - 1‖ ≤ epsilon) :
    ‖cmp89SourceSeparatedFinePrefixPrecision hL depth
        spacing epsilon a background budget fineSmall‖ ≤
      cmp89SourceSeparatedPrefixPrecisionUpperBound hL depth
        spacing epsilon a background budget fineSmall := by
  let Omega := cmp99SourceSeparatedGeneratedPhysicalFullCoarseRegion K Q
  let T := cmp89SourceSeparatedPrefixTower
    (spacing := spacing) (epsilon := epsilon) hL depth background budget
    fineSmall
  let r := cmp85LastPositivePrefix (depth + 1)
    (Nat.succ_pos depth)
  let b := cmp85SourcePrefixCountingCoefficient T a r
  have hQ : ‖(T.towerAt r.1).Qprime‖ ≤ 1 := by
    exact norm_cmp89SourceSeparatedFinalPrefix_Qprime_le_one
...
theorem cmp89SourceSeparatedFinePrefixPrecision_kernelBound
    (hL : 2 ≤ L) (depth : ℕ) {spacing epsilon a : ℝ}
    (hspacing : 0 < spacing)
    (background : GaugeConfig 4
      (cmp99RegionalLatticeSize L (2 * (K * Q)) (depth + 1)) (SUN Nc))
  ...
  exact norm_cmp89SourceSeparatedFinePrefixPrecision_le
    hL depth hspacing background budget fineSmall
...
theorem cmp89SourceSeparatedFinePrefixGreen_exponentialKernelBound
    ...
    (hrate : 0 < rate)
    (hbudget :
      cmp89SourceSeparatedPrefixPrecisionUpperBound
          (L := L) (K := K) (Q := Q) (Nc := Nc) hL depth
          spacing epsilon a background budget fineSmall *
        (Real.exp (rate * (L ^ (depth + 1) : ℕ)) - 1) *
        (((2 * L ^ (depth + 1) + 1) ^ 4 : ℕ) : ℝ) ≤
      cmp89SourceSeparatedPrefixCoercivity
          (L := L) (K := K) (Q := Q) (Nc := Nc) hL depth
          spacing epsilon a background budget fineSmall / 2) :
    let Omega := cmp99SourceSeparatedGeneratedPhysicalFullCoarseRegion K Q
    FinitePiLpExponentialKernelBound
      (cmp89SourceSeparatedFinePrefixGreen
        (L := L) (K := K) (Q := Q) (Nc := Nc) hL depth hspacing ha
        background budget fineSmall hsmall)
      (fun x y : ActiveGaugeRegion.Site
          (cmp99IteratedLiftActiveRegion (M := L) Omega (depth + 1)) =>
        finBoxDist x.1 y.1)
      (2 / cmp89SourceSeparatedPrefixCoercivity
        (L := L) (K := K) (Q := Q) (Nc := Nc) hL depth
        spacing epsilon a background budget fineSmall)
      rate := by
  dsimp only
  let Omega := cmp99SourceSeparatedGeneratedPhysicalFullCoarseRegion K Q
  let Kfine := cmp89SourceSeparatedFinePrefixPrecision hL depth
    spacing epsilon a background budget fineSmall
  let Cfine := cmp89SourceSeparatedFinePrefixGreen hL depth hspacing ha
    background budget fineSmall hsmall
  apply finitePiLpExponentialKernelBound_of_coercive
    ...
    Kfine Cfine
  · exact cmp89SourceSeparatedFinePrefixPrecision_finiteRange
      hL depth spacing epsilon a background budget fineSmall
  · exact cmp89SourceSeparatedFinePrefixPrecision_kernelBound
      hL depth hspacing background budget fineSmall
  · exact isCoerciveCLM_cmp85SourceGeneratedPrefixPrecision
      (d := 4) (M := L) (N := 2 * (K * Q)) (Nc := Nc)
      (by norm_num) hL Omega (depth + 1) hspacing ha 0 background
      budget.toRadiusChain fineSmall hsmall
      (cmp85LastPositivePrefix (depth + 1) (Nat.succ_pos depth))
  · exact cmp85SourceGeneratedPrefixPrecision_comp_green
      (d := 4) (M := L) (N := 2 * (K * Q)) (Nc := Nc)
      (by norm_num) hL Omega (depth + 1) hspacing ha 0 background
      budget.toRadiusChain fineSmall hsmall
      (cmp85LastPositivePrefix (depth + 1) (Nat.succ_pos depth))
...
theorem cmp89SourceSeparatedAmbientPrefixGreen_exponentialKernelBound
    (hL : 2 ≤ L) (depth : ℕ) {spacing epsilon a rate : ℝ}
    (hspacing : 0 < spacing) (ha : 0 < a)
    (background : GaugeConfig 4
      (cmp99RegionalLatticeSize L (2 * (K * Q)) (depth + 1)) (SUN Nc))
  ...
  let Cfine := cmp89SourceSeparatedFinePrefixGreen hL depth hspacing ha
    background budget fineSmall hsmall
  have hfine :=
    cmp89SourceSeparatedFinePrefixGreen_exponentialKernelBound
...
theorem cmp89SourceSeparatedAmbientPrefixPrecision_exponentialKernelBound
  ...
  have hfine : FinitePiLpExponentialKernelBound Kfine
    ...
    apply finitePiLpTypedExponentialKernelBound_of_finiteRange
      (beta := cmp89SourceSeparatedPrefixPrecisionUpperBound hL depth
        spacing epsilon a background budget fineSmall)
      (R := L ^ (depth + 1))
    · exact (cmp89SourceSeparatedPrefixPrecisionUpperBound_pos hL depth
        hspacing background budget fineSmall).le
    · exact hdecay
    · exact cmp89SourceSeparatedFinePrefixPrecision_finiteRange
        hL depth spacing epsilon a background budget fineSmall
...
theorem cmp96SourceSeparatedRegionalPrefixGreen_exponentialKernelBound
  ...
  exact cmp99RegionalDirichletGreen_exponentialKernelBound
    ...
    (cmp89SourceSeparatedAmbientPrefixPrecision_exponentialKernelBound
