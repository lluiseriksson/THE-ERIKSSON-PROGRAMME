...
common scope.  This is the finite column to which residue-zero aliasing will
later be applied.

No Fourier-series equality, coefficient bound, owner transport, regional
...
/-- The internally generated source-separated `G Q'^*` applied to one
literal coarse point source is exactly the normalized finite sum of the
complete CMP89 endpoint integrand, with no target-dependent readout supplied
by the caller. -/
theorem
