...
/-!
# Residual-preserving outer trace integration

The source residual from equation (1.36) and the large-field `P` penalty from
equation (2.22) are scalar only after the inner Gaussian has been integrated.
This module retains that exact scalar while using the trace-driven outer
Gaussian estimate.
-/
...
/-- Fixed-contour trace estimate with the literal equation-(1.36)/(2.22)
residual factor preserved outside the two Gaussian integrations. -/
theorem norm_analyticIntegrand_le_of_outerTraceInteractionEnergy_cutoff
  ...
  have hinnerPos :
    ...
    simpa [sub_eq_add_neg, Matrix.mul_neg] using
      (posDef_one_sub_localized_covariance_of_l2_opNorm_small
        S C.referenceRoot alpha halpha hrootSmall)
    ...
    ‖∫ x,
      ...
      have hinnerIntegral :=
        G.norm_innerIntegral_le_exp_residual_sub_cardPenalty_mul_eq224Majorant
          ...
          (-(alpha • cmp116Eq223CoordinateProjection S)) (r x)
          gamma residual
      ...
      have hmajor :=
        cmp116Eq224_localized_gaussianMajorant_le_sourceEnergy_card
          S C.referenceRoot alpha (r x) x sourceRate 0
...
/-- Shifted-polydisc boundary form of the residual-preserving trace-driven
outer estimate. -/
theorem nestedCauchyBoundaryBound_of_outerTraceInteractionEnergy_cutoff_onPolydiscs
    {nDelta nY lieDim : ℕ} {Bond Site E : Type*}
    {Psi Phi : Site → Type*}
    [Fintype Bond] [DecidableEq Bond] [Norm E]
    [Nonempty (Bond × Fin lieDim)]
  ...
  apply C.norm_analyticIntegrand_le_of_outerTraceInteractionEnergy_cutoff
    Y0 P sigma tau psi phi S alpha sourceRate determinantBound gamma
      residual (r sigma tau) halpha hrootSmall
