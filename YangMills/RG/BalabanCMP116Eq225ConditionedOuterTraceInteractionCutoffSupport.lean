...
/-!
# Conditioned outer trace bound on the literal cutoff support

The centered physical source route previously consumed an almost-everywhere
interaction estimate for every inner Gaussian coordinate.  That contract is
too strong for the cubic Taylor remainder, whose small-field estimate is
available only where the literal cutoff is nonzero.

This module proves the fixed-contour equation-(2.25) bound with the
source-faithful support-sensitive contract.  Outside the cutoff support the
inner integrand is definitionally zero; all determinant, source-energy, and
outer Gaussian constants are unchanged.
-/
...
/-- Fixed-contour conditioned equation-(2.25) estimate requiring the
interaction inequality only where the literal cutoff factor is nonzero. -/
theorem
    norm_analyticIntegrand_le_of_conditionedOuterTraceInteractionEnergy_cutoffSupport
...
/-- Mixed physical Cauchy boundary with the source-faithful interaction
estimate required only on the literal cutoff support. Weakening coordinates
use shifted discs, while source coordinates use centered discs. -/
theorem
    nestedCauchyBoundaryBound_of_conditionedOuterTraceInteractionEnergy_cutoffSupport_onShiftedCenteredPolydiscs
    {nDelta nY lieDim : ℕ} {Bond Site E : Type*}
    {Psi Phi : Site → Type*}
    [Fintype Bond] [DecidableEq Bond] [Norm E]
    [Nonempty (Bond × Fin lieDim)]
