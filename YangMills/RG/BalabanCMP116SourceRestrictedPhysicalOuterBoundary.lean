...
/-!
# Fully physical outer boundary bound for the restricted source contour

This module composes the source-specific contour constructor with the
carrier-linear determinant, trace, and bilateral-radius estimates.  The
source-facing theorem retains only the genuine domination and source-energy
premises of the inner Gaussian integral and one final scalar smallness budget.
-/
...
/-- The nested Cauchy boundary bound for the literal restricted source
contour.  Its determinant, symmetric trace test, and bilateral `R1` radius
are generated internally from the physical patch certificate. -/
theorem nestedCauchyBoundaryBound_of_sourcePi4PhysicalOuterReduction
    ...
    (alpha sourceRate sourceResidual : ℝ)
    ...
    CMP116Eq214NestedCauchyBoundaryBound nDelta nY
      ...
      ((Real.exp
        ...
        cmp116Eq225LocalizedSourceEnergyPrefactor S
          Csource.referenceRoot alpha sourceResidual) *
        Real.exp
  ...
  apply
    (C.withSourcePi4RestrictedComplexGaussianOfPhysicalContour
      ...
      hAhead hrho hrate hgeom Cert htri hDelta hDelta1
      hradius hradiusCap hseries hneumann
    ).nestedCauchyBoundaryBound_of_outerTraceInteractionEnergy_bilateralSmall_onPolydiscs
      Y0 P psi phi S alpha sourceRate sourceResidual
      (Real.exp
        (cmp116SourceRestrictedUniformContourDeterminantCost
            M Nc Delta radius (1 + radius) rate Ahead rho
            ‖cmp116PhysicalEndomorphismComplexMatrix K‖ *
          (Z0.card : ℝ)))
      r halpha hrootSmall hq0 hq1
