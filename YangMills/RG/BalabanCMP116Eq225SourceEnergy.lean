...
/-- Rank-localized determinant and residual-source part of (2.24).  Unlike
`cmp116Eq225SourceEnergyPrefactor`, its exponent is the physical carrier
cardinality rather than the full Gaussian dimension. -/
def cmp116Eq225LocalizedSourceEnergyPrefactor
    {ι : Type*} [Fintype ι] [DecidableEq ι]
    (S : Finset ι) (R : Matrix ι ι ℝ)
    (alpha sourceResidual : ℝ) : ℝ :=
...
/-- Source-energy reduction with the determinant localized to `rank P_S =
|S|`.  This is the volume-uniform form needed by (2.26). -/
theorem cmp116Eq224_localized_gaussianMajorant_le_sourceEnergy_card
    {ι : Type*} [Fintype ι] [DecidableEq ι]
    (S : Finset ι) (R : Matrix ι ι ℝ) (alpha : ℝ)
    (r x : ι → ℝ) (sourceRate sourceResidual : ℝ)
    (halpha : 0 ≤ alpha)
    ...
    cmp116Eq224GaussianMajorant R
        (-(alpha • cmp116Eq223CoordinateProjection S))
        (fun i => (r i : ℂ)) ≤
      cmp116Eq225LocalizedSourceEnergyPrefactor S R alpha sourceResidual *
        Real.exp
          ((cmp116Eq225SourceCoefficient R alpha * sourceRate) *
            ∑ i ∈ S, x i ^ 2) := by
...
/-- Fixed-contour endpoint with both Gaussian factors indexed by the physical
localized carrier. -/
theorem CMP116Eq214FiniteGaussianData.norm_analyticIntegrand_le_of_sourceEnergy_card
    {nDelta nY lieDim : ℕ} {Bond Ψ Φ E : Type*}
    [Fintype Bond] [DecidableEq Bond] [Norm E]
    (G : CMP116Eq214FiniteGaussianData nDelta nY Bond Ψ Φ E lieDim)
    (Y0 P : Finset Bond)
  ...
  apply norm_integral_le_of_localizedGaussianEnergy
    S (cmp116Eq225SourceCoefficient (G.covarianceRoot sigma tau) alpha *
      ...
      (outerBound *
        cmp116Eq225LocalizedSourceEnergyPrefactor S
          (G.covarianceRoot sigma tau) alpha sourceResidual) hbeta
  filter_upwards [] with x
  rw [norm_mul]
  have hinner :=
    G.norm_innerIntegral_le_eq224Majorant_of_l2OpNormSmallness
