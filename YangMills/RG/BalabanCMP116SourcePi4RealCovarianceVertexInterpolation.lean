...
/-- The finite vertex polynomial and the source covariance agree on the whole
real line of any one physical weakening coordinate.  Only the other
coordinates must lie in the certified region; the free scalar `t` is
arbitrary.  This supplies equality of derivatives even at contour endpoints.
-/
theorem
    cmp116FiniteMultiaffineInterpolation_sourcePi4FullRealCovariance_update_eq
    {M Q Nc R Δ : ℕ}
    [NeZero M] [NeZero Q] [NeZero (Nc ^ 2 - 1)]
    (anchor : FinBox 4 Q)
    (K : PhysicalEndomorphism M Q Nc)
    ...
    cmp116FiniteMultiaffineInterpolation
        (fun sigma =>
          cmp116SourcePi4FullRealWeakenedCovarianceMixedDerivative
            (R := R) anchor K hc hmass hK sigma ∅)
        base coordinates (Function.update s d t) =
      cmp116SourcePi4FullRealWeakenedCovarianceMixedDerivative
        (R := R) anchor K hc hmass hK (Function.update s d t) ∅ := by
  have hs0Shift :
  ...
  have hpoly :=
    cmp116FiniteMultiaffineInterpolation_update_eq_affine
      (fun sigma =>
        cmp116SourcePi4FullRealWeakenedCovarianceMixedDerivative
          (R := R) anchor K hc hmass hK sigma ∅)
      base coordinates hcoordinates s d (hcover d) t
  have hzero :=
    cmp116FiniteMultiaffineInterpolation_sourcePi4FullRealCovariance_eq_self
      anchor K hc hmass hK hAhead hrho hrate hgeom PatchCert htri hrange
      hΔ hΔ1 base (Function.update s d 0) coordinates hcoordinates hcover
      hRweak hbaseShift hbaseCap hs0Shift hs0Cap hsmall
  have hone :=
    cmp116FiniteMultiaffineInterpolation_sourcePi4FullRealCovariance_eq_self
      anchor K hc hmass hK hAhead hrho hrate hgeom PatchCert htri hrange
      hΔ hΔ1 base (Function.update s d 1) coordinates hcoordinates hcover
      hRweak hbaseShift hbaseCap hs1Shift hs1Cap hsmall
  have hphysical :=
    cmp116SourcePi4FullRealWeakenedCovarianceMixedDerivative_update_eq_affine
      anchor K hc hmass hK hAhead hrho hrate hgeom PatchCert htri hrange
      hΔ hΔ1 s ∅ d (by simp) hRweak hsShift hsCap hsmall t
  have hphysicalOne :=
    cmp116SourcePi4FullRealWeakenedCovarianceMixedDerivative_update_eq_affine
      anchor K hc hmass hK hAhead hrho hrate hgeom PatchCert htri hrange
      hΔ hΔ1 s ∅ d (by simp) hRweak hsShift hsCap hsmall 1
