...
/-- Factorwise strong measurability form of the generic raw integrability
compiler.  The connected-cover construction is measurability-preserving, so a
source proof only needs to show strong measurability of the raw localized
activities that actually enter `Λ`. -/
theorem integrable_appendixFHoleConnectedLocalActivity_globalEval_of_rawMetricDecay_of_factor_stronglyMeasurable
    {d L : ℕ} [NeZero L]
    {β : Type*} [MeasurableSpace β]
    {Ψ : Cube d L → Type*}
    (HF : HoleFamily d L)
    ...
    Integrable
      (fun φ : (∀ _ : Cube d L, β) =>
        (appendixFHoleConnectedLocalActivity
          HF z Λ H Y).globalEval ψ φ)
      (Measure.pi fun _ : Cube d L => μ) :=
  integrable_appendixFHoleConnectedLocalActivity_globalEval_of_rawMetricDecay_of_stronglyMeasurable
    HF z Λ H μ Y ψ hH₀ hH₀_one hκ₀ hκ hlocal hraw
...
/-- Factorwise strong measurability form of the rooted raw integrability
compiler.  This is the source-facing version closest to the CMP/Bałaban
localized activity package: prove each raw factor strongly measurable, and the
finite connected-cover compiler supplies measurability of the integrated
target activity. -/
theorem integrable_appendixFHoleConnectedLocalActivity_globalEval_of_rawMetricDecay_rooted_of_factor_stronglyMeasurable
    {d L : ℕ} [NeZero L]
    {β : Type*} [MeasurableSpace β]
    {Ψ : Cube d L → Type*}
    (HF : HoleFamily d L)
    ...
    Integrable
      (fun φ : (∀ _ : Cube d L, β) =>
        (appendixFHoleConnectedLocalActivity
          HF z Λ H Y).globalEval ψ φ)
      (Measure.pi fun _ : Cube d L => μ) :=
  integrable_appendixFHoleConnectedLocalActivity_globalEval_of_rawMetricDecay_rooted_of_stronglyMeasurable
    HF z Λ H μ hY ψ hH₀ hH₀_one hK₀ hκ₀ hκ hroot hraw
    (appendixFHoleConnectedLocalActivity_globalEval_stronglyMeasurable
      HF z Λ H Y ψ hmeas)
