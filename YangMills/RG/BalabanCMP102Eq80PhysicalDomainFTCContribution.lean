...
/-- FTC contribution of one complete physical domain after the finite sum
over all dependent coarse/fine choices of one outer word. -/
noncomputable def cmp102Eq80PhysicalLayerWordDomainFTCContribution
    {M Q Nc R n : ℕ}
    [NeZero M] [NeZero Q] [NeZero (Nc ^ 2 - 1)]
    [NeZero (2 * Q)] [NeZero (M * (2 * Q))]
    (anchor : FinBox 4 Q)
...
/-- For one outer word, the FTC integral of the finite choice sum is the
finite sum of its complete physical-domain contributions. -/
theorem
    ...
    (∫ t in (0 : ℝ)..1,
      ...
      ∑ Y : Finset (FinBox 4 (2 * Q)),
        cmp102Eq80PhysicalLayerWordDomainFTCContribution
          (R := R)
          anchor K hc hmass hK baseCoarseCovariance
          sigma layerWord D D₃ V₀ P T Δπ J A Y := by
  calc
    (∫ t in (0 : ℝ)..1,
      ∑ choice :
          CMP99SourcePi4CoarseFineWalkChoice M Q R layerWord,
