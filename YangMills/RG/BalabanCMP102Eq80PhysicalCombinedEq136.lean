...
The terminal cutoff uses the literal source carrier from equation (2.14):
the union of the direct source domains and the native Lemma-1 source domains.
It is deliberately distinct from the larger terminal region `Z0`.  Direct
and native source supports lie inside `Z0`, so nested bond projections reduce
exactly while the native residual continues to evaluate the literal global
correction `D(P_Z0 B)`.  No locality of `D` is asserted.  Nonvanishing of
the common source cutoff supplies both the direct equation-(80) cutoff and the
native small-field premise printed in (1.34).
...
theorem abs_cmp116Eq80Lemma1CombinedPhysicalResidual_le_eq136
  ...
  have hdirectCutoff :=
    cmp116SignedCutoff_ne_zero_of_smallFieldCarrier_subset
      (cmp102Eq80SourcePi4PhysicalY0_subset_combinedSourceSmallFieldCarrier
        anchor domains E) hcutoff
