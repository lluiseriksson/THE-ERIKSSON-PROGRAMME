...
The Dirichlet region is the finite-range thickening of one literal separated large-block cutoff. Its ambient side is `K * L^(depth+1)`, while the precision range is `L^(depth+1)`; `K` is never identified with `L`. The local precision is the compression of P7's exact CMP85 prefix precision, and its Green is the canonical inverse generated from the transported coercivity theorem.

No region, Green, inverse law, coercivity witness or action estimate is a caller input.
...
noncomputable def cmp96SourceSeparatedRegionalCell
...
theorem cmp96SourceSeparatedRegionalCell_hasFiniteRangeMargin
    (P : CMP95SourceSmoothPartitionProfile)
    (L K Q depth : ℕ) [NeZero L] [NeZero K] [NeZero Q] :
    CMP99RegionalSquarePartitionHasFiniteRangeMargin
      (cmp99SourceSeparatedLargeBlockSquarePartition
        (L := L) (K := K) (Q := Q) (depth := depth) P)
      (cmp96SourceSeparatedRegionalCell P L K Q depth)
      (L ^ (depth + 1)) :=
...
noncomputable def cmp96SourceSeparatedRegionalPrefixPrecision
    (P : CMP95SourceSmoothPartitionProfile)
...
noncomputable def cmp96SourceSeparatedRegionalPrefixGreen
    (P : CMP95SourceSmoothPartitionProfile)
    (hL : 2 ≤ L) (depth : ℕ) {spacing epsilon a : ℝ}
  ...
  cmp99RegionalDirichletGreen
    ...
    (cmp89SourceSeparatedAmbientPrefixPrecision hL depth
      spacing epsilon a background budget fineSmall)
    (cmp89SourceSeparatedPrefixCoercivity_pos hL depth
...
the identity. -/
theorem cmp96SourceSeparatedRegionalPrefixPrecision_comp_green
    (P : CMP95SourceSmoothPartitionProfile)
    (hL : 2 ≤ L) (depth : ℕ) {spacing epsilon a : ℝ}
    (hspacing : 0 < spacing) (ha : 0 < a)
    (background : GaugeConfig 4
    ...
    (hsmall : cmp99SourcePoincareErrorCoeff 4 L (depth + 1)
      spacing epsilon < 1)
    (cell : FinBox 4 Q) :
    (cmp96SourceSeparatedRegionalPrefixPrecision P hL depth
      spacing epsilon a background budget fineSmall cell).comp
        (cmp96SourceSeparatedRegionalPrefixGreen P hL depth
