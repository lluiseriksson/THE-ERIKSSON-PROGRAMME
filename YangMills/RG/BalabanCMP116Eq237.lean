...
/-- Membership in the repository-level fixed-`Z` `Z0'` union is exactly
membership in one fixed `(D,P)` branch.

This is a finite bookkeeping theorem.  It does not identify Balaban's source
post-(2.37) `Z0'` family with the repository union. -/
theorem cmp116Eq237GlobalZ0PrimeIndex_mem_iff
    {σ ιD ιP ιZ0 ιZ0' Ψ Φ : Type*}
    [DecidableEq ιZ0']
    (R : CMP116HResummation σ ιD ιP ιZ0 ιZ0' Ψ Φ)
    (Z : σ) (Z0' : ιZ0') :
    Z0' ∈ cmp116Eq237GlobalZ0PrimeIndex R Z ↔
      ∃ D, D ∈ R.DIndex Z ∧
        ∃ P, P ∈ R.PIndex Z D ∧
          Z0' ∈ cmp116Eq237Z0PrimeIndex R Z D P := by
  rw [cmp116Eq237GlobalZ0PrimeIndex, Finset.mem_biUnion]
  constructor
  · intro hZ0'
    rcases hZ0' with ⟨D, hD, hPUnion⟩
    rw [Finset.mem_biUnion] at hPUnion
    rcases hPUnion with ⟨P, hP, hbranch⟩
    exact ⟨D, hD, P, hP, hbranch⟩
  · rintro ⟨D, hD, P, hP, hbranch⟩
    exact ⟨D, hD, by
      rw [Finset.mem_biUnion]
      exact ⟨P, hP, hbranch⟩⟩

/-- A pointwise membership characterization of a source `Z0'` family yields
the repository global-index equality by finite extensionality.

The source theorem still has to prove the `hmem` premise from the printed
post-(2.37) summation family and the chosen source-to-Lean dictionary. -/
theorem cmp116Eq237SourceZ0PrimeIndex_eq_global_of_mem_iff
    {σ ιD ιP ιZ0 ιZ0' Ψ Φ : Type*}
    [DecidableEq ιZ0']
    (R : CMP116HResummation σ ιD ιP ιZ0 ιZ0' Ψ Φ)
    ...
    (hmem :
      ∀ Z Z0',
        Z0' ∈ sourceZ0PrimeIndex Z ↔
          ∃ D, D ∈ R.DIndex Z ∧
            ∃ P, P ∈ R.PIndex Z D ∧
              Z0' ∈ cmp116Eq237Z0PrimeIndex R Z D P) :
    sourceZ0PrimeIndex = cmp116Eq237GlobalZ0PrimeIndex R := by
  funext Z
  ext Z0'
  exact
    (hmem Z Z0').trans
      (cmp116Eq237GlobalZ0PrimeIndex_mem_iff R Z Z0').symm
...
/-- The source-side scalar factor in the CMP116 page-20 definition of
`epsilon2`, leaving the final `epsilon1` multiplier separate.

The visual source formula is
`epsilon2 = 2*E0*epsilon1*C1*alpha4^-1*alpha6^-1*M^q*exp(C2*kappa1)`.
This definition names the part multiplying `epsilon1`; it does not assert that
the source constants have already been extracted or matched to Lean data. -/
noncomputable def cmp116Lemma3Epsilon2SourceFactor
    (E0 C1 alpha4 alpha6 M C2 kappa1 : ℝ) (q : ℕ) : ℝ :=
  2 * E0 * C1 * alpha4⁻¹ * alpha6⁻¹ * M ^ q *
    Real.exp (C2 * kappa1)

/-- The CMP116 page-20 source shape for `epsilon2`. -/
noncomputable def cmp116Lemma3Epsilon2OfSourceConstants
    (E0 C1 alpha4 alpha6 M C2 kappa1 : ℝ) (q : ℕ)
    (epsilon1 : ℝ) : ℝ :=
  cmp116Lemma3Epsilon2SourceFactor
      E0 C1 alpha4 alpha6 M C2 kappa1 q *
    epsilon1

/-- The CMP116 page-20 source shape for the final Lemma-3 constant `C3`.

The distinct `O(1)` occurrence in Eq. (2.37) is represented by the explicit
majorant constant `C237`; this theorem layer does not collapse source constants
unless the caller supplies such a common majorant. -/
noncomputable def cmp116Lemma3C3OfSourceConstants
    (blockScale : ℕ)
    (C237 E0 C1 alpha4 alpha6 M C2 kappa1 : ℝ) (q : ℕ) : ℝ :=
  2 * (((blockScale : ℝ) + 2) ^ 4) * C237 *
    cmp116Lemma3Epsilon2SourceFactor
      E0 C1 alpha4 alpha6 M C2 kappa1 q
...
theorem cmp116Lemma3Epsilon2SourceFactor_nonneg
    ...
    0 ≤
      cmp116Lemma3Epsilon2SourceFactor
        E0 C1 alpha4 alpha6 M C2 kappa1 q := by
  unfold cmp116Lemma3Epsilon2SourceFactor
  positivity

/-- When `epsilon2` and `C3` are instantiated by the CMP116 page-20 source
formulas, the Eq. (2.37) post-`P` amplitude is exactly `C3 * epsilon1`.

This is the algebraic constant-hierarchy step behind the visual transition from
Eq. (2.37) to Lemma 3 / Eq. (2.38). -/
theorem cmp116Eq237Amplitude_eq_C3_mul_epsilon1_of_sourceConstants
    (blockScale : ℕ)
    (C237 E0 C1 alpha4 alpha6 M C2 kappa1 epsilon1 : ℝ)
    (q : ℕ) :
...
/-- Build the primitive Lemma-3 parameter record from the visual CMP116 page-20
`epsilon2` and `C3` source formulas.

This constructor discharges only the scalar nonnegativity fields generated by
the source constant hierarchy.  The scale separation remains a real source
restriction, and no activity estimate or resummation theorem is inferred. -/
noncomputable def of_sourceConstants
    (blockScale : ℕ)
    (delta epsilon1 kappa kappa1 : ℝ)
    (C237 E0 C1 alpha4 alpha6 M C2 : ℝ)
    (q : ℕ)
    (hsourceFactor_nonneg :
      0 ≤
        cmp116Lemma3Epsilon2SourceFactor
          E0 C1 alpha4 alpha6 M C2 kappa1 q)
    (hC237_nonneg : 0 ≤ C237)
    (hepsilon1_nonneg : 0 ≤ epsilon1)
    (hscale :
      2 * (blockScale : ℝ) * kappa ≤ (kappa1 - 1) / 2) :
    CMP116Lemma3Parameters where
  ...
  amplitude_nonneg := by
    have hscaleFactor :
        0 ≤ 2 * (((blockScale : ℝ) + 2) ^ 4) := by
      positivity
    have hC3 :
        0 ≤
          cmp116Lemma3C3OfSourceConstants
            blockScale C237 E0 C1 alpha4 alpha6 M C2 kappa1 q := by
      unfold cmp116Lemma3C3OfSourceConstants
      exact
        mul_nonneg
          (mul_nonneg hscaleFactor hC237_nonneg)
          hsourceFactor_nonneg
    exact mul_nonneg hC3 hepsilon1_nonneg

  epsilon2_nonneg := by
    unfold cmp116Lemma3Epsilon2OfSourceConstants
    exact mul_nonneg hsourceFactor_nonneg hepsilon1_nonneg

  scale_margin := hscale

/-- Build the primitive Lemma-3 parameter record from the visual CMP116 page-20
`epsilon2` and `C3` source formulas, discharging the scalar
`epsilon2`-factor nonnegativity from primitive nonnegative constants.

This removes only a scalar bookkeeping premise.  The scale separation, source
constant identification, and all analytic Eq. (2.37) estimates remain explicit
elsewhere. -/
noncomputable def of_nonneg_sourceConstants
    (blockScale : ℕ)
    (delta epsilon1 kappa kappa1 : ℝ)
    (C237 E0 C1 alpha4 alpha6 M C2 : ℝ)
    (q : ℕ)
  ...
  of_sourceConstants
    blockScale delta epsilon1 kappa kappa1
    C237 E0 C1 alpha4 alpha6 M C2 q
    (cmp116Lemma3Epsilon2SourceFactor_nonneg
      hE0 hC1 halpha4 halpha6 hM)
    hC237_nonneg hepsilon1_nonneg hscale

end CMP116Lemma3Parameters
...
/-- Variant of `cmp116PostPResidualSourceBound_of_eq237` using a pointwise
membership iff between the source `Z0'` summation family and the repository's
global fixed-`Z` union.

This is the dictionary-shaped entry point for a future source theorem: it
removes the arbitrary finite inclusion input `hindex`, but still requires the
fixed-`Z0'` Eq. (2.37) estimate and the final post-(2.37) source summation. -/
theorem cmp116PostPResidualSourceBound_of_eq237_sourceIndexMemIff
    {σ ιD ιP ιZ0 ιZ0' ιC Ψ Φ : Type*}
    [DecidableEq ιZ0']
    (hp : CMP116Lemma3Parameters)
    (R : CMP116HResummation σ ιD ιP ιZ0 ιZ0' Ψ Φ)
  ...
  refine
    cmp116PostPResidualSourceBound_of_eq237
      hp R localizationScale C237 Calpha5 alpha5
      sourceCard gapCard components componentMetric
      sourceZ0PrimeIndex postPSourceWeight pWeight
      hC237_nonneg hpWeight_nonneg ?_ heq237_fixed hpost_eq237
  intro Z D hD P hP Z0' hZ0'
  exact
    (hsourceIndex Z Z0').2
      (cmp116Eq237Z0PrimeIndex_subset_global R Z D P hD hP hZ0')
...
/-- Build the Eq. (2.37) majorization boundary from the visual CMP116
`epsilon2`/`C3` constant hierarchy.

Compared with supplying `CMP116Eq237MajorizationBoundary` directly, this route
discharges the amplitude comparison
`postPAmplitude <= C3 * epsilon1` from the source-shaped identities for
`epsilon2` and `C3`.  The analytic seven-delta bound and residual-exponent
absorption remain explicit inputs. -/
def of_sourceConstantIdentities
    {σ : ℕ → ℕ → Type*}
    (hp : ∀ _t _k, CMP116Lemma3Parameters)
    (sourceMetric : ∀ t k, σ t k → ℕ)
    (postPSourceWeight : ∀ t k, σ t k → ℝ)
    (residualExponent : ∀ t k, σ t k → ℝ)
    (C237 E0 C1 alpha4 alpha6 M C2 kappa1 : ℕ → ℕ → ℝ)
    (q : ℕ → ℕ → ℕ)
    ...
    calc
      ...
      _ =
        cmp116Lemma3C3OfSourceConstants
          ...
          (hp t k).epsilon1 := by
          exact
            cmp116Eq237Amplitude_eq_C3_mul_epsilon1_of_sourceConstants
              (hp t k).blockScale
              (C237 t k) (E0 t k) (C1 t k)
              (alpha4 t k) (alpha6 t k) (M t k)
              (C2 t k) (kappa1 t k) (hp t k).epsilon1
              (q t k)
      _ = (hp t k).C3 * (hp t k).epsilon1 := by
          rw [← hC3 t k]

  seven_delta_bound := hseven

  residual_absorbed := hresidual
