...
def background0
    (S : CMP99Eq360ComplexClosedPhysicalInput (Nc := Nc) regions hd hM spacing) :
...
theorem precisionPerturbation_eq_sourcePrecisionPerturbation
  ...
  have hlocal :
      S.baselineLaplacian - S.perturbedLaplacian =
        S.localLaplacianPerturbation := by
    unfold baselineLaplacian perturbedLaplacian localLaplacianPerturbation
    exact cmp99Eq360_complexRegionalLaplacian_sub_eq_localPerturbation
      Omega S.background0 S.background1 spacing
