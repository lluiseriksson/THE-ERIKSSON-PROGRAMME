import YangMills.RG.BalabanCMP85BlockGaussianAlgebra
/-!
typed bridge from the abstract noncommutative certificates to the three-space CMP85 Schur complement.
...
noncomputable def cmp85TypedStepProjector
    (R : K →L[ℝ] L) (Rdag : L →L[ℝ] K) : K →L[ℝ] K :=
  Rdag.comp R

/-- Fine Green sandwich on the intermediate field space. -/
noncomputable def cmp85TypedGreenSandwich
...
noncomputable def cmp85TypedSchurPrecision
    (Q : H →L[ℝ] K) (Qdag : K →L[ℝ] H)
    (R : K →L[ℝ] L) (Rdag : L →L[ℝ] K)
    (G : H →L[ℝ] H) (b c : ℝ) : K →L[ℝ] K :=
  b • ContinuousLinearMap.id ℝ K -
      b ^ 2 • cmp85TypedGreenSandwich Q Qdag G +
    c • cmp85TypedStepProjector R Rdag

/-- The exact one-step coisometry makes `Rdag R` a projector. -/
theorem cmp85TypedStepProjector_idempotent
    (R : K →L[ℝ] L) (Rdag : L →L[ℝ] K)
    (hcoiso : R.comp Rdag = ContinuousLinearMap.id ℝ L) :
    (cmp85TypedStepProjector R Rdag).comp
        (cmp85TypedStepProjector R Rdag) =
      cmp85TypedStepProjector R Rdag := by
  apply ContinuousLinearMap.ext
  intro x
  have hx := congrArg
    (fun T : L →L[ℝ] L => Rdag (T (R x))) hcoiso
  simpa only [cmp85TypedStepProjector,
...
in (2.41). -/
theorem cmp85TypedStepProjector_absorb_left
    (R : K →L[ℝ] L) (Rdag : L →L[ℝ] K)
    (hcoiso : R.comp Rdag = ContinuousLinearMap.id ℝ L) :
    R.comp (cmp85TypedStepProjector R Rdag) = R := by
  apply ContinuousLinearMap.ext
  intro x
...
theorem cmp85Typed_rightSchurBracket_eq_zero
    (Q : H →L[ℝ] K) (Qdag : K →L[ℝ] H)
    (R : K →L[ℝ] L) (Rdag : L →L[ℝ] K)
    (G : H →L[ℝ] H) (C : K →L[ℝ] K)
    (b c beta : ℝ)
  ...
  have hP : P * P = P := by
    ...
    exact cmp85TypedStepProjector_idempotent R Rdag hcoiso
  have hC' :
    ...
    simpa only [P, S, cmp85TypedSchurPrecision,
  ...
  have h := cmp85_rightSchurBracket_eq_zero
...
laws of `C`. -/
theorem cmp85Typed_leftSchurBracket_eq_zero
    (Q : H →L[ℝ] K) (Qdag : K →L[ℝ] H)
    (R : K →L[ℝ] L) (Rdag : L →L[ℝ] K)
    (G : H →L[ℝ] H) (C : K →L[ℝ] K)
    (b c beta : ℝ)
  ...
  have hC' :
    ...
    simpa only [P, S, cmp85TypedSchurPrecision,
      ...
      ContinuousLinearMap.id_apply, ContinuousLinearMap.comp_apply,
      ContinuousLinearMap.one_apply, one_smul, smul_smul] using hx
  have h := cmp85_leftSchurBracket_eq_zero
    (A := K →L[ℝ] K) b c beta P S C hP hC' hrec
  dsimp only at h ⊢
  apply ContinuousLinearMap.ext
  intro x
...
inserted here. -/
theorem cmp85Typed_rightAveragingIdentity
    (Q : H →L[ℝ] K) (Qdag : K →L[ℝ] H)
    (R : K →L[ℝ] L) (Rdag : L →L[ℝ] K)
    (G : H →L[ℝ] H) (C : K →L[ℝ] K)
    (b c : ℝ)
    ...
    R.comp
        (ContinuousLinearMap.id ℝ K +
          b ^ 2 • (cmp85TypedGreenSandwich Q Qdag G).comp C) =
      (b + c) • R.comp C := by
  let P := cmp85TypedStepProjector R Rdag
  let S := cmp85TypedGreenSandwich Q Qdag G
  have hRP : R.comp P = R :=
    cmp85TypedStepProjector_absorb_left R Rdag hcoiso
  apply ContinuousLinearMap.ext
  intro x
