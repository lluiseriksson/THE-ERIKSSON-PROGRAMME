...
/-- C56-T1: Continuity of f from Lipschitz continuity.
    Reduces hcont : Continuous f to hLip : LipschitzWith K f
    under [PseudoEMetricSpace G] on the gauge group domain.
    [NormedAddCommGroup E] on the codomain implies [PseudoEMetricSpace E]
    automatically via the norm-induced metric structure.
    Proof: LipschitzWith.continuous directly. --/
theorem kp_hcont_of_lipschitz_factor
    [PseudoEMetricSpace G]
    {E : Type*} [NormedAddCommGroup E]
    {K : NNReal} {f : G → E} (hLip : LipschitzWith K f) :
    Continuous f :=
  hLip.continuous

/-- C56-T2: Clay Yang-Mills from a norm-square plaquette energy, squared inverse
    temperature, and a Lipschitz amplitude.
    Packages C55-T2 and C56-T1:
    given f : G → E with hLip : LipschitzWith K f,
    [PseudoEMetricSpace G] on the gauge group (which implies [TopologicalSpace G]
    via instance synthesis from PseudoEMetricSpace → UniformSpace → TopologicalSpace),
    [BorelSpace G] and [BorelSpace E] on both spaces,
    hndef : ∀ g, plaquetteEnergy g = ‖f g‖ ^ 2, and hβdef : β = b ^ 2,
    derives Continuous f via C56-T1, then Measurable f via C55-T1,
    and concludes ClayYangMillsTheorem.
    Delegates to C55-T2 with kp_hcont_of_lipschitz_factor hLip supplying hcont.
    Note: [NormedAddCommGroup E] implies [PseudoEMetricSpace E] automatically. --/
theorem kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_sq_beta_lipschitz_factor
    [MeasurableInv G] [MeasurableMul₂ G]
    [PseudoEMetricSpace G] [BorelSpace G]
    {E : Type*} [NormedAddCommGroup E] [MeasurableSpace E] [BorelSpace E]
    {H : Type*} [NormedAddCommGroup H] [InnerProductSpace ℝ H]
    (μ : Measure G) [IsProbabilityMeasure μ]
    (plaquetteEnergy : G → ℝ) (f : G → E) (β b : ℝ) {K : NNReal} (Fobs : G → ℝ)
    (dnat : (N : ℕ) → ConcretePlaquette d N → ConcretePlaquette d N → ℕ)
    (T P₀ : H →L[ℝ] H) (γ C_T : ℝ) (Ω : H)
    (hgap : HasSpectralGap T P₀ γ C_T)
    (hvac : ‖Ω‖ = 1 ∧ P₀ = (innerSL ℝ Ω).smulRight Ω)
  ...
  kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_sq_beta_continuous_factor
    μ plaquetteEnergy f β b Fobs dnat T P₀ γ C_T Ω hgap hvac
    (kp_hcont_of_lipschitz_factor hLip) hβdef hndef hcorr hF


/-- C57-T1: Continuity of f from local Lipschitz continuity.
    Reduces hcont : Continuous f to hllip : LocallyLipschitz f
    under [PseudoEMetricSpace G] on the gauge group domain.
    [NormedAddCommGroup E] on the codomain implies [PseudoEMetricSpace E]
    automatically via the norm-induced metric structure.
    LocallyLipschitz is strictly weaker than LipschitzWith K:
    every K-Lipschitz map is locally Lipschitz, but not vice versa
    (e.g. f(x) = sqrt(|x|) on ℝ is locally Lipschitz but not globally).
    Proof: LocallyLipschitz.continuous directly. --/
theorem kp_hcont_of_locally_lipschitz_factor
    [PseudoEMetricSpace G]
    {E : Type*} [NormedAddCommGroup E]
    {f : G → E} (hllip : LocallyLipschitz f) :
    Continuous f :=
  hllip.continuous

/-- C57-T2: Clay Yang-Mills from a norm-square plaquette energy, squared inverse
    temperature, and a locally Lipschitz amplitude.
    Packages C55-T2 and C57-T1:
    given f : G → E with hllip : LocallyLipschitz f,
    [PseudoEMetricSpace G] on the gauge group (which implies [TopologicalSpace G]
    via instance synthesis from PseudoEMetricSpace → UniformSpace → TopologicalSpace),
    [BorelSpace G] and [BorelSpace E] on both spaces,
    hndef : ∀ g, plaquetteEnergy g = ‖f g‖ ^ 2, and hβdef : β = b ^ 2,
    derives Continuous f via C57-T1, then Measurable f via C55-T1,
    and concludes ClayYangMillsTheorem.
    Delegates to C55-T2 with kp_hcont_of_locally_lipschitz_factor hllip supplying hcont.
    LocallyLipschitz weakens C56's LipschitzWith K hypothesis: no global constant K required.
    Note: [NormedAddCommGroup E] implies [PseudoEMetricSpace E] automatically. --/
theorem kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_sq_beta_locally_lipschitz_factor
    [MeasurableInv G] [MeasurableMul₂ G]
    [PseudoEMetricSpace G] [BorelSpace G]
    {E : Type*} [NormedAddCommGroup E] [MeasurableSpace E] [BorelSpace E]
    {H : Type*} [NormedAddCommGroup H] [InnerProductSpace ℝ H]
  ...
  kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_sq_beta_continuous_factor
    μ plaquetteEnergy f β b Fobs dnat T P₀ γ C_T Ω hgap hvac
    (kp_hcont_of_locally_lipschitz_factor hllip) hβdef hndef hcorr hF


/-- C58-T1: Local Lipschitz continuity of f from C¹ smoothness.
    Reduces hllip : LocallyLipschitz f to hcd : ContDiff ℝ 1 f.
    [NormedAddCommGroup G] [NormedSpace ℝ G] on the gauge group domain implies
    [PseudoEMetricSpace G] automatically via NormedAddCommGroup → SeminormedAddCommGroup
    → PseudoMetricSpace → PseudoEMetricSpace, satisfying LocallyLipschitz's requirement.
    ContDiff ℝ 1 is strictly stronger than LocallyLipschitz:
    every C¹ function is locally Lipschitz, but not vice versa
    (e.g. f(x) = |x| on ℝ is locally Lipschitz but not C¹ at 0).
    Proof: ContDiff.locallyLipschitz directly. --/
theorem kp_hllip_of_contdiff_one_factor
    [NormedAddCommGroup G] [NormedSpace ℝ G]
    {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {f : G → E} (hcd : ContDiff ℝ 1 f) :
    LocallyLipschitz f :=
  hcd.locallyLipschitz

/-- C58-T2: Clay Yang-Mills from a norm-square plaquette energy, squared inverse
    temperature, and a C¹ amplitude.
    Packages C58-T1 and C57-T2:
    given f : G → E with hcd : ContDiff ℝ 1 f,
    [NormedAddCommGroup G] [NormedSpace ℝ G] on the gauge group (which implies
    [PseudoEMetricSpace G] via NormedAddCommGroup → PseudoMetricSpace → PseudoEMetricSpace),
    [BorelSpace G] and [BorelSpace E] on both spaces,
    hndef : ∀ g, plaquetteEnergy g = ‖f g‖ ^ 2, and hβdef : β = b ^ 2,
    derives LocallyLipschitz f via C58-T1, then concludes ClayYangMillsTheorem via C57-T2.
    C¹ smoothness weakens C57's LocallyLipschitz hypothesis: the caller specifies
    a smoothness degree rather than a local Lipschitz condition.
    Note: [NormedAddCommGroup G] [NormedSpace ℝ G] implies [PseudoEMetricSpace G] automatically. --/
theorem kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_sq_beta_contdiff_one_factor
    [MeasurableInv G] [MeasurableMul₂ G]
    ...
    (hF : ∀ (N : ℕ) [NeZero N] (A : GaugeConfig d N G) (p : ConcretePlaquette d N),
        Fobs (GaugeConfig.plaquetteHolonomy A p) = 1) :
    ClayYangMillsTheorem :=
  kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_sq_beta_locally_lipschitz_factor
    μ plaquetteEnergy f β b Fobs dnat T P₀ γ C_T Ω hgap hvac
...
/-- C61-T1: Beta nonnegativity from norm-square witness.
    Proves 0 ≤ β from hβdef : β = ‖b‖ ^ 2 for b in an
    arbitrary SeminormedAddCommGroup B. Genuine interface generalization. --/
...
/-- C61-T2: Full Clay packaging from norm-square beta witness.
    Generalizes C60-T2: b : ℝ replaced by b : B in SeminormedAddCommGroup,
    hβdef : β = b ^ 2 replaced by hβdef : β = ‖b‖ ^ 2.
    Delegates to C60-T2 with ‖b‖ as scalar real witness.
    NOTE: Interface generalization only, not mass gap progress. --/
theorem kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_norm_sq_beta_hasFDerivAt_factor
    [MeasurableInv G] [MeasurableMul₂ G]
    [NormedAddCommGroup G] [NormedSpace ℝ G] [BorelSpace G]
    {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E] [MeasurableSpace E] [BorelSpace E]
    {B : Type*} [SeminormedAddCommGroup B]
...
/-- C63-H: Continuity from a ContinuousLinearMap factor.
    Given F : G →L[ℝ] E, derives Continuous ⇑F directly via F.cont.
    Used as the hcont witness in C63-T1. --/
theorem kp_hcont_of_continuousLinearMap_factor
    [NormedAddCommGroup G] [NormedSpace ℝ G]
    {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E]
    (F : G →L[ℝ] E) :
    Continuous F :=
...
/-- C63-T1: Full Clay packaging from norm-square beta witness via ContinuousLinearMap factor.
    Combines C62-T1 (continuous_factor path, norm-square beta in SeminormedAddCommGroup)
    with automatic continuity extraction from CLM structure.
    Replaces hcont : Continuous f in C62-T1 by F : G →L[ℝ] E;
    derives Continuous F via kp_hcont_of_continuousLinearMap_factor.
    The gauge group G must additionally carry NormedAddCommGroup + NormedSpace ℝ structure
    (stronger than C62-T1 which only needs TopologicalSpace G).
    Delegates to C62-T1 passing ↑F as the function and F.cont as continuity witness.
    NOTE: Interface generalization only, not progress on the Yang-Mills mass gap. --/
theorem kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_norm_sq_beta_continuousLinearMap_factor
    [MeasurableInv G] [MeasurableMul₂ G]
    [NormedAddCommGroup G] [NormedSpace ℝ G]
    [BorelSpace G]
    {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E] [MeasurableSpace E] [BorelSpace E]
    ...
    {H : Type*} [NormedAddCommGroup H] [InnerProductSpace ℝ H]
    (μ : Measure G) [IsProbabilityMeasure μ]
    (plaquetteEnergy : G → ℝ) (F : G →L[ℝ] E) (β : ℝ) (b : B) (Fobs : G → ℝ)
    (dnat : (N : ℕ) → ConcretePlaquette d N → ConcretePlaquette d N → ℕ)
    (T P₀ : H →L[ℝ] H) (γ C_T : ℝ) (Ω : H)
    (hgap : HasSpectralGap T P₀ γ C_T)
    (hvac : ‖Ω‖ = 1 ∧ P₀ = (innerSL ℝ Ω).smulRight Ω)
  ...
  kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_norm_sq_beta_continuous_factor
    μ plaquetteEnergy F β b Fobs dnat T P₀ γ C_T Ω hgap hvac
    (kp_hcont_of_continuousLinearMap_factor F) hβdef hndef hcorr hF


/-- C64-H: Continuity from a linear isometry.
    A linear isometry F : G →ₗᵢ[ℝ] E is continuous because isometries
    are uniformly continuous. NOTE: Interface generalization only,
    not progress on the Yang-Mills mass gap. --/
theorem kp_hcont_of_linearIsometry_factor
    [NormedAddCommGroup G] [NormedSpace ℝ G]
    {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E]
    (F : G →ₗᵢ[ℝ] E) :
    Continuous F :=
...
/-- C64-T1: Full Clay packaging from norm-square beta witness and linear isometry amplitude.
    Reduces the continuous-linear-map interface (C63-T1) to the linear-isometry interface:
    instead of supplying F : G →L[ℝ] E, the caller supplies F : G →ₗᵢ[ℝ] E.
    Continuity is derived from the isometry structure via C64-H.
    NOTE: Interface generalization only, not progress on the Yang-Mills mass gap. --/
theorem kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_norm_sq_beta_linearIsometry_factor
    [MeasurableInv G] [MeasurableMul₂ G]
    [NormedAddCommGroup G] [NormedSpace ℝ G]
    [BorelSpace G]
    {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E] [MeasurableSpace E] [BorelSpace E]
...
/-- C66-H: Continuity from a continuous linear equivalence.
    Derived by applying ContinuousLinearEquiv.continuous.
    Analogous to C65-H but for the equiv type. --/
theorem kp_hcont_of_continuousLinearEquiv_factor
    [NormedAddCommGroup G] [NormedSpace ℝ G]
    {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E]
    (F : G ≃L[ℝ] E) :
    Continuous F :=
...
/-- C66-T1: Full Clay packaging via a continuous linear equivalence factor.
    Delegates to C63 packaging via F.toContinuousLinearMap. --/
theorem kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_norm_sq_beta_continuousLinearEquiv_factor
    [MeasurableInv G] [MeasurableMul₂ G]
    [NormedAddCommGroup G] [NormedSpace ℝ G]
    [BorelSpace G]
    {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E] [MeasurableSpace E] [BorelSpace E]
...
/-- C67-H: Continuity from an isometry.
    Derived by applying Isometry.continuous.
    For any pseudoemetric space map: distance-preservation implies continuity. --/
theorem kp_hcont_of_isometry_factor
    [PseudoEMetricSpace G]
    {E : Type*} [PseudoEMetricSpace E]
    {F : G → E} (hIso : Isometry F) :
    Continuous F :=
  hIso.continuous

/-- C67-T1: Clay YM packaging using an isometry amplitude factor.
    Delegates to C62 (continuous_factor) via Isometry.continuous. --/
theorem kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_norm_sq_beta_isometry_factor
    [MeasurableInv G] [MeasurableMul₂ G]
    [PseudoEMetricSpace G] [BorelSpace G]
    {E : Type*} [NormedAddCommGroup E] [MeasurableSpace E] [BorelSpace E]
    {B : Type*} [SeminormedAddCommGroup B]
    ...
    ClayYangMillsTheorem :=
  kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_norm_sq_beta_continuous_factor
    μ plaquetteEnergy F β b Fobs dnat T P₀ γ C_T Ω hgap hvac hIso.continuous hβdef hndef hcorr hF

/-- C68-H: Continuity from a homeomorphism.
    Derived by applying Homeomorph.continuous.
    For any topological space map: homeomorphisms are continuous by definition. --/
...
/-- C68-T1: Clay YM packaging using a homeomorphism amplitude factor.
    Delegates to C62 (continuous_factor) via Homeomorph.continuous. --/
theorem kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_norm_sq_beta_homeomorph_factor
    [MeasurableInv G] [MeasurableMul₂ G]
    [TopologicalSpace G] [BorelSpace G]
    {E : Type*} [NormedAddCommGroup E] [MeasurableSpace E] [BorelSpace E]
    {B : Type*} [SeminormedAddCommGroup B]
    {H : Type*} [NormedAddCommGroup H] [InnerProductSpace ℝ H]
    (μ : Measure G) [IsProbabilityMeasure μ]
    (plaquetteEnergy : G → ℝ) (F : G ≃ₜ E) (β : ℝ) (b : B) (Fobs : G → ℝ)
    (dnat : (N : ℕ) → ConcretePlaquette d N → ConcretePlaquette d N → ℕ)
    (T P₀ : H →L[ℝ] H) (γ C_T : ℝ) (Ω : H)
    (hgap : HasSpectralGap T P₀ γ C_T)
    (hvac : ‖Ω‖ = 1 ∧ P₀ = (innerSL ℝ Ω).smulRight Ω)
...
/-- C69-H: Continuity from a uniform equivalence.
    Derived by applying UniformEquiv.continuous.
    Every uniform equivalence is automatically continuous. --/
theorem kp_hcont_of_uniform_equiv_factor
    [UniformSpace G]
...
/-- C69-T1: Clay YM packaging using a uniform equivalence amplitude factor.
    Delegates to C62 (continuous_factor) via UniformEquiv.continuous.
    [UniformSpace G] subsumes [TopologicalSpace G] in Lean 4 Mathlib. --/
theorem kp_clay_from_normalized_rank_one_vacuum_projector_and_holonomy_normalized_observable_norm_sq_plaquetteEnergy_norm_sq_beta_uniform_equiv_factor
    [MeasurableInv G] [MeasurableMul₂ G]
    ...
    {H : Type*} [NormedAddCommGroup H] [InnerProductSpace ℝ H]
    (μ : Measure G) [IsProbabilityMeasure μ]
    (plaquetteEnergy : G → ℝ) (F : G ≃ᵤ E) (β : ℝ) (b : B) (Fobs : G → ℝ)
    (dnat : (N : ℕ) → ConcretePlaquette d N → ConcretePlaquette d N → ℕ)
    (T P₀ : H →L[ℝ] H) (γ C_T : ℝ) (Ω : H)
    (hgap : HasSpectralGap T P₀ γ C_T)
    (hvac : ‖Ω‖ = 1 ∧ P₀ = (innerSL ℝ Ω).smulRight Ω)
