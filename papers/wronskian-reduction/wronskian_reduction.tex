...
promising. The identity and the counterexamples are verified
symbolically and in exact arithmetic; nothing beyond the identity
and the counterexamples is claimed.
\end{abstract}
...
\]
The open positivity problem is $W(t) < 0$ for $t$ near $\pi$, where
$W$ is the left side of \eqref{eq:wronskian} with
$(a, b) = (A, B)$. By Theorem~\ref{thm:wronskian},
\[
...
\label{conj:main}
For every $\beta > 0$: $F_B > 0$ on $(0,\pi)$, and $F_A/F_B$ is
strictly decreasing on $(0,\pi)$; equivalently
$W(t) < 0$ on $(0,\pi)$.
\end{conjecture}
...
\end{center}
A Lean~4 formalization of Theorem~\ref{thm:wronskian} (a finite
antisymmetrization over \texttt{Finset} with the sine/cosine addition
formulas) is the natural next target of the programme's
formalization satellite; it is \emph{not} claimed here.
