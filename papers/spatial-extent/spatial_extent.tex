...
Spatial Extent Breaks the Vacuum, Machine-Checked\\
...
slice with different row sums, and conclude that no constant row sum exists:
$T\Om=\Om$ is \emph{false}.  The vacuum becomes a Perron vector with no closed
...
$L$ non-interacting copies of a two-state system; a tensor power always has
volume-independent spectrum, so its volume-independent rate is physically empty
...
system is $L$ copies of a two-state system with no interaction between them.  A
tensor power has volume-independent spectrum for trivial reasons.  We do not
...
with no closed form, and none of those statements can even be written down
without first solving for it.  The obstruction is therefore not that a proof got
