...
$\Gamma$-power series, the full classification of the parameter:
$\Bfam_{\nu,c}$ is a uniform upper bound for $\rho_\nu$ (all
$\nu \ge 0$, $x > 0$) exactly when $c \le \tfrac12$, and a uniform
lower bound exactly on natural complementary ranges, with explicit
counterexample witnesses.  The classification statement itself is
...
at any hypothetical critical contact.  The entire chain carries the
axiom oracle
...
phenomenon independently of the Lean theorems at $30$ parameter
...
means ordinary prose, always flagged; \emph{verified} marks an
...
$x_* > 0$ with $\rho_\nu(x_*) = \Bfam_{\nu,c}(x_*)$, and:
\begin{enumerate}
\item \emph{(window)} $\sqrt{1-c} \;\le\; x_* \;\le\;
\dfrac{2\hat{D}}{2c-1}$, where
$\hat{D} = (\nu+\tfrac32)^2-(\nu+c)^2$
(\lean{amosFamily\_crossing\_exists});
\item \emph{(strict threshold)} $x_* > x_\dagger :=
\dfrac{2(\nu+c)\sqrt{c(1-c)}}{2c-1}$
(\lean{crossing\_threshold\_strict});
\item \emph{(transversality)} at the crossing,
$D'(x_*) = \dfrac{\Bfam_{\nu,c}(x_*)}{x_*}
\Bigl(2c-1-\dfrac{\nu+c}{\sqrt{(\nu+c)^2+x_*^2}}\Bigr) > 0$
(\lean{crossing\_hasDerivAt}, \lean{crossing\_pos\_right});
\item \emph{(orientation)} $\rho_\nu < \Bfam_{\nu,c}$ on
$(0, x_*)$ and $\rho_\nu > \Bfam_{\nu,c}$ on $(x_*, \infty)$, and
conversely the sign of $D$ locates $x$ relative to $x_*$
(\lean{amosFamily\_crossing\_orientation});
\item \emph{(scale law)} $2(\nu+c)\sqrt{c(1-c)} \;\le\;
...
$a \mapsto x/(a+\sqrt{a^2+x^2})$ is strictly decreasing
(\lean{amosFamily\_anti}), $c \le \tfrac12$ gives
$\Bfam_{\nu,c} \ge \Bfam_{\nu,1/2} > \rho_\nu$ by
...
bound $\rho_\nu(x) < x/\bigl(2(\nu+1)(1-q_\nu)\bigr)$ with
$q_\nu = (x/2)^2/(\nu+1)$ (\lean{besselRatioReal\_lt\_product},
...
(\lean{crossing\_hasDerivAt\_everywhere} provides the second
derivative of $D$ everywhere along $D$'s own derivative function),
...
phenomenon \emph{independently of the Lean theorems}: its protocol
--- registered in the development's charter in its own commits,
strictly before the script existed and before any result was
observed --- mandates that verdicts be decided \emph{only} by ball
...
posteriori by the final ball width, which can only produce an
undecided verdict, never a wrong one.  All $30$ pairs pass all four
...
$\sim 3\times10^{-16}$) clears the threshold
$x_\dagger = 50250.44\ldots$ with certified
...
(hyperbolic-cotangent) form of $\rho_{1/2}$, for which
$x(\coth x - 1) \to \tfrac12$ collapses the crossing equation to
$x \approx a/(2c-1)$ at exponentially small error --- a paper-level
remark; the certified box needs no such structure.  The companion
...
development now contains the complete parameter classification ---
...
release \lean{c4-v1.0}; ai.viXra.org identifier pending at the time
of this release, July 2026.
