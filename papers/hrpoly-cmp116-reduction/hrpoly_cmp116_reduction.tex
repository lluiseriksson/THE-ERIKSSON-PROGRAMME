...
corresponding to equations (2.20)--(2.22) and a uniform scalar bound on the
physical source norm.  The determinant and inverse-source terms of equation
(2.24) are bounded internally and no longer appear as a terminal premise.
We do not prove the concrete kernel inputs, the uniform physical source-norm
estimate, equation (2.26), the polymer estimate \lean{hRpoly}, a continuum
...
leaves two named physical inputs: a domination of the physical integrand by a
real Gaussian, and a uniform scalar bound on the norm of its source.
...
$q=\alpha_5\opnorm R^2<1$, let $n$ be the finite coordinate dimension, and
take $A=-\alpha_5P_{Z_0}$.  The formal development proves
\begin{equation}\label{eq:fullmajorant}
 M_{2.24}(R,-\alpha_5P_{Z_0},r)
 \le
 \frac{1}{\sqrt{(1-q)^n}}
 \exp\!\left(
   \frac{\opnorm R^2\lVert r\rVert_2^2}{2(1-q)}
 \right).
\end{equation}
The proof diagonalizes the positive-semidefinite localized Hessian, bounds
each eigenvalue by $q$, obtains
$(1-q)^n\le\det(I-R^T\alpha_5P_{Z_0}R)$, and controls the inverse quadratic
form spectrally.  Thus a certificate
$\lVert r\rVert_2^2\le R_{\mathrm{src}}$ produces a uniform explicit
majorant without an independent determinant or matrix-inverse hypothesis.
...
\]
The strengthened terminal endpoint uses the premises
\begin{align}
 \tag{Dom}\label{eq:hdom}
 |\operatorname{innerIntegrand}_{G}(b)|&\le g_{A_{Z_0},r}(b)
 \quad\text{for }(R_{\mathrm{matrix}})_*\gauss\text{-a.e. }b,\\
 \tag{Src}\label{eq:hsource}
 \lVert r(\sigma,\tau,x)\rVert_2^2&\le R_{\mathrm{src}}
 \quad\text{uniformly on the Cauchy contours}.
\end{align}
It defines $M_{\mathrm{src}}$ by the right-hand side of
\eqref{eq:fullmajorant}, with $\lVert r\rVert_2^2$ replaced by
$R_{\mathrm{src}}$.
...
\eqref{eq:hdom} and \eqref{eq:hsource} hold, then the literal finite
...
\[
 ...
 \operatorname{CauchyRate}_{\Delta}\!\left(
   ...
   (M_{\mathrm{out}}M_{\mathrm{src}})\right).
\]
All Gaussian integrability, covariance-coordinate transport, projector
construction, shifted-Hessian positivity, determinant control, and inverse
source-quadratic control are discharged internally.
\end{theorem}

The Lean endpoint is
\nolinkurl{norm_term_le_cauchyRate_of_physicalGaussianReduction_sourceNorm}
in
...
the concrete kernel hypotheses used to obtain \eqref{eq:hdom},
\eqref{eq:hsource}, the
...
reduction completes 8,454 jobs.  The four theorem endpoints in the terminal
...
\lean{hrpoly-cmp116-main-reduction-v0.2-2026-07-15}.  The exact commands are
\begin{verbatim}
lake exe cache get
...
\eqref{eq:hdom}, and prove a model-specific uniform instance of
\eqref{eq:hsource}.  The determinant and inverse-source matrix analysis is
already internal.  The resulting scalar term bound must be factored and
resummed into the metric decay of
...
\[
 \text{concrete kernel bounds}+h_{\mathrm{source}}
 \longrightarrow (2.26)
...
physical kernel and source-norm estimates that still carry the analytic
