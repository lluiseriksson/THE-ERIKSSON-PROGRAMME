...
theory, and nothing at $\beta=0$, where for $L>0$ the coercivity constant
vanishes and the degeneracy is real.
...
that, with the sharp constant and, for $eta>0$, on both sides.

egin{theorem}[sharp bound, attained, minimal]\label{thm:norm}
Let $\Omega>0$ and $\lambda$ be Perron data for the symmetrised kernel.  Then
$\lVert TuVert_{\mathrm{site}}\le\lambda\lVert uVert_{\mathrm{site}}$ for
every $u$ and every $eta$; equality holds at $u=\sqrt w\,\Omega$; and any
...
entrywise non-negativity --- which is why the statement survives at $eta<0$
...
egin{theorem}[coercivity]\label{thm:coercive}
For $eta>0$, $\langle u,Tuangle_{\mathrm{site}}\ge
(e^eta-e^{-eta})^Ligl(\min_\sigma w(\sigma)igr)\lVert
uVert_{\mathrm{site}}^2$.
\end{theorem}
...
constant vanishes at $eta=0$ when $L>0$, which is correct --- the kernel is
degenerate there --- and equals $\min w>0$ when $L=0$ at every $eta$.

Consequently $T$ is injective and invertible for $eta>0$, and
$(a/\lambda)I\le T/\lambda\le I$ with $a>0$.  What this removes is the
normalisation obstacle before a functional calculus; it does not build one.
