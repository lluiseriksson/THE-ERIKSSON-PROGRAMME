...
\textbf{On the spectrum, precisely.}  The similarity $T\circ Q=Q\circ S$ with
$Q$ an isomorphism, and the equality of eigenvalue \emph{sets} that follows from
it, are machine-checked theorems.  The multiplicity, ordered-list, reality and
Perron-ratio components of the corresponding pre-registered gate, and the
characteristic-polynomial \emph{consequence} of the similarity, are
\textbf{not} formalised here.

\textbf{On the size of the operator.}  The physical operator norm of $T$ is
\emph{exactly} the Perron eigenvalue $\lambda$, at every $\beta$: bounded,
attained, and minimal, all three machine-checked.  For $\beta>0$ the operator is
moreover coercive, with explicit constant
$(e^\beta-e^{-\beta})^L\min_\sigma w(\sigma)$, hence injective and invertible,
so $T/\lambda$ is squeezed between a strictly positive scalar and the identity.

\textbf{What is not here.}  No Hamiltonian and no functional calculus:
$H=-\log(T/\lambda)$ is now a meaningful object to seek, and it is not built.
No spectral gap statement for $T$, no infinite volume, no continuum, no gauge
theory.  At $\beta=0$ with $L>0$ the coercive lower bound, the
invertible normalised operator and any Hamiltonian route are absent; the
exact norm, self-adjointness and semidefinite positivity survive there.

\textbf{The mathematics is not new.}  Reflection positivity for Ising-type
measures, through sites and through bonds, and its transfer-matrix reading, are
classical: Osterwalder and Schrader for the axioms, Osterwalder and Seiler for
the lattice setting, and Fr\"ohlich, Israel, Lieb and Simon for reflection
positivity of such measures; the transfer matrix of a finite spin chain is older
still.  No mathematical novelty is claimed for these finite-dimensional facts.
The contribution is their Lean formalisation --- the explicit site/bond
indexing, the quotient identification, and the mechanically checked
...
\paragraph{What is \emph{not} proved.}  There is no Hamiltonian here.  The
similarity with the symmetrised kernel is proved (Section~\ref{sec:spec}), and
with it the equality of eigenvalue \emph{sets}; but the Perron labelling, the
ratio to the Perron eigenvalue, multiplicities, ordered spectral data and
reality are \emph{not} formalised, and neither is a spectral gap statement for
$T$.  The norm \emph{is} controlled --- exactly, see
Section~\ref{sec:contraction} --- but no Hamiltonian and no functional calculus
are built.  There is no thermodynamic limit and no continuum limit.  The system
is a finite coupled $\Z_2$ slice, and every statement below is a statement about
it.

\paragraph{On novelty, stated at the front rather than at the back.}  The
mathematics is classical.  Reflection positivity for Ising-type measures through
sites and through bonds, and the reading of the resulting operator as a transfer
matrix, go back to Osterwalder--Schrader and to
Fr\"ohlich--Israel--Lieb--Simon, with the lattice formulation in
Osterwalder--Seiler; the transfer matrix of a finite spin chain is older still.
Nothing in this paper is offered as a new mathematical fact about such systems.
What is offered is machine checking: the site/bond index bookkeeping done
explicitly, the operator obtained as the unique solution of an equation rather
than posited, the null space identified and the quotient constructed, and every
statement carrying its own axiom report.
...
\textbf{R4 remains in the historical record as a gate, and is now only partly
superseded.}  Its similarity and eigenvalue-set components are superseded by
Theorem~\ref{thm:sim} and Corollary~\ref{cor:eig}, which are proved and cited.
Its multiplicity, ordered-list, reality and Perron-ratio observations remain
numerical, are cited in no proof, and no statement in this paper depends on
them.  The characteristic polynomial was never one of R4's components; it is a
standard consequence of the similarity, and it is not formalised here either.
...
\section{The size of the operator}\label{sec:contraction}

The reconstruction so far leaves the size of $T$ unknown.  This section closes
that, with the sharp constant and, for $\beta>0$, on both sides.

\begin{theorem}[sharp bound, attained, minimal]\label{thm:norm}
Let $\Omega>0$ and $\lambda$ be Perron data for the symmetrised kernel.  Then
$\lVert Tu\rVert_{\mathrm{site}}\le\lambda\lVert u\rVert_{\mathrm{site}}$ for
every $u$ and every $\beta$; equality holds at $u=\sqrt{w}\,\Omega$; and any
$c\ge0$ with the same property satisfies $\lambda\le c$.
\end{theorem}

The three parts are
\osline{SpatialReconstruction}{NORMLELINE}{siteQ\_transferOp\_le},
\osline{SpatialReconstruction}{NORMATTLINE}{siteQ\_transferOp\_perron} and
\osline{SpatialReconstruction}{NORMMINLINE}{perron\_norm\_constant\_minimal},
resting on the generic
\osline{SpatialReconstruction}{NORMGENLINE}{norm\_sq\_le\_perron}.

The proof is a weighted Cauchy--Schwarz in which the Perron vector is not a
bound but the \emph{weight} that makes an elementary inequality tight.  No
spectral theory is used, and no operator semidefiniteness is assumed --- only
entrywise non-negativity --- which is why the statement survives at $\beta<0$
where positivity of $T$ is unavailable.  The existence of the Perron data is
discharged from this lane's own Perron--Frobenius, so the statement is
unconditional.

\begin{theorem}[coercivity]\label{thm:coercive}
For $\beta>0$,
$\langle u,Tu\rangle_{\mathrm{site}}\ge
(e^{\beta}-e^{-\beta})^{L}\bigl(\min_\sigma w(\sigma)\bigr)
\lVert u\rVert_{\mathrm{site}}^{2}$.
\end{theorem}

\osline{SpatialReconstruction}{COERCLINE}{bondQ\_ge\_siteQ}, from the generic
induction \osline{SpatialReconstruction}{COERGENLINE}{spatialKernel\_coercive}.
The factor $\min_\sigma w(\sigma)$ is not decoration: the physical norm is
weighted, so passing from a Euclidean bound to this one costs exactly that
factor, and omitting it would state something false for weights below one.  The
constant vanishes at $\beta=0$ when $L>0$, which is correct --- the kernel is
degenerate there --- and equals $\min w>0$ when $L=0$ at every $\beta$.

Consequently $T$ is injective
(\osline{SpatialReconstruction}{INJLINE}{transferOp\_injective}) and invertible,
and $(a/\lambda)I\le T/\lambda\le I$ with $a>0$ ---
\osline{SpatialReconstruction}{NORMTWOLINE}{normalised\_two\_sided}, whose middle
term is the quadratic form of the normalised operator itself by
\osline{SpatialReconstruction}{NORMBRIDGELINE}{siteForm\_normalisedTransferOpL\_self}.
What this removes is the normalisation obstacle before a functional calculus; it
does not build one.

\paragraph{Exactly what fails at $\beta=0$.}  Not everything, and an earlier
version of this paper said ``nothing at $\beta=0$'', which contradicted the
sentence beside it claiming the exact norm at \emph{every} $\beta$.  What
survives at $\beta=0$ is self-adjointness, semidefinite positivity, the exact
norm $\lVert T\rVert_{\mathrm{site}}=\lambda$, and the attainment and
minimality of that constant; at $L=0$ even invertibility survives.  What fails,
and only for $L>0$, is the coercive lower bound --- and with it the invertible
normalised operator and any route to a Hamiltonian.

\section{What is open}\label{sec:open}

\begin{enumerate}
\item \textbf{The spectral data beyond the sets.}  What remains open is not the
similarity, which is proved (Theorem~\ref{thm:sim}), nor the equality of
eigenvalue sets (Corollary~\ref{cor:eig}).  It is the formal transport of
multiplicities, ordered spectral data, reality, and the Perron labelling and
ratio --- and, resting on those, the spectral-gap statement that earlier work in
this lane proves for the symmetrised kernel.  None of it is transported here.
\item \textbf{No Hamiltonian, and no functional calculus.}  This item used to
read ``no contraction bound''.  That is no longer true and the paper says so in
Section~\ref{sec:contraction}: $\|T\|_{\mathrm{site}}=\lambda$ exactly, and for
$\beta>0$ the operator is coercive and invertible.  What remains open is the
next step, not this one: $H=-\log(T/\lambda)$ is a meaningful object to seek and
is not constructed, and neither the spectral calculus nor a Hilbert-space
packaging of the physical metric is present.
...
\paragraph{The anchor is a curated replay, and the pre-registration is
certified by blob identity rather than by ancestry.}  The source anchor was
rebuilt from this campaign's base commit so that it would carry this paper's
changes and nothing from a concurrent line of work sharing the same clone.  That
purchase had a price, and it is disclosed rather than left to be discovered: the
pre-registration commits --- \texttt{2392c080} for the site-bridge gate,
\texttt{47d48fc2} for the reconstruction gate, \texttt{6e67629a} for this
version's job-count prediction --- are \textbf{not ancestors} of the anchor.  A
reader inspecting only the clean branch sees the judges and the Lean appear in
one commit, which is not how it happened.

The chronology is therefore certified by the original commits \emph{together
with} the fact that the judge files in the anchor are byte-identical to the
blobs that were pre-registered:
\texttt{judge\_site\_bridge.py} at blob
\texttt{bcfb0363edef76aa6873e057ad2081f2e11bebeb} and
\texttt{judge\_os\_reconstruction.py} at blob
\texttt{3e236e54d38dd263730bbedea368fbe232616428}, in the pre-registration
commit and in the anchor alike.  That is checked, not asserted, by
\repofile{scripts/verify\_prereg\_blobs.py}{scripts/verify\_prereg\_blobs.py},
which also reports the absent ancestry rather than hiding it.

\paragraph{What the build was, stated so that a green result cannot be read as
more than it is.}  Everything was elaborated on \emph{one} fresh Git clone at
the immutable anchor, using \emph{warm} pinned dependency and build artefacts:
this is \textbf{not} a from-scratch build.  Because a warm \texttt{.lake} makes
a green \texttt{lake build} unable to distinguish a re-elaborated module from
one whose artefact already matched, the two modules of this paper are also
elaborated \emph{directly}, by \texttt{lake env lean} on each source file,
before the full-core integration build and the oracle run.  File digests are
printed before those commands as well as after.
...
\quad at the original v1.0 campaign parent & JOBSCAMPAIGNBASE jobs \\ % @@JOBSCAMPAIGNBASE@@
...
\paragraph{Two campaigns, two predictions, and they are different numbers.}  For
\textbf{this version} the prediction registered before any count was taken was
$8469\to8469$, hence $\Delta=0$, because everything added since the v1.0 anchor
went into \emph{existing} modules and added no module and no import.  The
earlier $8468\to8469$, $\Delta=+1$, belongs to the \textbf{v1.0} campaign, where
one module did join the core; it is kept in the table as historical provenance
and is not a claim about this version.  Both predictions were hard-coded in the
filling script before the corresponding measurement existed, and the script
refuses to write the manuscript unless the measured absolute \emph{and} the
measured delta match what was registered.  Every count in the table comes from a
core build in the same build environment.
...
\section*{References for the classical material}

The facts this paper formalises are not new, and these are the sources for them.
None of the results below is re-proved here; they are named so that the
distinction between the mathematics and its mechanisation is on the page.

\begin{enumerate}\itemsep2pt
\item K.~Osterwalder and R.~Schrader, \emph{Axioms for Euclidean Green's
functions}, Comm.\ Math.\ Phys.\ \textbf{31} (1973), 83--112; and
\emph{II}, Comm.\ Math.\ Phys.\ \textbf{42} (1975), 281--305.
\item K.~Osterwalder and E.~Seiler, \emph{Gauge field theories on a lattice},
Ann.\ Physics \textbf{110} (1978), 440--471.
\item J.~Fr\"ohlich, R.~Israel, E.~H.~Lieb and B.~Simon, \emph{Phase
transitions and reflection positivity.~I. General theory and long range lattice
models}, Comm.\ Math.\ Phys.\ \textbf{62} (1978), 1--34.
\end{enumerate}
