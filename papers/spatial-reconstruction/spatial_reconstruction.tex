...
\newcommand{\anchor}{ANCHORPLACEHOLDER0000000000000000000000}
\newcommand{\osline}[3]{%
...
Machine-checked as \osline{SpatialOS}{SITEBIJLINE}{sum\_pathsAt\_eq}, with the
two directions
\osline{SpatialOS}{PASTJOINLINE}{pastSiteOf\_joinSite} and
\osline{SpatialOS}{FUTJOINLINE}{futSiteOf\_joinSite}.  The first needs the
...
Machine-checked as \osline{SpatialOS}{GWJOINLINE}{gibbsWeight\_joinSite}.  It
...
\osline{SpatialOS}{PRODWLINE}{prod\_w\_joinSite} holds for \emph{any} pair of
halves, agreeing at the boundary or not, while the kernel identity
\osline{SpatialOS}{PRODKLINE}{prod\_K\_joinSite} needs them to agree.  We state
...
Machine-checked as
\osline{SpatialOS}{SITEBRIDGELINE}{osPairingSiteCross\_eq\_gibbsSum}; the
diagonal case is \osline{SpatialOS}{SITEDIAGLINE}{osPairingSite\_eq\_gibbsSum}.
...
\osline{SpatialOS}{SITENNLINE}{gibbsSumSite\_reflected\_nonneg} and
\osline{SpatialOS}{SITEGRAMLINE}{gibbsSumSite\_reflected\_gram\_nonneg}.  The
...
\osline{SpatialReconstruction}{SITECOLLLINE}{siteForm\_collapse} and
\osline{SpatialReconstruction}{BONDCOLLLINE}{bondForm\_collapse}.
...
\osline{SpatialReconstruction}{STARLINE}{siteForm\_transferOp}.
...
\osline{SpatialReconstruction}{SALINE}{transferOp\_selfAdjoint}, via
\osline{SpatialReconstruction}{STARLEFTLINE}{siteForm\_transferOp\_left}.  The
...
\osline{SpatialReconstruction}{POSLINE}{transferOp\_nonneg}.  By
...
\osline{SpatialReconstruction}{SITENNVEC}{siteForm\_self\_nonneg}.
...
\osline{SpatialReconstruction}{SURJLINE}{collapse\_surjective}.  The witness is
...
ending at it --- \osline{SpatialReconstruction}{CONSTLINE}{const\_mem\_halvesAt}
--- and Gibbs weights are positive.

Consequently the null space of the site form on observables is exactly
$\ker\Phi$, and the quotient by it is the \emph{whole} space of boundary
vectors, of dimension $2^L$.  Gate R1 of Section~\ref{sec:gates} predicted that
dimension as an integer before this was written.
...
\osline{SpatialReconstruction}{MEASLINE}{osPairing\_transfer} composed with the
bond bridge, and stated on the path sum itself at
\osline{SpatialReconstruction}{MEASSUMLINE}{osPairing\_transfer\_gibbsSum}.  In
...
extracted --- \osline{SpatialOS}{BONDCROSSLINE}{osPairingBondCross\_eq\_gibbsSum}
...
coupled kernel; the ring-weighted case is known to be exactly the anisotropic
two-dimensional Ising transfer matrix, so it is not expected to be easy.
...
Everything was elaborated on a fresh Linux clone at that checkpoint, on the
sanctioned compilation plane, with the toolchain and the Mathlib revision pinned
in the repository and re-checked by the runner before any compilation started.
...
Quantity & Value \\
\midrule
Full core build & JOBSAFTER jobs, success \\
\quad same tree without this paper's modules & JOBSBEFORE jobs \\
\quad predicted delta, registered before measuring & $+1$ \\
Oracle commands, and answers & ORACLETOTAL \\
\quad with a non-standard axiom & ORACLENONSTD \\
Declarations in \texttt{SpatialOS.lean} & DECLSOS \\
Declarations in \texttt{SpatialReconstruction.lean} & DECLSREC \\
\quad of them in the oracle & DECLSINORACLE \\
Distinct axioms across all outputs & \texttt{propext}, \texttt{Quot.sound},
  \texttt{Classical.choice} \\
Occurrences of \texttt{sorryAx} & SORRYCOUNT \\
Project axioms & 0 \\
Errors & 0 \\
\bottomrule
\end{tabular}
...
in the runner before any count was taken, and it was registered as a
\emph{delta} rather than an absolute because no recorded baseline applies to
this campaign's base commit --- the counts in the repository's own standing
instructions and in its ledger are on branches that are not ancestors of it.
Both counts in the table therefore come from builds in the same runtime.
