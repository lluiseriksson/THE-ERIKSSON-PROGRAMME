...
$-1/2$.  This operator is not equal in general to the ordinary-edge mixed
operator of Driver--Hall--Kemp: an explicit SU(2) configuration separates them,
and the two also have different reverse resolutions.  The new Lean
...
$C_{\mathrm q}=C_{\mathrm{DHK}}$ in general
 & Refuted & Explicit SU(2) witness \\
Structural relation between $C_{\mathrm q}$ and $C_{\mathrm{DHK}}$
...
$C_{\mathrm q}$ and $C_{\mathrm{DHK}}$ are not equal in general.  Indeed, for
$a_1=a_2=a_3=\alpha=I$, $a_4=\beta=i\sigma_x$, and $A_k=c\,i\sigma_k$ with real
$c
eq0$, one has $C_{\mathrm{DHK}}=c^{2}$ and $C_{\mathrm q}=-3c^{2}$, so
$C_{\mathrm q}-C_{\mathrm{DHK}}=-4c^{2}
eq0$.  What remains open is not their
equality, but a structural comparison between the two operators and its
interaction with the concrete four-face heat density.
...
Equation~\eqref{eq:ward4} is the strongest compiled endpoint of this
paper, conditional only on the explicit 	exttt{SU2CrossingPauliWardData}
interface.  Calling its mixed operator the ordinary-edge operator
