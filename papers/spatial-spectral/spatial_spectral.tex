...
\newcommand{\anchor}{2c089eae9016671ea32cc1786eb2f7e9e96e6da4}
\newcommand{\osline}[3]{%
...
\emph{sharp}: some fluctuation observable attains it, by an argument that
splits at $\mathrm{specGap}=0$, where the maximiser may be a top index and no
...
\osline{SpatialSpectral}{188}{specGap}.
...
\osline{SpatialSpectral}{200}{specGap\_lt}.
...
\osline{SpatialSpectral}{221}{inner\_specBasis\_eq\_zero\_of\_top}.
...
\osline{SpatialSpectral}{279}{norm\_act\_le\_specGap}.

The proof is the expansion in the eigenbasis.  Writing $c_j$ for the coordinates
of $u$, the coordinates of $Ku$ are $\mu_jc_j$
(\osline{SpatialSpectral}{239}{inner\_specBasis\_act}), so by Parseval
(\osline{SpatialSpectral}{261}{eucNorm\_sq\_eq\_sum})
\[
...
(\osline{SpatialSpectral}{372}{perp\_invariant\_eigen}); the scalar drops out
because the pairing is bilinear.  Iterating gives
\osline{SpatialSpectral}{387}{iterate\_norm\_le\_specGap}.
...
\osline{SpatialSpectral}{421}{gibbs\_pathSum\_bound\_unconditional}.
...
\osline{SpatialSpectral}{461}{gibbs\_pathSum\_relative\_decay},
\osline{SpatialSpectral}{328}{specRatio\_lt\_one},
\osline{SpatialSpectral}{351}{norm\_act\_normalizedKernel\_le}.
...
(\osline{SpatialSpectral}{487}{exists\_nonzero\_dressed\_fluctuation}).
\end{remark}
...
\osline{SpatialSpectral}{574}{quadForm\_split}.
...
\osline{SpatialSpectral}{612}{quadForm\_lower}.
...
\osline{SpatialSpectral}{668}{gibbsCorr\_bound\_of\_partition\_lower}.
...
\osline{SpatialSpectral}{699}{gibbsCorr\_decay\_fixed\_extent}.
...
closed formula and not the least such index; and it depends on the observable
only through $c$ and $u$, which do not.  \textbf{This is a fixed-$L$ statement and nothing more}: the
rate is $\mathrm{specRatio}(L)$, the tabulated evidence above is that it tends
to $1$, and therefore the theorem does not survive the infinite-volume limit.
...
\osline{SpatialSpectral}{841}{exists\_attaining\_fluctuation}.
...
(\osline{SpatialSpectral}{806}{inner\_perron\_specBasis\_eq\_zero}); its norm
...
Together with Theorem~\ref{thm:bound} this is the statement that
$\mathrm{specGap}$ \emph{is} the operator norm of $K$ on the fluctuation
sector.  We do not additionally introduce that norm as a defined object and
prove an equation between it and $\mathrm{specGap}$; a bound together with its
attainment carries the same content, and we say which of the two we did.
...
Oracle commands & 2669 \\
\quad with axiom dependencies & 2643 \\
\quad axiom-free & 23 \\
