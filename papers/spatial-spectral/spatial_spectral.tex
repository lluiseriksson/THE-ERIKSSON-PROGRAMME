...
\newcommand{\anchor}{5a2713cb16db7d501473f92ceb67521314e485da}
\newcommand{\osline}[3]{%
...
normalised two-point function is bounded by $C\,\mathrm{specRatio}^{N}$ past an
explicit threshold.  That rate depends on the extent, so this is not
...
\osline{SpatialSpectral}{182}{specGap}.
...
\osline{SpatialSpectral}{194}{specGap\_lt}.
...
\osline{SpatialSpectral}{215}{inner\_specBasis\_eq\_zero\_of\_top}.
...
\osline{SpatialSpectral}{273}{norm\_act\_le\_specGap}.

The proof is the expansion in the eigenbasis.  Writing $c_j$ for the coordinates
of $u$, the coordinates of $Ku$ are $\mu_jc_j$
(\osline{SpatialSpectral}{233}{inner\_specBasis\_act}), so by Parseval
(\osline{SpatialSpectral}{255}{eucNorm\_sq\_eq\_sum})
\[
...
(\osline{SpatialSpectral}{366}{perp\_invariant\_eigen}); the scalar drops out
because the pairing is bilinear.  Iterating gives
\osline{SpatialSpectral}{381}{iterate\_norm\_le\_specGap}.
...
\osline{SpatialSpectral}{415}{gibbs\_pathSum\_bound\_unconditional}.
...
\osline{SpatialSpectral}{455}{gibbs\_pathSum\_relative\_decay},
\osline{SpatialSpectral}{322}{specRatio\_lt\_one},
\osline{SpatialSpectral}{345}{norm\_act\_normalizedKernel\_le}.
...
(\osline{SpatialSpectral}{481}{exists\_nonzero\_dressed\_fluctuation}).
\end{remark}
...
\osline{SpatialSpectral}{568}{quadForm\_split}.
...
\osline{SpatialSpectral}{606}{quadForm\_lower}.
...
\osline{SpatialSpectral}{662}{gibbsCorr\_bound\_of\_partition\_lower}.
...
\osline{SpatialSpectral}{693}{gibbsCorr\_decay\_fixed\_extent}.
The constant is explicit in the proof, $C = 2\|\sqrt{w}A\|^{2}/c^{2}$ up to the
$+1$ that keeps it positive for the zero observable, and $N_0$ is the index past
which the fluctuation part of the partition function has fallen below half of
its Perron part.  \textbf{This is a fixed-$L$ statement and nothing more}: the
...
Oracle commands & 2666 \\
\quad with axiom dependencies & 2640 \\
\quad axiom-free & 23 \\
