...
\newcommand{\anchor}{9704b3f3c0d576bb40c43caa5360fbd64dfa1643}
\newcommand{\osline}[3]{%
...
$\mathrm{specRatio}^{N_0} < \frac{c^{2}}{2(\|b_{\perp}\|^{2}+1)}$, not given as a
closed formula and not the least such index.  Here $c$ and $b_{\perp}$ are the
Perron coefficient and the orthogonal component of the dressed \emph{constant}
observable --- \emph{not} of $A$, so the threshold does not depend on the
observable and only $C$ does.  That rate depends on the extent, so this
...
\paragraph{Norm identified, but no restricted-operator interface.}
Theorems~\ref{thm:sharp} and~\ref{thm:isgreatest} identify $\mathrm{specGap}$ as
the greatest norm ratio on the fluctuation sector, so the norm \emph{is} pinned
down, and by a statement about an object.  What is not built is a
\texttt{Submodule} carrying $v^{\perp}$, with a \texttt{ContinuousLinearMap} on
it and an equation for \emph{its} norm.  Both theorems are also empty when the
state space has fewer than two points.
...
$c=\langle\Om,b\rangle$ and $u=b-c\,\Om$, so that $u\perp\Om$.  (This $u$ is the
$b_{\perp}$ of the abstract; it is built from $b$, never from $A$.)  Then
\[
...
top indices, since those contribute $0$ by definition.  Such an index does
supply an eigenvector --- the Perron one --- but not a \emph{non-Perron} one,
hence none in the fluctuation sector, which is what the argument needed.  The
correct proof splits, and the split is now in the formalisation.
...
\osline{SpatialSpectral}{886}{exists\_attaining\_fluctuation}.
...
(\osline{SpatialSpectral}{851}{inner\_perron\_specBasis\_eq\_zero}); its norm
...
\osline{SpatialSpectral}{939}{specGap\_isGreatest}.
