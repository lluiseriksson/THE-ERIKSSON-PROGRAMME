...
\newcommand{\anchor}{a70426f4c8a0ed733b4eee94fb01b158bc81fd08}
\newcommand{\osline}[3]{%
...
It gives no coupling-sensitive and no volume-uniform information, for two
independent reasons, and both are proved.  First,
...
identity $A-B=4$ between the two \emph{decoupled} even-sector eigenvalues drives
every estimate.  The blindness is proved two-sided, so it covers the symmetrised
convention $w^{1/2}Kw^{1/2}$ as well, and the positive eigenvector is proved to
dominate every eigenvalue, real or complex --- so its eigenvalue is the spectral
radius, which is the Perron statement this development needs and proves without
a Perron--Frobenius theorem in the library.
...
\item and the same computation gives it \emph{two-sided}:
      $R_{\,uAv}=R_A$ for nowhere-zero $u$ on the source and $v$ on the target.
      So the symmetrised convention $w^{1/2}Kw^{1/2}$ is invisible too, which
      settles by theorem what the companion paper could only record as prose.
\end{itemize}
...
\item and \emph{every} eigenvalue is dominated, real or complex:
      $|\mu|\le\lambda_+$.  So $\lambda_+$ is the spectral radius.  This is the
      one consequence of Perron--Frobenius the paper needs, and it is proved
      here rather than cited, because the pinned library has no
      Perron--Frobenius theorem at all.
\end{itemize}
...
\item \textbf{The Birkhoff contraction theorem itself is not mechanised.}  What
is machine-checked is the algebra that forces the blindness and the
degeneration: the invariance of $R_A$, the exact witness, the resulting bounds.
That $\tanh(\Delta/4)$ is a contraction factor for the projective metric is
classical \cite{Birkhoff,Bushell} and enters only as the reason those bounds are
the ones a reader would want; no theorem below takes it as a hypothesis.  A
reader who wants the whole chain inside the kernel should read the title as
naming the obstruction, not the classical interpretation of it.
\item \textbf{The spectral-radius claim covers all eigenvalues, but not
uniqueness or simplicity.}  Theorems~\ref{thm:dominates}
and~\ref{thm:dominates-c} prove $|\mu|\le\lambda_+$ for every eigenvalue, real
or complex, with any nonzero eigenvector; that is what makes $\lambda_+$ the
spectral radius.  Uniqueness of the positive eigenvector up to scale,
simplicity of $\lambda_+$, and completeness of the spectrum are \emph{not}
covered --- those are the rest of Perron--Frobenius, and the library has none of
it (Remark~\ref{rem:pf}).
...
The obvious objection is that the source-only convention is doing the work.  It
is not, and the same computation shows why.

\begin{theorem}[The blindness is two-sided]\label{thm:twosided}
Let $u,v$ be nowhere zero.  Then
$R_{\,u\,A\,v} = R_A$, where $(uAv)(\sigma,\tau)=u(\sigma)A(\sigma,\tau)v(\tau)$.
\end{theorem}

\osline{SpatialBirkhoff}{258}{crossRatio\_twoSided}.

\begin{corollary}[The symmetrised kernel is invisible as well]\label{cor:sym}
$R_{\widetilde K_\gamma} = R_K$ for
$\widetilde K_\gamma(\sigma,\tau)=w_\gamma(\sigma)^{1/2}K(\sigma,\tau)
w_\gamma(\tau)^{1/2}$.
\end{corollary}

\osline{SpatialBirkhoff}{280}{crossRatio\_symCoupledKernel}, with the kernel
itself at \osline{SpatialBirkhoff}{273}{symCoupledKernel}.  The companion paper
could record the corresponding statement only as prose; here it is a theorem,
and it removes the last reading on which the blindness could be an artifact of
the convention.
...
\begin{theorem}[The positive eigenvector dominates]\label{thm:dominates}
Let $A$ be entrywise nonnegative with a strictly positive eigenvector $v$,
$Av=\lambda v$.  Then every real eigenvalue $\mu$ of $A$, with any nonzero real
eigenvector, satisfies $|\mu|\le\lambda$.  In particular, for the coupled
two-site kernel with $\beta,\gamma>0$, every real eigenvalue is dominated by
$\lambda_+$.
\end{theorem}

\osline{SpatialPerron}{401}{abs\_eigenvalue\_le\_of\_pos\_eigenvector} and, for
the kernel of this paper,
\osline{SpatialPerron}{454}{evenTop\_dominates}.

\begin{proof}
Scale the competing eigenvector until it touches the positive one: let $t$ be
the maximum of $|w_i|/v_i$, attained at $i_0$.  Then
$|\mu|\,|w_{i_0}| = |\sum_j A_{i_0j}w_j| \le \sum_j A_{i_0j}|w_j|
\le t\sum_j A_{i_0j}v_j = t\lambda v_{i_0} = \lambda|w_{i_0}|$, and
$|w_{i_0}|>0$.
\end{proof}

\begin{theorem}[The same bound over $\C$, hence the spectral radius]
\label{thm:dominates-c}
The statement of Theorem~\ref{thm:dominates} holds verbatim for complex
eigenpairs: if $w:\iota\to\C$ is not identically zero and $Aw=\mu w$ with
$\mu\in\C$, then $|\mu|\le\lambda$.  Consequently $\lambda_+$ is the
\emph{spectral radius} of the coupled two-site kernel --- it dominates every
eigenvalue, and it is itself one, with a strictly positive eigenvector.
\end{theorem}

\osline{SpatialPerron}{472}{norm\_eigenvalue\_le\_of\_pos\_eigenvector} and
\osline{SpatialPerron}{518}{evenTop\_dominates\_complex}.  The proof is the real
one with the complex modulus in place of the absolute value; the triangle
inequality is all that the argument ever used, so nothing else changes.

\begin{remark}[What this does and does not settle]
Theorems~\ref{thm:dominates} and~\ref{thm:dominates-c} are the consequence of
Perron--Frobenius that this paper needs, and they are proved here rather than
cited, because the pinned library has no Perron--Frobenius
(Remark~\ref{rem:pf}).  With Theorem~\ref{thm:vacuum} they say: $\lambda_+$ is
the spectral radius of the coupled two-site kernel, and the strictly positive
eigenvector carries it.  What is still \emph{not} settled is uniqueness of that
eigenvector up to scale, simplicity of $\lambda_+$, and completeness of the
spectrum --- so `the' vacuum remains, strictly, `a' strictly positive
eigenvector carrying the spectral radius.
\end{remark}
...
Definition~\ref{def:cr} builds the cross-ratio from nothing.  So the one
consequence this paper needs had to be constructed rather than cited, and
Theorems~\ref{thm:dominates} and~\ref{thm:dominates-c} construct it.  What is
machine-checked is exactly what is stated: the eigen-equation, strict positivity
of the eigenvector, and domination in modulus of every eigenvalue, real or
complex --- hence the identification of $\lambda_+$ as the spectral radius.
What remains outside the development is uniqueness up to scale, simplicity of
$\lambda_+$, and spectral completeness.
\end{remark}
...
Direct dense \emph{numerical} diagonalisation of the $2^L\times2^L$ coupled
kernel over
...
\paragraph{The general statement was cheaper than the special one.}
Theorem~\ref{thm:blind} was proved first for a source-only weight, because that
is the case the companion paper needs.  Theorem~\ref{thm:twosided} --- two
factors instead of one --- is the same proof with one more pair of terms to
cancel, and it subsumes the special case, covers the symmetrised convention, and
removes an entire class of referee objection.  The lesson is narrow but real:
when a cancellation argument is driven by \emph{where} a factor sits rather than
\emph{what} it is, generalising costs nothing and buys the objection.
...
Oracle commands & 2542 \\
\quad with axiom dependencies & 2519 \\
\quad axiom-free & 23 \\
