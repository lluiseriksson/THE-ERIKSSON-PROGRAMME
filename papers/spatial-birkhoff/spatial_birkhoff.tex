...
\newcommand{\anchor}{57f3786f04cdcbc7a19b63f5d72f823a9f13e690}
\newcommand{\osline}[3]{%
...
dominate every real eigenvalue --- which is the Perron statement this
development can certify without a Perron--Frobenius theorem in the library.
...
\item and \emph{every} real eigenvalue is dominated: $|\mu|\le\lambda_+$ for any
      real eigenpair.  So $\lambda_+$ is the spectral radius over real
      eigenpairs.  This is the one consequence of Perron--Frobenius the paper
      needs, and it is proved here rather than cited, because the pinned library
      has no Perron--Frobenius theorem at all.
\end{itemize}
...
\item \textbf{The spectral-radius claim is over \emph{real} eigenpairs only.}
Theorem~\ref{thm:dominates} proves $|\mu|\le\lambda_+$ for every real eigenvalue
with a nonzero real eigenvector, which is what makes $\lambda_+$ the spectral
radius in the sense used here.  Complex eigenpairs, and uniqueness of the
positive eigenvector up to scale, are \emph{not} covered --- those are the rest
of Perron--Frobenius, and the library has none of it (Remark~\ref{rem:pf}).
...
Theorem~\ref{thm:dominates} is the consequence of Perron--Frobenius that this
paper needs, and it is proved here rather than cited, because the pinned library
has no Perron--Frobenius (Remark~\ref{rem:pf}).  With
Theorem~\ref{thm:vacuum} it says: $\lambda_+$ is the spectral radius of the
coupled two-site kernel over real eigenpairs, and the strictly positive
eigenvector is the vacuum in that sense.  What is still \emph{not} settled is
uniqueness of the positive eigenvector up to scale, the complex part of the
spectrum, and completeness --- so `the' vacuum remains, strictly, `a' strictly
positive eigenvector carrying the largest real eigenvalue.
\end{remark}
...
Oracle commands & 2540 \\
\quad with axiom dependencies & 2517 \\
\quad axiom-free & 23 \\
