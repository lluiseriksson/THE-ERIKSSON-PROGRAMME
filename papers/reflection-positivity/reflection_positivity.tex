...
\newcommand{\anchor}{bbadf68cb9d2433e2e3f88f910e7240acb6ae21d}
\newcommand{\osfile}[2]{%
...
\title{A Machine-Checked Reflection-Positivity Framework\\
for $\Z_N$ Lattice Gauge Theory,\\
with the $\Z_2$ Wilson Instance}
\author{Lluis Eriksson}
\date{July 27, 2026 (version 1.1)}
...
\section{One instance carrying all four ingredients}
\label{sec:endpoint}
...
one-point system — so a single named endpoint is given in which nothing is
degenerate.

\begin{theorem}[endpoint]
\label{thm:endpoint}
Consider the $\Z_2$ gauge system with two edges exchanged by the time
reflection and one plaquette whose boundary is both of them, carrying the
genuine Wilson factor $e^{\beta s}$.  Its plaquette holonomy is $x+y$, which in
$\Z_2$ equals $x-y$: the plaquette \emph{straddles the reflection plane}, so the
entire Gibbs weight is the crossing kernel and there is no half-space factor at
all.  Then for every $\beta \ge 0$ and every observable $F$ of one half,
$\langle F\rangle_{\mathrm{OS}}$ is a non-negative real.  For $\beta > 0$ the
weight takes two distinct values, so the Gibbs measure is genuinely non-uniform
and the reflection genuinely exchanges the two edges.
\end{theorem}

This is reflection positivity obtained by \emph{discharging} a gauge system, a
splitting and a character expansion — not by assuming a pairing, and not with a
constant weight.

\begin{remark}[the size of the instance]
\label{rem:size}
It is a two-edge system.  A full temporal box, with spatial extent, several
time slices and plaquettes of all three kinds — wholly on one side, wholly on
the other, and straddling — is \emph{not} treated here.  The splitting
structure is precisely the interface such a box would have to discharge, and
discharging it is geometric bookkeeping this paper does not perform.  What the
endpoint establishes is that the three steps compose on a system whose weight
is the real Wilson factor at positive coupling, not that any physically
interesting lattice has been handled.
\end{remark}
...
Thm.~\ref{thm:endpoint} & \lean{z2Wilson\_reflectionPositive},
  \lean{z2WilsonSystem\_nondegenerate}, \lean{z2WilsonSystem\_refl\_ne\_id} & E \\
\bottomrule
\end{longtable}
\normalsize
...
E = \osfile{Z2Endpoint}{YangMills/OS/Z2Endpoint.lean}.
