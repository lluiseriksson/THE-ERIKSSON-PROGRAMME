...
No new Lean module is claimed by this paper.  Historical source files in the
larger programme are not treated as current compiled artifacts, and no
\texttt{.olean} verification is inferred from their presence.  The failed
volume-uniform projective branch was killed before spending remote theorem
prover time.  A future formalization should begin with
$\Theta(A\otimes B)=\Theta(A)\Theta(B)$ and the finite equality
classification, and should be compiled in the designated remote environment.
