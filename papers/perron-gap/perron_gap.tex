...
\newcommand{\anchor}{6aa26bcedc4b008f4e5acaa708da343bf1f1100b}
\newcommand{\osline}[3]{%
...
hence $w$ nonnegative, hence $p=0$.  We also deliver the vacuum in Euclidean
normalisation, $\|\Om\|=1$ with $T\Om=\Om$, and the symmetry of the symmetrised
kernel --- the two interfaces the next module needs.
...
\item \textbf{Real eigenpairs.}  Peripheral separation is proved for real
eigenvalues.  Complex eigenvalues are dominated in modulus by $\lambda$
\cite{ErikssonBirkhoff} but are not separated from it here.
\item \textbf{Algebraic simplicity} is still not proved, and neither is
...
at every extent \osline{PerronGap}{353}{coupled\_gap\_of\_sourceWeight}.
...
decoupled kernel is.  This is the bridge to a self-adjoint operator; the
self-adjointness itself, and the variational characterisation it unlocks, belong
to the module that consumes it and are not proved here.
...
Oracle commands & 2579 \\
\quad with axiom dependencies & 2556 \\
\quad axiom-free & 23 \\
