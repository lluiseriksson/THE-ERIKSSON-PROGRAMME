...
intervals) and certifies $\mathrm{Crit}<0$ on all of $[1/20,3]$; the
...
\emph{All other pairs:} each unit increase of $m+n$ multiplies the
bound $a_nb_m+a_mb_n\le2mn\,(e^*)^8\kappa^{4(m+n)-2}
[((m-1)!m!)^{-2}n!^{-4}+((n-1)!n!)^{-2}m!^{-4}]$ by at most
$\kappa^4\cdot(\text{ratio of weights and factorials})\le
\kappa^4(2q)^4<10^{-2}$ at $\kappa\le1/40$, $q\le$ any index reached
from the base cases; summing the resulting geometric series doubles the
$(1,4)$-term at most. Collecting:
\[
\mathrm{Crit}\le-\kappa^{10}(1-10^{-6})+0.71\,\kappa^{14}
=-\kappa^{10}\bigl(1-10^{-6}-0.71\,\kappa^4\bigr)<0,
\]
since $\kappa^4\le40^{-4}<4\cdot10^{-7}$.
\end{proof}
...
interval-computed tails; a built-in full stability pass at precision
$+70$; canonical-run transcript archived; an Arb twin is a scheduled
upgrade). Bessel enclosures use the positive
