...
cancellations force the closed second-order law
$E=\cos(t/2)(1-c(t)/\beta)+O(\beta^{-2})$,
...
reporting success (the box count is invocation-dependent --- the
archived transcript of the canonical run is committed alongside the
script). For $0<\beta\le1/20$, Lemma~\ref{lem:smallbeta} applies.
\end{proof}
...
$2mn\le s^2/2$), $pq(q^2+p^2)\le s^4$, and at most $s$ pairs per level,
\[
\frac{\pi^3}{48}\!\!\sum_{s\ge6}\!\!\;(\text{level }s)
\le\frac{\pi^3}{48}(e^*)^8\sum_{s\ge6}s^7\kappa^{4s-2}
...
tails computed in intervals with a formally derived and per-box
asserted geometric ratio (from the termwise-exact
$I_{n+1}(x)\le xI_n(x)/(2(n+1))$); a built-in full stability pass at
precision $+70$; canonical-run transcript archived; an Arb twin is a
scheduled upgrade). Bessel enclosures use the positive
