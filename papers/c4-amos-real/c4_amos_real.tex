...
% REGIME: tricotomy; the ONLY slots from v1 are the two declared
% [BIB-ID-SLOT]s (release blockers, charter-registered); tag-scoped
% manuscript hashes; same-lane self-citations from v1.
...
limit.  Every theorem below is stated for every real $\nu \ge 0$ and
every $x > 0$, over the $\Gamma$-power series
\[
\lean{besselIReal}\ \nu\ x \;=\;
...
ordinary prose, always flagged.  The repository's largest namespace
...
already real-parameter in its ordinate --- and the calibration
engine.  New in this arc, the real-order object
$\lean{besselIReal}\ \nu\ x$ above, its ratio
...
zone this is proved directly from the series, with three
...
A certified interval-arithmetic companion (protocol pre-registered
in the arc charter before the run) evaluates the margin
...
$[2.78243072419\times10^{-6} \pm 2.5\times10^{-19}]$ reproduces,
...
registry plus the 40 of this arc), exactly
\begin{center}
...
in the commit message; the certified-grid transcripts (v1
superseded-kept, v2 the authoritative portable pair); and six
...
together with the release revision of this manuscript; compare
against \lean{git show <rev>:path}, never a worktree checkout.  The
release tag \lean{c4-v1.0} marks the release commit, placed after
the pre-release audit desks of the repository's protocol close
(audit registry: \lean{docs/C4-CHARTER.md}); the release-tree audit
is recorded in the manifest, post-tag by sequencing, with any fixes
in post-tag commits; tags never move.

\section{Conclusion}

An Amos-type bound for modified Bessel ratios is now a theorem of
the pinned formal development at every real order $\nu \ge 0$,
proved from the $\Gamma$-power series with no analytic input, with
...
repository and, absent a prior formalization, possibly the first
...
identifier pending \textbf{[BIB-ID-SLOT]}, July 2026.
...
\lean{c3-v1.0.1}; ai.viXra.org identifier pending
\textbf{[BIB-ID-SLOT]}, July 2026.
