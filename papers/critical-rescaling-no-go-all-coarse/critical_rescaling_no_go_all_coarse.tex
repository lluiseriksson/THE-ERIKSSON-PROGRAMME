...
\hypersetup{
  ...
  pdfkeywords={lattice gauge theory, block averaging, Poincare inequality, Fourier mode, Lean 4}
}
...
\date{August 4, 2026 (version 1.2)}
...
sufficient coercivity theorem.  We prove that no scalar rescaling can yield
coercivity on the full one-cochain space with a constant uniform in the block
side.  For every $L\ge2$, every fixed $N'\ge1$, and $N_c\ge2$,
...
$s_L$.  Every admissible full-space Poincar\'e constant obeys
...
has mean-zero transverse modes in its exact kernel, and no scalar multiplier
can lift them.  The first Fourier mode on the within-block cycle makes this
obstruction both local to every block and spectrally sharp at low momentum.

The main result has five components.
...
\codepath{critical_rescaling_fourier_no_go_sources_v1.2.zip} has SHA-256
\begin{center}
\lean{DB9EA587A8F889062B9C0B77A1995C4BC76764251284DF62473745225D36AFEC}.
\end{center}
The packet contains the seven theorem/oracle files, the v1.2 manifest, the
independent audit record, the Lean toolchain, and the pinned Lake files.  The
toolchain is Lean
...
\end{verbatim}
The source release originally completed successfully with 8184 jobs for the
focused theorem build and 8481 jobs for the aggregate core.  An independent
reproduction on August 4, 2026 used a Colab Pro+ CPU/high-RAM runtime with
eight processors and no GPU.  It recomputed all seven LF-normalized source
hashes, checked that the pinned manifest was semantically unchanged, rebuilt
the focused theorem with 8184 successful jobs, and obtained the expected
axiom list for all nine oracle declarations.  A code-token scan found no
\lean{sorry}, \lean{admit}, or project \lean{axiom}.  The runtime was then
disconnected and deleted.  Raw/LF/CRLF hashes, log hashes, endpoint attacks,
and the manifest line-ending incident are recorded in
...
remove an exact kernel.  The constant-sector wall selects
