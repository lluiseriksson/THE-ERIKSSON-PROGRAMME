...
% bd2c18394ce8a55ec73413c11a9f311bbf002248, full core build 8422 jobs,
% 2431 oracle commands (see Section 8).
...
\newcommand{\anchor}{bd2c18394ce8a55ec73413c11a9f311bbf002248}
\newcommand{\osfile}[2]{%
...
whose rate is exactly the verified non-vacuum transfer eigenvalue
$\tanh\beta$; equivalently, the normalised transfer operator has spectral gap
$1-\tanh\beta > 0$, and the corresponding mass is
$-\log(\tanh\beta) > 0$ for $\beta > 0$.  All three are machine-checked.
...
The bound is at fixed finite size and is \emph{not} volume-uniform.  The pairing
of this system is definite --- proved, not assumed --- so the
Gelfand--Naimark--Segal quotient is the identity and therefore does no work;
a system with a degenerate pairing still needs it.  The underlying
...
\item the identification $\Ex_n[\overline{A(\sigma_0)}\,B(\sigma_n)]
      ...
      observables;
\item the non-vacuum bound $\|T_\beta - |\Om\rangle\langle\Om|\| \le a-b = \tanh\beta$,
      hence spectral gap $1-\tanh\beta$, and for $\beta > 0$ mass
      $-\log(\tanh\beta)$;
...
\item \textbf{The bound is not volume-uniform.}  It is a statement at fixed finite
...
pairing of this system is already definite --- Remark~\ref{rem:complete} proves
it --- so the quotient is the identity map.  That is an evasion of the
difficulty, not a solution of it; a system with a degenerate pairing still
requires it.
...
\begin{theorem}[Non-vacuum spectral bound]\label{thm:gap}
For $\beta \ge 0$,
...
\begin{remark}[Three quantities, kept apart]\label{rem:terminology}
With the vacuum eigenvalue normalised to $1$, three different numbers are in
play, and we name them as the literature does.
\begin{itemize}
\item $\tanh\beta$ is the \emph{non-vacuum eigenvalue}: the spectral radius of
$T_\beta$ restricted to the orthogonal complement of the vacuum, equivalently
the contraction rate.  This is what Theorem~\ref{thm:gap} bounds and what
appears as the decay rate in Theorem~\ref{thm:endpoint}.
\item $1-\tanh\beta$ is the \emph{spectral gap} of the transfer operator,
strictly positive for every $\beta$: \osline{Z2Transfer}{331}{z2\_spectral\_gap\_pos}.
\item $-\log(\tanh\beta)$ is the \emph{mass} --- the gap of $H$ in
$T_\beta = e^{-H}$ --- strictly positive for $\beta > 0$:
\osline{Z2Transfer}{348}{z2\_mass\_pos}.
\end{itemize}
Earlier drafts of this development, and the identifiers
\lean{z2TransferOp\_gap}, \lean{connCorrC\_le\_of\_gap} and
\lean{sharp\_clustering\_iff\_gap}, use ``gap'' for the first of the three.
Those names are retained because companion developments already consume
them and their permalinks are fixed; an identifier is not a claim, and what
each of them states is the non-vacuum bound.  In this paper's prose the
three are kept distinct.
\end{remark}
...
$T_\beta$.  Its rate is exactly the non-vacuum eigenvalue bounded in
Theorem~\ref{thm:gap}; the spectral gap and the mass are the derived
quantities of Remark~\ref{rem:terminology}.  That the measure-side decay
rate equals the non-vacuum transfer eigenvalue, from which the spectral gap
and the mass follow, is the content of the reconstruction, and here it is
checked rather
than asserted.
...
\begin{remark}[What ``complete'' means here]\label{rem:complete}
The reconstruction quotients by the null space of the pairing, and a reader
is entitled to ask whether the operator of this paper is literally the
descent of time translation to that quotient or is instead read off the
bond kernel and then matched to the measure.  It is the latter, and that is
deliberate --- an independently constructed operator is what makes
Theorem~\ref{thm:ident} a theorem rather than a definition.  What makes the
chain nonetheless complete is that the quotient step is present and is
proved to be the identity, not assumed to be.  The $n=0$ instance of
Theorem~\ref{thm:ident} \emph{is} the pairing of this system, and
\osline{Z2Identification}{417}{z2\_pairing\_nondegenerate} shows its null
space is zero; \osline{Z2Identification}{399}{z2Obs\_injective} shows
$A \mapsto A\Om$ is injective, so that map is the reconstruction map.  The
general-$n$ identification then says that $T_\beta$ implements the time
translation on its image.  In this definite finite model the
Osterwalder--Seiler quotient is therefore canonically trivial, and the
operator obtained from the normalised bond kernel realises the same
reconstructed Hilbert-space dynamics.  ``Complete'' means every link of the
chain is present and composes; it does not mean that every link was hard.
\end{remark}
...
Oracle commands & 2431 \\
\quad with axiom dependencies & 2409 \\
\quad axiom-free & 22 \\
...
Spectral gap $1-\tanh\beta>0$ & \osline{Z2Transfer}{331}{z2\_spectral\_gap\_pos} \\
Mass $-\log(\tanh\beta)>0$ & \osline{Z2Transfer}{348}{z2\_mass\_pos} \\
...
Reconstruction map injective & \osline{Z2Identification}{399}{z2Obs\_injective} \\
Quotient is the identity & \osline{Z2Identification}{417}{z2\_pairing\_nondegenerate} \\
