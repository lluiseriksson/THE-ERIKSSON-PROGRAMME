...
optimal extension/obstruction across negative orders, an elementary
proof without asymptotics, the correction of an earlier unit-step
paper statement, and a Lean~4/Mathlib formalization of the complete
optimal-domain theorem.  The Lean
...
order rather than compare two arbitrary orders.  We also checked the
negative-order individual-ratio bounds of Hornik--Gr\"un
\cite{HornikGrun2024} and the accessible primary record for the later
modified-Lommel monotonicity paper of Mao--Tian \cite{MaoTian}; neither
source states the cross-order iff boundary below.  The candidate
contribution isolated here is the optimal negative-order
classification in Theorem~\ref{thm:domain}, together with its formal
verification.  A targeted primary-source search found no prior
statement of this exact boundary; that conclusion remains explicitly
subject to external priority review.
\end{remark}
...
boundary and obstruction in Theorem~\ref{thm:domain}.  The priority
claim is not machine-checkable and remains a candidate pending an
independent bibliographic review.  Source presence is not reported as
...
first-contact route from the candidate contribution: the optimal
negative-order classification, now formalized end to end.  Numerical
