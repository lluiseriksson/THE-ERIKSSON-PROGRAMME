...
\medskip
\noindent\textbf{Target 1 (pure real algebra).} The calibration lemma,
with consistent naming ($t$ the ratio variable, $b$ the Bessel
argument):
\begin{verbatim}
lemma amos_calibration (a b t : Real)
    (ha : 0 < a) (hb : 0 < b) (ht : 0 < t)
    (hU : t < b / (a + Real.sqrt (a ^ 2 + b ^ 2))) :
    2 * a / b < 1 / t - t := by
  sorry -- ~30 lines: (s-a)*(s+a) = b^2 with s := Real.sqrt _,
        -- then strict antitonicity of fun t => 1/t - t on (0, oo)
\end{verbatim}
This is Lemma~\ref{lem:calibration} verbatim; no Bessel functions
appear.

\medskip
\medskip
\noindent\textbf{Target 2 (the chain, recurrences as hypotheses).}
Writing $I_0, I_1, I_2$ for the values $I_\nu(x), I_{\nu+1}(x),
I_{\nu+2}(x)$, the unit step is pure ordered-field algebra over two
named hypotheses:
\begin{verbatim}
lemma unit_step_of_recurrence_and_amos
    (x nu I0 I1 I2 : Real) (hx : 0 < x) (hnu : 0 <= nu)
    (hI0 : 0 < I0) (hI1 : 0 < I1) (hI2 : 0 < I2)
    (hrec  : I0 - I2 = (2 * (nu + 1) / x) * I1)
    (hamos : I1 / I0 <
      x / ((nu + 1/2) + Real.sqrt ((nu + 1/2) ^ 2 + x ^ 2))) :
    I1 / I0 - I2 / I1 < 1 / x := by
  sorry -- rho1 = I0/I1 - 2*(nu+1)/x from hrec, then
        -- amos_calibration with a := nu + 1/2 (0 < a from hnu),
        -- b := x, t := I1/I0
\end{verbatim}
The analytic facts \eqref{eq:recurrence}, \eqref{eq:derivative},
\eqref{eq:amos} enter only as the named assumptions \texttt{hrec} and
\texttt{hamos}, to be discharged if and when a Bessel library exists in
Mathlib. (The \texttt{sorry} placeholders mark the two proof
obligations; they are the targets, not claims.)
These two targets are the standing goals of the
