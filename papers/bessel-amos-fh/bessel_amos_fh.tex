...
Feynman--Hellmann identity. The algebraic core of the proof (the calibration lemma and the
unit-step chain) is machine-checked in Lean~4/Mathlib --- no
\texttt{sorry}, axiom oracle exactly Lean's three standard axioms ---
and we report an independent high-precision numerical audit of every
inequality used.
\end{abstract}
...
0 \;<\; \rho_\nu(x) - \rho_{\nu+1}(x) \;<\; \frac{1}{x}
\end{equation}
holds. The two sides have different provenance: the left inequality is
the classical Tur\'an-type inequality \cite{BariczTuran}, which we
import; the elementary content of this note is the \emph{right}
inequality, i.e.\ the unit-step monotonicity \eqref{eq:unitstep}. Consequently, for every $\nu_0 \ge 0$ the map
...
\emph{Second}, the proof yields the upper bound of
\eqref{eq:quantitative} directly in quantitative form, with an explicit
strict margin at every $(\nu, x)$ inherited from the strict Amos margin
$U_\nu - \rho_\nu > 0$; the lower bound (positivity of the difference)
is not reproved here but imported from the Tur\'an literature
\cite{BariczTuran}.
...
verification of the algebraic core (no \texttt{sorry}; standard axioms
only).
...
formalizable without any Bessel analysis, and this has been carried
out: the calibration lemma and the unit-step chain are \emph{proved} in
Lean~4/Mathlib, with no \texttt{sorry} and no project axioms. The
verification file \texttt{FHBesselAmos.lean} accompanies this note in
the repository and was checked with \texttt{lake env lean} against
Lean \texttt{4.30.0-rc2}, Mathlib commit \texttt{cd3b69b}. The axiom
oracle returns, for each of the three theorems, exactly Lean's three
standard axioms:
\begin{verbatim}
'amos_calibration' depends on axioms:
    [propext, Classical.choice, Quot.sound]
'unit_step_of_recurrence_and_amos' depends on axioms:
    [propext, Classical.choice, Quot.sound]
'logderiv_unit_step_increase' depends on axioms:
    [propext, Classical.choice, Quot.sound]
\end{verbatim}

The proved signatures, verbatim:
\begin{verbatim}
theorem amos_calibration (a b t : Real)
    (ha : 0 < a) (hb : 0 < b) (ht : 0 < t)
    (hU : t < b / (a + Real.sqrt (a ^ 2 + b ^ 2))) :
    2 * a / b < 1 / t - t

theorem unit_step_of_recurrence_and_amos
    (x nu I0 I1 I2 : Real) (hx : 0 < x) (hnu : 0 <= nu)
    (hI0 : 0 < I0) (hI1 : 0 < I1)
    ...
    I1 / I0 - I2 / I1 < 1 / x
\end{verbatim}
together with a third theorem, \texttt{logderiv\_unit\_step\_increase},
expressing the conclusion in the closed form of Step~1:
$(\rho_{\nu+1} + (\nu+1)/x) - (\rho_\nu + \nu/x) > 0$. Positivity of
\texttt{I2} turned out not to be needed and is not assumed.

\emph{Honest scope of the formalization.} What is machine-checked is
the ordered-field implication [three-term recurrence $+$ Amos bound
$\Rightarrow$ unit step], with the two analytic inputs entering as the
named hypotheses \texttt{hrec} and \texttt{hamos}; these are the
classical facts \eqref{eq:recurrence} and \eqref{eq:amos}
\cite{DLMF, Amos, Segura2011, RuizSegura}, to be discharged inside Lean
if and when a modified-Bessel library exists in Mathlib. The file is
archived both alongside this note and in the
\texttt{lean-transfer-matrix} satellite of the programme
(\url{https://github.com/lluiseriksson/lean-transfer-matrix}).
