...
$\mathcal{D}_r(S) = \operatorname{ran} E([-r,r])$ for self-adjoint $S$, so
that density of $\mathcal{D}_r(S)$ forces $\sigma(S) \subseteq [-r,r]$ ---
is a statement about spectral subspaces, and criteria of this kind are
standard material in local spectral theory, where
$\limsup_n \|S^n v\|^{1/n}$ is the local spectral radius at $v$.  We have
not located the precise statement in the form used here and have not
performed an exhaustive search; rather than assert priority we state the
expectation that it is known, and we invite the closest antecedent.  What
we do claim is the mechanization, the packaging of the criterion for
\emph{families} of observables with per-member constants, and
