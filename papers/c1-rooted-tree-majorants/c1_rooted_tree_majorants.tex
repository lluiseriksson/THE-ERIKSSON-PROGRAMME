...
\cite{mathlib20}; the upstream repository's formalized cluster
expansion (sharp Kotecký--Preiss criterion, Mayer--Ursell identity,
and a volume-uniform Wilson-loop area law) is described in the
author's preprint \cite{Eriksson26b}.  The Catalan/plane-tree layer
contributes upstream API (a \lean{PlaneTree} companion prepared for
separate submission to Mathlib).
...
\bibitem{Eriksson26b} L.~Eriksson,
\emph{A machine-checked volume-uniform Wilson-loop area law via a
formalized cluster expansion}, preprint, ai.viXra.org:2607.0005,
July 2026.
