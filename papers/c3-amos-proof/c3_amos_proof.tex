...
The companion paper of this series \cite{ErikssonC2} formalized the
...
--- every statement that carried the bound as a hypothesis ---
...
ordinary prose, always flagged.  The repository's largest
...
holds at every order $\ge 1$ (every order at which that step is
defined) --- each now a theorem with no analytic hypothesis.  Lean names:
\begin{center}\small
...
\qquad (0 < x \le 1/4),
\end{equation}
Two further lines convert \eqref{eq:zonekey} into the zone claim:
since $2n+1+x^2 \ge 1$, \eqref{eq:zonekey} also gives the crude
bound $I_{n+1}(x) \le x\,I_n(x)$, hence $x\,I_{n+1}^2 \le
x^2\,I_{n+1}I_n$; multiplying \eqref{eq:zonekey} by $I_n > 0$ and
inserting this,
\[
(2n+1)\,I_{n+1}I_n + x\,I_{n+1}^2 \;\le\;
(2n+1)\,I_{n+1}I_n + x^2\,I_{n+1}I_n \;<\; x\,I_n^2,
\]
which, by the identity $\psi_n \cdot (I_{n+1} I_n) = x\,(I_n^2 -
I_{n+1}^2)$, is precisely $\psi_n(x) > 2n+1$ on the zone --- these
are the two auxiliary steps inside \lean{besselPsi\_zone}.
...
$\psi_n > 2n+1$ on $(1/4, c)$ (at $1/4$ itself this holds by the
...
$s > t$ is the barrier conclusion $\psi_n > 2n+1$ divided by $x$,
and $\rho_n \RHS_n > 0$ --- so, since $1 + \rho_n \RHS_n > 0$,
positivity passes to $\RHS_n - \rho_n$; this is the closing step of
...
\eqref{eq:amos} with certified strict interval enclosures at all
$1206$ points of a pre-registered grid \cite{ErikssonC2}.  Relative to the present
...
(\textbf{8166 jobs}); the axiom oracle printed, for all
\textbf{28} registered statements (the eighteen of \cite{ErikssonC2}
plus the ten headline statements of this paper), exactly
\begin{center}
\lean{[propext, Classical.choice, Quot.sound]};
\end{center}
\noindent the oracle log is committed under \lean{scripts/} at the
same commit.  At the release revision the oracle registry was
extended by the six supporting lemmas named in the map below, so
that every ``exact'' row carries a direct rather than transitive
oracle witness: \textbf{34} entries, every one printing the same
axiom list, with the green rebuild (run 9, again 8166 jobs) and the
extended log committed under \lean{scripts/}.
The full run history of this development --- nine build transcripts
including every failed attempt, each committed the day it was
produced with its diagnosis in the commit message, and one
re-registration of an attempt budget recorded in the audit charter
(\lean{docs/C3-CHARTER.md}, Amendment~1) --- is committed under
\lean{scripts/} and in that charter file; per the repository's
...
\lean{RELEASE-MANIFEST.md} in this paper's directory, committed
together with this manuscript; compare against
\lean{git show <rev>:path}, never a worktree checkout.  The release
tag \lean{c3-v1.0} marks the release commit of this manuscript,
placed after the pre-release audit desks of the repository's
protocol closed (audit registry: \lean{docs/C3-CHARTER.md}); the
release-tree audit is recorded in the manifest, post-tag by
sequencing, with any fixes in post-tag commits; tags never move.
...
consumer statement of the companion development is unconditional.  Limitations: integer orders
