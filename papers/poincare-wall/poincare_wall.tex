...
\midrule
\endhead
CT endpoint, fixed volume & \path{flatGaugeFixedCovariance_CT_fixedVolume} & \texttt{PhysicalShell}\newline\texttt{CombesThomasEndpoint} \\
CT positive-rate witness & \path{exists_flatGaugeFixedCovariance_CT_fixedVolume} & \texttt{PhysicalShell}\newline\texttt{CombesThomasEndpoint} \\
$\dim_\R \su(n)=n^2-1$ & \path{finrank_suLie} & \texttt{SUNAdjoint}\newline\texttt{Dimension} \\
Isometric transport & \path{suLieCoordIso} & \texttt{SUNAdjoint}\newline\texttt{ModelInstance} \\
True adjoint model & \path{matrixSUNAdjointModel} & \texttt{SUNAdjoint}\newline\texttt{ModelInstance} \\
Sector witness ($N_c\ge2$) & \path{exists_pos_constantSector} & \texttt{Physical}\newline\texttt{PoincareWall} \\
Wall, fixed volume & \path{flatPoincare_constantSector_lower_bound} & \texttt{Physical}\newline\texttt{PoincareWall} \\
Wall, linear form ($d\ge3$) & \path{flatPoincare_linear_lower_bound} & \texttt{Physical}\newline\texttt{PoincareWall} \\
Coercivity upper bound & \path{flatPoincare_coercivity_upper_bound} & \texttt{Physical}\newline\texttt{PoincareWall} \\
No volume-uniform $c_0$ & \path{no_volumeUniform_coercivity_via_flatPoincare} & \texttt{Physical}\newline\texttt{PoincareWall} \\
Gate false ($d\ge3$, $N_c\ge2$) & \path{volumeUniformFlatPoincareGate_false} & \texttt{Physical}\newline\texttt{PoincareWall} \\
Per-volume non-vacuity & \path{perVolume_flatPoincare_family} & \texttt{Physical}\newline\texttt{PoincareWall} \\
Fluctuation predicate & \path{IsFluctuationCochain} & \texttt{PhysicalPoincare}\newline\texttt{SectorQuotient} \\
Full $\Rightarrow$ quotient & \path{quotientFlatPoincare_of_flatPoincare} & \texttt{PhysicalPoincare}\newline\texttt{SectorQuotient} \\
Non-transfer of the witness & \path{constant_not_fluctuation} & \texttt{PhysicalPoincare}\newline\texttt{SectorQuotient} \\
Quotient gate predicate & \path{VolumeUniformQuotientPoincareGate} & \texttt{PhysicalPoincare}\newline\texttt{SectorQuotient} \\
Square profile, zero sum & \path{sum_squareSign} & \texttt{PhysicalPoincare}\newline\texttt{LowModeFalsifier} \\
Mode is fluctuation & \path{squareModeCochain_isFluctuation} & \texttt{PhysicalPoincare}\newline\texttt{LowModeFalsifier} \\
Mode norm, exact & \path{norm_sq_squareModeCochain} & \texttt{PhysicalPoincare}\newline\texttt{LowModeFalsifier} \\
Exact transported Hodge ratio & \path{flatGaugeHodgeK0_inner_blockScaleSquareModeCochain_eq_div} & \texttt{PhysicalPoincare}\newline\texttt{LowModeBlock} \\
Physical block contraction & \path{norm_sq_flatBlockConstraintQCLM_le_inv_mul} & \texttt{PhysicalPoincare}\newline\texttt{LowModeBlock} \\
Rayleigh numerator bound & \path{blockScaleSquareMode_rayleigh_numerator_le} & \texttt{PhysicalPoincare}\newline\texttt{LowModeBlock} \\
Quotient linear lower bound & \path{quotientPoincare_squareMode_linear_lower_bound} & \texttt{PhysicalPoincare}\newline\texttt{LowModeBlock} \\
Quotient gate false & \path{volumeUniformQuotientPoincareGate_false} & \texttt{PhysicalPoincare}\newline\texttt{LowModeBlock} \\
\bottomrule
\end{longtable}
\endgroup
...
checkpoint \lean{a17d7816}, sealed together with its transcript and live
documentation by commit \lean{33072662}; this manuscript revision is
...
names); the committed
transcript
\lean{docs/oracle-transcripts/ORACLE-20260714-a17d7816.txt} records the
...
\paragraph{Verification ledger.}  The append-only ledger
\lean{docs/VERIFICATION-LEDGER.md} documents every checkpoint of the
