% The volume-uniform Poincare wall — machine-checked obstruction paper.
% Lane: hRpoly / P4-CT / P4-ADJ / W-1..W-3a (ledger Addenda 467-503).
...
% 12ca1a87 (sealed 64969b35); this manuscript added in a strictly
...
\title{The Volume-Uniform Poincar\'e Wall:\\
a Machine-Checked Obstruction for the Flat Block-Poincar\'e Route\\
...
lattice Yang--Mills, we report a machine-checked \emph{negative} result
and the machine-checked infrastructure that makes it meaningful.  The
positive substrate: (i) a fixed-volume Combes--Thomas chain for
...
coordinates --- so that no statement below is discharged by a trivial
model.  The headline theorem is the \emph{wall}: under the block
normalization actually used by the formalized chain, every flat
...
impression.  We further formalize the constant-sector quotient interface
(the wall witness provably does not transfer to the fluctuation space;
the quotient gate is neither proved nor refuted) and the exact witness
family for its one-sided falsifier.  Everything stated here is checked
...
and reproduction instructions are included.  No claim of progress on the
Clay problem is made: the distance to the mass-gap problem is unchanged
and effectively $0\%$.
\end{abstract}
...
coercive finite-range operator has an inverse with exponentially decaying
kernel, at a rate governed by the coercivity constant.  Whoever supplies
...
The outcome has three layers, and we state them with the labels they
deserve.
...
(Section~\ref{sec:adj}).  No theorem below is satisfied merely by the
trivial model.

\textbf{Negative (the wall; the headline).}  Under the block
...
Two further layers are \emph{result-neutral} and are reported as such.
The \emph{constant-sector quotient interface}
(Section~\ref{sec:quotient}) restricts the Poincar\'e question to the
orthogonal complement of the constant sector; the wall witness provably
does not transfer there, and the volume-uniform quotient gate is
\emph{open} --- neither proved nor refuted.  The \emph{falsifier witness
family} (Section~\ref{sec:falsifier}) is an exact square-wave fluctuation
mode intended for a one-sided test of that gate: it can refute the gate
if its Rayleigh quotient degenerates; a bounded quotient on it would only
eliminate one candidate.  We are explicit about this asymmetry because
the formal record must not overclaim in either direction.

\subsection*{Honest scope}

This paper proves no mass gap, no volume-uniform Combes--Thomas bound,
and nothing about the continuum limit; the distance to the Clay
problem~\cite{JaffeWitten} is unchanged by these results and is
effectively $0\%$.  The wall is a theorem about \emph{one specific
route under one specific normalization} --- precisely the route the
formalized fixed-volume chain consumes.  It does not exclude a
volume-uniform Combes--Thomas estimate obtained after quotienting the
harmonic sector, rescaling the block map (the wall must then be
re-audited: it may merely move), or extracting coercivity from the
interacting Wilson Hessian directly.  The value of the negative result
is navigational: it converts a would-be silent dead end into a
machine-checked boundary, before any downstream construction is built
on it.
...
tilt budget such that the covariance kernel obeys
\[
|K^{-1}(x,y)| \;\le\; \frac{2}{c}\; e^{-\theta\, d(x,y)} .
\]
\end{theorem}
...
\lean{no\_volumeUniform\_coercivity\_via\_flatPoincare}]\label{thm:noc}
...
\begin{remark}[the open gate, stated as open]
The volume-uniform quotient gate
(\lean{VolumeUniformQuotientPoincareGate}) is neither proved nor refuted
in the formalization, and this paper makes no claim in either direction.
Non-transfer removes the W-1 witness, not the possibility of a wall:
slowly varying near-harmonic modes are the natural candidate
counter-witness, which motivates the next section.
\end{remark}

\section{The falsifier witness family}\label{sec:falsifier}
...
can be evaluated exactly.  The repository provides the family and its
two exact identities (\lean{PhysicalPoincareLowModeFalsifier.lean}); the
form evaluation itself (Hodge term, block term, Rayleigh quotient,
endpoint decision) is registered future work, and we state the
discipline that governs it.

\begin{definition}[the lowest square-wave mode; \lean{squareModeCochain}]
...
\begin{remark}[one-sided discipline]
The registered continuation evaluates
$R \;=\; \bigl(\inn{A}{K_0A} + \|QA\|^2\bigr)/\|A\|^2$
on this family, with no normalization that silently cancels
$L$-dependence.  Two outcomes are admissible: a volume sequence with
$R\to0$, which formally refutes the quotient gate (a second wall); or a
uniform positive lower bound \emph{on this mode only}, which eliminates
one candidate and leaves the gate open.  Only a spectral-minimality or
all-modes argument could convert a favorable computation into a proof of
the gate, and no such argument is claimed.
\end{remark}
...
\midrule
CT endpoint, fixed volume & \lean{flatGaugeFixedCovariance\_CT\_fixedVolume} & \lean{PhysicalShellCombesThomasEndpoint} \\
CT positive-rate witness & \lean{exists\_flatGaugeFixedCovariance\_CT\_fixedVolume} & \lean{PhysicalShellCombesThomasEndpoint} \\
$\dim_\R \su(n)=n^2-1$ & \lean{finrank\_suLie} & \lean{SUNAdjointDimension} \\
Isometric transport & \lean{suLieCoordIso} & \lean{SUNAdjointModelInstance} \\
True adjoint model & \lean{matrixSUNAdjointModel} & \lean{SUNAdjointModelInstance} \\
Sector witness ($N_c\ge2$) & \lean{exists\_pos\_constantSector} & \lean{PhysicalPoincareWall} \\
Wall, fixed volume & \lean{flatPoincare\_constantSector\_lower\_bound} & \lean{PhysicalPoincareWall} \\
Wall, linear form ($d\ge3$) & \lean{flatPoincare\_linear\_lower\_bound} & \lean{PhysicalPoincareWall} \\
Coercivity upper bound & \lean{flatPoincare\_coercivity\_upper\_bound} & \lean{PhysicalPoincareWall} \\
No volume-uniform $c_0$ & \lean{no\_volumeUniform\_coercivity\_via\_flatPoincare} & \lean{PhysicalPoincareWall} \\
Gate false ($d\ge3$, $N_c\ge2$) & \lean{volumeUniformFlatPoincareGate\_false} & \lean{PhysicalPoincareWall} \\
Per-volume non-vacuity & \lean{perVolume\_flatPoincare\_family} & \lean{PhysicalPoincareWall} \\
Fluctuation predicate & \lean{IsFluctuationCochain} & \lean{PhysicalPoincareSectorQuotient} \\
Full $\Rightarrow$ quotient & \lean{quotientFlatPoincare\_of\_flatPoincare} & \lean{PhysicalPoincareSectorQuotient} \\
Non-transfer of the witness & \lean{constant\_not\_fluctuation} & \lean{PhysicalPoincareSectorQuotient} \\
Quotient gate (open) & \lean{VolumeUniformQuotientPoincareGate} & \lean{PhysicalPoincareSectorQuotient} \\
Square profile, zero sum & \lean{sum\_squareSign} & \lean{PhysicalPoincareLowModeFalsifier} \\
Mode is fluctuation & \lean{squareModeCochain\_isFluctuation} & \lean{PhysicalPoincareLowModeFalsifier} \\
Mode norm, exact & \lean{norm\_sq\_squareModeCochain} & \lean{PhysicalPoincareLowModeFalsifier} \\
\bottomrule
\end{tabular}
\end{center}

Every listed name is exercised by the committed axiom-oracle script; on
the frozen tree each prints exactly
...
checkpoint \lean{12ca1a87}, sealed by commit \lean{64969b35}; this
manuscript is added in a strictly editorial later commit that changes no
Lean file.
...
\lean{Build completed successfully (8410 jobs)}.

\paragraph{Axiom oracle.}  \lean{lake env lean oracle\_check.lean}
executes $2194$ \lean{\#print axioms} invocations; the committed
transcript
\lean{docs/oracle-transcripts/ORACLE-20260714-12ca1a87.txt} records the
full raw output with provenance header: $2174$ results on a nonempty
subset of \lean{[propext, Classical.choice, Quot.sound]} (finely
$2108+49+17$) and $20$ axiom-free results; zero \lean{sorryAx}, zero
nonstandard axiom sets.  SHA-256 hashes (git blob form, LF):
script \lean{4376FA7E}\ldots, raw output \lean{3FAF5D32}\ldots.

\paragraph{Verification ledger.}  The append-only ledger
\lean{docs/VERIFICATION-LEDGER.md} documents every checkpoint of the
results here (Addenda 467--503), including the measurement-discipline
...
The registered next steps, in order: evaluation of the Hodge term on the
witness family (two jump slices); evaluation of the block term and the
Rayleigh quotient; the endpoint decision for the quotient gate (second
wall or candidate-eliminated); and, independently of the outcome, the
two other registered routes (rescaled block map with re-audited sector
normalization; interacting-Hessian coercivity).  Each will be reported
with the same discipline --- exact identities, declared
