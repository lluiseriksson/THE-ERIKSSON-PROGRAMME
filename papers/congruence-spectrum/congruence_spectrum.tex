...
The hypercube instance is the fusion bound. Every theorem is machine-checked in
Lean~4.
\end{abstract}
...
\[
  \frac{|\lambda_1(T)|}{\lambda_0(T)}\;\le\;\tanh\!\big(\Delta(T)/4\big)
\]
is classical \cite{birkhoff1957,evesonnussbaum1995}. \emph{That is the only
input we import.} The two facts that make it bite are elementary, and both are
proved here.
...
Lemmas~\ref{lem:deltainv} and~\ref{lem:deltaval} and the Birkhoff bound,
\[
...
\emph{Lower bound.} Concentrating on an argmin pair $\{p,q\}$ leaves the block
$\begin{psmallmatrix}1&\mu\\ \mu&1\end{psmallmatrix}$ exactly, for every
$\varepsilon>0$ (Proposition~\ref{prop:orbit}), of ratio $(1-\mu)/(1+\mu)$; and
the value is not attained because $\Delta$ is a strict maximum only in the
limit.
\end{proof}
...
positive diagonals, on families chosen to break it, before the proof was found,, never exceeded
...
Theorem~\ref{thm:lcp}. Two imported classical inputs are cited rather than
reproved, and both are labelled where they are used: the continuity of
eigenvalues invoked after Proposition~\ref{prop:orbit}, and the Birkhoff bound
$|\lambda_1|/\lambda_0\le\tanh(\Delta/4)$ used in Theorem~\ref{thm:lcp}. No
statement in this paper is conjectural.

\begin{center}
\begin{tabular}{ll}
\toprule
commit & \texttt{954f3b2d} \\
module \texttt{sha256} & \texttt{b479f550b5ac7a261e8dd0ed} \\
                       & \texttt{7735416eec09cfb239759e4b973130b848e2296c} \\
...
axiom oracle & 36/36 declarations on \texttt{[propext, Classical.choice, Quot.sound]} \\
\texttt{sorryAx} & none \\
\bottomrule
\end{tabular}
\end{center}
