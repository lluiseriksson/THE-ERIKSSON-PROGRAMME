...
If $M_{ii}=1$ and $\mu\le M_{ij}\le 1$ for all $i,j$ with $\mu>0$, then
$\Delta(M)=2\log(1/\mu)$ exactly.
\end{lemma}
...
(Theorem~\ref{thm:rigid}, Lemma~\ref{lem:change}), the fusion identity
(Theorem~\ref{thm:fusion}), the wall (Theorem~\ref{thm:wall}), the orbit
statement (Proposition~\ref{prop:orbit}), and the algebraic and eigenvector
ingredients of Lemma~\ref{lem:bond}, Theorem~\ref{thm:exch} and
Theorem~\ref{thm:lcp}
against a pinned Mathlib in \texttt{YangMills/OS/CongruenceSpectrum.lean}, and
\textbf{has been verified}, including the two lemmas that carry
...
monotonicity. \emph{Their interpretation in terms of the spectrally defined $r$
...
\cite{birkhoff1957,evesonnussbaum1995} were checked against the publishers'
records, but the internal numbering of the results inside those papers was not,
so Fact~\ref{fact:birkhoff} is attributed to the papers rather than to a
numbered statement in them.
