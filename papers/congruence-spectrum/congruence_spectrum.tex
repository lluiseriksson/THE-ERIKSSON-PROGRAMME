...
\emph{This is the only mathematical input we import}, and it is imported as a
statement, not reproved. The two facts that make it bite are elementary, and
...
positive diagonals, on families chosen to break it, and run before the proof was found, never exceeded
...
The results named in the next sentence, together with the algebraic lemmas
supporting Theorem~\ref{thm:lcp}, are written in Lean~4 --- the rigid half
(Theorem~\ref{thm:rigid}, Lemma~\ref{lem:change}), the one-bond spectrum with
its sign hypothesis (Lemma~\ref{lem:bond}, Remark~\ref{rem:sign}), the fusion
identity (Theorem~\ref{thm:fusion}), the wall
(Theorem~\ref{thm:wall}), the orbit statement (Proposition~\ref{prop:orbit}) and
the exchangeable computation (Theorem~\ref{thm:exch})
