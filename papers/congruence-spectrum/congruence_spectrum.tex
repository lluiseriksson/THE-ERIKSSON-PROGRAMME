...
the value of Theorem~\ref{thm:lcp} can be read off the minimum cross-ratio with
...
Let $M$ be symmetric with $M_{ii}=1$ and $\mu\le M_{ij}\le 1$ for all $i,j$,
where $\mu>0$. Then $\Delta(M)=2\log(1/\mu)$ exactly.
\end{lemma}
...
load-bearing import of Theorem~\ref{thm:lcp}, and a reader must be able to go to
one line of one source and check it. Remark~\ref{rem:phiform} reduces the
...
\bibitem{seneta2006} E.~Seneta,
  \emph{Non-negative Matrices and Markov Chains}, rev.\ ed.,
