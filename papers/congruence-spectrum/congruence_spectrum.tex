...
\section{Which congruence is extremal: pairs beat clusters}
...
The reading is the point. \textbf{The more sites a uniform concentration keeps,
the smaller the ratio it can reach.} So among uniform concentrations the best is
the smallest nondegenerate one --- a \emph{pair} --- and at $m=2$ the value is
...
That settles the uniform family. For arbitrary weights we have only a
...
Every theorem and lemma stated above --- the rigid half
...
(Theorem~\ref{thm:wall}) and the exchangeable computation
(Theorem~\ref{thm:exch}) --- is written in Lean~4 against a pinned Mathlib in
\texttt{YangMills/OS/CongruenceSpectrum.lean}, and \textbf{has been verified}.
Only Conjecture~\ref{conj:lcp} is not, and it carries the word
\emph{conjecture} in every sentence that states it.

\begin{center}
\begin{tabular}{ll}
\toprule
commit & \texttt{6c41bf1d} \\
module \texttt{sha256} & \texttt{4939fc4dc0e3f8190b0684ba} \\
                       & \texttt{3a76296ef8e9dcf9077818909ebe2369ef1b8f82} \\
...
axiom oracle & 23/23 declarations on \texttt{[propext, Classical.choice, Quot.sound]} \\
\texttt{sorryAx} & none \\
\bottomrule
\end{tabular}
\end{center}
