...
% 673d1df122ac7110e6f7b52795b44a02d38d8d24, full core build 8423 jobs,
% 2460 oracle commands (see Section 7).
...
\newcommand{\anchor}{673d1df122ac7110e6f7b52795b44a02d38d8d24}
\newcommand{\osline}[3]{%
...
from the Boltzmann weights.  For $\beta>0$ the reconstructed physical space is
two-dimensional.  Integrating out the future collapses four observables onto two
states, and that collapse is the null space.  We prove: the pairing factors through an
...
The degeneracy is the mechanism the reconstruction exists to handle, and the one
expected to reappear in systems with larger half-space algebras.  What is not claimed: this is two
...
For $\beta>0$ the reconstructed physical space is two-dimensional.  A four-dimensional space
...
This is the mechanism, and it is the one expected to reappear in systems with
larger half-space algebras: a lattice with spatial extent has half-space
observables in far greater number than its transfer operator has states.  We
state that as an expectation about the systems targeted next, not as a theorem.
...
\item an explicit $A\neq 0$ with $\Phi A = 0$;
\item $\bigl(\C^{2\times 2}\bigr)/\ker\Phi \cong \C^2$, the isomorphism
      induced by $\Phi$;
\item and, for $\beta>0$, the form the quotient carries is positive definite,
      so it is a pre-Hilbert space --- though the identification with
      $\C^2$ is linear, not isometric.
\end{itemize}
...
$\Z_2$ variable per slice; for $\beta>0$ the physical space is two-dimensional
(and one-dimensional at $\beta=0$, by Remark~\ref{rem:betapos}).
...
\begin{remark}[Why $\beta>0$, said rather than avoided]\label{rem:betapos}
...
definiteness does not: at $\beta=0$ the whole line $v_0=-v_1$ becomes null, so the
null space of the pairing is strictly larger than $\ker\Phi$ and the reconstructed
space drops from two dimensions to one.  That is a theorem here, not an
asserted caveat: \osline{Z2Quotient}{398}{kForm\_not\_definite\_at\_zero}
exhibits the witness.  The hypotheses in the formalisation record exactly this
...
\begin{remark}[Which inner product the quotient carries]\label{rem:inner}
Theorem~\ref{thm:quot} is a \emph{linear} isomorphism, and it is worth saying
precisely what that does and does not settle.  The Gelfand--Naimark--Segal
inner product does not transport to the standard Euclidean structure of
$\C^2$; it transports to the kernel-weighted form
\[
  \inn{v}{w}_{k_\beta} \;=\; \sum_{b,c} \overline{v(b)}\, k_\beta(b,c)\, w(c),
\]
and the pairing of this paper is exactly that form pulled back along $\Phi$:
\osline{Z2Quotient}{304}{reflPairing\_eq\_kForm}.  For $\beta>0$ this form is
positive definite --- \osline{Z2Quotient}{336}{kForm\_definite} --- so the
quotient is a genuine pre-Hilbert space and not merely a vector space, which
is what entitles us to call it the Gelfand--Naimark--Segal quotient.  An
\emph{isometric} identification with standard Euclidean $\C^2$ would
additionally require the positive square root of $k_\beta$; that is not
constructed here, and no isometry is claimed.
\end{remark}
...
\paragraph{Independently defined, over a shared primitive.}  The pairing and
the reconstruction map are defined independently over the same primitive
Boltzmann bond weight $k_\beta$.  Neither is defined from the other --- that is
the property non-circularity needs --- and their first interaction is
Theorem~\ref{thm:factor}.  This is what makes
...
Oracle commands & 2460 \\
\quad with axiom dependencies & 2438 \\
\quad axiom-free & 22 \\
