...
\date{August 2, 2026 -- research preprint, version 0.5}
...
0.5 additionally formalizes the scalar improper integral, the Bochner
transport from a resolvent-family kernel estimate to the exact coefficient
$2/\sqrt c$, and the quantitative operator bound
$\norm{K^{-1/2}}\le c^{-1/2}$.  The remaining gap is stated without
compression: Lean does not yet identify the canonical spectral root with the
operator-valued resolvent integral (nor prove integrability of that canonical
inverse family).  Thus the localized physical root certificate remains
...
also \formalized.  Lean now checks the scalar Stieltjes integral and a generic
Bochner theorem that transports an integrable resolvent-family kernel bound
to the exact $2/\sqrt c$ coefficient.  The complete Stieltjes passage in
Theorem~\ref{thm:root} remains \proved{} at paper level because identification
of the formal spectral root with the canonical shifted-inverse integral is
...
The scalar identity in \eqref{eq:scalar} is now independently
\formalized{} as \path{integral_Ioi_inv_add_sq}, together with its
integrability theorem.  More importantly, the step from the pointwise
resolvent estimate to the integrated single-bond bound is no longer merely a
paper calculation.  For any Bochner-integrable operator family $F(t)$, Lean
proves
\[
  \norm{(F(t)\delta_pv)(q)}
...
$w(q,p)=e^{-\theta\dist(q,p)}$.  This theorem closes the integration and
single-bond evaluation step itself; it deliberately does not assert that the
abstract family $F$ is the canonical shifted inverse or that its integral is
the spectral root.
...
bound.  The integrated-kernel theorem derives the correct item-6 bound for the
Stieltjes integral of any integrable family satisfying the shift-uniform
estimate.  Item 6 for the \emph{canonical spectral root} still awaits
identification of that root with this integral.
...
smallness hypothesis by coercivity plus finite range.  Version 0.5 closes the
scalar integral, its exact constant, the abstract Bochner kernel transport,
and the quantitative root norm; neither route yet proves the physical
single-bond kernel bound for the canonical root without the remaining
...
matrix root through an orthonormal basis, and the post-identification kernel
transport is now machine-checked by Bochner integration and continuous
coordinate evaluation.  The boxed equality is the remaining engineering
question: construct the canonical shifted-inverse family as an integrable Lean
term and identify its integral with the spectral root.
...
estimate.  The remaining obligation is now sharply localized at the
operator-valued Stieltjes identification, including integrability of the
canonical inverse family.  Supplying the root kernel predicate as an
...
representation; (ii) the canonical shifted-inverse family cannot be made
Bochner integrable and identified with the spectral root without assuming the
desired root-kernel conclusion; (iii) the root certificate requires a
...
Bochner kernel transport are in \path{StieltjesKernelIntegration.lean}.
...
version 0.5 cites the integration commit and updates the formal/open boundary.
...
\lean{shift-uniform-ct-v0.5}.  The detached \path{ARTIFACT.md} manifest records
...
on August 2, 2026.  Two earlier attempts to rebuild the full
...
full-core rebuild is claimed.  A targeted oracle checked eighteen declarations:
seven resolvent declarations, five root-layer declarations, two quantitative
adapter declarations, and four scalar/Bochner integration declarations.  Each
printed exactly
\[
...
three root modules, or the Stieltjes-integration module.
...
version 0.5 now formalizes its sharp coercivity norm bound, the scalar improper
integral, and the Bochner transfer of the integrated resolvent estimate to the
single-bond kernel predicate.  What remains is narrower and genuinely
operator-theoretic: prove integrability of the canonical shifted-inverse
family and identify its normalized integral with that spectral root.  Until
this equality is formalized, the localized physical certificate remains
conditional; the fixed-volume Poincar\'e wall is unchanged.
