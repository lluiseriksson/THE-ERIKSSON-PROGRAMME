...
\date{August 2, 2026 -- research preprint, version 0.6}
...
of its normalized Stieltjes integral.  The remaining gap is now one equality:
Lean does not yet identify that integral with the separately constructed
spectral root.  Thus the localized physical root certificate remains
conditional on this identification; fixed-volume coercivity is still not
volume-uniform.
\end{abstract}
...
also \formalized.  Lean now constructs the canonical shifted-inverse family,
proves it continuous and Bochner integrable, and proves the exact
$2/\sqrt c$ exponential kernel bound for its normalized Stieltjes integral.
The complete passage in Theorem~\ref{thm:root} remains \proved{} at paper level
because identification of the formal spectral root with that integral is
still \openstatus.  Consequently unconditional discharge of the existing
\path{PhysicalLocalizedCovarianceRootCertificate} is also open.  No continuum
limit, mass gap, or volume-uniform positive coercivity constant is claimed.
...
\begin{equation}\label{eq:formal-integral-bound}
 ...
 \le \frac{2}{\sqrt c}e^{-\theta\dist(q,p)}.
\end{equation}
This formalizes localization of the canonical integral.  The remaining step
is to prove in Lean that this operator equals the separately constructed
spectral root $K^{-1/2}$.
...
bound.  The canonical-family theorem and \eqref{eq:formal-integral-bound}
derive the correct item-6 bound for the actual shifted-inverse Stieltjes
integral.  Item 6 for the \emph{canonical spectral root} still awaits the
single identification of that root with this already-localized integral.
...
neither route yet transfers that bound to the spectral root without the
remaining Stieltjes identification.
...
matrix root through an orthonormal basis.  The integral in the boxed equality
is now a canonical, continuous, integrable Lean term, and the
post-identification kernel transport is machine-checked by Bochner integration
and continuous coordinate evaluation.  Only the boxed equality itself remains.
...
inverse family.  The remaining obligation is exactly its equality with the
spectral root.  Supplying the root kernel predicate as an
assumption still does not prove decay; the certificate adapter uses it only to
isolate this exact frontier.
...
not assumptions.  Only identification of its integral with the formal
spectral root remains open.
...
representation; (ii) the canonical shifted-inverse integral cannot be
identified with the spectral root without assuming the desired root-kernel
conclusion; (iii) the root certificate requires a
...
constants, and endpoint.  None is currently observed, but (ii) is the real
...
Canonical shifted-inverse integral equals spectral root & not yet implemented & open \\
Theorem~\ref{thm:root} as one end-to-end Lean declaration & this manuscript & paper \\
\bottomrule
\end{longtable}
\normalsize
...
version 0.5 cites the integration commit, and version 0.6 cites the canonical
shifted-inverse commit and updates the formal/open boundary.
...
\lean{shift-uniform-ct-v0.6}.  The detached \path{ARTIFACT.md} manifest records
...
on August 2, 2026.  On the same date the canonical-family target completed
successfully with 8203 jobs, and a fresh \path{YangMillsCore} build completed
successfully with 8470 jobs.  A targeted oracle checked twenty-six
declarations: the prior eighteen plus positive-root uniqueness and seven
canonical-family results.  Each printed exactly
\[
...
proves the localized bound for its normalized integral.  What remains is one
genuinely operator-theoretic equality: identify that integral with the
spectral root.  Until this equality is formalized, the localized physical
certificate remains conditional; the fixed-volume Poincar\'e wall is
unchanged.
