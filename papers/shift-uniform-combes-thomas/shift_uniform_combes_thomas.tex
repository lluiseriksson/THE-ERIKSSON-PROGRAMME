...
% Stieltjes-integration artifact: 73179102;
% canonical shifted-inverse artifact: 9f19b2d7;
% canonical Stieltjes-root identification artifact: b44907f0.
...
\date{August 2, 2026 -- research preprint, version 0.7}
...
\]
The cancellation identity, shifted coercivity, uniform defect estimate,
resolvent kernel theorem, and the self-adjoint Stieltjes-root passage are
formalized in Lean~4 on the repository's actual physical gauge-cochain types.
In particular,
\[
...
linear-map root is also constructed and verified spectrally in Lean.  Version
0.5 additionally formalizes the scalar improper integral, generic Bochner
kernel transport, and the quantitative operator bound
$\norm{K^{-1/2}}\le c^{-1/2}$.  Version 0.6 constructs the canonical family
$F(t)=(K+t^2\Id)^{-1}$, proves its exact inverse and resolvent identities,
operator-norm continuity, Bochner integrability, and exponential localization
of its normalized Stieltjes integral.  Version 0.7 evaluates this integral on
an orthonormal eigenbasis, proves positivity and the exact square identity,
identifies it with the separately constructed spectral root by uniqueness,
and discharges the root-kernel field of the physical certificate.  The
operator bridge is therefore closed in the stated finite-dimensional model.
Fixed-volume coercivity is still not volume-uniform.
\end{abstract}
...
\formalized.  The positive continuous-linear-map square root, its exact
square, positivity, symmetry, and the bound $\norm{K^{-1/2}}\le c^{-1/2}$ are
also \formalized.  Lean constructs the canonical shifted-inverse family,
proves it continuous and Bochner integrable, identifies its normalized
Stieltjes integral exactly with the spectral root, and proves the
$2/\sqrt c$ exponential kernel bound for that root.  The complete passage in
Theorem~\ref{thm:root} is now \formalized, including an end-to-end constructor
for \path{PhysicalLocalizedCovarianceRootCertificate} under its explicit
base-covariance and Combes--Thomas premises.  No continuum limit, mass gap, or
volume-uniform positive coercivity constant is claimed.
...
included with a complete proof and is also represented by named Lean
declarations.

\begin{theorem}[localized inverse square root; \formalized]\label{thm:root}
...
The scalar identity in \eqref{eq:scalar} is independently \formalized{} as
\path{integral_Ioi_inv_add_sq}, together with its integrability theorem.  The
step from the pointwise resolvent estimate to the integrated single-bond bound
is also no longer merely a paper calculation.  For any Bochner-integrable
operator family $F(t)$, Lean proves
\[
  \norm{(F(t)\delta_pv)(q)}
  \le \frac{A}{c+t^2}w(q,p)\norm v
  \quad\Longrightarrow\quad
  \norm{\left(\frac{2}{\pi}\int_0^\infty F(t)\,dt\right)
    \delta_pv(q)}
  \le \frac{A}{\sqrt c}w(q,p)\norm v.
\]
The exponential specialization takes $A=2$ and
$w(q,p)=e^{-\theta\dist(q,p)}$.  Version 0.6 removes the abstract-family
premise.  Lean defines
\[
 F_K(t):=(K+t^2\Id)^{-1},
\]
proves both inverse identities, and checks the resolvent identity
\[
 F_K(t)-F_K(u)=(u^2-t^2)F_K(t)F_K(u).
\]
Coercivity gives
\[
 \norm{F_K(t)}\le(c+t^2)^{-1},\qquad
 \norm{F_K(t)-F_K(u)}\le |u^2-t^2|c^{-2}.
\]
Thus $F_K$ is continuous and Bochner integrable, and the physical capstone is
a Lean theorem with no family or integrability hypothesis left to the caller:
\begin{equation}\label{eq:formal-integral-bound}
 \norm{\left(\frac{2}{\pi}\int_0^\infty F_K(t)\,dt\right)_{q,p}}_{\op}
 \le \frac{2}{\sqrt c}e^{-\theta\dist(q,p)}.
\end{equation}
Version 0.7 closes the identification.  On an orthonormal eigenbasis Lean
evaluates $F_K(t)$ and its normalized integral componentwise, proves that the
integral is positive, and establishes
\begin{equation}\label{eq:formal-square}
 \left(\frac{2}{\pi}\int_0^\infty F_K(t)\,dt\right)^2=K^{-1}.
\end{equation}
Uniqueness of the positive square root then gives the exact operator identity
\begin{equation}\label{eq:formal-identification}
 \frac{2}{\pi}\int_0^\infty F_K(t)\,dt=K^{-1/2}.
\end{equation}
Rewriting \eqref{eq:formal-integral-bound} with
\eqref{eq:formal-identification} proves the claimed kernel bound for the
canonical spectral root itself.
...
$B\circ B=C$, positivity, and symmetry.  The generic physical adapter derives
items 2--5.  A new coercive specialization now fills item 3 with the
quantitative constant $\sqrt{c^{-1}}$, rather than using $\norm B$ as its own
bound.  The canonical-family theorem, \eqref{eq:formal-integral-bound}, and the
exact identification \eqref{eq:formal-identification} now derive item 6 for
the \emph{canonical spectral root}.  The end-to-end theorem
\path{physicalLocalizedCovarianceRootCertificate_of_shiftUniform} therefore
installs items 2--6 without taking a separate root-kernel assumption.
...
smallness hypothesis by coercivity plus finite range.  Version 0.5 closed the
scalar integral, its exact constant, generic Bochner kernel transport, and the
quantitative root norm.  Version 0.6 closes construction, continuity,
integrability, and localization of the canonical shifted-inverse integral.
Version 0.7 transfers that bound to the spectral root and closes the physical
certificate adapter under the stated finite-volume hypotheses.
...
\\
\longrightarrow \boxed{B=\text{canonical shifted-inverse Stieltjes integral}}
\\
\longrightarrow \text{formal integrated single-bond kernel bound}
\\
\longrightarrow
\lean{PhysicalLocalizedCovarianceRootCertificate}.
\end{gathered}}
\end{equation}
The first arrow is machine-checked by transporting a real positive-semidefinite
matrix root through an orthonormal basis.  The boxed equality is checked by
componentwise evaluation on the symmetric operator's orthonormal eigenbasis,
positivity, the exact square identity, and uniqueness of positive roots.  The
last two arrows are checked by Bochner integration, continuous coordinate
evaluation, and the physical certificate constructor.  The next scientific
frontier is not another operator identity: it is a lower bound on $c$ uniform
in volume (or a different route that avoids needing one).
...
in version 0.3.1.  Version 0.5 also closes the scalar improper integral,
quantitative norm estimate, and generic integrated single-bond kernel
estimate.  Version 0.6 proves uniqueness of positive roots and closes
construction, continuity, integrability, and localization of the canonical
inverse family.  Version 0.7 proves its equality with the spectral root,
transports the localized bound, and supplies the end-to-end certificate
constructor.  The generic older adapters remain conditional by design, but
the new shift-uniform constructor does not assume its desired root-kernel
conclusion.
...
$\norm L<1$; the canonical real root and its localized kernel bound are now
formal under the explicit shift-uniform hypotheses. \\
\bottomrule
\end{tabular}
\end{center}
...
resolvent map.}  Correct for an arbitrary choice of right inverses.  Version
0.6 instead defines the canonical inverse $(K+t^2\Id)^{-1}$ and proves its
operator-norm continuity from the exact resolvent identity and coercivity
bound.  Its measurability and Bochner integrability are therefore theorems,
not assumptions.  Version 0.7 evaluates its integral on an orthonormal
eigenbasis and identifies it with the formal spectral root.  This attack is
closed at finite volume.
...
representation; (ii) a volume-uniform coercivity estimate cannot be obtained
in the intended gauge-fixed regime or replaced by a different viable route;
(iii) the root certificate requires a
volume-uniform $c$ from the already refuted block normalization; or (iv) a
prior formalization is found with the same physical types, hypotheses,
constants, and endpoint.  The former operator-identification kill criterion
has been discharged; volume-uniform analysis is now the real technical
bottleneck.
...
\path{PhysicalGaugeCovariancePositiveRoot.lean}.  Scalar integration and
Bochner kernel transport are in \path{StieltjesKernelIntegration.lean}.  The
canonical shifted inverse, its integrability, and its directly integrated
physical bound are in \path{CoerciveCovarianceStieltjes.lean}.
...
\path{covarianceSqrtOfIsCoerciveCLM_isPositive} & same & \textsc{exact} \\
\path{norm_covarianceSqrtOfIsCoerciveCLM_le} & same & \textsc{exact} \\
...
\path{physicalLocalizedCovariance}\newline
\path{RootCertificate_of_}\newline
\path{coercive_precision} & same & \textsc{conditional} \\
\path{integrableOn_Ioi_inv_add_sq} &
\path{StieltjesKernelIntegration.lean} & \textsc{exact} \\
\path{integral_Ioi_inv_add_sq} & same & \textsc{exact} \\
\path{physicalCovarianceKernelBound_}\newline
\path{stieltjesIntegralOperator} & same & \textsc{exact} \\
\path{physicalCovariance_exponentialKernelBound_}\newline
\path{stieltjesIntegralOperator} & same & \textsc{exact} \\
\path{eq_of_isPositive_of_comp_self_eq} &
\path{FiniteDimensionalRealPositive}\newline\path{Sqrt.lean} & \textsc{exact} \\
\path{shiftedCovarianceFamily_comp_precision} &
\path{CoerciveCovarianceStieltjes.lean} & \textsc{exact} \\
\path{precision_comp_shiftedCovarianceFamily} & same & \textsc{exact} \\
\path{norm_shiftedCovarianceFamily_le} & same & \textsc{exact} \\
\path{shiftedCovarianceFamily_sub} & same & \textsc{exact} \\
\path{continuous_shiftedCovarianceFamily} & same & \textsc{exact} \\
\path{integrableOn_shiftedCovarianceFamily} & same & \textsc{exact} \\
\path{physicalShiftedCovarianceFamily_}\newline
\path{stieltjes_exponentialKernelBound} & same & \textsc{exact} \\
Eigenvalue positivity and three eigenvector-action declarations & same &
\textsc{exact} \\
Positivity of the shifted family and its Stieltjes integral & same &
\textsc{exact} \\
\path{stieltjesIntegralOperator_shifted_}\newline
\path{comp_self} and \path{eq_covarianceSqrt} & same & \textsc{exact} \\
Physical root-kernel bound and
\path{RootCertificate_of_shiftUniform} & same & \textsc{exact} \\
Theorem~\ref{thm:root}, assembled from the named declarations above &
formal source & \textsc{formalized} \\
\bottomrule
\end{longtable}
\normalsize
...
is not circular.
The scalar-integration and quantitative-norm extension is the immutable commit
\begin{center}
\small\lean{73179102f2bc2cdec77b4b448cf116b5128fdd4f}.
\end{center}
It adds \path{StieltjesKernelIntegration.lean}, strengthens the coercive root
and physical adapter, registers all new declarations in both oracles, and adds
verification-ledger Addendum 572.  This version-0.5 manuscript cites that
already-fixed formal source.
The canonical-family extension is the immutable commit
\begin{center}
\small\lean{9f19b2d7c3dbcee1cc95683c9bc1ea29629c6b91}.
\end{center}
It adds \path{CoerciveCovarianceStieltjes.lean}, positive-root uniqueness,
the aggregate import, the expanded focused oracle, and verification-ledger
Addendum 573.  This version-0.6 manuscript cites that already-fixed source.
The exact Stieltjes-root identification is the subsequent immutable commit
\begin{center}
\small\lean{b44907f005515c7f841471a8da6e870402f0e5ec}.
\end{center}
It adds eigenbasis evaluation, positivity and the exact square of the
canonical integral, its equality with the spectral root, the transported
physical root-kernel bound, the end-to-end certificate constructor, ten oracle
registrations, and verification-ledger Addendum 574.  This version-0.7
manuscript cites that already-fixed source.
...
identifiers in the PDF.  Version 0.4 cites the separate positive-root commit;
version 0.5 cites the integration commit, version 0.6 cites the canonical
shifted-inverse commit, and version 0.7 cites the exact root-identification
commit and closes that formal boundary.
Neither alters the immutable version-0.3 source archive or its hash.  The
annotated local release tag for the present editorial tree is
\lean{shift-uniform-ct-v0.7}.  The detached \path{ARTIFACT.md} manifest records
the manuscript commit and final PDF SHA-256 after rendering.  Those two values
cannot be printed inside the hashed PDF without circularly changing the PDF;
the release tag and manifest are the non-circular cryptographic index.
...
\end{verbatim}
The four resolvent checks and all three version-0.4 root-module builds completed
successfully on August 1, 2026.  The strengthened coercive-root target, the
physical adapter, and the Stieltjes-integration target completed successfully
on August 2, 2026.  On the same date the root-identification target completed
successfully with 8204 jobs, and a fresh \path{YangMillsCore} build completed
successfully with 8470 jobs.  A targeted oracle checked thirty-six
declarations: the prior twenty-six plus the ten root-identification and
physical-certificate results.  Each printed exactly
\[
...
\end{verbatim}
The development contains no \lean{sorry} in the two resolvent modules, the
three root modules, the Stieltjes-integration module, or the canonical-family
module.
...
The Stieltjes integral then produces exponential localization of $K^{-1/2}$
with amplitude $2/\sqrt c$.  This is not advertised as a new result in matrix
analysis.  Version 0.4 constructed the positive real continuous-linear root;
version 0.5 formalized its sharp coercivity norm bound, the scalar improper
integral, and generic Bochner kernel transport.  Version 0.6 constructs the
canonical shifted-inverse family, proves it continuous and integrable, and
proves the localized bound for its normalized integral.  Version 0.7 proves
that this integral is positive, squares to $K^{-1}$, and therefore equals the
canonical spectral root; it transports the localized bound and constructs the
physical root certificate without a root-kernel premise.  The finite-volume
operator chain is closed.  The fixed-volume Poincar\'e wall is unchanged:
neither volume-uniform coercivity nor an infinite-volume or continuum limit is
proved here.
