...
\newcommand{\anchor}{08a9050201287a31d20fc4339cc5c6c64b1391fb}
\newcommand{\osline}[3]{%
...
A Machine-Checked Perron Theorem for Strictly Positive Kernels,\\
and the Coupled-Slice Vacuum at Every Spatial Extent\\
\large existence by compactness, without invoking a fixed-point theorem
in the pinned \texttt{mathlib} revision}
...
The existence proof does not use a fixed-point theorem, because the pinned
\texttt{mathlib} revision contains none.  It maximises $r$ over the compact set of pairs $(r,x)$ with $x$ in the
...
The pinned \texttt{mathlib} revision has no Brouwer fixed-point theorem, so
existence is obtained without one.  That is a statement about this revision, not
about the library in perpetuity.
...
\osline{PerronKernel}{388}{sourceWeightedKernelL}, strictly positive when $w$
is: \osline{PerronKernel}{392}{sourceWeightedKernelL\_pos}.
...
\osline{PerronKernel}{402}{vacuum\_exists\_of\_sourceWeight},
\osline{PerronKernel}{422}{vacuum\_unique\_of\_sourceWeight},
\osline{PerronKernel}{433}{vacuum\_spectral\_radius\_of\_sourceWeight}.
For the concrete slice with a bond around the ring,
\osline{PerronKernel}{445}{spatialWeightRing} and
\osline{PerronKernel}{454}{coupled\_ring\_vacuum\_exists}.

\begin{theorem}[The normalised vacuum: $T\Om=\Om$ again]\label{thm:norm}
Let $T=\lambda^{-1}K_w$.  Then at every $L$ there is $\Om$ with $\Om>0$,
$\sum_\sigma\Om(\sigma)=1$, and $T\Om=\Om$.
\end{theorem}

\osline{PerronKernel}{355}{normalizedKernel},
\osline{PerronKernel}{362}{normalizedKernel\_fixes\_eigenvector},
\osline{PerronKernel}{413}{normalized\_vacuum\_of\_sourceWeight}, and for
the ring slice
\osline{PerronKernel}{462}{coupled\_ring\_normalized\_vacuum}.

\begin{remark}[The equation that failed, holding again]
This is worth stating separately because of \emph{which} equation it is.  The
companion chain obtained the vacuum from $T\Om=\Om$ for the \emph{uniform}
$\Om$, and \cite{ErikssonSpatial} proved exactly that statement false as soon as
a spatial coupling is present.  Theorem~\ref{thm:norm} restores the same
equation --- for the Perron $\Om$ instead of the uniform one, at every extent.
The normalisation $\sum\Om=1$ is free: the existence proof already produces its
vector inside the simplex.
\end{remark}
...
Oracle commands & 2564 \\
\quad with axiom dependencies & 2541 \\
\quad axiom-free & 23 \\
