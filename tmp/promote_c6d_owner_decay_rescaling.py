#!/usr/bin/env python3
"""Fail-closed promotion of the C6d owner-distance rescaling specialization.
...
def blob(source_sha: str, relative: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"C6D_OWNER_RESCALING_BLOB_FAILED={relative}:"
...
def promote_text(data: bytes) -> bytes:
    ...
    if header in text:
        ...
        raise RuntimeError("C6D_OWNER_RESCALING_SCRATCH_MARKER_MISSING")
    text = re.sub(
    ...
    if "import tmp." in text:
        raise RuntimeError("C6D_OWNER_RESCALING_TMP_IMPORT_REMAINS")
    if text.count("PRE-VALIDATION:") != 1:
        raise RuntimeError("C6D_OWNER_RESCALING_PRE_COUNT")
    if re.search(r"(?m)^\s*axiom\b|\b(?:sorry|admit|by\?|exact\?)\b", text):
        raise RuntimeError("C6D_OWNER_RESCALING_PLACEHOLDER")
...
def require_clean_blob(source_sha: str, relative: str) -> bytes:
    ...
    if status.returncode != 0 or status.stdout:
        raise RuntimeError(f"C6D_OWNER_RESCALING_DIRTY={relative}")
    data = blob(source_sha, relative)
    if (ROOT / relative).read_bytes().replace(b"\r\n", b"\n") != data:
        raise RuntimeError(f"C6D_OWNER_RESCALING_DIVERGED={relative}")
    return data
...
def main() -> int:
    ...
    if re.fullmatch(r"[0-9a-f]{40}", args.source_sha) is None:
        raise RuntimeError("C6D_OWNER_RESCALING_SOURCE_SHA_INVALID")
    ...
    for relative in SOURCES:
        ...
        if git("cat-file", "-e", f"{args.source_sha}:{target}").returncode == 0:
            raise RuntimeError(f"C6D_OWNER_RESCALING_TARGET_EXISTS={target}")
        if (ROOT / target).exists() or git(
            "status", "--porcelain=v1", "--", target
        ).stdout:
            raise RuntimeError(f"C6D_OWNER_RESCALING_TARGET_DIRTY={target}")
    ...
    if core_status.returncode != 0 or core_status.stdout:
        raise RuntimeError("C6D_OWNER_RESCALING_CORE_DIRTY")
    ...
    if audit_import in core:
        raise RuntimeError("C6D_OWNER_RESCALING_CORE_IMPORT_EXISTS")
    ...
    if not args.apply:
        print(
            "C6D_OWNER_RESCALING_PROMOTION_PREVIEW_OK "
    ...
    try:
        ...
        if actual != manifest:
            raise RuntimeError(
                f"C6D_OWNER_RESCALING_POSTWRITE={actual} EXPECTED={manifest}"
            )
    except Exception:
    ...
    print(
        "C6D_OWNER_RESCALING_PROMOTION_APPLY_OK "
        f"files={len(rows)} promoted={len(SOURCES)} "
        f"source_sha={args.source_sha} manifest_sha256={manifest}"
    )
    return 0
