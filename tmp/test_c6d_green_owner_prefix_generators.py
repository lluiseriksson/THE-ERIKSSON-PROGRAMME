...
def main() -> int:
    ...
    if len(manifest_paths) != 32 or len(set(manifest_paths)) != 32:
    ...
    if len(promotion.AUDIT_IMPORTS) != 16:
    ...
    with tempfile.TemporaryDirectory(prefix="c6d-green-owner-prefix-") as folder:
        ...
        if len(queue) != 33 or sum(row[2] is not None for row in queue) != 16:
        ...
        print(
            ...
            f"queue={len(queue)} axiom_blocks=18 checked_imports={checked_imports} "
