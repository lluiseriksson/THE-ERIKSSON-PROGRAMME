#!/usr/bin/env python3
"""Verify and preserve a durable Eq. (3.37) coordinate-boundary archive."""
...
def main() -> int:
    ...
    if not archive.is_file():
        raise RuntimeError(f"EQ337_DERIVATIVE_ARCHIVE_MISSING={archive}")
    if destination.exists():
        raise RuntimeError(f"EQ337_DERIVATIVE_DESTINATION_EXISTS={destination}")
    destination.parent.mkdir(parents=True, exist_ok=True)
    staging = Path(tempfile.mkdtemp(prefix=".eq337-derivative-", dir=destination.parent))
    try:
        copied = staging / "hrpoly-eq337-covariant-derivative-evidence.tar.gz"
        verifier_json = staging / "eq337-covariant-derivative-axioms.json"
        ...
        if child.returncode != 0:
            raise RuntimeError(
                "EQ337_DERIVATIVE_VERIFIER_FAILED "
        ...
        if result.get("status") != "EQ337_COVARIANT_DERIVATIVE_EVIDENCE_OK":
            raise RuntimeError("EQ337_DERIVATIVE_VERIFIER_STATUS")
        manifest = {
            "status": "EQ337_COVARIANT_DERIVATIVE_ARCHIVE_PACKAGE_OK",
        ...
        raise
    print(
        "EQ337_COVARIANT_DERIVATIVE_ARCHIVE_PACKAGE_OK "
