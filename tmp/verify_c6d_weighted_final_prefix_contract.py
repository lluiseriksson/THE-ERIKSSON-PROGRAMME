#!/usr/bin/env python3
"""Static contract for the weighted/final C6d cold evidence."""
...
def git_blob(repo: Path, source_sha: str, path: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"C6D_WEIGHTED_FINAL_GIT_BLOB_FAILED={path}:"
...
def stages() -> tuple[str, ...]:
    return (
        "c6d_weighted_final_prefix_materialize_dependencies",
        "c6d_weighted_final_prefix_prepare_build_dirs",
        *(
            stage
            for index, (module, _) in enumerate(MODULES, start=1)
            for stage in (
                f"c6d_weighted_final_prefix_{index:02d}_{module.lower()}_source",
                f"c6d_weighted_final_prefix_{index:02d}_{module.lower()}_audit",
            )
        ),
        "c6d_weighted_final_prefix_root",
    )
