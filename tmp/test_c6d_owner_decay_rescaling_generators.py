#!/usr/bin/env python3
"""Lightweight synthetic-commit test for C6d owner-distance rescaling."""
...
def load(name: str, path: Path):
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"C6D_OWNER_RESCALING_TEST_IMPORT_FAILED={path}")
...
def git(*args: str, input_bytes: bytes | None = None, env=None) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            "C6D_OWNER_RESCALING_TEST_GIT_FAILED=" + " ".join(args) + "\n"
...
def main() -> int:
    promotion = load(
        "c6d_owner_rescaling_promotion_test",
        ROOT / "tmp" / "promote_c6d_owner_decay_rescaling.py",
    )
    runner_gen = load(
        "c6d_owner_rescaling_runner_test",
        ROOT / "tmp" / "generate_c6d_owner_decay_rescaling_runner.py",
    )
    notebook_gen = load(
        "c6d_owner_rescaling_notebook_test",
        ROOT / "tmp" / "generate_c6d_owner_decay_rescaling_notebook.py",
    )
    verifier_gen = load(
        "c6d_owner_rescaling_verifier_test",
        ROOT / "tmp" / "generate_c6d_owner_decay_rescaling_verifier.py",
    )
    head = git("rev-parse", "HEAD").decode().strip()

    with tempfile.TemporaryDirectory(prefix="c6d-owner-rescaling-") as folder:
        ...
        for relative in promotion.PREREQUISITES:
            ...
            if b"PRE-VALIDATION:" not in data:
                raise RuntimeError(
                    f"C6D_OWNER_RESCALING_TEST_PREREQ_NOT_PRE={relative}"
                )
        ...
        audit_import = (
            "import YangMills.RG."
            "BalabanCMP99Eq360C6dSourceSeparatedOwnerDecayRescalingAudit"
        )
        if not core.endswith("\n"):
            core += "\n"
        rows.append(("YangMillsCore.lean", (core + audit_import + "\n").encode()))
        ...
        source_commit = git(
            ...
            input_bytes=b"synthetic C6d owner-rescaling source\n",
        ).decode().strip()

        runner_text = runner_gen.generate(source_commit)
        compile(runner_text, "synthetic-c6d-owner-rescaling-runner.py", "exec")
        ...
        if blobs_match is None or len(ast.literal_eval(blobs_match.group(1))) != 3:
            raise RuntimeError("C6D_OWNER_RESCALING_TEST_SOURCE_BLOBS")
        if queue_match is None:
            raise RuntimeError("C6D_OWNER_RESCALING_TEST_QUEUE_MISSING")
        queue = ast.literal_eval(queue_match.group(1))
        if len(queue) != 3 or sum(row[2] is not None for row in queue) != 1:
            raise RuntimeError("C6D_OWNER_RESCALING_TEST_QUEUE_SCOPE")
        ...
        runner_commit = git(
            ...
            input_bytes=b"synthetic C6d owner-rescaling runner\n",
        ).decode().strip()
        ...
        if len(cells) != 1:
            raise RuntimeError("C6D_OWNER_RESCALING_TEST_NOTEBOOK_CELL_COUNT")
        source = cells[0]["source"]
        compile(
            "".join(source) if isinstance(source, list) else source,
            "synthetic-c6d-owner-rescaling.ipynb",
            "exec",
        )
        ...
        compile(verifier_text, "synthetic-c6d-owner-rescaling-verifier.py", "exec")
        if "C6D_OWNER_RESCALING_EVIDENCE_OK" not in verifier_text:
            raise RuntimeError("C6D_OWNER_RESCALING_TEST_VERIFIER_SENTINEL")
        print(
            "C6D_OWNER_RESCALING_GENERATORS_OK "
