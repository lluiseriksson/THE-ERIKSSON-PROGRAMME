#!/usr/bin/env python3
"""Fail-closed promotion of the C6d source coercivity/Green chain.

Promotion preserves every PRE-VALIDATION notice and is not certification.
The promoted sources, the existing PRE-VALIDATION reachability pair and the
root must pass one fresh cold source/audit queue before any notice is removed.
"""
...
PATH_FILE = ROOT / "tmp" / "C6D-SOURCE-COERCIVITY-GREEN-PROMOTION-PATHS.txt"
CORE = "YangMillsCore.lean"
EXPECTED_PATHS = 18
SAME_GATE_PREREQUISITES = (
    "YangMills/RG/BalabanCMP99SourcePoincarePositiveRadiusReachability.lean",
    "YangMills/RG/BalabanCMP99SourcePoincarePositiveRadiusReachabilityAudit.lean",
)
SEALED_PREREQUISITES = (
    "YangMills/RG/BalabanCMP99SourceLocalizedRetainedTower.lean",
    "YangMills/RG/BalabanCMP99SourceLocalizedRetainedTowerAudit.lean",
...
def require_commit(source_sha: str) -> None:
    if re.fullmatch(r"[0-9a-f]{40}", source_sha) is None:
        raise RuntimeError("C6D_COERCIVITY_GREEN_SOURCE_SHA_INVALID")
    child = git("rev-parse", f"{source_sha}^{{commit}}")
    if child.returncode != 0 or child.stdout.decode().strip() != source_sha:
        raise RuntimeError("C6D_COERCIVITY_GREEN_SOURCE_COMMIT_MISMATCH")
...
def git_blob(source_sha: str, relative: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"C6D_COERCIVITY_GREEN_BLOB_FAILED={relative}:"
...
def selected_paths() -> list[str]:
    rows = [
        row.strip() for row in PATH_FILE.read_text(encoding="utf-8").splitlines()
        if row.strip()
    ]
    if len(rows) != EXPECTED_PATHS or len(set(rows)) != EXPECTED_PATHS:
        raise RuntimeError(
            f"C6D_COERCIVITY_GREEN_SCOPE={len(rows)}/{len(set(rows))}"
        )
    for index, row in enumerate(rows):
        if not row.startswith("tmp/") or not row.endswith(".draft.lean"):
            raise RuntimeError(f"C6D_COERCIVITY_GREEN_PATH_INVALID={row}")
        is_audit = Path(row).name.endswith("Audit.draft.lean")
        if is_audit != (index % 2 == 1):
            raise RuntimeError(f"C6D_COERCIVITY_GREEN_PAIR_ORDER={row}")
    return rows
...
def require_clean_blob(source_sha: str, relative: str) -> bytes:
    ...
    if status.returncode != 0 or status.stdout:
        raise RuntimeError(f"C6D_COERCIVITY_GREEN_DIRTY={relative}")
    data = git_blob(source_sha, relative)
    if (ROOT / relative).read_bytes().replace(b"\r\n", b"\n") != data:
        raise RuntimeError(f"C6D_COERCIVITY_GREEN_DIVERGED={relative}")
    return data
...
def require_absent(source_sha: str, relative: str) -> None:
    ...
    if status.returncode != 0 or status.stdout or probe.returncode == 0:
        raise RuntimeError(f"C6D_COERCIVITY_GREEN_DESTINATION_EXISTS={relative}")
    if (ROOT / relative).exists():
        raise RuntimeError(f"C6D_COERCIVITY_GREEN_DESTINATION_EXISTS={relative}")


def require_prerequisites(source_sha: str) -> None:
    for relative in SAME_GATE_PREREQUISITES:
        data = git_blob(source_sha, relative)
        if data.count(b"PRE-VALIDATION:") != 1:
            raise RuntimeError(
                f"C6D_COERCIVITY_GREEN_SAME_GATE_NOTICE={relative}"
            )
    for relative in SEALED_PREREQUISITES:
        if b"PRE-VALIDATION:" in git_blob(source_sha, relative):
            raise RuntimeError(
                f"C6D_COERCIVITY_GREEN_PREREQUISITE_UNSEALED={relative}"
            )
...
def retarget(data: bytes) -> bytes:
    text = re.sub(
        ...
        data.decode("utf-8"), flags=re.MULTILINE,
    )
    if "import tmp." in text:
        raise RuntimeError("C6D_COERCIVITY_GREEN_TMP_IMPORT_REMAINS")
    if text.count("PRE-VALIDATION:") != 1:
        raise RuntimeError("C6D_COERCIVITY_GREEN_PREVALIDATION_COUNT")
    if re.search(r"(?m)^\s*axiom\b|\b(?:sorry|admit|by\?|exact\?)\b", text):
        raise RuntimeError("C6D_COERCIVITY_GREEN_FORBIDDEN_PLACEHOLDER")
    return text.encode("utf-8")


def core_with_audits(data: bytes, selected: list[str]) -> bytes:
    imports = [
        "import YangMills.RG." + Path(row).name.removesuffix(".draft.lean")
        for row in selected if Path(row).name.endswith("Audit.draft.lean")
    ]
    imports.append(
...
def main() -> int:
    ...
    if core_status.returncode != 0 or core_status.stdout:
        raise RuntimeError("C6D_COERCIVITY_GREEN_CORE_DIRTY")
    ...
    if not args.apply:
        print(
            f"C6D_COERCIVITY_GREEN_PROMOTION_{mode}_OK files={len(rows)} "
    ...
    try:
        ...
        if actual != digest:
            raise RuntimeError(
                f"C6D_COERCIVITY_GREEN_POSTWRITE={actual} WANT={digest}"
            )
    except Exception:
