#!/usr/bin/env python3
"""Lightweight synthetic-commit test for the exact Green-decay gate."""
...
def load(name: str, path: Path):
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"C6D_EXACT_GREEN_DECAY_TEST_IMPORT_FAILED={path}")
...
def git(*args: str, input_bytes: bytes | None = None, env=None) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            "C6D_EXACT_GREEN_DECAY_TEST_GIT_FAILED=" + " ".join(args) + "\n"
...
def main() -> int:
    promotion = load(
        "c6d_exact_green_decay_promotion_test",
        ROOT / "tmp" / "promote_c6d_exact_green_decay.py",
    )
    runner_gen = load(
        "c6d_exact_green_decay_runner_test",
        ROOT / "tmp" / "generate_c6d_exact_green_decay_runner.py",
    )
    notebook_gen = load(
        "c6d_exact_green_decay_notebook_test",
        ROOT / "tmp" / "generate_c6d_exact_green_decay_notebook.py",
    )
    verifier_gen = load(
        "c6d_exact_green_decay_verifier_test",
        ROOT / "tmp" / "generate_c6d_exact_green_decay_verifier.py",
    )
    head = git("rev-parse", "HEAD").decode().strip()

    with tempfile.TemporaryDirectory(prefix="c6d-exact-green-decay-") as folder:
        ...
        source_commit = git(
            ...
            input_bytes=b"synthetic exact Green-decay source\n",
        ).decode().strip()

        runner_text = runner_gen.generate(source_commit)
        compile(runner_text, "synthetic-exact-green-decay-runner.py", "exec")
        ...
        if blobs_match is None or len(ast.literal_eval(blobs_match.group(1))) != 5:
            raise RuntimeError("C6D_EXACT_GREEN_DECAY_TEST_SOURCE_BLOBS")
        if queue_match is None:
            raise RuntimeError("C6D_EXACT_GREEN_DECAY_TEST_QUEUE_MISSING")
        queue = ast.literal_eval(queue_match.group(1))
        if len(queue) != 5 or sum(row[2] is not None for row in queue) != 2:
            raise RuntimeError("C6D_EXACT_GREEN_DECAY_TEST_QUEUE_SCOPE")
        ...
        runner_commit = git(
            ...
            input_bytes=b"synthetic exact Green-decay runner\n",
        ).decode().strip()

        notebook_text = notebook_gen.load_base().generate(
            source_commit,
            runner_commit,
            "c6d-exact-green-decay-v1",
        ...
        if len(cells) != 1:
            raise RuntimeError("C6D_EXACT_GREEN_DECAY_TEST_NOTEBOOK_CELL_COUNT")
        source = cells[0]["source"]
        compile(
            "".join(source) if isinstance(source, list) else source,
            "synthetic-exact-green-decay.ipynb",
            "exec",
        )
        ...
        compile(verifier_text, "synthetic-exact-green-decay-verifier.py", "exec")
        if "C6D_EXACT_GREEN_DECAY_EVIDENCE_OK" not in verifier_text:
            raise RuntimeError("C6D_EXACT_GREEN_DECAY_TEST_VERIFIER_SENTINEL")
        print(
            "C6D_EXACT_GREEN_DECAY_GENERATORS_OK "
