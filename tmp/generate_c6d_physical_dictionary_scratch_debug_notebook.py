#!/usr/bin/env python3
"""Generate the retained one-cell Colab launcher for physical dictionary debug."""
...
RUNNER_PATH = "scripts/colab_c6d_physical_dictionary_scratch_debug.py"
OUTPUT = ROOT / "scripts" / "colab_c6d_physical_dictionary_scratch_debug.ipynb"


def load_base():
    spec = importlib.util.spec_from_file_location("c6d_physical_dictionary_debug_notebook_base", BASE)
    if spec is None or spec.loader is None:
        raise RuntimeError("C6D_PHYSICAL_DICTIONARY_DEBUG_NOTEBOOK_BASE_IMPORT_FAILED")
...
def main() -> int:
    ...
    if len(cells) != 1:
        raise RuntimeError(f"C6D_PHYSICAL_DICTIONARY_DEBUG_NOTEBOOK_CELLS={len(cells)}")
    source = cells[0]["source"]
    compile("".join(source) if isinstance(source, list) else source, str(args.output), "exec")
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(content, encoding="utf-8", newline="\n")
    print(
        "C6D_PHYSICAL_DICTIONARY_DEBUG_NOTEBOOK_GENERATED "
