#!/usr/bin/env python3
"""Preserve one verified complex-recursion prerequisite notebook."""

from __future__ import annotations

import argparse
import hashlib
import json
from pathlib import Path
import re
import shutil
import subprocess
import sys
import tempfile


ROOT = Path(__file__).resolve().parents[1]
VERIFIER = (
    ROOT / "tmp" /
    "verify_eq337_complex_forced_recursion_prereq_evidence.py"
)
HASH_MARKERS = ("EVIDENCE_SHA256", "EVIDENCE_ARCHIVE_SHA256")


def sha256(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as stream:
        for chunk in iter(lambda: stream.read(1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest().upper()


def transcript(path: Path) -> str:
    notebook = json.loads(path.read_text(encoding="utf-8"))
    chunks: list[str] = []
    for cell in notebook.get("cells", []):
        for output in cell.get("outputs", []):
            text = output.get("text", "")
            if isinstance(text, list):
                chunks.extend(str(part) for part in text)
            elif isinstance(text, str):
                chunks.append(text)
    return "".join(chunks)


def exact_marker(text: str, name: str) -> str:
    values = re.findall(
        rf"(?m)^{re.escape(name)}=([A-Fa-f0-9]+)\s*$", text
    )
    if len(values) != 1:
        raise RuntimeError(f"MARKER_COUNT={name}:{len(values)} WANT=1")
    return values[0].upper()


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--notebook", type=Path, required=True)
    parser.add_argument("--destination", type=Path, required=True)
    parser.add_argument("--source-sha", required=True)
    parser.add_argument("--runner-rev", required=True)
    args = parser.parse_args()

    notebook = args.notebook.resolve()
    destination = args.destination.resolve()
    if not notebook.is_file():
        raise RuntimeError(f"NOTEBOOK_MISSING={notebook}")
    if destination.exists():
        raise RuntimeError(f"DESTINATION_EXISTS={destination}")
    text = transcript(notebook)
    runner_hashes = {name: exact_marker(text, name) for name in HASH_MARKERS}

    destination.parent.mkdir(parents=True, exist_ok=True)
    staging = Path(
        tempfile.mkdtemp(
            prefix=".eq337-complex-recursion-prereq-evidence-",
            dir=destination.parent,
        )
    )
    try:
        copied = staging / "executed-complex-recursion-prereq.ipynb"
        verifier_json = staging / "complex-recursion-prereq-axioms.json"
        shutil.copyfile(notebook, copied)
        child = subprocess.run(
            [
                sys.executable,
                str(VERIFIER),
                "--repo", str(ROOT),
                "--notebook", str(copied),
                "--source-sha", args.source_sha,
                "--runner-rev", args.runner_rev,
                "--json-out", str(verifier_json),
            ],
            cwd=ROOT,
            check=False,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
        )
        if child.returncode != 0:
            raise RuntimeError(
                "VERIFIER_FAILED "
                f"exit={child.returncode} stdout={child.stdout!r} "
                f"stderr={child.stderr!r}"
            )
        result = json.loads(verifier_json.read_text(encoding="utf-8"))
        if result.get("status") != (
            "EQ337_COMPLEX_RECURSION_PREREQ_EVIDENCE_OK"
        ):
            raise RuntimeError("VERIFIER_STATUS_MISMATCH")
        manifest = {
            "status": "EQ337_COMPLEX_RECURSION_PREREQ_PACKAGE_OK",
            "source_sha": result["source_sha"],
            "runner_revision": result["runner_revision"],
            "runner_hashes": runner_hashes,
            "files": {
                copied.name: sha256(copied),
                verifier_json.name: sha256(verifier_json),
            },
        }
        manifest_path = staging / "manifest.json"
        manifest_path.write_text(
            json.dumps(manifest, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
            newline="\n",
        )
        sums = staging / "SHA256SUMS"
        sums.write_text(
            "".join(
                f"{sha256(path)}  {path.name}\n"
                for path in sorted(staging.iterdir(), key=lambda item: item.name)
                if path.name != "SHA256SUMS"
            ),
            encoding="utf-8",
            newline="\n",
        )
        staging.replace(destination)
    except Exception:
        shutil.rmtree(staging, ignore_errors=True)
        raise

    print(
        "EQ337_COMPLEX_RECURSION_PREREQ_PACKAGE_OK "
        f"destination={destination} "
        f"manifest_sha256={sha256(destination / 'manifest.json')} "
        f"notebook_sha256={sha256(destination / copied.name)}"
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
