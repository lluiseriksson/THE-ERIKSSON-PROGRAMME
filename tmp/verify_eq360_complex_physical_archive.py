#!/usr/bin/env python3
"""Verify durable cold evidence for the source-specific Eq. (3.60) gate."""
...
CONTRACT_PATH = ROOT / "tmp" / "verify_eq360_complex_physical_contract.py"
...
def load(path: Path, name: str):
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_MODULE_LOAD_FAILED={path}")
...
def main() -> int:
    ...
    if re.fullmatch(r"[0-9a-f]{40}", args.source_sha) is None:
        raise RuntimeError("EQ360_COMPLEX_PHYSICAL_SOURCE_SHA_INVALID")
    contract = load(CONTRACT_PATH, "eq360_complex_physical_contract")
    util = load(ARCHIVE_UTIL_PATH, "eq360_complex_physical_archive_util")
    archive_path = args.archive.resolve()
    if not archive_path.is_file():
        raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_ARCHIVE_MISSING={archive_path}")

    with tarfile.open(archive_path, "r:gz") as archive:
        root, files = util.safe_members(archive)
        evidence_name = f"{root}/evidence.json"
        if evidence_name not in files:
            raise RuntimeError("EQ360_COMPLEX_PHYSICAL_EVIDENCE_MISSING")
        ...
        for key, wanted in required.items():
            if evidence.get(key) != wanted:
                raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_{key.upper()}_MISMATCH")
        ...
        if not isinstance(source_blobs, dict) or set(source_blobs) != source_paths:
            raise RuntimeError("EQ360_COMPLEX_PHYSICAL_SOURCE_BLOB_SCOPE")
        ...
        for relative, wanted in source_blobs.items():
            ...
            if measured != wanted:
                raise RuntimeError(
                    f"EQ360_COMPLEX_PHYSICAL_SOURCE_BLOB_MISMATCH={relative}"
                )
            boundary_hashes[relative] = measured

        records = evidence.get("records")
        if not isinstance(records, list) or not records:
            raise RuntimeError("EQ360_COMPLEX_PHYSICAL_RECORDS_MISSING")
        by_stage: dict[str, dict] = {}
        outputs: dict[str, str] = {}
        for record in records:
            if not isinstance(record, dict) or not isinstance(record.get("stage"), str):
                raise RuntimeError("EQ360_COMPLEX_PHYSICAL_RECORD_INVALID")
            stage = record["stage"]
            if stage in by_stage:
                raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_DUPLICATE_STAGE={stage}")
            if record.get("exit") != 0:
                raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_STAGE_EXIT={stage}")
            member = f"{root}/{stage}.stdout"
            if member not in files:
                raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_STDOUT_MISSING={stage}")
            output = util.read_member(archive, files[member])
            if util.sha256_bytes(output) != record.get("output_sha256"):
                raise RuntimeError(
                    f"EQ360_COMPLEX_PHYSICAL_STDOUT_HASH_MISMATCH={stage}"
                )
            outputs[stage] = output.decode("utf-8")
            by_stage[stage] = record
        if set(files) != {evidence_name} | {f"{root}/{s}.stdout" for s in by_stage}:
            raise RuntimeError("EQ360_COMPLEX_PHYSICAL_MEMBER_SCOPE")
        for stage in contract.stages():
            if stage not in by_stage:
                raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_REQUIRED_STAGE={stage}")
        ...
        for index, (module, expected_count) in enumerate(contract.MODULES, start=1):
            ...
            if len(declarations) != expected_count:
                raise RuntimeError(
                    f"EQ360_COMPLEX_PHYSICAL_AUDIT_COUNT={module}:{len(declarations)}"
                )
        ...
        for forbidden in contract.FORBIDDEN:
            if forbidden in compact:
                raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_FORBIDDEN_AXIOM={forbidden}")
        ...
        if missing:
            raise RuntimeError("EQ360_COMPLEX_PHYSICAL_MISSING=" + json.dumps(missing))
        if duplicate:
            raise RuntimeError(
                "EQ360_COMPLEX_PHYSICAL_DUPLICATE="
                + json.dumps(duplicate, sort_keys=True)
            )
        if invalid:
            raise RuntimeError(
                "EQ360_COMPLEX_PHYSICAL_NONSTANDARD="
