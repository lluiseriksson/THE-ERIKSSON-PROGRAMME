#!/usr/bin/env python3
"""Warm diagnostic through the positive-bond Eq. (3.51) adjoint expansion.

Run only after the exact Eq359 cold gate has emitted PASS and retained its
runtime.  The script reuses that build state, checks out the published
PRE-VALIDATION checkpoint below, materializes the regional stencil prefix,
compiles the already staged Eq351 factorization, and finally compiles the new
one-bond adjoint-expansion source/audit pair.  It is diagnostic evidence only:
it does not remove PRE-VALIDATION, edit the ledger, commit, push, or release
the runtime.
"""
...
def materialize(
    rows: tuple[tuple[str, str, dict[str, str]], ...],
) -> None:
    for source_name, target_name, replacements in rows:
        ...
        if "import tmp." in text:
            raise RuntimeError("WARM_TMP_IMPORT_REMAINS=" + source_name)
...
def main() -> int:
    ...
    print("WARM_POSITIVE_ADJOINT_FINAL_STATUS=PASS", flush=True)
    return 0
