#!/usr/bin/env python3
"""Validate and preserve one executed closed physical Eq. (3.37) notebook."""
...
VERIFIER = ROOT / "tmp" / "verify_eq337_closed_physical_recursion_evidence.py"
...
def exact_marker(text: str, name: str) -> str:
    ...
    if len(values) != 1:
        raise RuntimeError(
            f"EQ337_CLOSED_PHYSICAL_MARKER_COUNT={name}:{len(values)} WANT=1"
        )
...
def main() -> int:
    ...
    if not notebook.is_file():
        raise RuntimeError(f"EQ337_CLOSED_PHYSICAL_NOTEBOOK_MISSING={notebook}")
    if destination.exists():
        raise RuntimeError(f"EQ337_CLOSED_PHYSICAL_DESTINATION_EXISTS={destination}")
    runner_hashes = {
        name: exact_marker(transcript(notebook), name) for name in HASH_MARKERS
    }

    destination.parent.mkdir(parents=True, exist_ok=True)
    staging = Path(
        tempfile.mkdtemp(prefix=".eq337-closed-physical-", dir=destination.parent)
    )
    try:
        copied = staging / "executed-eq337-closed-physical-recursion.ipynb"
        ...
        if child.returncode != 0:
            raise RuntimeError(
                "EQ337_CLOSED_PHYSICAL_VERIFIER_FAILED "
        ...
        if result.get("status") != "EQ337_CLOSED_PHYSICAL_RECURSION_EVIDENCE_OK":
            raise RuntimeError("EQ337_CLOSED_PHYSICAL_VERIFIER_STATUS_MISMATCH")
        manifest = {
            "status": "EQ337_CLOSED_PHYSICAL_RECURSION_PACKAGE_OK",
    ...
    print(
        "EQ337_CLOSED_PHYSICAL_RECURSION_PACKAGE_OK "
        f"destination={destination} "
        f"manifest_sha256={sha256(destination / 'manifest.json')} "
        f"notebook_sha256={sha256(destination / copied.name)}"
    )
    return 0
