#!/usr/bin/env python3
"""Fail-closed promotion of the finite source-specific Eq. (3.60) gate."""
...
def paths() -> list[str]:
    ...
    if len(rows) != EXPECTED_PATHS or len(set(rows)) != EXPECTED_PATHS:
        raise RuntimeError(
            f"EQ360_SCOPE={len(rows)}/{len(set(rows))} "
    ...
    if any(
        ...
        raise RuntimeError("EQ360_SCOPE_PATH_INVALID")
    return rows
...
def require_prerequisites(source_sha: str) -> None:
    for relative in REQUIRED_PREREQUISITES:
        ...
        if b"PRE-VALIDATION:" in data:
            raise RuntimeError(f"EQ360_PREREQUISITE_UNSEALED={relative}")
...
def main() -> int:
    ...
    try:
        ...
        if actual != digest:
            raise RuntimeError(f"EQ360_POSTWRITE_MISMATCH={actual} WANT={digest}")
    except Exception:
