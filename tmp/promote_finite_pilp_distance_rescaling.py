#!/usr/bin/env python3
"""Fail-closed promotion of the exact kernel-distance rescaling lemma.

The script reads the two tracked scratch blobs from an exact source commit,
adds the standard PRE-VALIDATION marker, rewrites the audit import and appends
only the audit import to ``YangMillsCore.lean``.  It never runs Lean and never
...
SOURCES = (
    "tmp/FinitePiLpTypedKernelDistanceRescaling.draft.lean",
    "tmp/FinitePiLpTypedKernelDistanceRescalingAudit.draft.lean",
)
...
def blob(source_sha: str, relative: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"DISTANCE_RESCALING_BLOB_FAILED={relative}:"
...
def promote_text(data: bytes) -> bytes:
    ...
    if header in text:
        text = text.replace(header, standard, 1)
    elif audit in text:
        text = text.replace(audit, "-- " + standard, 1)
    else:
        raise RuntimeError("DISTANCE_RESCALING_SCRATCH_MARKER_MISSING")
    text = re.sub(
    ...
    if "import tmp." in text:
        raise RuntimeError("DISTANCE_RESCALING_TMP_IMPORT_REMAINS")
    if text.count("PRE-VALIDATION:") != 1:
        raise RuntimeError("DISTANCE_RESCALING_PRE_COUNT")
    if re.search(r"(?m)^\s*axiom\b|\b(?:sorry|admit|by\?|exact\?)\b", text):
        raise RuntimeError("DISTANCE_RESCALING_PLACEHOLDER")
...
def require_clean_blob(source_sha: str, relative: str) -> bytes:
    ...
    if status.returncode != 0 or status.stdout:
        raise RuntimeError(f"DISTANCE_RESCALING_DIRTY={relative}")
    data = blob(source_sha, relative)
    if (ROOT / relative).read_bytes().replace(b"\r\n", b"\n") != data:
        raise RuntimeError(f"DISTANCE_RESCALING_DIVERGED={relative}")
    return data
...
def main() -> int:
    ...
    if core_status.returncode != 0 or core_status.stdout:
        raise RuntimeError("DISTANCE_RESCALING_CORE_DIRTY")
    ...
    if audit_import in core:
        raise RuntimeError("DISTANCE_RESCALING_CORE_IMPORT_EXISTS")
    ...
    if not args.apply:
        print(
            "DISTANCE_RESCALING_PROMOTION_PREVIEW_OK "
    ...
    try:
        ...
        if actual != manifest:
            raise RuntimeError(
                f"DISTANCE_RESCALING_POSTWRITE={actual} EXPECTED={manifest}"
            )
    except Exception:
    ...
    print(
        "DISTANCE_RESCALING_PROMOTION_APPLY_OK "
        f"files={len(rows)} promoted={len(SOURCES)} "
        f"source_sha={args.source_sha} manifest_sha256={manifest}"
    )
    return 0
