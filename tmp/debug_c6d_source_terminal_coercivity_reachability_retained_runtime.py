#!/usr/bin/env python3
"""Warm retained-runtime debug for the constructed C6d coercivity interval.

Diagnostic only. Exact Git blobs from one source SHA are overlaid on a retained
checkout without moving HEAD. The prerequisite source-terminal coercivity and
positive-radius Poincare debug queues must already be green in the same runtime.
A later cold source/audit/root gate remains mandatory before PRE-VALIDATION can
be removed.
"""
...
STAGES = (
    (
        "tmp/BalabanCMP99Eq360C6dSourceTerminalCoercivityReachability.draft.lean",
        None,
    ),
    (
        "tmp/BalabanCMP99Eq360C6dSourceTerminalCoercivityReachabilityAudit.draft.lean",
        1,
    ),
)
...
def git(repo: Path, *args: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            "C6D_REACHABILITY_DEBUG_GIT_FAIL=" + " ".join(args) + ":"
...
def axiom_gate(text: str, expected: int, stage: str) -> None:
    ...
    for forbidden in FORBIDDEN:
        if forbidden in compact:
            raise RuntimeError(
                f"C6D_REACHABILITY_DEBUG_FORBIDDEN={stage}:{forbidden}"
            )
    ...
    if actual != expected:
        raise RuntimeError(
            f"C6D_REACHABILITY_DEBUG_AXIOM_COUNT={stage}:{actual} WANT={expected}"
        )
    for block in blocks:
        names = {
        ...
        if unexpected:
            raise RuntimeError(
                f"C6D_REACHABILITY_DEBUG_AXIOM_UNEXPECTED={stage}:"
...
def main() -> int:
    ...
    if resolved != args.source_sha:
        raise RuntimeError("C6D_REACHABILITY_DEBUG_SOURCE_SHA_MISMATCH")
    ...
    for relative, _ in STAGES:
        ...
        if actual != expected:
            raise RuntimeError(f"C6D_REACHABILITY_DEBUG_BLOB_MISMATCH={relative}")
