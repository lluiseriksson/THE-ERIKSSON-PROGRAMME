#!/usr/bin/env python3
"""Generate the pinned cold runner for the canonical Eq. (3.51) inputs."""
...
OUTPUT = ROOT / "scripts" / "colab_eq351_regrouping_inputs_validation.py"
...
def git(*args: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            "EQ351_REGROUPING_INPUTS_GIT_FAILED=" + " ".join(args) + ":"
...
"""Cold validation of the three canonical Eq. (3.51) regrouping inputs.

The queue certifies the internally constructed positive-adjoint expansion,
negative-bond factorization and complex covariant divergence.  It does not
certify the three-species regional regrouping, Eq. (3.54), window 15, 20/41
or a TermSource.
"""
...
if measured != BASE_SHA256:
    raise RuntimeError("EQ351_REGROUPING_INPUTS_BASE_HASH_MISMATCH")
BASE_PATH.write_bytes(base_source)
spec = importlib.util.spec_from_file_location("eq351_regrouping_inputs_base", BASE_PATH)
if spec is None or spec.loader is None:
    raise RuntimeError("EQ351_REGROUPING_INPUTS_BASE_IMPORT_FAILED")
...
runner.ROOT = Path("/content/hrpoly-eq351-regrouping-inputs")
runner.EVIDENCE = Path("/content/hrpoly-eq351-regrouping-inputs-evidence")
runner.ARCHIVE = Path("/content/hrpoly-eq351-regrouping-inputs-evidence.tar.gz")
runner.PATH_MANIFEST = Path("/content/hrpoly-eq351-regrouping-inputs-paths.txt")
runner.SOURCE_BLOBS = {{
{source_blob_lines}
}}
...
queue = [
    (
        ...
        [
            "lake", "build",
            "YangMills.RG.BalabanCMP99Eq351ExponentialAdjointRemainder",
            "YangMills.RG.BalabanCMP99Eq351PhysicalComplexPositiveBondFactorization",
            "YangMills.RG.BalabanCMP99Eq351PhysicalComplexOrientedPerturbation",
            "YangMills.RG.BalabanCMP99Eq337PhysicalComplexPerturbedBackground",
            "YangMills.RG.BalabanCMP99Eq337PhysicalComplexPerturbedLinkRadius",
        ],
        None,
    ),
    (
...
queue.append(("eq351_regrouping_inputs_root", ["lake", "build", "YangMillsCore"], None))
runner.QUEUE = queue
...
def main() -> int:
    ...
    print(
        "EQ351_REGROUPING_INPUTS_RUNNER_GENERATED "
