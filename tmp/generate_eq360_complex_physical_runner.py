...
OUTPUT = ROOT / "scripts" / "colab_eq360_complex_physical_validation.py"
...
def render(source_sha: str, runner_rev: str) -> str:
    ...
    hashes = {path: digest(source_sha, path) for path in tracked}
    blob_lines = "\n".join(
        f"    {path!r}: {value!r}," for path, value in hashes.items()
    )
    module_lines = "\n".join(
...
BASE_URL = (
    ...
    "THE-ERIKSSON-PROGRAMME/" f"{{SOURCE_SHA}}/{BASE_RUNNER}"
)
...
def main() -> int:
    ...
    print(
        "EQ360_COMPLEX_PHYSICAL_RUNNER_GENERATED "
