#!/usr/bin/env python3
"""Remove PRE-VALIDATION only after exact Eq. (3.60) physical evidence."""
...
def load(path: Path, name: str):
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_SEAL_MODULE_LOAD_FAILED={path}")
...
def git_blob(source_sha: str, relative: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"EQ360_COMPLEX_PHYSICAL_SEAL_BLOB_FAILED={relative}:"
...
def read_evidence(path: Path) -> dict:
    ...
    if not isinstance(source_sha, str) or re.fullmatch(r"[0-9a-f]{40}", source_sha) is None:
        raise RuntimeError("EQ360_COMPLEX_PHYSICAL_SEAL_SOURCE_INVALID")
    if not isinstance(result.get("boundary_blob_sha256"), dict):
        raise RuntimeError("EQ360_COMPLEX_PHYSICAL_SEAL_HASHES_MISSING")
    return result
...
def require_clean_exact(result: dict, relative: str) -> bytes:
    ...
    if status.returncode != 0 or status.stdout:
        raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_SEAL_DIRTY={relative}")
    ...
    if wanted != measured:
        raise RuntimeError(
            f"EQ360_COMPLEX_PHYSICAL_SEAL_EVIDENCE_MISMATCH={relative}:"
            f"{measured}:{wanted}"
        )
    worktree = (ROOT / relative).read_bytes().replace(b"\r\n", b"\n")
    if worktree != data:
        raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_SEAL_WORKTREE_DIVERGED={relative}")
    return data
...
def main() -> int:
    ...
    remover = load(BASE_SEALER, "eq360_complex_physical_notice_remover")
    contract = load(CONTRACT_PATH, "eq360_complex_physical_seal_contract")
    rows = [
    ...
    try:
        ...
        for relative, _ in rows:
            if "PRE-VALIDATION:" in (ROOT / relative).read_text(encoding="utf-8-sig"):
                raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_PREVALIDATION_REMAINS={relative}")
        actual = digest([(relative, (ROOT / relative).read_bytes()) for relative, _ in rows])
        if actual != manifest_sha:
            raise RuntimeError(f"EQ360_COMPLEX_PHYSICAL_SEAL_POSTWRITE_MISMATCH={actual}")
    except Exception:
