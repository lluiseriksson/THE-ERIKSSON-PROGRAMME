...
ROOT = '/content/hrpoly-neumann-canonical-precision-promoted-cold-v1'
...
def verify(files):
    ...
    require(data['scope'] == "canonical fine-to-terminal spacing and counting coefficient normalization only; not boundary inverse, uniform B0 or window15", 'PHYSICAL_SCOPE')
    ...
    require(review['cold']['production_outputs'] == {'NeumannCanonicalPrecisionOffsetAction.olean': runner.PARENT_OLEAN_HASH}, 'PARENT_OUTPUT_REVIEW')
    ...
    commands['mathlib_repro'] = ['lake', 'env', 'lean', '-o', '/content/' + REV + '-evidence/mathlib_repro.olean',
        'tmp/' + REV + '/NeumannCanonicalSpacingNormalizationRepro.lean']
    commands['physical_prerequisites'] = ["lake","build","YangMills.RG.NeumannCanonicalPrecisionOffsetAction","YangMills.RG.BalabanCMP99SourceFlatGeneratedTerminalBlockCollapse"]
    for s in ('clean_before', 'clean_after'):
        commands[s] = ['git', 'diff', '--exit-code', 'HEAD', '--', 'YangMills', 'lean-toolchain', 'lake-manifest.json']
    for s, name in [('physical_draft', 'NeumannCanonicalSpacingNormalizationDraft.lean')]:
    ...
    for s, path in [('physical_draft', 'tmp/NeumannCanonicalSpacingNormalizationDraft.lean')]:
