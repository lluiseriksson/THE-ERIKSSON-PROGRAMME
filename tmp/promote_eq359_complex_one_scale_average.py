#!/usr/bin/env python3
"""Fail-closed PRE-VALIDATION promotion of the source-specific Eq. (3.59) pair."""
...
PATHS_FILE = ROOT / "tmp" / "EQ359-COMPLEX-ONE-SCALE-AVERAGE-DRAFT-PATHS.txt"
CORE = "YangMillsCore.lean"
EXPECTED_PATHS = 14
REQUIRED_PREREQUISITES = (
    "YangMills/RG/BalabanCMP99Eq337ComplexClosedRecursiveBackground.lean",
...
def paths() -> list[str]:
    ...
    if len(rows) != EXPECTED_PATHS or len(set(rows)) != EXPECTED_PATHS:
        raise RuntimeError(
            f"EQ359_SCOPE={len(rows)}/{len(set(rows))} "
            f"WANT={EXPECTED_PATHS}/{EXPECTED_PATHS}"
        )
    if any(not row.startswith("tmp/") or not row.endswith(".draft.lean") for row in rows):
        raise RuntimeError("EQ359_SCOPE_PATH_INVALID")
    return rows
...
def require_absent(source_sha: str, relative: str) -> None:
    status = git("status", "--porcelain=v1", "--", relative)
    if status.returncode != 0 or status.stdout:
        raise RuntimeError(f"EQ359_DESTINATION_DIRTY={relative}")
    probe = git("cat-file", "-e", f"{source_sha}:{relative}")
    if probe.returncode == 0 or (ROOT / relative).exists():
        raise RuntimeError(f"EQ359_DESTINATION_EXISTS={relative}")
...
def retarget_imports(data: bytes) -> bytes:
    ...
    if "import tmp." in text:
        raise RuntimeError("EQ359_TMP_IMPORT_REMAINS")
    if text.count("PRE-VALIDATION:") != 1:
        raise RuntimeError("EQ359_PREVALIDATION_COUNT_MISMATCH")
...
def require_prerequisites(source_sha: str) -> None:
    for relative in REQUIRED_PREREQUISITES:
        ...
        if b"PRE-VALIDATION:" in data:
            raise RuntimeError(f"EQ359_PREREQUISITE_UNSEALED={relative}")
...
def core_with_audits(data: bytes, selected: list[str]) -> bytes:
    ...
    if present:
        raise RuntimeError(f"EQ359_CORE_IMPORT_EXISTS={present!r}")
...
def main() -> int:
    ...
    if core_status.returncode != 0 or core_status.stdout:
        raise RuntimeError("EQ359_CORE_DIRTY")
    rows.append((CORE, core_with_audits(git_blob(args.source_sha, CORE), selected)))
    digest = manifest_digest(rows)

    if not args.apply:
        print(
            "EQ359_PROMOTION_PREVIEW_OK "
            f"files={len(rows)} promoted={EXPECTED_PATHS} source_sha={args.source_sha} "
    ...
    try:
        ...
        if actual != digest:
            raise RuntimeError(f"EQ359_POSTWRITE_MISMATCH={actual} WANT={digest}")
    except Exception:
