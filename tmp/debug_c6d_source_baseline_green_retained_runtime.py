#!/usr/bin/env python3
"""Warm retained-runtime debug for the constructed literal C6d Green.

Diagnostic only. Exact Git blobs from one source SHA are overlaid on the
already validated retained checkout and compiled once. The checkout commit is
never moved. A later fresh cold source/audit/root gate remains mandatory before
either PRE-VALIDATION notice can be removed.
"""
...
STAGES = (
    ("tmp/BalabanCMP99Eq360C6dSourceBaselineGreen.draft.lean", None),
    ("tmp/BalabanCMP99Eq360C6dSourceBaselineGreenAudit.draft.lean", 8),
)
...
def git(repo: Path, *args: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            "C6D_BASELINE_GREEN_DEBUG_GIT_FAIL=" + " ".join(args) + ":"
...
def axiom_gate(text: str, expected: int, stage: str) -> None:
    ...
    for forbidden in FORBIDDEN:
        if forbidden in compact:
            raise RuntimeError(
                f"C6D_BASELINE_GREEN_DEBUG_FORBIDDEN={stage}:{forbidden}"
            )
    ...
    if actual != expected:
        raise RuntimeError(
            f"C6D_BASELINE_GREEN_DEBUG_AXIOM_COUNT={stage}:{actual} "
            f"WANT={expected}"
        )
    for block in blocks:
        names = {
        ...
        if unexpected:
            raise RuntimeError(
                f"C6D_BASELINE_GREEN_DEBUG_AXIOM_UNEXPECTED={stage}:"
...
def main() -> int:
    ...
    if resolved != args.source_sha:
        raise RuntimeError("C6D_BASELINE_GREEN_DEBUG_SOURCE_SHA_MISMATCH")

    for relative, _ in STAGES:
        data = git(repo, "cat-file", "blob", f"{args.source_sha}:{relative}")
        expected = git(
            repo, "rev-parse", f"{args.source_sha}:{relative}"
        ).decode().strip()
        actual = hashlib.sha1(f"blob {len(data)}\0".encode() + data).hexdigest()
        if actual != expected:
            raise RuntimeError(
                f"C6D_BASELINE_GREEN_DEBUG_BLOB_MISMATCH={relative}"
            )
        target = repo / relative
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes(data)
        print(f"GREEN_DEBUG_BLOB={relative} GIT_SHA1={actual}", flush=True)

    for index, (relative, expected_axioms) in enumerate(STAGES, start=1):
        code, seconds, output = compile_one(repo, relative)
        print(output, end="", flush=True)
        print(
            f"GREEN_DEBUG_STAGE={index}_{Path(relative).stem} "
            f"EXIT={code} SECONDS={seconds:.3f}", flush=True,
        )
        if code != 0:
            print("GREEN_DEBUG_FINAL_STATUS=FAIL", flush=True)
            return code
    ...
    if git(repo, "rev-parse", "HEAD").decode().strip() != original_head:
        raise RuntimeError("C6D_BASELINE_GREEN_DEBUG_HEAD_MOVED")
    print(f"GREEN_DEBUG_ORIGINAL_HEAD={original_head}", flush=True)
    print(f"GREEN_DEBUG_SOURCE_SHA={args.source_sha}", flush=True)
    print("GREEN_DEBUG_AXIOM_HEADERS=8", flush=True)
    print("GREEN_DEBUG_FINAL_STATUS=PASS", flush=True)
    return 0
