#!/usr/bin/env python3
"""Generate the one-cell launcher for a pinned Eq. (3.59) Colab runner."""
...
RUNNER_PATH = "scripts/colab_eq359_complex_one_scale_average_validation.py"
REPO_RAW = (
...
def generate(
    ...
    if source_pin is None or source_pin.group(1) != source_sha:
        raise SystemExit("EQ359_RUNNER_SOURCE_PIN_MISMATCH")
    ...
    if revision_pin is None or revision_pin.group(1) != runner_rev:
        raise SystemExit("EQ359_RUNNER_REV_MISMATCH")
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "scripts" / "colab_eq359_complex_one_scale_average_validation.ipynb",
    )
