#!/usr/bin/env python3
"""Warm retained-runtime debug for the literal C6d coercivity chain.

Diagnostic only.  Exact Git blobs from one source SHA are overlaid on the
already validated retained checkout and compiled in dependency order.  The
checkout commit is never moved.  A later cold source/audit/root gate remains
mandatory before any PRE-VALIDATION mark can be removed.
"""
...
def git(repo: Path, *args: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            "C6D_SOURCE_COERCIVITY_DEBUG_GIT_FAIL=" + " ".join(args) + ":"
...
def axiom_gate(text: str, expected: int, stage: str) -> None:
    ...
    for forbidden in FORBIDDEN:
        if forbidden in compact:
            raise RuntimeError(
                f"C6D_SOURCE_COERCIVITY_DEBUG_FORBIDDEN={stage}:{forbidden}"
            )
    ...
    if actual != expected:
        raise RuntimeError(
            f"C6D_SOURCE_COERCIVITY_DEBUG_AXIOM_COUNT={stage}:{actual} "
            f"WANT={expected}"
        )
    for block in blocks:
        names = {
        ...
        if unexpected:
            raise RuntimeError(
                f"C6D_SOURCE_COERCIVITY_DEBUG_AXIOM_UNEXPECTED={stage}:"
...
def main() -> int:
    ...
    if resolved != args.source_sha:
        raise RuntimeError("C6D_SOURCE_COERCIVITY_DEBUG_SOURCE_SHA_MISMATCH")
    ...
    for relative, _ in STAGES:
        ...
        if actual != expected:
            raise RuntimeError(
                f"C6D_SOURCE_COERCIVITY_DEBUG_BLOB_MISMATCH={relative}"
            )
