#!/usr/bin/env python3
"""Generate the one-cell Colab launcher for a pinned C6d Step3 runner."""
...
def generate(source_sha: str, runner_checkpoint: str) -> str:
        ...
        "cells": [
            {
                ...
                "metadata": {"id": "c6d_step3_localized_precision"},
                "outputs": [],
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "scripts" / "colab_c6d_step3_localized_precision_validation.ipynb",
    )
    args = parser.parse_args()
    content = generate(args.source_sha, args.runner_checkpoint)
    notebook = json.loads(content)
    compile("".join(notebook["cells"][0]["source"]), str(args.output), "exec")
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(content, encoding="utf-8", newline="\n")
    print(
        "C6D_STEP3_NOTEBOOK_GENERATED "
