#!/usr/bin/env python3
"""Fail-closed promotion of the exact post-Green decay prefix.

This promotes only the D1 full-companion localization pair and the named
metric-transport pair.  It never runs Lean and never removes PRE-VALIDATION.
"""
...
SOURCES = (
    "tmp/BalabanCMP99SourceActiveRegionFullCompanionPrecisionDecay.draft.lean",
    "tmp/BalabanCMP99SourceActiveRegionFullCompanionPrecisionDecayAudit.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientMetric.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientMetricAudit.draft.lean",
)
PREREQUISITES = (
...
def blob(source_sha: str, relative: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"C6D_POST_GREEN_BLOB_FAILED={relative}:"
...
def promote_text(data: bytes) -> bytes:
    ...
    if "import tmp." in text:
        raise RuntimeError("C6D_POST_GREEN_TMP_IMPORT_REMAINS")
    if text.count("PRE-VALIDATION:") != 1:
        raise RuntimeError("C6D_POST_GREEN_PRE_COUNT")
    if re.search(r"(?m)^\s*axiom\b|\b(?:sorry|admit|by\?|exact\?)\b", text):
        raise RuntimeError("C6D_POST_GREEN_PLACEHOLDER")
...
def require_clean_blob(source_sha: str, relative: str) -> bytes:
    ...
    if status.returncode != 0 or status.stdout:
        raise RuntimeError(f"C6D_POST_GREEN_DIRTY={relative}")
    data = blob(source_sha, relative)
    if (ROOT / relative).read_bytes().replace(b"\r\n", b"\n") != data:
        raise RuntimeError(f"C6D_POST_GREEN_DIVERGED={relative}")
    return data
...
def main() -> int:
    ...
    if core_status.returncode != 0 or core_status.stdout:
        raise RuntimeError("C6D_POST_GREEN_CORE_DIRTY")
    core = blob(args.source_sha, CORE).decode("utf-8")
    imports = (
        "import YangMills.RG."
        "BalabanCMP99SourceActiveRegionFullCompanionPrecisionDecayAudit",
        "import YangMills.RG."
        "BalabanCMP99Eq360C6dSourceSeparatedAmbientMetricAudit",
    )
    for line in imports:
        if line in core:
            raise RuntimeError(f"C6D_POST_GREEN_CORE_IMPORT_EXISTS={line}")
    ...
    if not args.apply:
        print(
            "C6D_POST_GREEN_PROMOTION_PREVIEW_OK "
    ...
    try:
        ...
        if actual != manifest:
            raise RuntimeError(
                f"C6D_POST_GREEN_POSTWRITE={actual} EXPECTED={manifest}"
            )
    except Exception:
    ...
    print(
        "C6D_POST_GREEN_PROMOTION_APPLY_OK "
        f"files={len(rows)} promoted={len(SOURCES)} "
        f"source_sha={args.source_sha} manifest_sha256={manifest}"
    )
    return 0
