#!/usr/bin/env python3
"""Verify durable stage-output evidence for the source-specific Eq. (3.59) pair."""
...
CONTRACT_PATH = ROOT / "tmp" / "verify_eq359_complex_one_scale_average_contract.py"
...
def load(path: Path, name: str):
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"EQ359_ARCHIVE_MODULE_LOAD_FAILED={path}")
...
def main() -> int:
    ...
    if re.fullmatch(r"[0-9a-f]{40}", args.source_sha) is None:
        raise RuntimeError("EQ359_ARCHIVE_SOURCE_SHA_INVALID")

    contract = load(CONTRACT_PATH, "eq359_contract")
    util = load(ARCHIVE_UTIL_PATH, "eq359_archive_util")
    repo = args.repo.resolve()
    archive_path = args.archive.resolve()
    if not archive_path.is_file():
        raise RuntimeError(f"EQ359_ARCHIVE_MISSING={archive_path}")

    with tarfile.open(archive_path, "r:gz") as archive:
        root, files = util.safe_members(archive)
        evidence_name = f"{root}/evidence.json"
        if evidence_name not in files:
            raise RuntimeError("EQ359_ARCHIVE_EVIDENCE_MISSING")
        evidence = json.loads(util.read_member(archive, files[evidence_name]).decode())
        if evidence.get("runner_rev") != args.runner_rev:
            raise RuntimeError("EQ359_ARCHIVE_RUNNER_REV_MISMATCH")
        if evidence.get("source_sha") != args.source_sha:
            raise RuntimeError("EQ359_ARCHIVE_SOURCE_SHA_MISMATCH")
        if evidence.get("status") != "PASS":
            raise RuntimeError(f"EQ359_ARCHIVE_STATUS={evidence.get('status')}")
        if evidence.get("mathlib_sha") != EXPECTED_MATHLIB:
            raise RuntimeError("EQ359_ARCHIVE_MATHLIB_MISMATCH")
        if evidence.get("toolchain_asset_sha256") != EXPECTED_TOOLCHAIN_ASSET:
            raise RuntimeError("EQ359_ARCHIVE_TOOLCHAIN_MISMATCH")
        ...
        if not isinstance(source_blobs, dict) or set(source_blobs) != source_paths:
            raise RuntimeError("EQ359_ARCHIVE_SOURCE_BLOB_SCOPE")
        for relative, wanted in source_blobs.items():
            measured = util.sha256_bytes(
                contract.git_blob(repo, args.source_sha, relative)
            )
            if measured != wanted:
                raise RuntimeError(f"EQ359_ARCHIVE_SOURCE_BLOB_MISMATCH={relative}")

        records = evidence.get("records")
        if not isinstance(records, list) or not records:
            raise RuntimeError("EQ359_ARCHIVE_RECORDS_MISSING")
        by_stage: dict[str, dict] = {}
        outputs: dict[str, str] = {}
        for record in records:
            if not isinstance(record, dict) or not isinstance(record.get("stage"), str):
                raise RuntimeError("EQ359_ARCHIVE_RECORD_INVALID")
            stage = record["stage"]
            if stage in by_stage:
                raise RuntimeError(f"EQ359_ARCHIVE_DUPLICATE_STAGE={stage}")
            if record.get("exit") != 0:
                raise RuntimeError(f"EQ359_ARCHIVE_STAGE_EXIT={stage}")
            member_name = f"{root}/{stage}.stdout"
            if member_name not in files:
                raise RuntimeError(f"EQ359_ARCHIVE_STDOUT_MISSING={stage}")
            output = util.read_member(archive, files[member_name])
            if util.sha256_bytes(output) != record.get("output_sha256"):
                raise RuntimeError(f"EQ359_ARCHIVE_STDOUT_HASH_MISMATCH={stage}")
        ...
        if set(files) != expected_members:
            raise RuntimeError("EQ359_ARCHIVE_MEMBER_SCOPE")
        for stage in contract.stages():
            if stage not in by_stage:
                raise RuntimeError(f"EQ359_ARCHIVE_REQUIRED_STAGE={stage}")
        ...
        for index, (module, expected_count) in enumerate(contract.MODULES, start=1):
            ...
            if len(declarations) != expected_count:
                raise RuntimeError(f"EQ359_ARCHIVE_AUDIT_COUNT={module}")
            declarations_by_module[module] = declarations
            expected.extend(declarations)
            audit_stages.append(f"eq359_{index:02d}_{module.lower()}_audit")
        if len(expected) != 32 or len(set(expected)) != 32:
            raise RuntimeError("EQ359_ARCHIVE_DECLARATION_SCOPE")

        audit_text = "\n".join(outputs[stage] for stage in audit_stages)
        compact = re.sub(r"\s+", "", audit_text)
        for forbidden in contract.FORBIDDEN:
            if forbidden in compact:
                raise RuntimeError(f"EQ359_FORBIDDEN_AXIOM={forbidden}")
        ...
        if missing:
            raise RuntimeError("EQ359_MISSING=" + json.dumps(missing))
        if duplicate:
            raise RuntimeError("EQ359_DUPLICATE=" + json.dumps(duplicate))
        if invalid:
            raise RuntimeError("EQ359_NONSTANDARD=" + json.dumps(invalid))
