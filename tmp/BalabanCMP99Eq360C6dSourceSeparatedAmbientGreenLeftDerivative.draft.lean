...
evidence.  It depends on the unsealed exact D2 value bound, C6d background
...
/-- The literal positive-depth left derivative of the exact C6d Green has
the source scale `ell` and the same owner rate as the value action. -/
theorem cmp99Eq360C6dSourceSeparatedAmbientGreen_leftDerivative_blockLocalizedSupBound
    (hdepth : 0 < depth)
