#!/usr/bin/env python3
"""Generate the cold Colab runner for exact per-depth C6d Green decay."""
...
OUTPUT = ROOT / "scripts" / "colab_c6d_exact_green_decay_validation.py"
...
def load_base():
    spec = importlib.util.spec_from_file_location("c6d_exact_green_decay_base", BASE)
    if spec is None or spec.loader is None:
        raise RuntimeError("C6D_EXACT_GREEN_DECAY_RUNNER_BASE_IMPORT_FAILED")
...
def generate(source_sha: str) -> str:
    ...
    replacements = (
        (
            ...
            "Fresh Colab gate for exact per-depth C6d Green decay.",
        ),
        (
            ...
            "source/audit pairs, all seven public axiom readouts and every repository\n"
            "consumer through ``YangMillsCore``. Passing proves only per-depth D2; it\n"
            "does not prove uniform B0/delta0, attain window 15, move ``20/41`` or\n"
            "inhabit ``TermSource``.",
        ),
        (
    ...
    for old, new in replacements:
        if old not in content:
            raise RuntimeError("C6D_EXACT_GREEN_DECAY_RUNNER_REPLACEMENT_MISSING")
...
def main() -> int:
    ...
    print(
        "C6D_EXACT_GREEN_DECAY_RUNNER_GENERATED "
        f"source_sha={args.source_sha} files=5 stages=5 axiom_blocks=7 "
        "root=YangMillsCore "
