#!/usr/bin/env python3
"""Validate and preserve one executed Eq337 complex-Ubar notebook."""
...
def exact_marker(text: str, name: str) -> str:
    ...
    if len(values) != 1:
        raise RuntimeError(f"EQ337_UBAR_MARKER_COUNT={name}:{len(values)} WANT=1")
...
def main() -> int:
    ...
    if not notebook.is_file():
        raise RuntimeError(f"EQ337_UBAR_NOTEBOOK_MISSING={notebook}")
    if destination.exists():
        raise RuntimeError(f"EQ337_UBAR_DESTINATION_EXISTS={destination}")
    text = transcript(notebook)
    runner_hashes = {name: exact_marker(text, name) for name in HASH_MARKERS}

    destination.parent.mkdir(parents=True, exist_ok=True)
    staging = Path(tempfile.mkdtemp(prefix=".eq337-ubar-evidence-", dir=destination.parent))
    try:
        copied = staging / "executed-eq337-complex-ubar-radius.ipynb"
        verifier_json = staging / "eq337-complex-ubar-radius-axioms.json"
        ...
        if result.get("status") != "EQ337_COMPLEX_UBAR_RADIUS_EVIDENCE_OK":
            raise RuntimeError("EQ337_UBAR_VERIFIER_STATUS_MISMATCH")
        manifest = {
    ...
    print(
        "EQ337_COMPLEX_UBAR_RADIUS_PACKAGE_OK "
