#!/usr/bin/env python3
"""Verify durable cold evidence for the next C6d real-slice gate."""
...
def load(path: Path, name: str):
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"C6D_NEXT_REAL_SLICE_ARCHIVE_MODULE_LOAD_FAILED={path}")
...
def main() -> int:
    ...
    if re.fullmatch(r"[0-9a-f]{40}", args.source_sha) is None:
        raise RuntimeError("C6D_NEXT_REAL_SLICE_ARCHIVE_SOURCE_SHA_INVALID")

    contract = load(CONTRACT_PATH, "c6d_next_real_slice_archive_contract")
    util = load(ARCHIVE_UTIL_PATH, "c6d_next_real_slice_archive_util")
    repo = args.repo.resolve()
    archive_path = args.archive.resolve()
    if not archive_path.is_file():
        raise RuntimeError(f"C6D_NEXT_REAL_SLICE_ARCHIVE_MISSING={archive_path}")

    with tarfile.open(archive_path, "r:gz") as archive:
        root, files = util.safe_members(archive)
        evidence_name = f"{root}/evidence.json"
        if evidence_name not in files:
            raise RuntimeError("C6D_NEXT_REAL_SLICE_ARCHIVE_EVIDENCE_MISSING")
        ...
        for key, wanted in required.items():
            if evidence.get(key) != wanted:
                raise RuntimeError(
                    f"C6D_NEXT_REAL_SLICE_ARCHIVE_{key.upper()}_MISMATCH"
                )
        ...
        if not isinstance(source_blobs, dict) or set(source_blobs) != source_paths:
            raise RuntimeError("C6D_NEXT_REAL_SLICE_ARCHIVE_SOURCE_BLOB_SCOPE")
        for relative, wanted in source_blobs.items():
            ...
            if measured != wanted:
                raise RuntimeError(
                    f"C6D_NEXT_REAL_SLICE_ARCHIVE_SOURCE_BLOB_MISMATCH={relative}"
                )

        records = evidence.get("records")
        if not isinstance(records, list) or not records:
            raise RuntimeError("C6D_NEXT_REAL_SLICE_ARCHIVE_RECORDS_MISSING")
        by_stage: dict[str, dict] = {}
        outputs: dict[str, str] = {}
        for record in records:
            if not isinstance(record, dict) or not isinstance(record.get("stage"), str):
                raise RuntimeError("C6D_NEXT_REAL_SLICE_ARCHIVE_RECORD_INVALID")
            stage = record["stage"]
            if stage in by_stage:
                raise RuntimeError(f"C6D_NEXT_REAL_SLICE_ARCHIVE_DUPLICATE_STAGE={stage}")
            if record.get("exit") != 0:
                raise RuntimeError(f"C6D_NEXT_REAL_SLICE_ARCHIVE_STAGE_EXIT={stage}")
            member_name = f"{root}/{stage}.stdout"
            if member_name not in files:
                raise RuntimeError(f"C6D_NEXT_REAL_SLICE_ARCHIVE_STDOUT_MISSING={stage}")
            output = util.read_member(archive, files[member_name])
            if util.sha256_bytes(output) != record.get("output_sha256"):
                raise RuntimeError(
                    f"C6D_NEXT_REAL_SLICE_ARCHIVE_STDOUT_HASH_MISMATCH={stage}"
                )
        ...
        if set(files) != expected_members:
            raise RuntimeError("C6D_NEXT_REAL_SLICE_ARCHIVE_MEMBER_SCOPE")
        for stage in contract.stages():
            if stage not in by_stage:
                raise RuntimeError(f"C6D_NEXT_REAL_SLICE_ARCHIVE_REQUIRED_STAGE={stage}")
        ...
        for index, (module, expected_count) in enumerate(contract.MODULES, start=1):
            ...
            declarations = contract.PRINT_RE.findall(audit_blob.decode("utf-8"))
            if len(declarations) != expected_count:
                raise RuntimeError(
                    f"C6D_NEXT_REAL_SLICE_ARCHIVE_AUDIT_COUNT={module}:"
                    f"{len(declarations)}"
                )
        ...
        for forbidden in contract.FORBIDDEN:
            if forbidden in compact:
                raise RuntimeError(f"C6D_NEXT_REAL_SLICE_FORBIDDEN_AXIOM={forbidden}")
        ...
        if missing:
            raise RuntimeError("C6D_NEXT_REAL_SLICE_MISSING=" + json.dumps(missing))
        if duplicate:
            raise RuntimeError(
                "C6D_NEXT_REAL_SLICE_DUPLICATE=" + json.dumps(duplicate, sort_keys=True)
            )
        if invalid:
            raise RuntimeError(
                "C6D_NEXT_REAL_SLICE_NONSTANDARD=" + json.dumps(invalid, sort_keys=True)
            )

    result = {
        "status": "C6D_NEXT_REAL_SLICE_EVIDENCE_OK",
        "source_sha": args.source_sha,
