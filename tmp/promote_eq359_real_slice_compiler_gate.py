#!/usr/bin/env python3
"""Fail-closed promotion for the finite Eq. (3.59) compact real-slice gate."""
...
REQUIRED_PREREQUISITES = (
    ...
    "YangMills/RG/BalabanCMP99ComplexTransportedBlockAverage.lean",
    "YangMills/RG/BalabanCMP99ComplexPhysicalRegionalTower.lean",
)
...
def require_commit(source_sha: str) -> None:
    if re.fullmatch(r"[0-9a-f]{40}", source_sha) is None:
        raise RuntimeError("EQ359_REAL_SLICE_SOURCE_SHA_INVALID")
    child = git("rev-parse", f"{source_sha}^{{commit}}")
    if child.returncode != 0 or child.stdout.decode().strip() != source_sha:
        raise RuntimeError("EQ359_REAL_SLICE_SOURCE_COMMIT_MISMATCH")
...
def git_blob(source_sha: str, relative: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"EQ359_REAL_SLICE_BLOB_FAILED={relative}:"
...
def paths() -> list[str]:
    ...
    if len(rows) != EXPECTED_PATHS or len(set(rows)) != EXPECTED_PATHS:
        raise RuntimeError(
            f"EQ359_REAL_SLICE_SCOPE={len(rows)}/{len(set(rows))} "
            f"WANT={EXPECTED_PATHS}/{EXPECTED_PATHS}"
        )
    if any(not row.startswith("tmp/") or not row.endswith(".draft.lean")
           for row in rows):
        raise RuntimeError("EQ359_REAL_SLICE_SCOPE_PATH_INVALID")
    return rows
...
def require_absent(source_sha: str, relative: str) -> None:
    status = git("status", "--porcelain=v1", "--", relative)
    if status.returncode != 0 or status.stdout:
        raise RuntimeError(f"EQ359_REAL_SLICE_DESTINATION_DIRTY={relative}")
    probe = git("cat-file", "-e", f"{source_sha}:{relative}")
    if probe.returncode == 0 or (ROOT / relative).exists():
        raise RuntimeError(f"EQ359_REAL_SLICE_DESTINATION_EXISTS={relative}")
...
def retarget_imports(data: bytes) -> bytes:
    ...
    if "import tmp." in text:
        raise RuntimeError("EQ359_REAL_SLICE_TMP_IMPORT_REMAINS")
    if text.count("PRE-VALIDATION:") != 1:
        raise RuntimeError("EQ359_REAL_SLICE_PREVALIDATION_COUNT_MISMATCH")
...
def require_prerequisites(source_sha: str) -> None:
    for relative in REQUIRED_PREREQUISITES:
        ...
        if b"PRE-VALIDATION:" in data:
            raise RuntimeError(
                f"EQ359_REAL_SLICE_PREREQUISITE_UNSEALED={relative}"
            )
...
def core_with_audits(data: bytes, selected: list[str]) -> bytes:
    ...
    if present:
        raise RuntimeError(f"EQ359_REAL_SLICE_CORE_IMPORT_EXISTS={present!r}")
...
def main() -> int:
    ...
    try:
        ...
        if actual != digest:
            raise RuntimeError(
                f"EQ359_REAL_SLICE_POSTWRITE_MISMATCH={actual} WANT={digest}"
            )
    except Exception:
    ...
    print(
        "EQ359_REAL_SLICE_PROMOTION_APPLY_OK "
        f"files={len(rows)} promoted={EXPECTED_PATHS} "
        f"source_sha={args.source_sha} manifest_sha256={digest}"
    )
    return 0
