#!/usr/bin/env python3
"""Fail-closed promotion of the source-specific Eq. (3.51) remainder gate."""
...
def paths() -> list[str]:
    ...
    if len(rows) != EXPECTED_PATHS or len(set(rows)) != EXPECTED_PATHS:
        raise RuntimeError(
            f"EQ351_SCOPE={len(rows)}/{len(set(rows))} "
    ...
    if any(
        ...
        raise RuntimeError("EQ351_SCOPE_PATH_INVALID")
    return rows
...
def retarget_imports(data: bytes) -> bytes:
    ...
    if "import tmp." in text:
        raise RuntimeError("EQ351_TMP_IMPORT_REMAINS")
    if text.count("PRE-VALIDATION:") != 1:
        raise RuntimeError("EQ351_PREVALIDATION_COUNT_MISMATCH")
    if re.search(r"(?m)^\s*(?:sorry|admit|axiom)\b|\b(?:by\?|exact\?)\b", text):
        raise RuntimeError("EQ351_FORBIDDEN_PLACEHOLDER")
...
def require_prerequisites(source_sha: str) -> None:
    for relative in REQUIRED_PREREQUISITES:
        ...
        if b"PRE-VALIDATION:" in data:
            raise RuntimeError(f"EQ351_PREREQUISITE_UNSEALED={relative}")
...
def main() -> int:
    ...
    try:
        ...
        if actual != digest:
            raise RuntimeError(f"EQ351_POSTWRITE_MISMATCH={actual} WANT={digest}")
    except Exception:
