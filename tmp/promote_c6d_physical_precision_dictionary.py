#!/usr/bin/env python3
"""Fail-closed promotion of the C6d physical precision dictionary.

This promotes only the rectangular reindex algebra, Step-7b background,
covariant derivative/Laplacian dictionary and complete precision dictionary.
It never runs Lean and never removes PRE-VALIDATION.
"""
...
def blob(source_sha: str, relative: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"C6D_PHYSICAL_DICTIONARY_BLOB_FAILED={relative}:"
...
def promote_text(data: bytes) -> bytes:
    ...
    if text.count("SCRATCH ONLY:") != 1:
        raise RuntimeError("C6D_PHYSICAL_DICTIONARY_SCRATCH_MARKER_COUNT")
    standard = (
    ...
    if len(matches) != 1:
        raise RuntimeError("C6D_PHYSICAL_DICTIONARY_SCRATCH_PROSE_UNRECOGNIZED")
    ...
    if "import tmp." in text:
        raise RuntimeError("C6D_PHYSICAL_DICTIONARY_TMP_IMPORT_REMAINS")
    if text.count("PRE-VALIDATION:") != 1:
        raise RuntimeError("C6D_PHYSICAL_DICTIONARY_PRE_COUNT")
    if re.search(r"(?m)^\s*axiom\b|\b(?:sorry|admit|by\?|exact\?)\b", text):
        raise RuntimeError("C6D_PHYSICAL_DICTIONARY_PLACEHOLDER")
...
def require_clean_blob(source_sha: str, relative: str) -> bytes:
    ...
    if status.returncode != 0 or status.stdout:
        raise RuntimeError(f"C6D_PHYSICAL_DICTIONARY_DIRTY={relative}")
    data = blob(source_sha, relative)
    if (ROOT / relative).read_bytes().replace(b"\r\n", b"\n") != data:
        raise RuntimeError(f"C6D_PHYSICAL_DICTIONARY_DIVERGED={relative}")
    return data
...
def main() -> int:
    ...
    if re.fullmatch(r"[0-9a-f]{40}", args.source_sha) is None:
        raise RuntimeError("C6D_PHYSICAL_DICTIONARY_SOURCE_SHA_INVALID")
    resolved = git("rev-parse", f"{args.source_sha}^{{commit}}")
    if resolved.returncode != 0 or resolved.stdout.decode().strip() != args.source_sha:
        raise RuntimeError("C6D_PHYSICAL_DICTIONARY_SOURCE_COMMIT_MISMATCH")

    for relative in PREREQUISITES:
        if b"PRE-VALIDATION:" in blob(args.source_sha, relative):
            raise RuntimeError(f"C6D_PHYSICAL_DICTIONARY_PREREQ_UNSEALED={relative}")

    rows: list[tuple[str, bytes]] = []
    for relative in SOURCES:
        target = destination(relative)
        if git("cat-file", "-e", f"{args.source_sha}:{target}").returncode == 0:
            raise RuntimeError(f"C6D_PHYSICAL_DICTIONARY_TARGET_EXISTS={target}")
        if (ROOT / target).exists() or git(
            "status", "--porcelain=v1", "--", target
        ).stdout:
            raise RuntimeError(f"C6D_PHYSICAL_DICTIONARY_TARGET_DIRTY={target}")
    ...
    if core_status.returncode != 0 or core_status.stdout:
        raise RuntimeError("C6D_PHYSICAL_DICTIONARY_CORE_DIRTY")
    ...
    for line in imports:
        if line in core:
            raise RuntimeError(f"C6D_PHYSICAL_DICTIONARY_CORE_IMPORT_EXISTS={line}")
    ...
    if not args.apply:
        print(
            "C6D_PHYSICAL_DICTIONARY_PROMOTION_PREVIEW_OK "
    ...
    try:
        ...
        if actual != manifest:
            raise RuntimeError(
                f"C6D_PHYSICAL_DICTIONARY_POSTWRITE={actual} EXPECTED={manifest}"
            )
    except Exception:
    ...
    print(
        "C6D_PHYSICAL_DICTIONARY_PROMOTION_APPLY_OK "
        f"files={len(rows)} promoted={len(SOURCES)} "
        f"source_sha={args.source_sha} manifest_sha256={manifest}"
    )
    return 0
