#!/usr/bin/env python3
"""Bounded hot debug for the C6d terminal-coercivity scratch pair.

This is diagnostic only.  It materializes two exact Git blobs into an already
validated retained checkout, compiles source then audit, and never edits the
checkout commit.  A later cold source/audit/root gate remains mandatory.
"""
...
def git(repo: Path, *args: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            "C6D_COERCIVITY_DEBUG_GIT_FAIL=" + " ".join(args) + ":"
...
def compile_one(repo: Path, relative: str) -> tuple[int, float, str]:
    ...
    return child.returncode, seconds, child.stdout.decode(errors="replace")
...
def axiom_gate(text: str) -> None:
    ...
    for forbidden in FORBIDDEN:
        if forbidden in compact:
            raise RuntimeError(f"C6D_COERCIVITY_DEBUG_FORBIDDEN={forbidden}")
    blocks = list(OUTPUT_RE.finditer(text))
    pure = list(NO_AXIOM_RE.finditer(text))
    if len(blocks) + len(pure) != EXPECTED_AXIOM_HEADERS:
        raise RuntimeError(
            "C6D_COERCIVITY_DEBUG_AXIOM_COUNT="
            f"{len(blocks) + len(pure)} WANT={EXPECTED_AXIOM_HEADERS}"
        )
    for block in blocks:
        names = {name.strip() for name in block.group(2).split(",") if name.strip()}
        unexpected = names - ALLOWED
        if unexpected:
            raise RuntimeError(
                "C6D_COERCIVITY_DEBUG_AXIOM_UNEXPECTED="
...
def main() -> int:
    ...
    if resolved != args.source_sha:
        raise RuntimeError("C6D_COERCIVITY_DEBUG_SOURCE_SHA_MISMATCH")
    ...
    for relative in PATHS:
        ...
        if actual != expected:
            raise RuntimeError(f"C6D_COERCIVITY_DEBUG_BLOB_MISMATCH={relative}")
    ...
    axiom_gate(audit_output)
    if git(repo, "rev-parse", "HEAD").decode().strip() != original_head:
        raise RuntimeError("C6D_COERCIVITY_DEBUG_HEAD_MOVED")
