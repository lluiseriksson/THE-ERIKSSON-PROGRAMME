#!/usr/bin/env python3
"""Remove PRE-VALIDATION only after exact physical Eq. (3.51) evidence."""
...
def load(path: Path, name: str):
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"EQ351_SEAL_MODULE_LOAD_FAILED={path}")
...
def git_blob(source_sha: str, relative: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"EQ351_SEAL_BLOB_FAILED={relative}:"
...
def read_evidence(path: Path) -> dict:
    ...
    if not isinstance(source_sha, str) or re.fullmatch(r"[0-9a-f]{40}", source_sha) is None:
        raise RuntimeError("EQ351_SEAL_SOURCE_INVALID")
    if not isinstance(result.get("boundary_blob_sha256"), dict):
        raise RuntimeError("EQ351_SEAL_HASHES_MISSING")
    return result
...
def main() -> int:
    ...
    remover = load(BASE_SEALER, "eq351_notice_remover")
    contract = load(CONTRACT_PATH, "eq351_seal_contract")
    rows = [
        (
            relative,
    ...
    try:
        ...
        for relative, _ in rows:
            if "PRE-VALIDATION:" in (ROOT / relative).read_text(encoding="utf-8-sig"):
                raise RuntimeError(f"EQ351_PREVALIDATION_REMAINS={relative}")
        actual = digest(
            [(relative, (ROOT / relative).read_bytes()) for relative, _ in rows]
        )
        if actual != manifest_sha:
            raise RuntimeError(f"EQ351_SEAL_POSTWRITE_MISMATCH={actual}")
    except Exception:
