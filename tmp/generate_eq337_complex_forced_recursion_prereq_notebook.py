#!/usr/bin/env python3
"""Generate the stable one-cell launcher for the complex prereq gate."""
...
RUNNER_PATH = (
    "scripts/colab_eq337_complex_forced_recursion_prereq_validation.py"
)
DEFAULT_OUTPUT = (
    ROOT / "scripts" /
    "colab_eq337_complex_forced_recursion_prereq_validation.ipynb"
)
REPO_RAW = (
...
def require_commit(sha: str, label: str) -> None:
    ...
    if git("rev-parse", f"{sha}^{{commit}}") != sha:
        raise SystemExit(f"{label}_SHA_RESOLUTION_MISMATCH")
...
def generate(source_sha: str, runner_checkpoint: str, runner_rev: str) -> str:
    ...
    if source_pin is None or source_pin.group(1) != source_sha:
        raise SystemExit("COMPLEX_RECURSION_PREREQ_SOURCE_PIN_MISMATCH")
    ...
    if revision_pin is None or revision_pin.group(1) != runner_rev:
        raise SystemExit("COMPLEX_RECURSION_PREREQ_REV_MISMATCH")
...
def main() -> int:
    ...
    print(
        "EQ337_COMPLEX_RECURSION_PREREQ_NOTEBOOK_GENERATED "
