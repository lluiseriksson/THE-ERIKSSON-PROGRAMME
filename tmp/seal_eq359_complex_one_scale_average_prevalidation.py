#!/usr/bin/env python3
"""Remove PRE-VALIDATION only after exact Eq. (3.59) cold evidence."""
...
def load(path: Path, name: str):
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"EQ359_SEAL_MODULE_LOAD_FAILED={path}")
...
def git_blob(source_sha: str, relative: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"EQ359_SEAL_BLOB_FAILED={relative}:"
...
def read_evidence(path: Path) -> dict:
    ...
    if result.get("status") != "EQ359_COMPLEX_ONE_SCALE_AVERAGE_EVIDENCE_OK":
        raise RuntimeError("EQ359_SEAL_STATUS_MISMATCH")
    if result.get("expected_declarations") != 32:
        raise RuntimeError("EQ359_SEAL_DECLARATIONS_MISMATCH")
    source_sha = result.get("source_sha")
    if not isinstance(source_sha, str) or re.fullmatch(r"[0-9a-f]{40}", source_sha) is None:
        raise RuntimeError("EQ359_SEAL_SOURCE_INVALID")
    if not isinstance(result.get("boundary_blob_sha256"), dict):
        raise RuntimeError("EQ359_SEAL_HASHES_MISSING")
    return result
...
def paths(contract) -> list[str]:
    ...
    if len(rows) != 14 or len(set(rows)) != 14:
        raise RuntimeError("EQ359_SEAL_SCOPE_INVALID")
    return rows
...
def main() -> int:
    ...
    remover = load(BASE_SEALER, "eq359_notice_remover")
    contract = load(CONTRACT_PATH, "eq359_seal_contract")
    rows = [
        (
            relative,
    ...
    if not args.apply:
        print(
            "EQ359_SEAL_PREVIEW_OK "
            f"files=14 source_sha={result['source_sha']} "
    ...
    try:
        ...
        for relative, _ in rows:
            if "PRE-VALIDATION:" in (ROOT / relative).read_text(encoding="utf-8-sig"):
                raise RuntimeError(f"EQ359_PREVALIDATION_REMAINS={relative}")
        actual = digest(
            [(relative, (ROOT / relative).read_bytes()) for relative, _ in rows]
        )
        if actual != manifest_sha:
            raise RuntimeError(f"EQ359_SEAL_POSTWRITE_MISMATCH={actual}")
    except Exception:
