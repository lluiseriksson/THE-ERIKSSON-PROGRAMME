#!/usr/bin/env python3
"""Generate the fail-closed verifier for the C6d physical dictionary."""
...
DEFAULT_RUNNER = ROOT / "scripts" / "colab_c6d_physical_precision_dictionary_validation.py"
DEFAULT_NOTEBOOK = ROOT / "scripts" / "colab_c6d_physical_precision_dictionary_validation.ipynb"
DEFAULT_OUTPUT = ROOT / "tmp" / "verify_c6d_physical_precision_dictionary_evidence.py"
MODULES = [
...
def notebook_cell_source(path: Path) -> str:
    ...
    if len(cells) != 1:
        raise RuntimeError(f"C6D_PHYSICAL_DICTIONARY_VERIFIER_NOTEBOOK_CODE_CELLS={len(cells)}")
    source = cells[0].get("source")
    if isinstance(source, list):
        return "".join(source)
    if isinstance(source, str):
        return source
    raise RuntimeError("C6D_PHYSICAL_DICTIONARY_VERIFIER_NOTEBOOK_SOURCE_INVALID")
...
def runner_metadata(path: Path) -> tuple[str, str, dict[str, str]]:
    ...
    if source is None or revision is None or blobs is None:
        raise RuntimeError("C6D_PHYSICAL_DICTIONARY_VERIFIER_RUNNER_METADATA_MISSING")
    parsed = ast.literal_eval(blobs.group(1))
    if not isinstance(parsed, dict) or not all(
        isinstance(key, str) and isinstance(value, str) for key, value in parsed.items()
    ):
        raise RuntimeError("C6D_PHYSICAL_DICTIONARY_VERIFIER_SOURCE_BLOBS_INVALID")
...
def generated_text(source_sha: str, runner: Path, notebook: Path) -> str:
    ...
    if runner_source != source_sha:
        raise RuntimeError(
            f"C6D_PHYSICAL_DICTIONARY_VERIFIER_SOURCE={runner_source} EXPECTED={source_sha}"
        )
    ...
    headers = repr(EXPECTED_AXIOM_HEADERS)
    audit_stages = repr({module: f"{index:02d}_{module.removeprefix('Balaban').lower()}_audit" for index, module in enumerate(MODULES, start=1)})
    queue = []
    for index, module in enumerate(MODULES, start=1):
        slug = module.removeprefix("Balaban").lower()
        queue.extend((f"{index:02d}_{slug}_focal", f"{index:02d}_{slug}_audit"))
    queue.append("05_c6d_physical_precision_dictionary_yang_mills_core_root")
...
EXPECTED_AXIOM_HEADERS = {headers}
QUEUE_STAGES = {queue!r}
...
def main() -> int:
    ...
    print(
        "C6D_PHYSICAL_DICTIONARY_VERIFIER_GENERATED "
