#!/usr/bin/env python3
"""Generate the one-cell launcher for a pinned closed Eq. (3.37) runner."""
...
RUNNER_PATH = "scripts/colab_eq337_closed_physical_recursion_validation.py"
REPO_RAW = (
...
def generate(
    ...
    if source_pin is None or source_pin.group(1) != source_sha:
        raise SystemExit("EQ337_CLOSED_RUNNER_SOURCE_PIN_MISMATCH")
    ...
    if revision_pin is None or revision_pin.group(1) != runner_rev:
        raise SystemExit("EQ337_CLOSED_RUNNER_REV_MISMATCH")
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "scripts" / "colab_eq337_closed_physical_recursion_validation.ipynb",
    )
    ...
    print(
        "EQ337_CLOSED_PHYSICAL_NOTEBOOK_GENERATED "
