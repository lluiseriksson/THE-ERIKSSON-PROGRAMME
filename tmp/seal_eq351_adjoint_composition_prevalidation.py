#!/usr/bin/env python3
"""Retire Eq. (3.50) PRE-VALIDATION only from exact cold evidence."""
...
def load(path: Path, name: str):
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"EQ351_ADJOINT_SEAL_MODULE_LOAD_FAILED={path}")
...
def git_blob(source_sha: str, relative: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"EQ351_ADJOINT_SEAL_BLOB_FAILED={relative}:"
...
def read_evidence(path: Path) -> dict:
    ...
    if not isinstance(source_sha, str) or re.fullmatch(r"[0-9a-f]{40}", source_sha) is None:
        raise RuntimeError("EQ351_ADJOINT_SEAL_SOURCE_INVALID")
    if not isinstance(result.get("boundary_blob_sha256"), dict):
        raise RuntimeError("EQ351_ADJOINT_SEAL_HASHES_MISSING")
    return result
...
def paths(contract) -> list[str]:
    ...
    if len(rows) != 4 or len(set(rows)) != 4:
        raise RuntimeError("EQ351_ADJOINT_SEAL_SCOPE_INVALID")
    return rows
...
def require_clean_exact(result: dict, relative: str) -> bytes:
    ...
    if status.returncode != 0 or status.stdout:
        raise RuntimeError(f"EQ351_ADJOINT_SEAL_DIRTY={relative}")
    ...
    if wanted != measured:
        raise RuntimeError(
            f"EQ351_ADJOINT_SEAL_EVIDENCE_MISMATCH={relative}:{measured}:{wanted}"
        )
    worktree = (ROOT / relative).read_bytes().replace(b"\r\n", b"\n")
    if worktree != data:
        raise RuntimeError(f"EQ351_ADJOINT_SEAL_WORKTREE_DIVERGED={relative}")
    return data
...
def main() -> int:
    ...
    remover = load(BASE_SEALER, "eq351_adjoint_notice_remover")
    contract = load(CONTRACT_PATH, "eq351_adjoint_seal_contract")
    rows = [
        (
            relative,
    ...
    if not args.apply:
        print(
            "EQ351_ADJOINT_SEAL_PREVIEW_OK "
            f"files=4 source_sha={result['source_sha']} "
    ...
    try:
        ...
        for relative, _ in rows:
            if "PRE-VALIDATION:" in (ROOT / relative).read_text(encoding="utf-8-sig"):
                raise RuntimeError(f"EQ351_ADJOINT_PREVALIDATION_REMAINS={relative}")
        actual = digest(
            [(relative, (ROOT / relative).read_bytes()) for relative, _ in rows]
        )
        if actual != manifest_sha:
            raise RuntimeError(f"EQ351_ADJOINT_SEAL_POSTWRITE_MISMATCH={actual}")
    except Exception:
