#!/usr/bin/env python3
"""Fail-closed promotion of the exact C6d Green owner-value prefix.

The promoted prefix contains the positive- and zero-depth owner-input action,
owner-distance decay and block-localized value pairs.  This script performs
text transport only: it never runs Lean and never removes PRE-VALIDATION.
Both exact D2 Green-decay pairs must already be sealed in the selected source
commit.
"""

from __future__ import annotations

import argparse
import hashlib
from pathlib import Path
import re
import subprocess


ROOT = Path(__file__).resolve().parents[1]
CORE = "YangMillsCore.lean"
PROMOTION_SENTINEL = "C6D_GREEN_OWNER_PREFIX"
SOURCES = (
    "tmp/BalabanCMP99Eq342LeftDerivativeFromValueBound.draft.lean",
    "tmp/BalabanCMP99Eq342LeftDerivativeFromValueBoundAudit.draft.lean",
    "tmp/BalabanCMP99Eq342RightAdjointFromValueBound.draft.lean",
    "tmp/BalabanCMP99Eq342RightAdjointFromValueBoundAudit.draft.lean",
    "tmp/BalabanCMP99Eq342LaplacianFromLeftDerivativeBound.draft.lean",
    "tmp/BalabanCMP99Eq342LaplacianFromLeftDerivativeBoundAudit.draft.lean",
    "tmp/BalabanCMP99Eq342LeftDerivativeAtTerminalSpacing.draft.lean",
    "tmp/BalabanCMP99Eq342LeftDerivativeAtTerminalSpacingAudit.draft.lean",
    "tmp/BalabanCMP99Eq342RightAdjointAtTerminalSpacing.draft.lean",
    "tmp/BalabanCMP99Eq342RightAdjointAtTerminalSpacingAudit.draft.lean",
    "tmp/BalabanCMP99Eq342LaplacianAtTerminalSpacing.draft.lean",
    "tmp/BalabanCMP99Eq342LaplacianAtTerminalSpacingAudit.draft.lean",
    "tmp/BalabanCMP99Eq342SourceLocalizedCertificateAssembler.draft.lean",
    "tmp/BalabanCMP99Eq342SourceLocalizedCertificateAssemblerAudit.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerInputAction.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerInputActionAudit.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerInputActionZeroDepth.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerInputActionZeroDepthAudit.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerDecay.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerDecayAudit.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerDecayZeroDepth.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerDecayZeroDepthAudit.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenBlockLocalizedOwnerDecay.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenBlockLocalizedOwnerDecayAudit.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenBlockLocalizedOwnerDecayZeroDepth.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenBlockLocalizedOwnerDecayZeroDepthAudit.draft.lean",
    "tmp/BalabanCMP99SourcePhysicalLocalizedRegionNonempty.draft.lean",
    "tmp/BalabanCMP99SourcePhysicalLocalizedRegionNonemptyAudit.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenLeftDerivative.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenLeftDerivativeAudit.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenRightAdjoint.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenRightAdjointAudit.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenLaplacian.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenLaplacianAudit.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenZeroDepthActions.draft.lean",
    "tmp/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenZeroDepthActionsAudit.draft.lean",
)
PREREQUISITES = (
    "YangMills/RG/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenDecay.lean",
    "YangMills/RG/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenDecayAudit.lean",
    "YangMills/RG/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenDecayZeroDepth.lean",
    "YangMills/RG/BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenDecayZeroDepthAudit.lean",
)
AUDIT_IMPORTS = (
    "import YangMills.RG.BalabanCMP99Eq342LeftDerivativeFromValueBoundAudit",
    "import YangMills.RG.BalabanCMP99Eq342RightAdjointFromValueBoundAudit",
    "import YangMills.RG.BalabanCMP99Eq342LaplacianFromLeftDerivativeBoundAudit",
    "import YangMills.RG.BalabanCMP99Eq342LeftDerivativeAtTerminalSpacingAudit",
    "import YangMills.RG.BalabanCMP99Eq342RightAdjointAtTerminalSpacingAudit",
    "import YangMills.RG.BalabanCMP99Eq342LaplacianAtTerminalSpacingAudit",
    "import YangMills.RG.BalabanCMP99Eq342SourceLocalizedCertificateAssemblerAudit",
    "import YangMills.RG.BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerInputActionAudit",
    "import YangMills.RG.BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerInputActionZeroDepthAudit",
    "import YangMills.RG.BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerDecayAudit",
    "import YangMills.RG.BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenOwnerDecayZeroDepthAudit",
    "import YangMills.RG.BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenBlockLocalizedOwnerDecayAudit",
    "import YangMills.RG.BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenBlockLocalizedOwnerDecayZeroDepthAudit",
    "import YangMills.RG.BalabanCMP99SourcePhysicalLocalizedRegionNonemptyAudit",
    "import YangMills.RG.BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenLeftDerivativeAudit",
    "import YangMills.RG.BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenRightAdjointAudit",
    "import YangMills.RG.BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenLaplacianAudit",
    "import YangMills.RG.BalabanCMP99Eq360C6dSourceSeparatedAmbientGreenZeroDepthActionsAudit",
)


def git(*args: str) -> subprocess.CompletedProcess[bytes]:
    return subprocess.run(
        ["git", "-c", "safe.directory=*", *args],
        cwd=ROOT,
        check=False,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    )


def blob(source_sha: str, relative: str) -> bytes:
    child = git("cat-file", "blob", f"{source_sha}:{relative}")
    if child.returncode != 0:
        raise RuntimeError(
            f"C6D_GREEN_OWNER_PREFIX_BLOB_FAILED={relative}:"
            + child.stderr.decode(errors="replace")
        )
    return child.stdout


def destination(relative: str) -> str:
    name = Path(relative).name.removesuffix(".draft.lean")
    return f"YangMills/RG/{name}.lean"


def promote_text(data: bytes) -> bytes:
    text = data.decode("utf-8")
    if text.count("SCRATCH ONLY:") != 1:
        raise RuntimeError("C6D_GREEN_OWNER_PREFIX_SCRATCH_MARKER_COUNT")
    standard = (
        "PRE-VALIDATION: source present; its `.olean` is not yet materialized "
        "and the result is not compiler-verified."
    )
    source_pattern = re.compile(
        r"SCRATCH ONLY: this file is neither imported nor compiler-verified and is not\n"
        r"evidence\.[^\n]*"
    )
    if source_pattern.search(text):
        text, count = source_pattern.subn(standard, text, count=1)
    else:
        audit = "-- SCRATCH ONLY: no compiler or axiom-oracle verdict is claimed."
        audit_doc = "/-! SCRATCH ONLY: no compiler or axiom-oracle verdict is claimed. -/"
        if text.count(audit) == 1:
            text = text.replace(audit, "-- " + standard, 1)
            count = 1
        elif text.count(audit_doc) == 1:
            text = text.replace(audit_doc, "/-! " + standard + " -/", 1)
            count = 1
        else:
            raise RuntimeError("C6D_GREEN_OWNER_PREFIX_SCRATCH_PROSE_UNRECOGNIZED")
    if count != 1:
        raise RuntimeError("C6D_GREEN_OWNER_PREFIX_SCRATCH_REPLACE_COUNT")
    text = re.sub(
        r"^import tmp\.([A-Za-z0-9_]+)\.draft$",
        r"import YangMills.RG.\1",
        text,
        flags=re.MULTILINE,
    )
    if "import tmp." in text:
        raise RuntimeError("C6D_GREEN_OWNER_PREFIX_TMP_IMPORT_REMAINS")
    if text.count("PRE-VALIDATION:") != 1:
        raise RuntimeError("C6D_GREEN_OWNER_PREFIX_PRE_COUNT")
    if "SCRATCH ONLY:" in text:
        raise RuntimeError("C6D_GREEN_OWNER_PREFIX_SCRATCH_REMAINS")
    if re.search(r"(?m)^\s*axiom\b|\b(?:sorry|admit|by\?|exact\?)\b", text):
        raise RuntimeError("C6D_GREEN_OWNER_PREFIX_PLACEHOLDER")
    return text.encode("utf-8")


def require_clean_blob(source_sha: str, relative: str) -> bytes:
    status = git("status", "--porcelain=v1", "--", relative)
    if status.returncode != 0 or status.stdout:
        raise RuntimeError(f"C6D_GREEN_OWNER_PREFIX_DIRTY={relative}")
    data = blob(source_sha, relative)
    if (ROOT / relative).read_bytes().replace(b"\r\n", b"\n") != data:
        raise RuntimeError(f"C6D_GREEN_OWNER_PREFIX_DIVERGED={relative}")
    return data


def digest(rows: list[tuple[str, bytes]]) -> str:
    payload = b"".join(
        f"{hashlib.sha256(data).hexdigest().upper()}  {relative}\n".encode()
        for relative, data in rows
    )
    return hashlib.sha256(payload).hexdigest().upper()


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--source-sha", required=True)
    parser.add_argument("--apply", action="store_true")
    args = parser.parse_args()
    if re.fullmatch(r"[0-9a-f]{40}", args.source_sha) is None:
        raise RuntimeError("C6D_GREEN_OWNER_PREFIX_SOURCE_SHA_INVALID")
    resolved = git("rev-parse", f"{args.source_sha}^{{commit}}")
    if resolved.returncode != 0 or resolved.stdout.decode().strip() != args.source_sha:
        raise RuntimeError("C6D_GREEN_OWNER_PREFIX_SOURCE_COMMIT_MISMATCH")

    for relative in PREREQUISITES:
        if b"PRE-VALIDATION:" in blob(args.source_sha, relative):
            raise RuntimeError(f"C6D_GREEN_OWNER_PREFIX_PREREQ_UNSEALED={relative}")

    rows: list[tuple[str, bytes]] = []
    for relative in SOURCES:
        target = destination(relative)
        if git("cat-file", "-e", f"{args.source_sha}:{target}").returncode == 0:
            raise RuntimeError(f"C6D_GREEN_OWNER_PREFIX_TARGET_EXISTS={target}")
        if (ROOT / target).exists() or git(
            "status", "--porcelain=v1", "--", target
        ).stdout:
            raise RuntimeError(f"C6D_GREEN_OWNER_PREFIX_TARGET_DIRTY={target}")
        rows.append((target, promote_text(require_clean_blob(args.source_sha, relative))))

    core_status = git("status", "--porcelain=v1", "--", CORE)
    if core_status.returncode != 0 or core_status.stdout:
        raise RuntimeError("C6D_GREEN_OWNER_PREFIX_CORE_DIRTY")
    core = blob(args.source_sha, CORE).decode("utf-8")
    for line in AUDIT_IMPORTS:
        if line in core:
            raise RuntimeError(f"C6D_GREEN_OWNER_PREFIX_CORE_IMPORT_EXISTS={line}")
    if not core.endswith("\n"):
        core += "\n"
    rows.append((CORE, (core + "\n".join(AUDIT_IMPORTS) + "\n").encode("utf-8")))

    manifest = digest(rows)
    if not args.apply:
        print(
            f"{PROMOTION_SENTINEL}_PROMOTION_PREVIEW_OK "
            f"files={len(rows)} promoted={len(SOURCES)} "
            f"source_sha={args.source_sha} manifest_sha256={manifest}"
        )
        return 0

    originals = {
        relative: (ROOT / relative).read_bytes() if (ROOT / relative).exists() else None
        for relative, _ in rows
    }
    written: list[str] = []
    try:
        for relative, data in rows:
            target = ROOT / relative
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_bytes(data)
            written.append(relative)
        actual = digest(
            [(relative, (ROOT / relative).read_bytes()) for relative, _ in rows]
        )
        if actual != manifest:
            raise RuntimeError(
                f"C6D_GREEN_OWNER_PREFIX_POSTWRITE={actual} EXPECTED={manifest}"
            )
    except Exception:
        for relative in reversed(written):
            original = originals[relative]
            if original is None:
                (ROOT / relative).unlink(missing_ok=True)
            else:
                (ROOT / relative).write_bytes(original)
        raise

    print(
        f"{PROMOTION_SENTINEL}_PROMOTION_APPLY_OK "
        f"files={len(rows)} promoted={len(SOURCES)} "
        f"source_sha={args.source_sha} manifest_sha256={manifest}"
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
