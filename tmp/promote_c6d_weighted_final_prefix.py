#!/usr/bin/env python3
"""Fail-closed promotion of the finite post-prefix C6d graph.

The four source/audit pairs were validated together as scratch objects by the
hot C6d graph.  Promotion is still deliberately separate from certification:
this script copies the exact Git blobs, retargets only ``tmp.*.draft`` imports,
keeps every PRE-VALIDATION notice, and adds only the four audits to
``YangMillsCore``.  A fresh cold gate remains mandatory before sealing.
"""
...
PATH_FILE = ROOT / "tmp" / "C6D-WEIGHTED-FINAL-PREFIX-DRAFT-PATHS.txt"
...
def require_commit(source_sha: str) -> None:
    if re.fullmatch(r"[0-9a-f]{40}", source_sha) is None:
        raise RuntimeError("C6D_WEIGHTED_FINAL_SOURCE_SHA_INVALID")
    child = git("rev-parse", f"{source_sha}^{{commit}}")
    if child.returncode != 0 or child.stdout.decode().strip() != source_sha:
        raise RuntimeError("C6D_WEIGHTED_FINAL_SOURCE_COMMIT_MISMATCH")
...
def git_blob(source_sha: str, relative: str) -> bytes:
    ...
    if child.returncode != 0:
        raise RuntimeError(
            f"C6D_WEIGHTED_FINAL_BLOB_FAILED={relative}:"
...
def selected_paths() -> list[str]:
    ...
    if len(rows) != EXPECTED_PATHS or len(set(rows)) != EXPECTED_PATHS:
        raise RuntimeError(
            f"C6D_WEIGHTED_FINAL_SCOPE={len(rows)}/{len(set(rows))} "
    ...
    if any(
        ...
        raise RuntimeError("C6D_WEIGHTED_FINAL_SCOPE_PATH_INVALID")
    for index in range(0, EXPECTED_PATHS, 2):
        if "Audit.draft.lean" in rows[index]:
            raise RuntimeError("C6D_WEIGHTED_FINAL_SOURCE_ORDER_INVALID")
        if not rows[index + 1].endswith("Audit.draft.lean"):
            raise RuntimeError("C6D_WEIGHTED_FINAL_AUDIT_ORDER_INVALID")
    return rows
...
def require_absent(source_sha: str, relative: str) -> None:
    status = git("status", "--porcelain=v1", "--", relative)
    if status.returncode != 0 or status.stdout:
        raise RuntimeError(f"C6D_WEIGHTED_FINAL_DESTINATION_DIRTY={relative}")
    probe = git("cat-file", "-e", f"{source_sha}:{relative}")
    if probe.returncode == 0 or (ROOT / relative).exists():
        raise RuntimeError(f"C6D_WEIGHTED_FINAL_DESTINATION_EXISTS={relative}")
...
def require_sealed_prerequisites(source_sha: str) -> None:
    for relative in REQUIRED_SEALED_PREREQUISITES:
        ...
        if b"PRE-VALIDATION:" in data:
            raise RuntimeError(
                f"C6D_WEIGHTED_FINAL_PREREQUISITE_UNSEALED={relative}"
            )
...
def retarget_imports(data: bytes) -> bytes:
    ...
    if "import tmp." in text:
        raise RuntimeError("C6D_WEIGHTED_FINAL_TMP_IMPORT_REMAINS")
    if text.count("PRE-VALIDATION:") != 1:
        raise RuntimeError("C6D_WEIGHTED_FINAL_PREVALIDATION_COUNT_MISMATCH")
    if re.search(r"(?m)^\s*axiom\b|\b(?:sorry|admit|by\?|exact\?)\b", text):
        raise RuntimeError("C6D_WEIGHTED_FINAL_FORBIDDEN_PLACEHOLDER")
...
def core_with_audits(data: bytes, selected: list[str]) -> bytes:
    ...
    if len(audits) != EXPECTED_PATHS // 2:
        raise RuntimeError("C6D_WEIGHTED_FINAL_CORE_AUDIT_SCOPE")
    text = data.decode("utf-8")
    for audit in audits:
        if audit in text:
            raise RuntimeError(f"C6D_WEIGHTED_FINAL_CORE_IMPORT_EXISTS={audit}")
...
def main() -> int:
    ...
    try:
        ...
        if actual != digest:
            raise RuntimeError(
                f"C6D_WEIGHTED_FINAL_POSTWRITE_MISMATCH={actual} WANT={digest}"
            )
    except Exception:
    ...
    print(
        "C6D_WEIGHTED_FINAL_PROMOTION_APPLY_OK "
        f"files={len(rows)} promoted={EXPECTED_PATHS} "
        f"source_sha={args.source_sha} manifest_sha256={digest}"
    )
    return 0
