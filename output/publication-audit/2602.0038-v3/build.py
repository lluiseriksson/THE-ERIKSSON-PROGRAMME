...
def main() -> int:
    ...
    front = ART / f"{JOB}.pdf"
    old = INPUTS / "2602.0038v2-public.pdf"
    writer = PdfWriter()
    writer.append(str(front))
    writer.append(str(old))
    writer.add_metadata({"/Title": "Zero-Momentum Propagator Bound in a Simplified Gribov-Zwanziger Lattice Measure", "/Author": "Lluis Eriksson", "/Subject": "Version 3 retitling and scope correction"})
    provisional = ART / "2602.0038v3.pdf"
    with provisional.open("wb") as stream:
        writer.write(stream)
    digest = sha(provisional)
    pages = len(PdfReader(str(provisional)).pages)
    final = ART / f"2602.0038v3-{digest[:8]}-{pages}pp.pdf"
    if final.exists():
        final.unlink()
    provisional.replace(final)
    result = {"status": "BUILT-NOT-INDEPENDENTLY-AUDITED", "final_pdf": final.name, "final_sha256": digest, "pages": pages, "front_pages": len(PdfReader(str(front)).pages), "public_v2_pages": len(PdfReader(str(old)).pages), "public_v2_sha256": sha(old), "source_sha256": sha(SRC / "front_note.tex")}
