...
ART = ROOT / "artifacts"
INPUT = ROOT / "inputs" / "2602.0038v2-public.pdf"
...
def main() -> int:
    ...
    check(sha(INPUT) == "306d2336ad3c415fce994ba20ee0488ba20739ed5002877fc7a79745c530da16", "public v2 SHA")
    reader = PdfReader(str(final), strict=True)
    check(not reader.is_encrypted and len(reader.pages) == 11, "final unencrypted 11 pages")
    active = (ROOT / "src" / "front_note.tex").read_text(encoding="utf-8") + (ROOT / "SUBMISSION-ID.txt").read_text(encoding="utf-8")
    for phrase in ("necessary condition", "does not by itself prove", "No continuum-limit or Clay-problem conclusion", "preserved 10-page v2"):
        check(phrase in re.sub(r"\s+", " ", active), f"scope phrase: {phrase}")
    ...
    check(len(final_pages) == 11 and len(input_pages) == 10, "rendered page counts")
    for index, old in enumerate(input_pages):
        with Image.open(final_pages[index + 1]).convert("RGB") as a, Image.open(old).convert("RGB") as b:
            check(a.size == b.size and ImageChops.difference(a, b).getbbox() is None, f"final page {index + 2} pixel-equals public v2 page {index + 1}")
