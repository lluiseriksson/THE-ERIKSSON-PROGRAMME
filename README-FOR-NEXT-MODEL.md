...
core green at **8427 jobs**, with Mathlib pinned and every headline oracle-clean
...
`08a90502` (on `main`).  The current publication crosswalk, including the five
byte-matched public O-lane PDFs and the two pending/frozen artifacts, is
...
lake build YangMillsCore    # 8427 jobs at the latest recorded checkpoint; fast with warm cache
```
