#!/usr/bin/env python3
"""V2 reader correction: remove two stale fields from the prior field-action cohort. Original v1 FAIL is retained; no source or compilation is changed. No Lean, Git or subprocess calls.
...
BLOBS = {
    ...
    "YangMills/RG/NeumannGeneratedCompleteFibreAudit.lean": "b624c8b6fe44c8de3a28b41dfd3a6e822914b1906b62c18f591c184ccc2ed5db"
}
NAMES = {
    "fibre_audit": [
        "YangMills.RG.neumann_mem_iteratedLift_iff_terminalOwner",
        "YangMills.RG.neumannGeneratedBlockSitesToActiveFibre",
        "YangMills.RG.neumannGeneratedCompleteFibreOffsetEquiv",
        "YangMills.RG.neumannGeneratedCompleteFibreOffsetEquiv_integerCoordinates",
        "YangMills.RG.sum_neumannGeneratedCompleteFibre_eq_offsets",
        "YangMills.RG.sum_neumannGeneratedOwnerIndicator_eq_offsets"
    ]
}
...
COMMANDS = {
    "fibre_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannGeneratedCompleteFibre"
    ],
    "fibre_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannGeneratedCompleteFibreAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-complete-fibre-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
            parent_diagnostic_source='b7ac45acae587b0175526006f5a8d9a4247dea7c',
        parent_diagnostic_archive_sha256='79e156198c179fab5cfefc88bcf5b70fec788bbd90302a5d64f5c1a2713813da',
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="constructed complete active fibre, offsets and integer-coordinate sum dictionary; not regional inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-complete-fibre-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannGeneratedCompleteFibre.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-complete-fibre-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostic_source='b7ac45acae587b0175526006f5a8d9a4247dea7c',
        parent_diagnostic_archive_sha256='79e156198c179fab5cfefc88bcf5b70fec788bbd90302a5d64f5c1a2713813da',
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        ...
        scope="constructed complete active fibre, offsets and integer-coordinate sum dictionary; not regional inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    for name in ["NeumannGeneratedCompleteFibre.olean"]:
    ...
    print('NEUMANN_COMPLETE_FIBRE_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_COMPLETE_FIBRE_COLD_EVIDENCE_VERIFIED')
