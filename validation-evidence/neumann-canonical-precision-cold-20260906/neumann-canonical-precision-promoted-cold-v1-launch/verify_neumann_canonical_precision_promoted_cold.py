#!/usr/bin/env python3
"""Exact two-name canonical Neumann precision cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
BLOBS = {
    ...
    "YangMills/RG/NeumannCanonicalPrecisionOffsetActionAudit.lean": "db71a0145473359db229db9c82b76be3a40c4098231a2f6f8e99e12e618b5b61"
}
NAMES = {
    "canonical_precision_audit": [
        "YangMills.RG.neumannCanonicalPrecision_eq_explicitCountingMass",
        "YangMills.RG.neumannCanonicalPrecision_apply_eq_completeOffsets"
    ]
}
NAMES = {s: set(names) for s, names in NAMES.items()}
COMMANDS = {
    "canonical_precision_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannCanonicalPrecisionOffsetAction"
    ],
    "canonical_precision_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannCanonicalPrecisionOffsetActionAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-canonical-precision-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
            parent_diagnostic_source='8aa5fe1ff3d166ad67debbda3aa4a23ff659950c',
        parent_diagnostic_archive_sha256='f70e866d0c13490518ce56693d4f59edf8a62e460b6aae721a6070bad7bc8060',
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="literal canonical Neumann precision finite-offset action; not Fourier normalization, regional inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-canonical-precision-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannCanonicalPrecisionOffsetAction.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-canonical-precision-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostic_source='8aa5fe1ff3d166ad67debbda3aa4a23ff659950c',
        parent_diagnostic_archive_sha256='f70e866d0c13490518ce56693d4f59edf8a62e460b6aae721a6070bad7bc8060',
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        ...
        scope="literal canonical Neumann precision finite-offset action; not Fourier normalization, regional inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    for name in ["NeumannCanonicalPrecisionOffsetAction.olean"]:
    ...
    for word in ('sorryAx', 'ofReduceBool', 'Other.axiom'):
        ...
        stage = 'canonical_precision_audit'
    ...
    print('NEUMANN_CANONICAL_PRECISION_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_CANONICAL_PRECISION_COLD_EVIDENCE_VERIFIED')
