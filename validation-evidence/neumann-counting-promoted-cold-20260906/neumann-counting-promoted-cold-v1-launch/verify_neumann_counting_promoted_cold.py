#!/usr/bin/env python3
"""Read-only promoted counting reflection/dictionary cold-graph evidence verifier. No Lean, Git or subprocess calls.
...
REV = 'neumann-counting-promoted-cold-v1'
...
BLOBS = {
    "YangMills/RG/NeumannGeneratedCountingMassReflection.lean": "e97f83ada28c0300d80119b9d1b289001975148caf539805ca09cbbaa9666ae6",
    "YangMills/RG/NeumannGeneratedCountingMassReflectionAudit.lean": "5c0769526ec970014d322ac2c13fb3f992fb16287516c7733d516735e572bc4f",
    "YangMills/RG/NeumannRetainedCountingMassDictionary.lean": "aee40a157a0538c87ccd0f1ab157919d9b2ccfc94efd453de4d844b65bf875d9",
    "YangMills/RG/NeumannRetainedCountingMassDictionaryAudit.lean": "592092738e1b3f8c7039e528350c4a5efeece55a483ae37ca24479d78bcc7776"
}
NAMES = {
    "reflection_audit": [
        "YangMills.RG.neumannGeneratedFullSiteReflection_involutive",
        "YangMills.RG.neumannGeneratedTerminalOwner_reflection",
        "YangMills.RG.neumannGeneratedFullCountingMass_reflection"
    ],
    "dictionary_audit": [
        "YangMills.RG.neumannFlatGeneratedCountingMass_eq_explicit",
        "YangMills.RG.neumannFlatRetainedTerminalCountingMass_eq_explicit"
    ]
}
...
COMMANDS = {
    "reflection_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannGeneratedCountingMassReflection"
    ],
    "reflection_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannGeneratedCountingMassReflectionAudit.lean"
    ],
    "dictionary_focal": [
        "lake",
        "build",
    ...
    "dictionary_audit": [
        ...
        "env",
        "lean",
        "YangMills/RG/NeumannRetainedCountingMassDictionaryAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-counting-promoted-cold-contract.json'])
    for field, value in dict(source_sha=SOURCE,
            parent_diagnostic_source='06a928178b22c42c4ecc5387461b808ee3a19dea',
        parent_diagnostic_archive_sha256='ed2cda8e2fac4c51a1d49b140b5956fd0ba05134b9e0f2dddf4c1fecfa522d92',
        retained_dictionary_source='214b6923f3523d44282d5772fa6ab09782864f10',
        retained_dictionary_archive_sha256='19e1ee0269978842e7d5bb13c43eebb71019d540fce3169a5f4bdc3d095792cf',
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-counting-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannGeneratedCountingMassReflection.olean", "NeumannRetainedCountingMassDictionary.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-counting-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostic_source='06a928178b22c42c4ecc5387461b808ee3a19dea',
        parent_diagnostic_archive_sha256='ed2cda8e2fac4c51a1d49b140b5956fd0ba05134b9e0f2dddf4c1fecfa522d92',
        retained_dictionary_source='214b6923f3523d44282d5772fa6ab09782864f10',
        retained_dictionary_archive_sha256='19e1ee0269978842e7d5bb13c43eebb71019d540fce3169a5f4bdc3d095792cf',
    ...
    production = {}
    for name in ["NeumannGeneratedCountingMassReflection.olean", "NeumannRetainedCountingMassDictionary.olean"]:
    ...
    for word in ('sorryAx', 'ofReduceBool', 'Other.axiom'):
        ...
        record = next(r for r in changed['records'] if r['stage'] == stage)
        record['output_sha256'] = sha(bad[stage + '.log'])
        bad[stage + '.json'] = json.dumps(record).encode()
        bad['evidence.json'] = json.dumps(changed).encode(); bads.append(bad)
    for bad in bads:
        try:
            verify(bad, gate, old)
    ...
    print('NEUMANN_COUNTING_PROMOTED_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_COUNTING_PROMOTED_COLD_EVIDENCE_VERIFIED')
