#!/usr/bin/env python3
"""Exact four-name Neumann internal-bond stencil cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
SOURCE = '81f35765ad50f8217bca20bc4530d99b3bf05103'
REV = 'neumann-internal-bond-stencil-promoted-cold-v1'
...
BLOBS = {
    ...
    "YangMills/RG/NeumannInternalBondStencilAudit.lean": "687a4f29c01cf13709f31f4e6202933b278a30603eefbc9e54eecd3e227db2a8"
}
NAMES = {
    "internal_bond_stencil_audit": [
        "YangMills.RG.neumannInternalBond_restrict_eq_extend_adjoint",
        "YangMills.RG.neumannInternalBond_derivative_eq_restrict",
        "YangMills.RG.neumannInternalBond_adjoint_eq_extended_divergence",
        "YangMills.RG.neumannInternalBond_laplacian_apply"
    ]
}
NAMES = {s: set(names) for s, names in NAMES.items()}
COMMANDS = {
    "internal_bond_stencil_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannInternalBondStencil"
    ],
    "internal_bond_stencil_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannInternalBondStencilAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-internal-bond-stencil-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
            parent_diagnostic_source='c0174bbcb8f9b2b791cdb2c546206b468fd4e3cf',
        parent_diagnostic_archive_sha256='b084a31292392611b28e051d16295aa313e755fb1ef7b819a6201458b586fe76',
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="actual Neumann internal-bond adjoint and finite divergence only; not reflected Green boundary law, regional inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-internal-bond-stencil-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannInternalBondStencil.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-internal-bond-stencil-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostic_source='c0174bbcb8f9b2b791cdb2c546206b468fd4e3cf',
        parent_diagnostic_archive_sha256='b084a31292392611b28e051d16295aa313e755fb1ef7b819a6201458b586fe76',
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        ...
        scope="actual Neumann internal-bond adjoint and finite divergence only; not reflected Green boundary law, regional inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    for name in ["NeumannInternalBondStencil.olean"]:
    ...
    for word in ('sorryAx', 'ofReduceBool', 'Other.axiom'):
        ...
        stage = 'internal_bond_stencil_audit'
    ...
    print('NEUMANN_INTERNAL_BOND_STENCIL_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_INTERNAL_BOND_STENCIL_COLD_EVIDENCE_VERIFIED')
