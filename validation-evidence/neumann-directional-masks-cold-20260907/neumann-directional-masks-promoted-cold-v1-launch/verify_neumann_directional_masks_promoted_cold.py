#!/usr/bin/env python3
"""Exact four-name Neumann directional rectangle masks cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
REV = 'neumann-directional-masks-promoted-cold-v1'
...
COMMANDS = {
    ...
    "directional_masks_audit": [
        ...
        "env",
        "lean",
        "YangMills/RG/NeumannRectangleDirectionalMasksAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-directional-masks-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="actual directional torus rectangle bond masks including full-period sides; not physical carrier choice, reflection covariance, regional inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-directional-masks-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
...
def self_test(gate, old):
    ...
    files['neumann-directional-masks-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostic_source='adfef2b5b883a839e24b03ceedd118b32428dace',
        parent_diagnostic_archive_sha256='7968f4f35b0ed2a8eb7fca405260be9d528fa0319cef044b7c53b67dd1df318d',
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        ...
        scope="actual directional torus rectangle bond masks including full-period sides; not physical carrier choice, reflection covariance, regional inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    ...
    print('NEUMANN_DIRECTIONAL_MASKS_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_DIRECTIONAL_MASKS_COLD_EVIDENCE_VERIFIED')
