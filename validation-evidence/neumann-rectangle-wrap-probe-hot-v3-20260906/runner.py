"""Bounded rectangle wrap-bond HOT template, not executable until parent-cold pins are filled.
...
SOURCE = '0a8cef1f347f37a397aedbc4ecdae3ebb983c4b9'
BASE = '81f35765ad50f8217bca20bc4530d99b3bf05103'
REV = 'neumann-rectangle-wrap-probe-hot-v3'
...
PINS = {
    ...
    "tmp/NeumannRectangleWrapProbeDraft.lean": "bdc33d7b56f1215d43cc13596f0a23c28a32d952a51c4ab685bfe1a5dc7eef5c",
...
NAMES = {"physical_draft": ["neumannRectangle_siteFit_retains_wrapBond"]}
...
def main():
    ...
    try:
        ...
        for stage, path in [('physical_draft', 'NeumannRectangleWrapProbeDraft.lean')]:
            op = OUT / (stage + '.olean')
        ...
        (OUT / 'result.json').write_text(json.dumps(dict(status=status, source=SOURCE,
            ...
            scope='finite retained torus wrap-bond witness only; not a norm bound, nonperiodic boundary inverse, uniform B0 or window15'), sort_keys=True) + '\n')
