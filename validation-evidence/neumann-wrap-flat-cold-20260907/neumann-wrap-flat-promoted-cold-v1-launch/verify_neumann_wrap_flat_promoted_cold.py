#!/usr/bin/env python3
"""Exact three-name Neumann wrap/flat cold reader. Independent contract and output checks. No Lean, Git or subprocess calls.
...
COMMANDS = {
    "wrap_probe_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannRectangleWrapProbe"
    ],
    "wrap_probe_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannRectangleWrapProbeAudit.lean"
    ],
    "flat_action_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannFlatInternalBondAction"
    ],
    "flat_action_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannFlatInternalBondActionAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-wrap-flat-promoted-cold-contract.json'])
    expected_owner = dict(source_sha=SOURCE,
        ...
        project_build_cache_restored=False,
            ...
            queue=list(COMMANDS), scope="actual torus wrap witness and flat internal-bond derivative/Laplacian only; not integer boundary identification, regional inverse, uniform B0 or window15", parser_self_test={'accepted': 2, 'rejected': 9})
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-wrap-flat-promoted-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannRectangleWrapProbe.olean","NeumannFlatInternalBondAction.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-wrap-flat-promoted-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostics=[{"source":"baf5fa136663fe71a9cf07dacd4e365b0026942e","archive_sha256":"0f9875ee8fdb56e62df2cb0281c67b1b675876828cda7b359582cc60c14b391d"},{"source":"69a5308c8e5635b95b59c6d5c88d4b5b789f851c","archive_sha256":"0e7af449f57a9390481ae3384944561f8ff215c23cdb273cc4525982eee6c427"}],
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
        durable_base_sha256=WRAPPER_HASH, axiom_gate_sha256=GATE_HASH,
        audit_expected={s: sorted(n) for s,n in NAMES.items()}, queue=list(COMMANDS),
        scope="actual torus wrap witness and flat internal-bond derivative/Laplacian only; not integer boundary identification, regional inverse, uniform B0 or window15", parser_self_test={'accepted':2,'rejected':9})).encode()
    production = {}
    ...
    print('NEUMANN_WRAP_FLAT_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_WRAP_FLAT_COLD_EVIDENCE_VERIFIED')
