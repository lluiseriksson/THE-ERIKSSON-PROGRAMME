#!/usr/bin/env python3
"""Fresh cold cohort for three block-average and three generated field-action declarations.
...
def main():
    ...
    runner = base.runner
    runner.RUNNER_REV = 'neumann-average-field-cohort-cold-v1'
    ...
    runner.QUEUE = [
        ("average_focal", ["lake","build","YangMills.RG.NeumannIntegerBlockImageAverage"], None),
        ("average_audit", ["lake","env","lean","YangMills/RG/NeumannIntegerBlockImageAverageAudit.lean"], audit_names["average_audit"]),
        ("field_focal", ["lake","build","YangMills.RG.NeumannGeneratedAverageFieldAction"], None),
        ("field_audit", ["lake","env","lean","YangMills/RG/NeumannGeneratedAverageFieldActionAudit.lean"], audit_names["field_audit"]),
    ]
    ...
    def make_evidence(status, opened):
        ...
        (runner.EVIDENCE / 'neumann-average-field-cohort-cold-contract.json').write_text(json.dumps({
            'source_sha': SOURCE,
            ...
            'scope': 'finite block-image averaging and actual generated Q/weighted/counting full-field actions; not complete physical fibre reindexing, inverse, B0 or window15',
        }, sort_keys=True) + '\n', encoding='utf-8')
        outputs = {}
