"""Physical R3 solver covariance HOT diagnostic; no inverse hypothesis or cold claim."""
import hashlib
import json
import os
...
def main():
    ...
    try:
        ...
        for name,path,digest in [('NeumannActualCoordinateSolutionDraft.lean','tmp/NeumannActualCoordinateSolutionDraft.lean',PIN),('axiom-gate.py','scripts/full_green_owner_exact_axiom_gate.py',GATE)]:
        ...
        source=scratch/'NeumannActualCoordinateSolutionDraft.lean'
        source.write_bytes((OUT/source.name).read_bytes())
        run('prerequisites',['lake','build','YangMills.RG.BalabanCMP89Eq246AliasPrecisionUniqueness','YangMills.RG.BalabanCMP89Eq246FullSolutionDomain'])
        text=run('physical',['lake','env','lean','-o',str(OUT/'NeumannActualCoordinateSolutionDraft.olean'),str(source.relative_to(ROOT))])
