"""Bounded Neumann internal-bond stencil HOT template, not executable until parent-cold pins are filled.

Parent-cold pins must come from independently preserved spacing cold evidence.
...
SOURCE = 'c0174bbcb8f9b2b791cdb2c546206b468fd4e3cf'
BASE = 'dd9354a87d2e9d60dac69aa79f287b53e45ba207'
REV = 'neumann-internal-bond-stencil-hot-v1'
ROOT = Path('/content/hrpoly-neumann-canonical-spacing-promoted-cold-v1')
OUT = Path('/content/' + REV + '-evidence')
ARCHIVE = Path(str(OUT) + '.tar.gz')
RAW = 'https://raw.githubusercontent.com/lluiseriksson/THE-ERIKSSON-PROGRAMME/'
PINS = {
    "YangMills/RG/BalabanCMP89SourceNeumannRegionalPrecision.lean": "7e0abf18c2501c5755cbda6e4a47634bfb4ca17418205c73411ed9c7f9ffa078",
    "YangMills/RG/BalabanCMP99ActiveRegionSourceCovariantAdjointStencil.lean": "6d0361611bf705f2e3bdf798ac9ef8b5edfe2dcb6e85f18cf71075bff53958a4",
    "scripts/full_green_owner_exact_axiom_gate.py": "016ca4daf0cd06c8016ece106334cc10a4c332c0a58f7f383f03c6f6b3e287c2",
    "tmp/NeumannInternalBondStencilDraft.lean": "74df0523ce0e4ba673b4d69cf4c320b604ffe9f672aac1d7cbba0254341b791c",
    "tmp/NeumannInternalBondStencilRepro.lean": "010947773f37cdf9c9bd2fdd2051f5f5fe42e6ccde596dbf1e5fc83a69f01958"
}
...
# Actual independently preserved cold1164 pins; source and mathematical statements unchanged.
PARENT_OUTER_SHA = '33830a24341b533cff91184a3e0967f6ce5f1e23654c6fbccf584bd7e23f485d'
REVIEW_REF = 'fc28cbb63274fbe71e6cae88fed5d464aa6efdba'
REVIEW_PATH = 'validation-evidence/neumann-canonical-spacing-cold-20260906/independent-local-verification.json'
REVIEW_HASH = 'ff717adf491d3403d767a74e8ddaca70c6a0c39578bbe2ad84a61ab370bd0e50'
PARENT_OLEAN_HASH = 'dbda0bf9590ece51553179d34eb947ceb5e543faba9333c11c938768da8020a2'
...
def main():
    ...
    parent = Path('/content/neumann-canonical-spacing-promoted-cold-v1-preservation-20260906.tar.gz')
    assert sha(parent.read_bytes()) == PARENT_OUTER_SHA, 'PARENT_ARCHIVE'
    parent_status = json.loads(Path('/content/neumann-canonical-spacing-promoted-cold-v1-launch/launch-final-status.json').read_text())
    assert parent_status == dict(status='PASS', source=BASE, revision='neumann-canonical-spacing-promoted-cold-v1', cold_seal=True), 'PARENT_STATUS'
    OUT.mkdir()
    ...
    try:
        ...
        assert review['cold']['production_outputs'] == {'NeumannCanonicalSpacingNormalization.olean': PARENT_OLEAN_HASH}, 'REVIEW_OUTPUT'
        assert sha((ROOT / '.lake/build/lib/lean/YangMills/RG/NeumannCanonicalSpacingNormalization.olean').read_bytes()) == PARENT_OLEAN_HASH, 'PARENT_OUTPUT_CHANGED'
        ...
        run('mathlib_repro', ['lake', 'env', 'lean', '-o', str(repro_op),
            str((scratch / 'NeumannInternalBondStencilRepro.lean').relative_to(ROOT))])
        assert repro_op.stat().st_size > 0
        outputs[repro_op.name] = sha(repro_op.read_bytes())
        run('physical_prerequisites', ["lake","build","YangMills.RG.BalabanCMP89SourceNeumannRegionalPrecision","YangMills.RG.BalabanCMP99ActiveRegionSourceCovariantAdjointStencil"], timeout=600)
        for stage, path in [('physical_draft', 'NeumannInternalBondStencilDraft.lean')]:
            op = OUT / (stage + '.olean')
        ...
        (OUT / 'result.json').write_text(json.dumps(dict(status=status, source=SOURCE,
            ...
            scope='actual Neumann internal-bond adjoint and finite divergence only; not reflected Green boundary law, regional inverse, uniform B0 or window15'), sort_keys=True) + '\n')
