#!/usr/bin/env python3
"""Read-only promoted half-cell block-reflection cold-graph evidence verifier. No Lean, Git or subprocess calls.
...
BLOBS = {
    "YangMills/RG/NeumannHalfCellBlockReflection.lean": "29b6ec8806af759d3d8d470b474c708772189715d2131e87db6f820a1ea7744c",
    "YangMills/RG/NeumannHalfCellBlockReflectionAudit.lean": "ffa7a566f6e0a5bdbff1cc4206f1315b8708b01902ea4a3b24a83dca126f6657"
}
NAMES = {
    "reflection_audit": [
        "YangMills.RG.neumannHalfCellReflection_involutive",
        "YangMills.RG.blockSite_neumannHalfCellReflection",
        "YangMills.RG.neumannHalfCellReflection_sameOwner_iff"
    ]
}
...
COMMANDS = {
    "reflection_focal": [
        "lake",
        "build",
        "YangMills.RG.NeumannHalfCellBlockReflection"
    ],
    "reflection_audit": [
        "lake",
        "env",
        "lean",
        "YangMills/RG/NeumannHalfCellBlockReflectionAudit.lean"
    ]
}
...
def verify(files, gate, old):
    ...
    require(contract.get('source_sha') == SOURCE and
            ...
            'GATE_CONTRACT')
    owner = json.loads(files['neumann-halfcell-block-reflection-cold-contract.json'])
    for field, value in dict(source_sha=SOURCE,
            parent_diagnostic_source='e46c93e3aa337bcf0c0feea19d61e97522f22861',
        parent_diagnostic_archive_sha256='92ceef59e36c9bb49b9e8595a77ba44f52bc89170a04b935d58f0a73c4a54bc0',
    ...
    expected_files = {'evidence.json', 'gate-contract.json', 'neumann-halfcell-block-reflection-cold-contract.json', 'preflight.json'}
    indexed = {}
    for r in records:
        stage = r['stage']
        indexed[stage] = r
    ...
    require(set(production) == set(["NeumannHalfCellBlockReflection.olean"]), 'PRODUCTION_OUTPUT_SET')
...
def self_test(gate, old):
    ...
    files['neumann-halfcell-block-reflection-cold-contract.json'] = json.dumps(dict(source_sha=SOURCE,
        parent_diagnostic_source='e46c93e3aa337bcf0c0feea19d61e97522f22861',
        parent_diagnostic_archive_sha256='92ceef59e36c9bb49b9e8595a77ba44f52bc89170a04b935d58f0a73c4a54bc0',
        project_build_cache_restored=False,
        durable_base_commit='ddf6fdc1882edddbf063389aab4d455a8ed30801',
    ...
    production = {}
    for name in ["NeumannHalfCellBlockReflection.olean"]:
    ...
    print('NEUMANN_HALFCELL_REFLECTION_VERIFIER_SELF_TEST=PASS synthetic=1 rejected=' + str(len(bads)))
...
def main():
    ...
    print('NEUMANN_HALFCELL_REFLECTION_COLD_EVIDENCE_VERIFIED')
