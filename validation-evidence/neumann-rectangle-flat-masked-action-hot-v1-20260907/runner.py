"""Bounded actual flat rectangular action HOT; blocked until parent evidence is pinned.

Run only after the directional-mask cold archive is independently preserved.
...
SOURCE = 'f2196b046117a59b9d4d1717887bc127fa34690f'
BASE = '74fc4f4dcd115558de6cf882d17540fe57d9bd27'
REV = 'neumann-rectangle-flat-masked-action-hot-v1'
ROOT = Path('/content/hrpoly-neumann-directional-masks-promoted-cold-v1')
...
PINS = {
    ...
    "YangMills/RG/NeumannFlatInternalBondAction.lean": "10beb5d8b11f6e01a7a289eb36b5974d92b04d9e6b12a1f60a3c962f9360e2a0",
    "scripts/full_green_owner_exact_axiom_gate.py": "016ca4daf0cd06c8016ece106334cc10a4c332c0a58f7f383f03c6f6b3e287c2",
    "tmp/NeumannRectangleFlatMaskedActionDraft.lean": "4f416540b2172baac471404c7d6d0c63184e9a27f815f254355bf409195edc01"
}
SOURCE_REFS = {p: (BASE if p.startswith('YangMills/') else SOURCE) for p in PINS}
NAMES = {"physical_draft": ["neumannRectangleFlat_laplacian_directionalMasks"]}
# Deliberately unset until actual independently preserved parent cold evidence.
PARENT_OUTER_SHA = '62bf2ebca204725f973e7a7794dd472e67b3b45f4af8ee8b71f97d235e73f021'
REVIEW_REF = '974673e242dfcb794f5610b81fdcf65110b665ed'
REVIEW_PATH = 'validation-evidence/neumann-directional-masks-cold-20260907/independent-local-verification.json'
REVIEW_HASH = 'd39d0f23e0b4ea0121bd2ddd160c0c0dbe95131c359eab233228d659500b9261'
PARENT_OLEAN_HASH = '74b7bba1cabed6ad72b19aa501bd86f2b48b5fa0c00ea956df7b60e2351ce798'
...
def main():
    ...
    parent = Path('/content/neumann-directional-masks-promoted-cold-v1-preservation-20260907.tar.gz')
    assert sha(parent.read_bytes()) == PARENT_OUTER_SHA, 'PARENT_ARCHIVE'
    parent_status = json.loads(Path('/content/neumann-directional-masks-promoted-cold-v1-launch/launch-final-status.json').read_text())
    assert parent_status == dict(status='PASS', source=BASE, revision='neumann-directional-masks-promoted-cold-v1', cold_seal=True), 'PARENT_STATUS'
    OUT.mkdir()
    ...
    try:
        ...
        assert review['cold']['production_outputs']['NeumannRectangleDirectionalMasks.olean'] == PARENT_OLEAN_HASH, 'REVIEW_OUTPUT'
        assert sha((ROOT / '.lake/build/lib/lean/YangMills/RG/NeumannRectangleDirectionalMasks.olean').read_bytes()) == PARENT_OLEAN_HASH, 'PARENT_OUTPUT_CHANGED'
        ...
        gate.self_test()
        run('physical_prerequisites', ["lake","build","YangMills.RG.NeumannRectangleDirectionalMasks","YangMills.RG.NeumannFlatInternalBondAction"], timeout=600)
        for stage, path in [('physical_draft', 'NeumannRectangleFlatMaskedActionDraft.lean')]:
            op = OUT / (stage + '.olean')
        ...
        (OUT / 'result.json').write_text(json.dumps(dict(status=status, source=SOURCE,
            ...
            scope='literal flat rectangular Laplacian with directional full-period masks; not integer boundary identification, physical carrier choice, reflection covariance, regional inverse, uniform B0 or window15'), sort_keys=True) + '\n')
