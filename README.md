...
| **Last closed** | **v2.11.0 F3-count bucket to graph-animal reduction** — F3-count translation layer, no bar movement. Added `plaquetteGraphPreconnectedSubsetsAnchoredCard` plus subset/cardinality bridges in `YangMills/ClayCore/LatticeAnimalCount.lean`: every shifted connecting-cluster bucket `p ∈ X ∧ q ∈ X ∧ PolymerConnected X ∧ X.card = n + ⌈dist(p,q)⌉₊` is bounded by the anchored graph-animal bucket `p ∈ X ∧ X.card = n + ⌈dist(p,q)⌉₊ ∧ ((plaquetteGraph d L).induce {x | x ∈ X}).Preconnected`. This isolates the remaining F3-count proof as a pure bounded-degree graph-animal/Klarner estimate before the existing `1296^n` decoder route. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
