...
| **Last closed** | **v2.18.0 total-size graph-animal decoder target + shifted-count canary** — F3-count audit refinement, no bar movement. Added `physicalGraphAnimalShiftedCountBound_zero_card_le_one`, which records that the exact shifted target `card ≤ K^n` forces the `n = 0` minimal-connector bucket to have cardinality at most one; this is stronger than the usual BFS/Klarner total-size animal count. Added `PhysicalConnectingClusterGraphAnimalTotalWordDecoderBound K`, whose decoder words have length `n + ⌈dist(p,q)⌉₊` and yield the standard bound `K^(n + ⌈dist(p,q)⌉₊)`. Next analytic move: absorb the distance factor into the decay rate, using an effective parameter like `K * r`. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
