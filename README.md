...
| **Last closed** | **v2.13.0 graph-animal 1296 F3-count package bridge** — F3-count packaging, no bar movement. Added `PhysicalConnectingClusterGraphAnimalShiftedCountBound1296`, `physicalShiftedConnectingClusterCountBoundExp_of_graphAnimalShiftedCount1296`, and `physicalShiftedF3CountPackageExp_of_graphAnimalShiftedCount1296` in `YangMills/ClayCore/LatticeAnimalCount.lean`. A proof of the pure combinatorial target `#(preconnected plaquetteGraph animals containing p,q with card = n + ⌈dist(p,q)⌉₊) ≤ 1296^n` now directly produces the downstream `PhysicalShiftedF3CountPackageExp` consumed by the v2.09/v2.10 physical exponential F3 route. Remaining work: prove that shifted graph-animal count. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
