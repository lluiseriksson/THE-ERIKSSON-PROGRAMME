...
| **Last closed** | **balabanH1_from_wilson_activity_enriched constructor (P2d-β)** — pure-additive enriched constructor `YangMills.balabanH1_from_wilson_activity_enriched {N_c : ℕ} [NeZero N_c] (wab : WilsonPolymerActivityBound N_c) : BalabanH1 N_c` in `YangMills/ClayCore/WilsonPolymerActivity.lean`, a one-line composition `h1_of_small_field_bound (SmallFieldActivityBound.ofWilsonActivity wab)` that wires the P2d-α enriched small-field constructor (v0.44.0) through `h1_of_small_field_bound` (`SmallFieldBound.lean`) to produce a first-class `BalabanH1 N_c` term whose small-field activity profile is `activity n := A₀ · r^(n+2)` — polymer-faithful — instead of identically zero (the legacy `balabanH1_from_wilson_activity` route via the trivial-activity shortcut `smallFieldBound_of_wilsonActivity`, kept intact). Demonstrates the P2d-α enriched constructor es not decorative: the polymer pair `(A₀, r)` propagates cleanly through the SFA → BalabanH1 chain. Pure additive: zero downstream breakage. Oracles of all four top-level decls in the module (`smallFieldBound_of_wilsonActivity`, `balabanH1_from_wilson_activity`, `SmallFieldActivityBound.ofWilsonActivity`, `balabanH1_from_wilson_activity_enriched`) print `[propext, Classical.choice, Quot.sound]` at build. Commit `c9ac61b`, +22/−0. (2026-04-24) |
