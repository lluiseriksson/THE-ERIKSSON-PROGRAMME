...
| **Last closed** | **v1.38.0 global Mayer data restricts to the physical F3 route** — compatibility sharpening, no bar movement. Added `ConnectedCardDecayMayerData.toPhysical`, `ShiftedF3MayerPackage.toPhysical`, and `PhysicalOnlyShiftedF3MayerCountPackage.ofGlobalMayer` in `YangMills/ClayCore/ClusterRpowBridge.lean`. A future all-dimensions Mayer proof can now feed the preferred fully physical `d = 4` route by restriction, while the minimal Clay-critical assumptions remain `PhysicalShiftedF3MayerPackage N_c wab` plus `PhysicalShiftedF3CountPackage`. This keeps the older global route and the new physical-only route definitionally aligned instead of maintaining parallel APIs. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
