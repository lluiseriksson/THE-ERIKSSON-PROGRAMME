...
![Core build](https://img.shields.io/badge/lake_build_YangMillsCore-green_(8397_jobs)-success)
...
```bash
lake build YangMillsCore          # the verified core — green, 8397 jobs
...
checkpoint:** 2026-07-04, `origin/main` commit `18b16135`.  The verification
...
core build remains
`lake build YangMillsCore` green at **8397 jobs**.
...
* the latest recorded full core build is still the 8397-job core build, while
  `origin/main` has advanced to `18b16135`.
...
```mermaid
graph TD
    subgraph core["YangMillsCore  (8397 jobs, oracle-clean)"]
...
| Build the core | `lake build YangMillsCore` | `Build completed successfully (8397 jobs)` |
