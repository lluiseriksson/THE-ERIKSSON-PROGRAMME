...
| **Last closed** | **v1.58.0 finite-volume connecting-cluster buckets vanish outside range** — count-side finite-support cleanup, no bar movement. Added `connecting_cluster_count_eq_zero_of_not_mem_range` in `YangMills/ClayCore/ConnectingClusterCount.lean`: if `n ∉ Finset.range (Fintype.card (ConcretePlaquette d L) + 1)`, then the bucket of connected polymers containing `p` and `q` with cardinality `n + ⌈siteLatticeDist p.site q.site⌉₊` has cardinality `0`. This packages the v1.57 universe-size cutoff in the exact range language used by `ClusterRpowBridge.lean`; it does not prove the uniform lattice-animal estimate. Build `lake build YangMills.ClayCore.ConnectingClusterCount` green; the new declaration prints `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
