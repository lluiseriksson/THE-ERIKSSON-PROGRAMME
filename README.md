...
| **Last closed** | **v1.54.0 count-dimension monotonicity commutes with F3 packaging** — F3 package-level normalization, no bar movement. Added simp-visible compatibility lemmas showing `mono_count_dim` commutes with `ofSubpackages` and `toPhysicalOnly` for the global, mixed physical, and fully physical package routes. Future scripts can now enlarge the count polynomial exponent either before or after assembling/restricting F3 packages and Lean reduces the two paths definitionally. The live mathematical obligations remain unchanged: construct the volume-uniform shifted count package and the Mayer/Ursell package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; all five new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
