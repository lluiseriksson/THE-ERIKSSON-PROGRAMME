...
| **Last closed** | **v1.85.0 lattice-animal plaquette graph scaffold** — F3-count infrastructure, no bar movement. Added `YangMills/ClayCore/LatticeAnimalCount.lean`, with `siteLatticeDist_symm`, the nearest-neighbour `plaquetteGraph d L` on `ConcretePlaquette d L`, adjacency projection/constructor lemmas, chain packaging, ambient reachability, induced-subgraph reachability, and induced `Preconnected`. This moves the graph translation assumed by the F3-count sketch into the main Lean tree: any nodup `PolymerConnected`-style chain whose consecutive plaquette sites satisfy `siteLatticeDist ≤ 1` is now a chain in `plaquetteGraph`, `PolymerConnected X` supplies such an internal chain between any two plaquettes of `X`, and `polymerConnected_plaquetteGraph_induce_preconnected` packages the induced graph `(plaquetteGraph d L).induce {x | x ∈ X}` as preconnected. Remaining mathematical work: prove the actual uniform exponential lattice-animal bucket bound, e.g. `ShiftedConnectingClusterCountBoundExp 1 ((4 * d * d : ℕ) : ℝ)` or the tighter `(2 * d - 1)` route, then combine it with the Mayer/Ursell activity package. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
