...
| **Last closed** | **v1.86.0 plaquette graph local-neighbor bucket** — F3-count infrastructure, no bar movement. Extended `YangMills/ClayCore/LatticeAnimalCount.lean` with `plaquetteSiteBall`, `plaquetteGraph_neighborFinset_eq_filter`, `plaquetteGraph_neighborFinset_subset_siteBall`, and `plaquetteGraph_degree_le_siteBall_card`. The neighbor finset of `plaquetteGraph d L` is now identified exactly with the concrete filter `q ≠ p ∧ siteLatticeDist p.site q.site ≤ 1`, and its degree is bounded by the local site-distance bucket cardinality. This isolates the finite branching object needed for the later walk/tree encoding proof of a uniform exponential lattice-animal bound. Remaining mathematical work: prove that bucket uniformly in `L` (then assemble `ShiftedConnectingClusterCountBoundExp` and feed the Mayer/Ursell package). Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
