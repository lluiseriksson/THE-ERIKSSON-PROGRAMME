...
| **Last closed** | **v1.73.0 direct `mono_dim` application canaries for shifted F3 counts** — F3 count-frontier ergonomics, no bar movement. Added `ShiftedF3CountPackageAt.mono_dim_apply`, `ShiftedF3CountPackageDim.mono_dim_apply`, `PhysicalShiftedF3CountPackage.mono_dim_apply`, and `ShiftedF3CountPackage.mono_dim_apply`, making the common exponent-raising maneuver directly applicable at `C_conn` and `dim + k`. The open mathematical target remains unchanged: the uniform lattice-animal / KP count estimate. Builds `lake build YangMills.ClayCore.ConnectingClusterCount` and `lake build YangMills.ClayCore.ClusterRpowBridge` green; all four new declarations remain `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
