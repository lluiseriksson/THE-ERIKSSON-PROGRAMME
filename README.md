...
| **Last closed** | **LargeFieldActivityBound.h_dominated fixed-E₀ refactor (P2c)** — the over-strong `∀ E₀ > 0, exp(−p₀(g)) ≤ E₀ · g²` quantifier in `LargeFieldActivityBound.h_dominated` is promoted to the fixed-constant form `exp(−p₀(g)) ≤ E₀ · g²` with `E₀ : ℝ` and `hE0 : 0 < E₀` exposed as first-class struct fields. This matches exactly what `YangMills.superPoly_dominance_at_specific` (v0.41.0 analytic core) produces, structurally closing the P2a ↔ `LargeFieldActivityBound` integration gap flagged in the v0.41.0 scope comment. API touchpoints `lf_dominance_gives_hlf_le`, `balabanHyps_of_bounds`, `all_balaban_hyps_from_bounds` updated; repo-wide pre-deployment recon confirmed zero downstream breakage (only `LargeFieldBound.lean` + `LargeFieldDominance.lean` reference the structure). Commit `f940d58`, +40/−19 across 2 files. Oracle: `[propext, Classical.choice, Quot.sound]`. (2026-04-23) |
