...
| **Last closed** | **v1.32.0 physical F3 subpackage bundle certificate exposed** — terminal certificate convenience, no bar movement. Added `connectedCorrDecayBundle_of_physicalShiftedF3Subpackages_siteDist_measurableF` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`, sending independently-produced `mayer : ShiftedF3MayerPackage N_c wab` and `count : PhysicalShiftedF3CountPackage` to the same auditable `ConnectedCorrDecayBundle` at `physicalClayDimension = 4`. Audit lemmas pin the bundle rate as `kpParameter wab.r` and prefactor as `clusterPrefactorShifted wab.r count.C_conn mayer.A₀ count.dim + 2 * exp (kpParameter wab.r)` by definitional equality. This keeps the physical route inspectable whether future F3 scripts build the single package first or keep the two halves separate. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
