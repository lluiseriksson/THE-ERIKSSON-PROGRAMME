...
| **Last closed** | **v0.72.0 F3 endpoints into `ClayConnectedCorrDecay`** — pure-additive, no bar movement. Adds `clayConnectedCorrDecay_of_shiftedCountBound_mayerData_ceil` and `clayConnectedCorrDecay_of_shiftedCountBound_connectedCardDecayActivities_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The preferred shifted F3 packages now feed not only `ClusterCorrelatorBound` and `ClayYangMillsMassGap N_c`, but also the physically named hub `ClayConnectedCorrDecay N_c` via field-for-field projection. Both packaged and raw `K / hK_abs_le / h_mayer` entry styles are covered. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
