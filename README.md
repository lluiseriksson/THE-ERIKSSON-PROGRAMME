...
| **Last closed** | **v0.60.0 finite-volume `TruncatedActivities` from cardinality decay** — pure-additive, no bar movement. Adds `TruncatedActivities.ofCardDecay` in `YangMills/ClayCore/MayerExpansion.lean`, constructing the abstract Mayer/KP activity object on a finite polymer index type from the canonical global estimate `|K Y| ≤ A₀ * r^Y.card` with `0 ≤ A₀` and `0 ≤ r`; `K_bound Y := A₀ * r^Y.card`, and summability is discharged automatically from finite support. This pairs with v0.59.0: the terminal F3 wrapper now consumes global cardinality decay, and the Mayer layer can produce a `TruncatedActivities` object from the same shape. Builds `lake build YangMills.ClayCore.MayerExpansion` and `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
