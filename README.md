...
| **Last closed** | **v1.19.0 shifted F3 count package gets direct constructor and applicator** — F3 count-interface cleanup, no bar movement. Added `ShiftedF3CountPackage.ofBound`, `ShiftedF3CountPackage.apply`, `ofBound_C_conn`, and `ofBound_dim` in `YangMills/ClayCore/ClusterRpowBridge.lean`. A future proof of the global shifted lattice-animal estimate can now be packaged from `ShiftedConnectingClusterCountBound C_conn dim` and consumed directly as a `ShiftedF3CountPackage`, without first projecting to a finite-volume `toAt` package. The mathematical frontier remains proving `ShiftedConnectingClusterCountBound` itself plus constructing the nonabelian Mayer package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
