...
| **Last closed** | **v1.29.0 single-package physical F3 frontier exposed** — F3 interface consolidation, no bar movement. Added `PhysicalShiftedF3MayerCountPackage N_c wab` in `YangMills/ClayCore/ClusterRpowBridge.lean`, bundling exactly `mayer : ShiftedF3MayerPackage N_c wab` and `count : PhysicalShiftedF3CountPackage`. New consumers `physicalClusterCorrelatorBound_of_physicalShiftedF3MayerCountPackage` and `physicalStrong_of_physicalShiftedF3MayerCountPackage_siteDist_measurableF` give the direct route `PhysicalShiftedF3MayerCountPackage → PhysicalClusterCorrelatorBound → ClayYangMillsPhysicalStrong` at `physicalClayDimension = 4`. The mathematical obligations are unchanged: construct the Mayer/activity package and prove the physical shifted count bound for `N_c ≥ 2`; terminal packaging is now one object. Build `lake build YangMills.ClayCore.ClusterRpowBridge YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; new helpers print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
