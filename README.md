...
| **Last closed** | **v2.10.0 physical-only exponential F3 L8 endpoint** — F3 terminal routing, no bar movement. Added wrappers in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean` carrying `PhysicalOnlyShiftedF3MayerCountPackageExp` to `ConnectedCorrDecayBundle` and the non-vacuous `ClayYangMillsPhysicalStrong` endpoint, plus a subpackage form for `PhysicalShiftedF3MayerPackage` + `PhysicalShiftedF3CountPackageExp` + `count.K * wab.r < 1`. This composes with v2.09, so the live route from `PhysicalConnectingClusterBaselineExtraWordDecoderCovers1296` now reaches the terminal physical-strong layer once the physical Mayer package and smallness input are supplied. Remaining mathematical work: construct the decoder and physical Mayer/activity package. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
