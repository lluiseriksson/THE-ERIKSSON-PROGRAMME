...
![Core build](https://img.shields.io/badge/lake_build_YangMillsCore-green_(8340_jobs)-success)
...
```bash
lake build YangMillsCore          # the verified core — green, 8340 jobs
...
2026-06-23, `origin/main` commit
`479b046`.  The latest recorded core build in the verification ledger is
`lake build YangMillsCore` green at **8340 jobs**.
...
| P4 physical-operator vertical slice | `62% [######....]` | physical cochains, gauge-fixed covariance, and fixed-volume flat Hodge/Poincare closure are in Lean |
...
* The P4 material now includes deterministic gauge-fixed precision
  ...
  cochains, a fixed-volume flat Hodge/block Poincare closure, and the
  finite-torus curl/divergence classification.  It
  still does not construct the physical Yang-Mills Hessian.
...
```mermaid
graph TD
    subgraph core["YangMillsCore  (8340 jobs, oracle-clean)"]
...
| Build the core | `lake build YangMillsCore` | `Build completed successfully (8340 jobs)` |
...
| Document | What it is |
|---|---|
| [`docs/VERIFICATION-LEDGER.md`](docs/VERIFICATION-LEDGER.md) | **The record.** Verbatim oracle outputs for every headline, 263 addenda. Start here to check any claim. |
