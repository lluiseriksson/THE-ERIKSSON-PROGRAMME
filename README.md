...
| **Last closed** | **v2.04.0 finite-word decoder bridge for F3 count** — F3-count infrastructure, no bar movement. Added `PhysicalConnectingClusterExtraWordDecoderBound K`: every shifted physical connecting-cluster bucket of extra size `n` is covered by a decoder from fixed-alphabet words `Fin n → Fin K`. Lean now proves `physicalShiftedConnectingClusterCountBoundExp_of_extraWordDecoder`, sending that word-decoder directly to `PhysicalShiftedConnectingClusterCountBoundExp 1 K`; for the physical route the live target is therefore `PhysicalConnectingClusterExtraWordDecoderBound 1296`. This is the correct BFS/Klarner contract: a cluster code is a finite word, not necessarily a literal graph walk. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
