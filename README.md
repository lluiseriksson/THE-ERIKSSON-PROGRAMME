...
![Core build](https://img.shields.io/badge/lake_build_YangMillsCore-green_(8407_jobs)-success)
...
```bash
lake build YangMillsCore          # the verified core — green, 8407 jobs
...
checkpoint:** 2026-07-13, measured at `73271e4e` (on `main`).  The verification
...
core build remains
`lake build YangMillsCore` green at **8407 jobs**.
...
* the latest recorded full core build is still the 8407-job core build, while
  the measured checkpoint has advanced to `73271e4e` (on `main`).
...
```mermaid
graph TD
    subgraph core["YangMillsCore  (8407 jobs, oracle-clean)"]
...
| Build the core | `lake build YangMillsCore` | `Build completed successfully (8407 jobs)` |
