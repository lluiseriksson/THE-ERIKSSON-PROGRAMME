...
| **Last closed** | **v1.20.0 single shifted F3 package exposes activities and count directly** — final single-package interface cleanup for the current F3 route, no bar movement. Added direct API for `ShiftedF3MayerCountPackage` in `YangMills/ClayCore/ClusterRpowBridge.lean`: `toTruncatedActivities`, `toTruncatedActivities_K`, `toTruncatedActivities_K_bound_le_cardDecay`, `toTruncatedActivities_K_bound_eq_zero_of_not_connected`, `wilsonConnectedCorr_eq_toTruncatedActivities_connectingSum`, and `apply_count`. Once the preferred package is constructed, downstream code can now obtain finite-volume truncated activities, support/cardinality bounds, the Mayer identity, and the shifted lattice-animal count inequality from the same object without manual projection through `mayerPackage` and `countPackage`. The mathematical frontier remains constructing that package for `N_c ≥ 2`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
