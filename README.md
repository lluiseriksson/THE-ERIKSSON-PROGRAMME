...
| **Last closed** | **v1.90.0 ternary displacement site-neighbor code** — F3-count infrastructure, no bar movement. Extended `YangMills/ClayCore/LatticeAnimalCount.lean` with `intTernaryCode`, `siteNeighborTernaryCode`, `finBox_eq_of_siteDisplacement_eq`, `siteNeighborTernaryCode_injective`, and `siteNeighborBallBoundDim_of_ternary_displacements`. The new bridge proves `SiteNeighborBallBoundDim d (3 ^ d)` from the coordinate fact that every site-neighbor displacement lies in `{-1,0,1}`. This leaves exactly the Euclidean-coordinate lemma from `siteLatticeDist ≤ 1` as the next local geometry witness before the bound feeds through v1.88 into the plaquette graph degree bound. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces are empty-or-canonical subsets, with the final bridge at `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
