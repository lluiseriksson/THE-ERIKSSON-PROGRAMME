...
| **Last closed** | **v0.79.0 local finite-volume shifted count bound** — combinatorial audit, no bar movement. Adds `ShiftedConnectingClusterCountBoundAt d L C_conn dim` in `YangMills/ClayCore/ConnectingClusterCount.lean`, the fixed-lattice version of the shifted connecting-cluster count frontier, plus `shiftedConnectingClusterCountBoundAt_finite`, proving every finite plaquette lattice has the trivial local bound with `C_conn = Fintype.card (Finset (ConcretePlaquette d L)) + 1` and `dim = 0`. This deliberately does **not** claim the global uniform lattice-animal estimate required by F3; it separates the already-formal finite-volume counting fact from the remaining uniform/asymptotic burden packaged by `ShiftedConnectingClusterCountBound`. Build `lake build YangMills.ClayCore.ConnectingClusterCount` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
