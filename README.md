...
| **Last closed** | **v0.67.0 named shifted count frontier for terminal F3** — pure-additive, no bar movement. Adds `ShiftedConnectingClusterCountBound C_conn dim` and `.apply` in `YangMills/ClayCore/ConnectingClusterCount.lean`, plus terminal wrapper `clay_theorem_of_shiftedCountBound_connectedCardDecayActivities_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The preferred F3 endpoint now consumes one named lattice-animal package `#bucket(n) ≤ C_conn * (n+1)^dim`, the raw connected-cardinality-decay activity bound, and the Mayer/Ursell identity. Builds `lake build YangMills.ClayCore.ConnectingClusterCount` and `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
