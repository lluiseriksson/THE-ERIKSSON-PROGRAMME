...
| **Last closed** | **v1.95.0 finite plaquette-walk coding interface** — F3-count infrastructure, no bar movement. Added `PlaquetteWalk d L n p` to `YangMills/ClayCore/LatticeAnimalCount.lean`, representing an edge-length `n` plaquette-graph walk from `p` as a finite function `Fin (n+1) → ConcretePlaquette d L` with starting-point and adjacency proofs. Added the coding bridge `plaquetteWalk_card_le_of_injective_code`: any injective code `PlaquetteWalk d L n p → (Fin n → Fin D)` yields `Fintype.card (PlaquetteWalk d L n p) ≤ D ^ n`, plus the fixed-dimension interface `PlaquetteWalkCodeBoundDim`. This isolates the next target: build the actual BFS neighbor-choice code from `PlaquetteGraphBranchingBoundDim physicalClayDimension 1296`. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
