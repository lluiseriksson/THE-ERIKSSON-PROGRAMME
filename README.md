...
| **Last closed** | **v0.77.0 split preferred F3 package into Mayer/count halves** — pure-additive, no bar movement. Adds `ShiftedF3MayerPackage N_c wab` for the Mayer/activity burden (`A₀`, positivity, `ConnectedCardDecayMayerData`) and `ShiftedF3CountPackage` for the combinatorial burden (`C_conn`, positivity, `dim`, `ShiftedConnectingClusterCountBound`) in `YangMills/ClayCore/ClusterRpowBridge.lean`. The existing terminal API `ShiftedF3MayerCountPackage N_c wab` remains preferred; `ShiftedF3MayerCountPackage.ofSubpackages` recombines the two halves, `.mayerPackage` / `.countPackage` project them back out, and the projection/recombination roundtrip is `rfl`. This sharpens the active F3 frontier without claiming either analytic half is proved: the Mayer/activity producer and shifted connecting-cluster count producer can now be attacked independently, then mechanically feed all downstream `ClusterCorrelatorBound`, `ClayWitnessHyp`, `ClayYangMillsMassGap`, `ClayConnectedCorrDecay`, and weak `ClayYangMillsTheorem` endpoints. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
