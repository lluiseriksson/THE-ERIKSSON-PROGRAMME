...
| **Last closed** | **v1.76.0 `mono_count_dim` endpoint canaries for F3 correlator bounds** — F3 proof-spine/API ergonomics, no bar movement. Added `physicalClusterCorrelatorBound_of_physicalOnlyShiftedF3MayerCountPackage_mono_count_dim`, `physicalClusterCorrelatorBound_of_physicalShiftedF3MayerCountPackage_mono_count_dim`, and `clusterCorrelatorBound_of_shiftedF3MayerCountPackage_mono_count_dim` in `YangMills/ClayCore/ClusterRpowBridge.lean`. After an F3 Mayer/count package is assembled, callers can raise only the count polynomial exponent by `k` and still reach the physical or Wilson-facing `ClusterCorrelatorBound` endpoint directly, with the Mayer half preserved and the prefactor exposing `dim + k`. The open mathematical target remains unchanged: the uniform lattice-animal / KP count estimate. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; all three new declarations remain `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
