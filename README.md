...
| **Last closed** | **v2.15.0 graph-animal count route to L8 physical-strong endpoint** — terminal F3 routing, no bar movement. Added `physicalStrong_of_graphAnimalShiftedCount1296_siteDist_measurableF` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`, importing the graph-animal route from `YangMills/ClayCore/LatticeAnimalF3Bridge.lean`. The current physical F3 inputs `PhysicalConnectingClusterGraphAnimalShiftedCountBound1296` + `PhysicalShiftedF3MayerPackage N_c wab` + `(1296 : ℝ) * wab.r < 1`, together with the usual bounded measurable observable hypotheses, now produce `ClayYangMillsPhysicalStrong` directly. Remaining mathematical work: prove the graph-animal count, physical Mayer package, and smallness regime. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
