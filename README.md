...
| **Last closed** | **v0.89.0 Poincare-to-covariance axiom converted to explicit SZ input** — audit cleanup with one real axiom deletion, no Clay bar movement. Removed `poincare_to_covariance_decay` from `YangMills/P8_PhysicalGap/StroockZegarlinski.lean`; the SZ bridge now takes `hCov : ∀ L, HasCovarianceDecay (gibbsFamily L) 2 (2 / α_star)` explicitly and repackages it to `ExponentialClustering`. This does not prove Poincare-to-covariance decay; it stops declaring that functional-analysis step as a global axiom. Build `lake build YangMills.P8_PhysicalGap.StroockZegarlinski` green. Pinned oracles: `sz_lsi_to_clustering_bridge` is `[propext, Classical.choice, Quot.sound]`; `sz_lsi_to_clustering` and `lsi_to_spectralGap` add only `hille_yosida_semigroup`. No `poincare_to_covariance_decay` axiom remains. No `sorry`. (2026-04-24) |
