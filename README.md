...
| **Last closed** | **v1.28.0 physical F3 route reaches `ClayYangMillsPhysicalStrong`** — terminal physical-route sharpening, no bar movement. Added L8 wrappers `connectedCorrDecayBundle_of_physicalClusterCorrelatorBound_siteDist`, `physicalStrong_of_physicalClusterCorrelatorBound_siteDist_measurableF`, and `physicalStrong_of_physicalShiftedF3Subpackages_siteDist_measurableF` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`. The preferred physical path now consumes exactly `wab : WilsonPolymerActivityBound N_c`, `mayer : ShiftedF3MayerPackage N_c wab`, and `count : PhysicalShiftedF3CountPackage`, then produces the non-vacuous `ClayYangMillsPhysicalStrong` endpoint at `physicalClayDimension = 4` for any fixed `β > 0` and measurable unit-bounded Wilson plaquette observable `F`. This does not prove F3; it removes the remaining terminal packaging gap after `PhysicalClusterCorrelatorBound`. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; new wrappers print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
