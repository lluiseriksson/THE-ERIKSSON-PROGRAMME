...
| **Last closed** | **v1.41.0 global-to-physical Mayer restriction is simp-visible** — proof-script ergonomics, no bar movement. Added `ConnectedCardDecayMayerData.toPhysical_K`, `ConnectedCardDecayMayerData.toPhysical_toTruncatedActivities`, and `ShiftedF3MayerPackage.toPhysical_toTruncatedActivities` in `YangMills/ClayCore/ClusterRpowBridge.lean`. Future analytic scripts may prove the older all-dimensions Mayer package first, then restrict it into the preferred Clay route at `physicalClayDimension = 4`; the raw activity kernel `K` and bundled `TruncatedActivities` now rewrite by `simp` after restriction, without unfolding constructors. This keeps the all-dimensions Mayer route aligned with the fully physical F3 package while the live obligations remain `PhysicalShiftedF3MayerPackage N_c wab` and `PhysicalShiftedF3CountPackage`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
