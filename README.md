...
| **Last closed** | **v1.18.0 shifted F3 Mayer package exposes its activity consumer** — F3 package API cleanup, no bar movement. Added package-level API in `YangMills/ClayCore/ClusterRpowBridge.lean`: `ShiftedF3MayerPackage.toTruncatedActivities`, `toTruncatedActivities_K`, `toTruncatedActivities_K_bound_le_cardDecay`, `toTruncatedActivities_K_bound_eq_zero_of_not_connected`, and `wilsonConnectedCorr_eq_toTruncatedActivities_connectingSum`. These lift the `ConnectedCardDecayMayerData` activity and Mayer-identity API to the actual Mayer-side package object used by the single-package shifted F3 interface. Future nonabelian Mayer/Ursell constructors can now hand around a `ShiftedF3MayerPackage` and expose finite-volume activities, support/cardinality bounds, and the Wilson connected-correlator identity directly. The mathematical frontier remains constructing that package for `N_c ≥ 2` plus the global shifted count package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
