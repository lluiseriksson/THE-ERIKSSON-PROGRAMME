...
| **Last closed** | **v1.55.0 L8 F3 bundles expose mono-count-dimension constants** — terminal audit/endpoint normalization, no bar movement. Added six `ConnectedCorrDecayBundle` canaries showing that after `mono_count_dim k`, the mixed physical, fully physical, and global-to-physical F3 bundle endpoints still expose mass `kpParameter wab.r` and the enlarged shifted prefactor `clusterPrefactorShifted ... (old_dim + k) + 2 * Real.exp (kpParameter wab.r)`. Future F3 scripts can absorb additional polynomial count factors without losing auditable L8 constants. The live mathematical obligations remain unchanged: construct the volume-uniform shifted count package and the Mayer/Ursell package. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; all six new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
