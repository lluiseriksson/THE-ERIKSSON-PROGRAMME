...
![Core build](https://img.shields.io/badge/lake_build_YangMillsCore-green_(8401_jobs)-success)
...
```bash
lake build YangMillsCore          # the verified core — green, 8401 jobs
...
checkpoint:** 2026-07-04, `origin/main` commit `3ad2c811`.  The verification
...
core build remains
`lake build YangMillsCore` green at **8401 jobs**.
...
* the latest recorded full core build is still the 8401-job core build, while
  `origin/main` has advanced to `3ad2c811`.
...
```mermaid
graph TD
    subgraph core["YangMillsCore  (8401 jobs, oracle-clean)"]
...
| Build the core | `lake build YangMillsCore` | `Build completed successfully (8401 jobs)` |
