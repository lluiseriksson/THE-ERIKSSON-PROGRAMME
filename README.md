...
| **Last closed** | **v1.09.0 SU(1) zero-activity canary for shifted F3 Mayer** — canary/interface check, no bar movement. Added `shiftedF3MayerPackage_su1_zero` in `YangMills/ClayCore/ClusterRpowBridge.lean`: for any `wab : WilsonPolymerActivityBound 1`, it constructs `ShiftedF3MayerPackage 1 wab` with `A₀ := 1` and raw truncated activity `K := 0`. The Mayer identity closes from `wilsonConnectedCorr_su1_eq_zero`, and the constructed connecting sum is zero. This confirms that the preferred F3 Mayer boundary accepts the already-closed singleton collapse; it is not the `N_c ≥ 2` Mayer/Ursell construction. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
