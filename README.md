...
| **Last closed** | **v1.35.0 physical count uniformization criterion exposed** — count-frontier upgrade path, no bar movement. Added `ShiftedConnectingClusterCountBoundDim.ofAtFamily`, `ShiftedF3CountPackageDim.ofAtFamily`, and `PhysicalShiftedF3CountPackage.ofAtFamily` in `YangMills/ClayCore/ConnectingClusterCount.lean`. The physical constructor consumes `h_at : ∀ L [NeZero L], PhysicalShiftedConnectingClusterCountBoundAt L C_conn dim` with the same `C_conn` and `dim` for every finite volume, and returns the uniform `PhysicalShiftedF3CountPackage`. This pins the next genuine count task: upgrade the fixed-`L` canary to a family of bounds with volume-independent constants. Build `lake build YangMills.ClayCore.ConnectingClusterCount` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
