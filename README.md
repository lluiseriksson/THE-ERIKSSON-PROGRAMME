...
| **Last closed** | **v0.64.0 shifted KP series reaches `ClusterCorrelatorBound`** — pure-additive, no bar movement. Adds `TruncatedActivities.two_point_decay_from_cluster_tsum_shifted`, `clusterPrefactorShifted_rpow_ceil_le_exp`, and `clusterCorrelatorBound_of_truncatedActivities_ceil_shifted` in `YangMills/ClayCore/ClusterRpowBridge.lean`. This propagates v0.63.0's natural lattice-animal profile `C_conn * (n+1)^dim * A₀ * r^(n+dist)` from the series layer to the abstract F3 two-point decay bridge, using `clusterPrefactorShifted r C_conn A₀ dim` while preserving the same exponential decay shape. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
