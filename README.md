...
| **Last closed** | **v0.92.0 SU(N) Dirichlet contraction axiom converted to explicit input** — audit cleanup with one real axiom deletion, no Clay bar movement. Removed `sunDirichletForm_contraction` from `YangMills/P8_PhysicalGap/SUN_DirichletCore.lean`; the strong form is now `sunDirichletForm_isDirichletFormStrong_of_contraction`, which takes the normal-contraction estimate explicitly and builds `IsDirichletFormStrong`. This does not prove Beurling-Deny normal contraction; it stops declaring it globally. Build `lake build YangMills.P8_PhysicalGap.SUN_DirichletCore` green. Pinned oracle for the explicit-input wrapper: `[lieDerivReg_all, propext, sunGeneratorData, Classical.choice, Quot.sound, Experimental.LieSUN.matExp_traceless_det_one]`. Non-Experimental Lean axiom count now 3. No `sorry`. (2026-04-24) |
