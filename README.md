...
| **Last closed** | **v1.83.0 physical `d = 4` exponential F3 correlator endpoint** — consumer-side closure, no bar movement. Added `physicalClusterCorrelatorBound_of_expCountBound_mayerData_ceil` and `physicalClusterCorrelatorBound_of_physicalMayerData_expCount_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`. These consume `PhysicalShiftedConnectingClusterCountBoundExp C_conn K` plus Mayer/activity data and produce `PhysicalClusterCorrelatorBound N_c r (clusterPrefactorExp r K C_conn A₀)` under `0 < K` and `K * r < 1`. This lets a future proof land directly at the Clay-relevant physical `d = 4`, uniform-in-volume exponential count frontier, without first proving an all-dimensions count theorem. Remaining mathematical work: prove physical/global Mayer-Ursell activity decay and the uniform physical exponential count bound. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
