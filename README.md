...
| **Last closed** | **v1.45.0 global Mayer + physical count exposes auditable L8 bundle** — endpoint plumbing, no bar movement. Added `connectedCorrDecayBundle_of_globalMayer_physicalCount_siteDist_measurableF` plus mass/prefactor equations in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`. The route `mayer : ShiftedF3MayerPackage N_c wab` + `count : PhysicalShiftedF3CountPackage` now exposes an auditable `ConnectedCorrDecayBundle` before projecting to `ClayYangMillsPhysicalStrong`, with `m = kpParameter wab.r` and `C = clusterPrefactorShifted wab.r count.C_conn mayer.A₀ count.dim + 2 * exp (kpParameter wab.r)`. The live mathematical obligations remain the Mayer/Ursell package and the physical uniform count package. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
