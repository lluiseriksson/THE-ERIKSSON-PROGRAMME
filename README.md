...
| **Last closed** | **v2.00.0 decoder-form BFS target bridges to physical F3 count** — F3-count infrastructure, no bar movement. Added `PhysicalConnectingClusterExtraWalkDecoderBound`, an implementation-oriented restatement of the extra-walk coding target: provide a decoder from `PlaquetteWalk physicalClayDimension L n p` to physical plaquette finsets whose image covers every shifted connecting-cluster bucket element. Lean then chooses one preimage walk per bucket element; `physicalConnectingClusterExtraWalkCodeOfDecoder_injective` proves the chosen code is injective because decoding recovers the original bucket. Terminal bridge: `physicalShiftedConnectingClusterCountBoundExp_of_extraWalkDecoder : PhysicalConnectingClusterExtraWalkDecoderBound → PhysicalShiftedConnectingClusterCountBoundExp 1 1296`. The actual decoder/coverage theorem remains the open BFS/tree construction; downstream F3-count algebra is closed. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
