...
| **Last closed** | **v0.90.0 terminal ClayAssembly oracle pinned as core-only** — audit correction, no Clay bar movement. Updated `YangMills/P8_PhysicalGap/ClayAssembly.lean` comments and added `#print axioms` canaries for `yangMills_existence_massGap`, `clay_millennium_yangMills`, and `clay_millennium_yangMills_strong`. All three now print exactly `[propext, Classical.choice, Quot.sound]`; the old comment claiming a dependency on legacy `holleyStroock_sunGibbs_lsi` was stale. This does not change the honesty warning: `ClayYangMillsTheorem := ∃ m_phys : ℝ, 0 < m_phys` is still the weak/vacuous endpoint audited in v0.47.0; the non-vacuous Clay work remains at `ClayYangMillsPhysicalStrong`, `ClusterCorrelatorBound`, and the RG/LSI fronts. Build `lake build YangMills.P8_PhysicalGap.ClayAssembly` green. No new axioms. No `sorry`. (2026-04-24) |
