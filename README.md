...
| **Last closed** | **v1.42.0 `ofGlobalMayer` exposes physical activities by simp** — proof-script ergonomics, no bar movement. Added `PhysicalOnlyShiftedF3MayerCountPackage.ofGlobalMayer_toTruncatedActivities` in `YangMills/ClayCore/ClusterRpowBridge.lean`. Future analytic scripts can prove the older all-dimensions `ShiftedF3MayerPackage` first, combine it with a physical count package via `PhysicalOnlyShiftedF3MayerCountPackage.ofGlobalMayer`, and rewrite the resulting fully physical package activities directly as `mayer.toTruncatedActivities β F p q` without unfolding constructors. This tightens the global-Mayer → preferred `physicalClayDimension = 4` route while the live obligations remain `PhysicalShiftedF3MayerPackage N_c wab` and `PhysicalShiftedF3CountPackage`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; the new declaration prints `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
