...
| **Last closed** | **v1.82.0 packaged exponential F3 route to Clay mass-gap hub** — API/packaging closure, no bar movement. Added `clusterCorrelatorBound_of_expCountBound_mayerData_ceil`, `ShiftedF3MayerCountPackageExp`, its `ofSubpackages` / projection / `apply_count` canaries, and the endpoint chain `clusterCorrelatorBound_of_shiftedF3MayerCountPackageExp`, `clayWitnessHyp_of_shiftedF3MayerCountPackageExp`, `clayMassGap_of_shiftedF3MayerCountPackageExp`, `clayConnectedCorrDecay_of_shiftedF3MayerCountPackageExp`, and `clay_theorem_of_shiftedF3MayerCountPackageExp` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The package bundles `ConnectedCardDecayMayerData`, `ShiftedConnectingClusterCountBoundExp C_conn K`, and `K * wab.r < 1`; the authentic endpoint returns `ClayYangMillsMassGap N_c` with prefactor `clusterPrefactorExp wab.r K C_conn A₀`. Remaining mathematical work: prove Mayer/Ursell activity decay and the uniform exponential lattice-animal count estimate. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
