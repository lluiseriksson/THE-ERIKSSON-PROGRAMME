...
| **Last closed** | **v1.47.0 global shifted count restricts directly to physical count** — ClayCore API/frontier sharpening, no bar movement. Added `ShiftedF3CountPackage.toPhysical` plus `toPhysical_C_conn`, `toPhysical_dim`, and `toPhysical_apply` in `YangMills/ClayCore/ClusterRpowBridge.lean`. A future global count proof `count : ShiftedF3CountPackage` can now be consumed as `count.toPhysical : PhysicalShiftedF3CountPackage`, with `C_conn` and `dim` preserved by simp. This is the count-side analogue of `ShiftedF3MayerPackage.toPhysical`; the live mathematical obligations remain the Mayer/Ursell package and either the physical count package or the stronger global count package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
