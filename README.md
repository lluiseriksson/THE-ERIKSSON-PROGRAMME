...
| **Last closed** | **v1.36.0 physical Mayer-only F3 route exposed** — physical-frontier sharpening, no bar movement. Added `PhysicalConnectedCardDecayMayerData`, `PhysicalShiftedF3MayerPackage`, `physicalClusterCorrelatorBound_of_physicalMayerData_shiftedCount_ceil`, and `physicalClusterCorrelatorBound_of_physicalShiftedF3Subpackages` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The `d = 4` physical F3 route no longer needs global `ConnectedCardDecayMayerData` over all dimensions: it can consume Mayer/activity data only at `physicalClayDimension = 4`, uniformly in finite volume `L`, together with `PhysicalShiftedF3CountPackage`. Remaining physical obligations are now exactly physical Mayer data plus physical volume-uniform count. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
