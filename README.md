...
| **Last closed** | **v0.71.0 unpackaged authentic F3 mass-gap endpoint** — pure-additive, no bar movement. Adds `clayMassGap_of_shiftedCountBound_connectedCardDecayActivities_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`, giving the older raw `K / hK_abs_le / h_mayer` entry style the same non-vacuous output as the preferred packaged API: `ClayYangMillsMassGap N_c`. Internally it builds `ConnectedCardDecayMayerData` and routes through `clayMassGap_of_shiftedCountBound_mayerData_ceil`; the packaged API remains preferred, but both public F3 terminal styles now avoid making the weak `ClayYangMillsTheorem` the primary audited endpoint. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
