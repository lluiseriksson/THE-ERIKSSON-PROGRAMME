...
| **Last closed** | **v1.01.0 Boltzmann-integrability variant of the `PhysicalStrong` bridge** — structural bridge to the first non-vacuous Clay target, no bar movement. Added `physicalStrong_of_clayConnectedCorrDecay_siteDist_of_boltzmannIntegrable` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`: the v1.00 canonical-distance bridge no longer needs the caller to provide `IsProbabilityMeasure (gibbsMeasure ... β)` directly; it derives it from the standard Boltzmann-weight integrability hypothesis via `gibbsMeasure_isProbability`. Remaining local inputs stay explicit: `0 < β`, `|F| ≤ 1`, and integrability of `W_p`, `W_q`, and `W_p*W_q` under the tilted Gibbs measure. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-24) |
