...
| **Last closed** | **balabanHyps_from_wilson_activity end-to-end constructor (P2f-α)** — pure-additive end-to-end constructor `YangMills.balabanHyps_from_wilson_activity {N_c : Nat} [NeZero N_c] (wab : WilsonPolymerActivityBound N_c) (profile : LargeFieldProfile) (h_lf_bound_at) (h_dominated) : BalabanHyps N_c` in `YangMills/ClayCore/LargeFieldDominance.lean`, which closes the structural loop of the α-stack: takes a `WilsonPolymerActivityBound` (from which P2d-α produces the SFA) plus the analytic large-field inputs (`profile`, `h_lf_bound_at`, `h_dominated`) at the canonical `g_bar = wab.r`, and emits a full `BalabanHyps N_c` via `balabanHyps_of_bounds`. Constructs the LFA inline with constants matching `SmallFieldActivityBound.ofWilsonActivity` (`E₀ := A₀+1`, `κ := -log r`, `ḡ := r`) so `hg_eq / hk_eq / hE0_eq` close by `rfl rfl rfl`. Does **not** use `LargeFieldActivityBound.ofSuperPoly` (P2e-α, kept intact), which `Classical.choose`s its `g_bar` and breaks the equality with `wab.r`; the analytic large-field content (`h_lf_bound`, `h_dominated`) is instead left as caller-supplied hypotheses at the canonical `g_bar = wab.r`. α-stack now topologically saturated (P2c-α / P2d-α / P2d-β / P2e-α / P2f-α); further additive constructors will not move the README percentage. Pure additive: zero downstream breakage. Oracles of all five top-level decls in `LargeFieldDominance.lean` (`superPolyProfile`, `superPoly_dominance`, `superPoly_dominance_at_specific`, `LargeFieldActivityBound.ofSuperPoly`, `balabanHyps_from_wilson_activity`) print `[propext, Classical.choice, Quot.sound]` at build. Commit `3961827`, +45/−0. (2026-04-24) |
