...
| **Last closed** | **v0.51.0 F3 terminal wrapper — connected finite KP data to current Clay endpoint** — pure-additive, no bar movement. Adds `clay_theorem_of_connectedFiniteBounds_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`, composing the connected finite-sum bridge `clusterCorrelatorBound_of_connectedFiniteBounds_ceil` with `clay_theorem_from_wilson_activity`; `clusterPrefactor_pos` discharges the terminal prefactor positivity. The remaining F3 work is now exactly exposed as `h_mayer` (Mayer/Ursell identity), `h_zero` (disconnected support cancellation), and `h_connected_bound` (connected finite Kotecký-Preiss lattice-animal estimate). Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
