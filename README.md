...
| **Last closed** | **v1.66.0 direct Mayer finite-sum KP comparison** — finite-volume F3 infrastructure, no bar movement. Added `finiteConnectingSum_le_of_cardBucketBounds_tsum` and `finiteConnectingSum_le_of_cardBucketBounds_tsum_shifted` in `YangMills/ClayCore/ClusterRpowBridge.lean`: given disconnected-support cancellation and the usual bucketwise KP bounds on the canonical finite plaquette range, the finite Mayer connecting sum is bounded directly by the unshifted or shifted KP `tsum`. This packages v1.65's Mayer finite-sum bucket `tsum` identity into the immediate inequality consumer, so callers no longer have to route manually through the connected finite-sum API. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; both new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
