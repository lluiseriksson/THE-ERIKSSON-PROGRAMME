...
| **Last closed** | **v1.69.0 physical shifted endpoints use Mayer-facing KP consumer** — F3 proof-spine cleanup, no bar movement and no public statement changes. Refactored `physicalClusterCorrelatorBound_of_shiftedCountBound_mayerData_ceil` and `physicalClusterCorrelatorBound_of_physicalMayerData_shiftedCount_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean` so both bound the finite Mayer connecting sum directly via `finiteConnectingSum_le_of_cardBucketBounds_tsum_shifted` after rewriting `TruncatedActivities.connectingBound_eq_finset_sum`, instead of first proving a connected finite-sum estimate and transporting it across `finiteConnectingSum_eq_connectedFiniteSum`. This aligns the physical shifted F3 endpoints with the Mayer-facing shifted bucket-`tsum` proof spine. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; both declarations remain `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
