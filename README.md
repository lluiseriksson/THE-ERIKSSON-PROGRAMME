...
| **Last closed** | **v2.09.0 lattice-animal physical F3 handoff bridge** — F3 route infrastructure, no bar movement. Added `YangMills/ClayCore/LatticeAnimalF3Bridge.lean`, with `physicalOnlyShiftedF3MayerCountPackageExp_of_baselineExtraWordDecoderCovers1296` and `physicalClusterCorrelatorBound_of_baselineExtraWordDecoderCovers1296`. The live physical exponential route is now a named handoff: `PhysicalConnectingClusterBaselineExtraWordDecoderCovers1296` + `PhysicalShiftedF3MayerPackage N_c wab` + `(1296 : ℝ) * wab.r < 1` feeds the v2.08 physical-only exponential package and then `PhysicalClusterCorrelatorBound`. Simp canaries pin the produced count constants `C_conn = 1`, `K = 1296`. The remaining mathematical work is still constructing the baseline-plus-word decoder and the physical Mayer/activity package. Build `lake build YangMills.ClayCore.LatticeAnimalF3Bridge` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
