...
| **Last closed** | **v1.44.0 global Mayer + physical count feeds L8 directly** — endpoint plumbing, no bar movement. Added `physicalStrong_of_globalMayer_physicalCount_siteDist_measurableF` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`, sending `mayer : ShiftedF3MayerPackage N_c wab` plus `count : PhysicalShiftedF3CountPackage` directly to the non-vacuous `ClayYangMillsPhysicalStrong` endpoint at `physicalClayDimension = 4`, using `PhysicalOnlyShiftedF3MayerCountPackage.ofGlobalMayer` internally. Future scripts that prove the older all-dimensions Mayer package first no longer need to manually assemble the intermediate fully physical package before entering L8. The live mathematical obligations remain the Mayer/Ursell package and the physical uniform count package. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; the new declaration prints `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
