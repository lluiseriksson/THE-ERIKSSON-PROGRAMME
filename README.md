...
| **Last closed** | **v1.37.0 fully physical F3 package reaches `PhysicalStrong`** — physical-frontier sharpening, no bar movement. Added `PhysicalOnlyShiftedF3MayerCountPackage` in `YangMills/ClayCore/ClusterRpowBridge.lean`, bundling exactly `mayer : PhysicalShiftedF3MayerPackage N_c wab` plus `count : PhysicalShiftedF3CountPackage`, with package-level finite-volume consumers and `physicalClusterCorrelatorBound_of_physicalOnlyShiftedF3MayerCountPackage`. Added the L8 route in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`: `connectedCorrDecayBundle_of_physicalOnlyShiftedF3MayerCountPackage_siteDist_measurableF`, audit equalities for mass/prefactor, and `physicalStrong_of_physicalOnlyShiftedF3MayerCountPackage_siteDist_measurableF`, plus the subpackage variant. The physical `d = 4` Clay route now runs from the exact remaining F3 assumptions to `PhysicalClusterCorrelatorBound → ConnectedCorrDecayBundle → ClayYangMillsPhysicalStrong`. Remaining analytic obligations are unchanged: prove physical Mayer/Ursell data and physical volume-uniform shifted count for `N_c ≥ 2`. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
