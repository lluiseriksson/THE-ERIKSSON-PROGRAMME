...
| **Last closed** | **v0.73.0 `ClusterCorrelatorBound` to `ClayWitnessHyp`** — pure-additive, no bar movement. Adds `clayWitnessHyp_of_clusterCorrelatorBound` in `YangMills/ClayCore/ClusterCorrelatorBound.lean`, repackaging `ClusterCorrelatorBound N_c r C_clust` as the older analytic bundle `ClayWitnessHyp N_c` field-for-field. A future proof of `ClusterCorrelatorBound` now feeds `CharacterExpansionData`, `ClayYangMillsMassGap`, `ClayConnectedCorrDecay`, and `ClayWitnessHyp` without extra glue. Build `lake build YangMills.ClayCore.ClusterCorrelatorBound` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
