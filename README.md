...
| **Last closed** | **v2.06.0 physical 1296 decoder target specialization** — F3-count infrastructure, no bar movement. Added `PhysicalConnectingClusterBaselineExtraWordDecoderCovers1296` and `physicalShiftedConnectingClusterCountBoundExp_of_baselineExtraWordDecoderCovers1296` in `YangMills/ClayCore/LatticeAnimalCount.lean`. The live F3-count target is now the single physical statement `PhysicalConnectingClusterBaselineExtraWordDecoderCovers1296`; Lean proves it immediately implies `PhysicalShiftedConnectingClusterCountBoundExp 1 1296`. This removes the last decorative parameter from the BFS/Klarner target, leaving the next proof step focused purely on constructing the baseline-plus-word decoder. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
