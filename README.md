...
![Core build](https://img.shields.io/badge/lake_build_YangMillsCore-green_(8403_jobs)-success)
...
```bash
lake build YangMillsCore          # the verified core — green, 8403 jobs
...
checkpoint:** 2026-07-04, `origin/main` commit `a89feb98`.  The verification
...
core build remains
`lake build YangMillsCore` green at **8403 jobs**.
...
* the latest recorded full core build is still the 8403-job core build, while
  `origin/main` has advanced to `a89feb98`.
...
```mermaid
graph TD
    subgraph core["YangMillsCore  (8403 jobs, oracle-clean)"]
...
| Build the core | `lake build YangMillsCore` | `Build completed successfully (8403 jobs)` |
