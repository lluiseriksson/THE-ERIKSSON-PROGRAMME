...
| **Last closed** | **v1.78.0 `mono_count_dim` subpackage terminal canaries for F3** — F3 proof-spine/API wiring, no bar movement. Added `clusterCorrelatorBound_of_shiftedF3Subpackages_mono_count_dim`, `clayMassGap_of_shiftedF3Subpackages_mono_count_dim`, `clayConnectedCorrDecay_of_shiftedF3Subpackages_mono_count_dim`, `clay_theorem_of_shiftedF3Subpackages_mono_count_dim`, plus mass/prefactor projection canaries in `YangMills/ClayCore/ClusterRpowBridge.lean`. If the Mayer/activity half and the count half are produced independently, a later count-dimension raise can now reach the correlator, authentic mass-gap structure, connected-decay hub, or weak theorem endpoint directly without first constructing/unfolding a combined package. The open mathematical target remains unchanged: the uniform lattice-animal / KP count estimate. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; all eight new declarations remain `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
