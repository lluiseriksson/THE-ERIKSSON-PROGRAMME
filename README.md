...
| **Last closed** | **v1.10.0 uniform-rpow small-β frontier lands directly in `PhysicalStrong`** — terminal bridge cleanup, no bar movement. Added `physicalStrong_of_uniformRpow_small_beta_siteDist_measurableF` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`: `0 < β₀`, `β₀ < 1`, `0 < C`, and `WilsonUniformRpowBound N_c β₀ C` now yield the canonical site-distance `ClayYangMillsPhysicalStrong` endpoint for `(sunHaarProb N_c)` and `wilsonPlaquetteEnergy N_c` under the local assumptions `0 < β`, `|F| ≤ 1`, and `Measurable F`. This composes the small-β route in `ZeroMeanCancellation.lean` with the v1.06-v1.09 L8 physical bridge; the remaining analytic input is still the Mayer/Kotecky-Preiss proof of `WilsonUniformRpowBound`. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
