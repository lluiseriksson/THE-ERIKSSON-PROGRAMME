...
| **Last closed** | **v0.63.0 shifted KP series for nonempty cardinality buckets** — pure-additive, no bar movement. Adds shifted KP support in `YangMills/ClayCore/ClusterSeriesBound.lean`: `connecting_cluster_tsum_summable_shifted`, `connecting_cluster_partial_sum_le_tsum_shifted`, `clusterPrefactorShifted`, `clusterPrefactorShifted_pos`, and `connecting_cluster_tsum_le_shifted`. This supports the natural lattice-animal profile `C_conn * (n+1)^dim * A₀ * r^(n+dist)` alongside the older `C_conn * n^dim * A₀ * r^(n+dist)` form, avoiding the artificial zero bound at the minimal bucket `n = 0` when `dim > 0`. Build `lake build YangMills.ClayCore.ClusterSeriesBound` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
