...
| **Last closed** | **v1.15.0 shifted F3 Mayer/count package field coherence exposed** — package-coherence cleanup, no bar movement. Added `[simp]` field lemmas in `YangMills/ClayCore/ClusterRpowBridge.lean` for `ShiftedF3MayerCountPackage.ofSubpackages`, `mayerPackage`, and `countPackage`: combining independent Mayer/count packages and projecting them now recovers `A₀`, `data`, `C_conn`, and `dim` without unfolding definitions. This reduces downstream friction exactly at the F3 assembly boundary; the mathematical frontier remains constructing the nonabelian Mayer package and the global shifted count package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
