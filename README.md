...
| **Last closed** | **v0.99.0 local bounded-observable Wilson connected-correlation estimate** — analytic glue, no Clay bar movement. Added `wilsonConnectedCorr_abs_le_two_of_unitBound` in `YangMills/ClayCore/ClusterCorrelatorBound.lean`: for `|F| ≤ 1`, it proves `|wilsonConnectedCorr ... β F p q| ≤ 2` from explicit inputs that the tilted Gibbs measure is a probability measure and the one-point/two-point Wilson observables are integrable. This does not replace F1/F2/F3 or prove exponential decay; it supplies the local finite-distance bound needed later when combining separated-distance cluster decay with all-pair physical mass-profile statements. Builds `lake build YangMills.ClayCore.ClusterCorrelatorBound` and `lake build YangMills.ClayCore.ClusterRpowBridge` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-24) |
