...
| **Last closed** | **v1.14.0 global shifted count package projects transparently to finite volumes** — F3-count interface cleanup, no bar movement. Added `ShiftedF3CountPackage.toAt_C_conn`, `ShiftedF3CountPackage.toAt_dim`, and `ShiftedF3CountPackage.toAt_apply` in `YangMills/ClayCore/ClusterRpowBridge.lean`: a global `ShiftedF3CountPackage` restricted by `pkg.toAt d L` now definitionally preserves `C_conn` and `dim`, and has a direct finite-volume application theorem. Once the global uniform lattice-animal package is proved, finite-volume consumers can inspect and apply its restriction without unfolding `toAt`. The remaining mathematical content is still `ShiftedConnectingClusterCountBound` itself. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
