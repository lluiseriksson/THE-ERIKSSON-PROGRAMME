...
- **What's next.** `ClusterCorrelatorBound`: the F3 summability/factoring bridge now reaches the exact Wilson-facing target, and the canonical `⌈siteLatticeDist⌉₊` geometry is closed. The live analytic inputs are the Mayer/Ursell identity for `wilsonConnectedCorr` and the `connectingBound` cluster-series estimate.
...
| **Last closed** | **v0.48.0 F3 bridge — truncated activities → `ClusterCorrelatorBound`** — pure-additive, no bar movement; proves `clusterCorrelatorBound_of_truncatedActivities` and the canonical-ceiling variant `clusterCorrelatorBound_of_truncatedActivities_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The theorem composes `TruncatedActivities.abs_connectingSum_le_connectingBound` + `connecting_cluster_tsum_le` and lands the exact Wilson-facing target `ClusterCorrelatorBound N_c r (clusterPrefactor r C_conn A₀ dim)`. The canonical variant also proves the `⌈siteLatticeDist⌉₊` geometric comparison via `clusterPrefactor_rpow_ceil_le_exp`, so the live F3 inputs are now isolated to the Mayer/Ursell identity for `wilsonConnectedCorr` and the `connectingBound` cluster-series estimate. Root `lake build YangMills` is green after namespace-collision cleanup. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
