...
| **Last closed** | **v0.49.0 F3 counting interface — canonical buckets + finite `connectingBound`** — pure-additive, no bar movement. Adds `ceil_siteLatticeDist_le_polymer_card` (`PolymerDiameterBound.lean`), `connected_polymer_card_eq_extra_add_dist` (`ConnectingClusterCount.lean`), and `TruncatedActivities.connectingBound_eq_finset_sum` (`MayerExpansion.lean`). These expose the exact `n + ⌈siteLatticeDist p.site q.site⌉₊` cardinality buckets for connected polymers containing `p,q`, and rewrite the abstract Mayer `connectingBound` as a finite sum over polymers containing `p,q` whenever the index type is finite. This aligns the finite Mayer bound, polymer-diameter geometry, and lattice-animal counting interface required by the remaining Kotecký-Preiss estimate. Root `lake build YangMills` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
