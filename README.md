...
| **Last closed** | **v1.81.0 exponential F3 bridge reaches `ClusterCorrelatorBound`** — bridge infrastructure, no bar movement. Extended the KP-compatible exponential route in `YangMills/ClayCore/ClusterRpowBridge.lean` with `TruncatedActivities.two_point_decay_from_cluster_tsum_exp`, `clusterPrefactorExp_rpow_ceil_le_exp`, `clusterCorrelatorBound_of_truncatedActivities_ceil_exp`, `finiteConnectingSum_le_of_cardBucketBounds_tsum_exp`, `cardBucketSum_le_of_count_and_pointwise_exp`, `clusterCorrelatorBound_of_cardBucketBounds_ceil_exp`, and `clusterCorrelatorBound_of_count_cardDecayBounds_ceil_exp`. The public endpoint consumes `ShiftedConnectingClusterCountBoundExp C_conn K` plus global activity decay `(T β F p q).K_bound Y ≤ A₀ * r ^ Y.card` and yields `ClusterCorrelatorBound N_c r (clusterPrefactorExp r K C_conn A₀)` under `0 < K` and `K * r < 1`. The remaining mathematical work is still the uniform exponential lattice-animal count estimate and Mayer/Ursell activity package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
