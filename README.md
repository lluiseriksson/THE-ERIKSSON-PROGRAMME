...
| **Last closed** | **v1.74.0 `mono_count_dim` application canaries for combined F3 packages** — F3 package ergonomics, no bar movement. Added `PhysicalOnlyShiftedF3MayerCountPackage.mono_count_dim_apply_count`, `PhysicalShiftedF3MayerCountPackage.mono_count_dim_apply_count`, and `ShiftedF3MayerCountPackage.mono_count_dim_apply_count` in `YangMills/ClayCore/ClusterRpowBridge.lean`, making package-level exponent-raising directly usable without manual projection. After `mono_count_dim k`, the count estimate is available at the unchanged count constant and exponent `dim + k`. The open mathematical target remains unchanged: the uniform lattice-animal / KP count estimate. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; all three new declarations remain `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
