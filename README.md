...
| **Last closed** | **v0.55.0 F3 terminal bucket-estimate wrappers** — pure-additive, no bar movement. Adds `clusterCorrelatorBound_of_cardBucketBounds_ceil` and `clay_theorem_of_cardBucketBounds_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`, composing the bucket decomposition, KP partial-sum comparison, and connected finite bridge. The F3 consumer now takes the exact lattice-animal bucket shape `bucket_sum(n,p,q) ≤ C_conn * n^dim * A₀ * r^(n + ⌈dist⌉₊)`; together with `h_mayer` and disconnected support cancellation `h_zero`, this yields `ClusterCorrelatorBound`, and with `WilsonPolymerActivityBound` the current `ClayYangMillsTheorem` endpoint. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
