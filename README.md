...
| **Last closed** | **v0.83.0 name the uniform rpow small-beta frontier** — pure-additive, no bar movement. Adds `WilsonUniformRpowBound N_c β C` and `yang_mills_final_small_beta_of_uniformRpow` in `YangMills/ClayCore/ZeroMeanCancellation.lean`. The new predicate names the exact uniform rpow-shape connected-correlator input consumed by the small-β terminal wrapper: one constant `C` and one scale `β` work uniformly across all finite lattices, positive inverse couplings, bounded test functions, and plaquette pairs. This deliberately distinguishes the terminal input from the looser `WilsonLinkIndependence` predicate, where the constant is existential at each call site. Build `lake build YangMills.ClayCore.ZeroMeanCancellation` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
