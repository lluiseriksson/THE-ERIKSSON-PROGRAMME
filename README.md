...
| **Last closed** | **v1.96.0 finite neighbor-choice coding from branching bounds** — F3-count infrastructure, no bar movement. Added `finsetCodeOfCardLe` / `finsetCodeOfCardLe_injective` and `PlaquetteNeighborChoiceCodeBoundDim` to `YangMills/ClayCore/LatticeAnimalCount.lean`. Any finset `s` with `s.card ≤ D` now injects into `Fin D` by enumerating `s` as `Fin s.card` via `Finset.equivFin` and casting along the bound. Applied to the plaquette graph, this turns `PlaquetteGraphBranchingBoundDim d D` into a concrete per-step neighbor alphabet `PlaquetteNeighborChoiceCodeBoundDim d D`, with physical endpoint `PlaquetteNeighborChoiceCodeBoundDim physicalClayDimension 1296`. This supplies the local alphabet for the next bridge `PlaquetteNeighborChoiceCodeBoundDim d D → PlaquetteWalkCodeBoundDim d D`, which will assemble per-step choices into an injective finite-walk word code and hence a `D^n` walk-count bound. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
