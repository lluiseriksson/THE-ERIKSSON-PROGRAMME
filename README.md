...
| **Last closed** | **v1.07.0 direct `ClusterCorrelatorBound` to `PhysicalStrong` bridge** — structural bridge cleanup to the first non-vacuous Clay target, no bar movement. Added `physicalStrong_of_clusterCorrelatorBound_siteDist_measurableF` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`: `ClusterCorrelatorBound N_c r C_clust`, `0 < r`, `r < 1`, `0 < C_clust`, `0 < β`, `|F| ≤ 1`, and `Measurable F` now directly yield the canonical site-distance `ClayYangMillsPhysicalStrong` endpoint for `(sunHaarProb N_c)` and `wilsonPlaquetteEnergy N_c`. All finite-volume regularity boilerplate is handled internally by v1.02-v1.06. F1/F2/F3 remain the active analytic front. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
