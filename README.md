...
| **Last closed** | **v1.80.0 exponential KP series prefactor for F3 counts** — consumer-side companion to the exponential count frontier, no bar movement. Added `connecting_cluster_tsum_summable_exp`, `connecting_cluster_summand_nonneg_exp`, `connecting_cluster_partial_sum_le_tsum_exp`, `connecting_cluster_tsum_eq_factored_exp`, `inner_sum_pos_exp`, `clusterPrefactorExp`, `clusterPrefactorExp_pos`, and `connecting_cluster_tsum_le_exp` in `YangMills/ClayCore/ClusterSeriesBound.lean`. These factor `∑' n, C_conn * K^n * A₀ * r^(n + dist)` as `clusterPrefactorExp r K C_conn A₀ * r^dist` under `0 < r`, `0 < K`, and `K * r < 1`. The open mathematical target remains the uniform exponential lattice-animal count estimate and its bridge into `ClusterCorrelatorBound`. Build `lake build YangMills.ClayCore.ClusterSeriesBound` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
