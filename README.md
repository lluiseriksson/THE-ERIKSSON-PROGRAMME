...
| **Last closed** | **v1.08.0 shifted F3 packages land directly in `PhysicalStrong`** — structural bridge packaging to the first non-vacuous Clay target, no bar movement. Added `physicalStrong_of_shiftedF3MayerCountPackage_siteDist_measurableF` and `physicalStrong_of_shiftedF3Subpackages_siteDist_measurableF` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`: a `WilsonPolymerActivityBound N_c` plus either a `ShiftedF3MayerCountPackage N_c wab`, or independent `ShiftedF3MayerPackage N_c wab` and `ShiftedF3CountPackage`, now yields the canonical site-distance `ClayYangMillsPhysicalStrong` endpoint for `(sunHaarProb N_c)` and `wilsonPlaquetteEnergy N_c` under the local assumptions `0 < β`, `|F| ≤ 1`, and `Measurable F`. This composes the shifted F3 `ClusterCorrelatorBound` producers with the v1.07 direct physical bridge. The live analytic work is now sharply exposed: construct the shifted F3 packages themselves (Mayer/Ursell identity plus Kotecky-Preiss count/series content). Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
