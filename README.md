...
| **Last closed** | **v0.80.0 packaged local finite-volume count data** — combinatorial audit, no bar movement. Adds `ShiftedF3CountPackageAt d L` in `YangMills/ClayCore/ConnectingClusterCount.lean`, packaging fixed-lattice shifted count data as `C_conn`, `hC`, `dim`, and `h_count : ShiftedConnectingClusterCountBoundAt d L C_conn dim`. The constructor `ShiftedF3CountPackageAt.finite` fills it with the v0.79 finite-volume bound (`C_conn = Fintype.card (Finset (ConcretePlaquette d L)) + 1`, `dim = 0`). This keeps the already-formal finite-volume layer reusable while leaving the global uniform `ShiftedConnectingClusterCountBound` as the genuine F3-count burden. Build `lake build YangMills.ClayCore.ConnectingClusterCount` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
