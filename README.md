...
| **Last closed** | **v1.40.0 physical Mayer package exposes its own consumers** — proof-script ergonomics, no bar movement. Added `PhysicalShiftedF3MayerPackage.toTruncatedActivities`, its `K` simp lemma, card-decay bound, disconnected-support cancellation, and Wilson connected-correlator identity in `YangMills/ClayCore/ClusterRpowBridge.lean`. Future physical Mayer/Ursell scripts can now work directly with `PhysicalShiftedF3MayerPackage N_c wab` before any count package is available. The two remaining physical F3 halves stay independently usable: `PhysicalShiftedF3MayerPackage N_c wab` and `PhysicalShiftedF3CountPackage`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
