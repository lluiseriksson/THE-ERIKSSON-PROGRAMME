...
| **Last closed** | **v0.58.0 F3 terminal endpoint from count + pointwise bounds** — pure-additive, no bar movement. Adds `clay_theorem_of_count_pointwiseBounds_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`, composing the count+pointwise `ClusterCorrelatorBound` bridge with `clay_theorem_from_wilson_activity`. With a `WilsonPolymerActivityBound`, the current `ClayYangMillsTheorem` endpoint follows directly from four factored F3 inputs: Mayer/Ursell identity `h_mayer`, disconnected support cancellation `h_zero`, lattice-animal bucket count `h_count`, and pointwise polymer activity estimate `h_pointwise`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
