...
| **Last closed** | **v0.91.0 Hille-Yosida semigroup axiom removed from P8** — audit cleanup with one real axiom deletion, no Clay bar movement. Removed `hille_yosida_semigroup` from `YangMills/P8_PhysicalGap/MarkovSemigroupDef.lean`; `StroockZegarlinski.lean` and `SUN_LiebRobin.lean` now take their `SymmetricMarkovTransport` inputs explicitly instead of constructing them from a global axiom. This does not formalize Beurling-Deny / Hille-Yosida; it localizes the C₀-semigroup construction as caller-supplied data. Builds `lake build YangMills.P8_PhysicalGap.StroockZegarlinski` and `lake build YangMills.P8_PhysicalGap.SUN_LiebRobin` green. Pinned oracles for `sz_lsi_to_clustering_bridge`, `sz_lsi_to_clustering`, `lsi_to_spectralGap`, and `sun_locality_to_covariance`: `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count now 4. No `sorry`. (2026-04-24) |
