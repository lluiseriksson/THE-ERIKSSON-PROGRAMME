...
| **Last closed** | **v1.06.0 concrete Wilson Boltzmann integrability discharged** — structural bridge cleanup to the first non-vacuous Clay target, no bar movement. Added `wilsonPlaquetteEnergy_boltzmann_integrable` and `physicalStrong_of_clayConnectedCorrDecay_siteDist_measurableF` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`. For the concrete Wilson plaquette energy, the finite-volume Boltzmann factor is integrable for `0 ≤ β` by boundedness of `wilsonPlaquetteEnergy N_c` and finiteness of the plaquette set. The cleanest bridge now takes only `ClayConnectedCorrDecay N_c`, `0 < β`, `|F| ≤ 1`, and `Measurable F`, and derives Gibbs probability, local Wilson-observable measurability/integrability, and the canonical site-distance `ClayYangMillsPhysicalStrong` endpoint. F1/F2/F3 remain the active analytic front. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-24) |
