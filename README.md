...
| **Last closed** | **v0.49.0 F3 counting interface — canonical buckets + finite `connectingBound`** — pure-additive, no bar movement. Adds `ceil_siteLatticeDist_le_polymer_card` (`PolymerDiameterBound.lean`), `connected_polymer_card_eq_extra_add_dist` (`ConnectingClusterCount.lean`), `TruncatedActivities.connectingBound_eq_finset_sum` (`MayerExpansion.lean`), and `clusterCorrelatorBound_of_finiteConnectingBounds_ceil` (`ClusterRpowBridge.lean`). These expose the exact `n + ⌈siteLatticeDist p.site q.site⌉₊` buckets for connected polymers containing `p,q`, rewrite the abstract Mayer `connectingBound` as a finite sum over polymers containing `p,q`, and give a Wilson-facing bridge that consumes that finite-sum estimate directly to produce `ClusterCorrelatorBound`. The remaining KP task is now stated in finite lattice-animal form. Root `lake build YangMills` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
