...
![Core build](https://img.shields.io/badge/lake_build_YangMillsCore-green_(8400_jobs)-success)
...
```bash
lake build YangMillsCore          # the verified core — green, 8400 jobs
...
checkpoint:** 2026-07-04, `origin/main` commit `692d3cf8`.  The verification
...
core build remains
`lake build YangMillsCore` green at **8400 jobs**.
...
* the latest recorded full core build is still the 8400-job core build, while
  `origin/main` has advanced to `692d3cf8`.
...
```mermaid
graph TD
    subgraph core["YangMillsCore  (8400 jobs, oracle-clean)"]
...
| Build the core | `lake build YangMillsCore` | `Build completed successfully (8400 jobs)` |
