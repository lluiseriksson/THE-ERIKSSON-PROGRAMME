...
| **Last closed** | **v0.75.0 single preferred shifted F3 package** — pure-additive, no bar movement. Adds `ShiftedF3MayerCountPackage N_c wab` and projections in `YangMills/ClayCore/ClusterRpowBridge.lean`. The remaining preferred F3 work is now one named Lean object bundling constants, `ConnectedCardDecayMayerData`, and `ShiftedConnectingClusterCountBound`; once supplied, it yields `ClusterCorrelatorBound`, `ClayWitnessHyp`, `ClayYangMillsMassGap`, `ClayConnectedCorrDecay`, and `ClayYangMillsTheorem`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
