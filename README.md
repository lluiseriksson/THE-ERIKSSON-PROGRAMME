...
| **Last closed** | **v2.05.0 baseline-plus-word decoder bridge** — F3-count infrastructure, no bar movement. Added `PhysicalConnectingClusterBaselineExtraWordDecoderCovers K` in `YangMills/ClayCore/LatticeAnimalCount.lean`, combining the distance-baseline correction with the finite-word BFS/Klarner bridge. A decoded bucket has shape `baseline ∪ decodeExtra word`, where `word : Fin n → Fin K` encodes the `n` extra plaquettes; Lean proves this target implies `PhysicalShiftedConnectingClusterCountBoundExp 1 K`. For the physical route the live target is now `PhysicalConnectingClusterBaselineExtraWordDecoderCovers 1296`. This avoids both audit traps: the cluster is not forced to be the literal range of a length-`n` walk, and the code object need not itself be a graph walk. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
