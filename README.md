...
| **Last closed** | **v0.96.0 SUN locality detached from experimental Dirichlet sidecar** — audit cleanup, no Clay bar movement. Removed the unused `SUN_DirichletCore` import from `YangMills/P8_PhysicalGap/SUN_LiebRobin.lean`; `sun_locality_to_covariance` is explicit-input and only needs the supplied Markov transport, variance decay, and Lieb-Robinson hypotheses. Build `lake build YangMills.P8_PhysicalGap.SUN_LiebRobin` green; pinned oracle is `[propext, Classical.choice, Quot.sound]`, so the locality theorem no longer inherits experimental Lie-derivative axioms through an unused import. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-24) |
