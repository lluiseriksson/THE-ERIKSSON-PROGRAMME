...
| **Last closed** | **v0.65.0 shifted bucket-count consumers for F3** — pure-additive, no bar movement. Adds `connectedFiniteSum_le_of_cardBucketBounds_shifted`, `connectedFiniteSum_le_of_cardBucketBounds_kp_shifted`, and `cardBucketSum_le_of_count_and_pointwise_shifted` in `YangMills/ClayCore/ClusterRpowBridge.lean`. These consume the realistic lattice-animal count `#bucket(n) ≤ C_conn * (n+1)^dim`, carrying the nonzero minimal-bucket profile through connected finite sums, cardinal buckets, count+pointwise estimates, and the shifted KP series. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
