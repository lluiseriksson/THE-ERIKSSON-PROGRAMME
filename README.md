...
| **Last closed** | **v0.54.0 F3 KP partial-sum comparison discharged** — pure-additive, no bar movement. Adds `connecting_cluster_summand_nonneg` and `connecting_cluster_partial_sum_le_tsum` in `YangMills/ClayCore/ClusterSeriesBound.lean`, plus `connectedFiniteSum_le_of_cardBucketBounds_kp` in `ClusterRpowBridge.lean`. The connected finite F3 consumer now discharges the standard nonnegative finite-partial-sum-to-`tsum` comparison internally; callers only need the actual lattice-animal bucket estimate. Builds green for `YangMills.ClayCore.ClusterSeriesBound` and `YangMills.ClayCore.ClusterRpowBridge`. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
