...
| **Last closed** | **LargeFieldActivityBound.ofSuperPoly constructor (P2e-α)** — the P2a analytic core (`superPoly_dominance_at_specific`, v0.41.0) and the P2c fixed-`E0` struct shape (v0.42.0) are wired together into a first-class term-mode constructor `YangMills.LargeFieldActivityBound.ofSuperPoly {N_c : Nat} [NeZero N_c] (A hA p hp E hE kappa hkappa h_lf_bound_at) : LargeFieldActivityBound N_c` in `YangMills/ClayCore/LargeFieldDominance.lean`. Extracts the small-enough-coupling witness via `Classical.choose` / `Classical.choose_spec` on `superPoly_dominance_at_specific hA hp hE`, packages `superPolyProfile A hA p hp` into `profile`, routes `h_lf_bound := h_lf_bound_at g spec.1 spec.2.1`, and discharges `h_dominated` via one-step sign normalisation (`-(A * …) = -A * …` by `ring`) + `exact spec.2.2`. Pure additive: zero downstream breakage. Oracle of `LargeFieldActivityBound.ofSuperPoly` prints `[propext, Classical.choice, Quot.sound]` at build. Commit `3be3c4c`, +39/−0. (2026-04-23) |
