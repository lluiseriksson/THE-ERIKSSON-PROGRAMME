...
| **Last closed** | **v0.62.0 F3 endpoint from connected cardinality-decay activities** — pure-additive, no bar movement. Adds `TruncatedActivities.ofConnectedCardDecay` and terminal wrapper `clay_theorem_of_count_connectedCardDecayActivities_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The constructor packages raw truncated activities with support-shaped bound `|K Y| ≤ if p ∈ Y ∧ q ∈ Y ∧ PolymerConnected Y then A₀ * r^Y.card else 0`; disconnected `K_bound` cancellation, global cardinality decay, and finite-volume summability then follow by construction. The terminal F3 endpoint now consumes only the raw connected cardinality-decay activity bound, the Mayer/Ursell identity for the constructed activity, and the lattice-animal bucket count. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
