...
| **Last closed** | **v0.97.0 experimental Dirichlet sidecar kept out of the main umbrella** — audit cleanup, no Clay bar movement. Removed `SUN_DirichletCore` and `SUN_DirichletForm` from the root aggregator `YangMills.lean`; both files remain available by direct import, but they currently depend on `YangMills.Experimental.LieSUN.LieDerivReg_v4` for concrete SU(N) Lie-derivative data and regularity. This keeps `lake build YangMills` from inheriting experimental Lie-derivative axioms through a convenience import. The concrete Dirichlet/Lie-derivative sidecar is still an explicit frontier, not mathematically closed. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-24) |
