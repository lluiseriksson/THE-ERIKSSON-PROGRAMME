...
| **Last closed** | **v1.27.0 physical `ClusterCorrelatorBound` endpoint exposed** — F3 physical-route sharpening, no bar movement. Added `PhysicalClusterCorrelatorBound` plus constructors from `ConnectedCardDecayMayerData + PhysicalShiftedConnectingClusterCountBound` and from `ShiftedF3MayerPackage + PhysicalShiftedF3CountPackage` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The new target is the `d = 4` analogue of `ClusterCorrelatorBound`: uniform over finite volumes `L`, with constants allowed to be physical-dimension constants. This is weaker than the all-dimensions global route by design, and gives the honest output of the four-dimensional F3 count frontier before terminal L8 packaging. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new constructors print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
