...
| **Last closed** | **v2.12.0 graph-animal shifted count interface** — F3-count frontier sharpening, no bar movement. Added `plaquetteGraphPreconnectedConnectingSubsetsShifted`, `PhysicalConnectingClusterGraphAnimalShiftedCountBound K`, and `physicalShiftedConnectingClusterCountBoundExp_of_graphAnimalShiftedCount` in `YangMills/ClayCore/LatticeAnimalCount.lean`. The remaining physical count task is now the pure combinatorial statement `#(preconnected plaquetteGraph animals containing p,q with card = n + ⌈dist(p,q)⌉₊) ≤ K^n`; proving it for `K = 1296` directly discharges `PhysicalShiftedConnectingClusterCountBoundExp 1 1296` and feeds the existing v2.09/v2.10 route. Remaining work: prove the shifted graph-animal estimate itself via BFS/Klarner baseline-plus-extra coding. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
