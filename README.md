...
| **Last closed** | **v1.53.0 Mayer/count F3 packages absorb count-dimension increases** — F3 package-level normalization, no bar movement. Added `mono_count_dim` wrappers for `PhysicalOnlyShiftedF3MayerCountPackage`, `PhysicalShiftedF3MayerCountPackage`, and global `ShiftedF3MayerCountPackage`, with simp lemmas pinning the Mayer half unchanged, `A₀` unchanged where present, `C_conn` unchanged, and `dim`/`count.dim = old dim + k`. This lets downstream F3 endpoint scripts absorb extra polynomial count factors directly at the combined package level rather than unpacking and rebuilding Mayer/count records. The live mathematical obligations remain unchanged: construct the volume-uniform shifted count package and the Mayer/Ursell package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; all thirteen new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
