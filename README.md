...
| **Last closed** | **v0.76.0 single-package F3 mass/prefactor canaries** — pure-additive, no bar movement. Adds four regression canaries in `YangMills/ClayCore/ClusterRpowBridge.lean` proving that the single-package F3 endpoints extract exactly `mass = kpParameter wab.r` and `prefactor = clusterPrefactorShifted wab.r pkg.C_conn pkg.A₀ pkg.dim` for both `ClayYangMillsMassGap` and `ClayConnectedCorrDecay`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
