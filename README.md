...
| **Last closed** | **v2.17.0 graph-animal word-decoder route to L8 physical-strong endpoint** — terminal F3 routing, no bar movement. The v2.16 target `PhysicalConnectingClusterGraphAnimalShiftedWordDecoderBound1296` now composes through `YangMills/ClayCore/LatticeAnimalF3Bridge.lean` to `PhysicalClusterCorrelatorBound`, and through `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean` to `ClayYangMillsPhysicalStrong` via `physicalStrong_of_graphAnimalWordDecoder1296_siteDist_measurableF`. Remaining mathematical work: construct the actual BFS/Klarner-style decoder, construct the physical Mayer package, and prove `(1296 : ℝ) * wab.r < 1`. Builds `lake build YangMills.ClayCore.LatticeAnimalF3Bridge` and `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
