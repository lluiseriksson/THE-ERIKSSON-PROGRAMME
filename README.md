...
| **Last closed** | **v0.95.0 final non-Experimental Lean axiom removed** — audit cleanup with one real axiom deletion, no Clay bar movement. Removed `physical_rg_rates_from_E26` from `YangMills/ClayCore/BalabanRG/PhysicalRGRates.lean`; the file now keeps only the `PhysicalRGRates` interface and projection theorems, while the direct witness lives in `PhysicalRGRatesWitness.lean` and is assembled from the current rate-side theorems (`rho_exp_contractive_from_E26`, `rho_in_unit_interval_from_E26`, `cP_linear_lb_from_E26`, `cLSI_linear_lb_from_E26`). `BalabanRGAxiomReduction.lean` now routes legacy public names through `physicalBalabanRGPackage` / `physical_uniform_lsi`. Builds green for `PhysicalRGRates`, `PhysicalRGRatesWitness`, and `BalabanRGAxiomReduction`. `git grep -n -E "^axiom " -- "*.lean" | Select-String -NotMatch "Experimental"` is empty. Non-Experimental Lean axiom count now 0. No `sorry`. This is an axiom-keyword cleanup, not a claim that the strong Clay physical target is fully closed. (2026-04-24) |
