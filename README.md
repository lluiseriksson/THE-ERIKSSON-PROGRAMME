...
| **Last closed** | **v1.87.0 plaquette site-ball orientation split** — F3-count infrastructure, no bar movement. Extended `YangMills/ClayCore/LatticeAnimalCount.lean` with `siteNeighborBall` and `plaquetteSiteBall_card_le_siteNeighborBall_card_mul_dir_sq`: the plaquette local bucket injects into `siteNeighborBall d L p.site × Fin d × Fin d`, so `(plaquetteSiteBall d L p).card ≤ (siteNeighborBall d L p.site).card * d * d`. This separates the remaining geometric site-neighborhood estimate from the pure plaquette-orientation factor, setting up the local branching constant for the later walk/tree encoding proof of `ShiftedConnectingClusterCountBoundExp`. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
