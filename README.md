...
| **Last closed** | **v1.63.0 direct bucket-`tsum` KP comparison** — finite-volume F3 infrastructure, no bar movement. Added `connectedFiniteSum_le_of_cardBucketBounds_tsum` and `connectedFiniteSum_le_of_cardBucketBounds_tsum_shifted` in `YangMills/ClayCore/ClusterRpowBridge.lean`: given the usual bucketwise KP bound on the canonical finite plaquette range, the connected finite sum is compared directly against the corresponding infinite KP series by rewriting it as the bucket `tsum`, applying termwise `Summable.tsum_le_tsum`, and using zero-outside-range plus KP nonnegativity off the finite range. This packages the v1.59-v1.62 bucket-series API into direct consumers; it does not prove the uniform lattice-animal estimate. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; both new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
