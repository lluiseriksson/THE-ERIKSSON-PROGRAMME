...
| **Last closed** | **v1.21.0 shifted F3 subpackage endpoints expose constants** — endpoint transparency for the preferred split F3 workflow, no bar movement. Added constant-projection lemmas in `YangMills/ClayCore/ClusterRpowBridge.lean`: `clayMassGap_of_shiftedF3Subpackages_mass_eq`, `clayMassGap_of_shiftedF3Subpackages_prefactor_eq`, `clayConnectedCorrDecay_of_shiftedF3Subpackages_mass_eq`, and `clayConnectedCorrDecay_of_shiftedF3Subpackages_prefactor_eq`. Once Mayer and count are proved independently, the resulting mass-gap and connected-decay objects expose decay rate `kpParameter wab.r` and prefactor `clusterPrefactorShifted wab.r count.C_conn mayer.A₀ count.dim` without unfolding package assembly. The mathematical frontier remains constructing the nonabelian Mayer package and global shifted count package for `N_c ≥ 2`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
