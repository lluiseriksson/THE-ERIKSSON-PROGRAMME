...
| **Last closed** | **v1.93.0 neighbor-finset branching form** — F3-count infrastructure, no bar movement. Repackaged the v1.92 plaquette-graph degree bound into the BFS-ready neighbor-cardinality API in `YangMills/ClayCore/LatticeAnimalCount.lean`: `plaquetteGraph_neighborFinset_card_le_ternary` proves `((plaquetteGraph d L).neighborFinset p).card ≤ (3 ^ d) * Fintype.card (Fin d) * Fintype.card (Fin d)`, and `plaquetteGraph_neighborFinset_card_le_physical_ternary` specializes this to `≤ 1296` in physical dimension 4. This is the finite-choice form needed to turn local branching into an exponential graph-walk/lattice-animal count. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
