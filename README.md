...
| **Last closed** | **v0.47.0 audit — frontier reclassification + weak-endpoint canaries (P2d-audit)** — audit-only, no bar movement; no mathematical pipeline change (one canary Lean file with zero downstream dependency). Three recordings: (1) `WilsonPolymerActivityBound.h_bound` reclassified from "active frontier" to **closed de facto for the current small-β / clipped Wilson activity producer** — `wilsonClusterActivity_abs_le` + `wilsonActivityBound_from_expansion` discharge it on `0 < β < 1`; no claim is made that Balaban CMP 116 Lemma 3 is formalised in full generality. (2) Active analytic frontier is `h_rpow` ≡ `ClusterCorrelatorBound N_c r C_clust`, the same statement modulo the proved bridge `clusterCorrelatorBound_of_rpow_bound` (`WilsonClusterProof.lean`) — this is F3 / Kotecký-Preiss on the `ClusterCorrelatorBound` front. (3) Both weak endpoints are formally audited: `ClayYangMillsTheorem := ∃ m_phys : ℝ, 0 < m_phys` and `ClayYangMillsStrong := ∃ m_lat, HasContinuumMassGap m_lat` are trivially inhabited and equivalent to `True`; wrappers that produce only these endpoints do not by themselves carry Clay-grade content. Per `L8_Terminal/ClayPhysical.lean`, the hierarchy is `ClayYangMillsPhysicalStrong → ClayYangMillsStrong → ClayYangMillsTheorem`, with `ClayYangMillsPhysicalStrong` the first non-vacuous level. Tripwire: `YangMills/L8_Terminal/ClayTrivialityAudit.lean` lands `clayYangMillsTheorem_trivial`, `clayYangMillsTheorem_iff_true`, `clayYangMillsStrong_trivial`, and `clayYangMillsStrong_iff_true`; all four `#print axioms` traces print `[propext, Classical.choice, Quot.sound]`, pinned in `AXIOM_FRONTIER.md` v0.47.0. L1 / L2 / L3 / OVERALL bars unchanged. (2026-04-24) |
