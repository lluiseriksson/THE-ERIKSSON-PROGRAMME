...
| **Last closed** | **v1.25.0 physical d=4 count package aliases exposed** — F3 count-interface sharpening, no bar movement. Added `physicalClayDimension := 4`, `PhysicalShiftedConnectingClusterCountBound`, and `PhysicalShiftedF3CountPackage` with `ofBound`, `apply`, `toAt`, field lemmas, and `toAt_apply` in `YangMills/ClayCore/ConnectingClusterCount.lean`. The physical combinatorial subtarget is now first-class as `PhysicalShiftedConnectingClusterCountBound C_conn dim`, definitionally the fixed-dimension frontier at `d = 4`, and its package projects to finite volumes by `toAt`. This does not prove the lattice-animal estimate; it makes the honest next target volume-uniform and four-dimensional rather than globally uniform over all `d`. Build `lake build YangMills.ClayCore.ConnectingClusterCount` green; `physicalClayDimension` prints no axioms and the new package declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
