...
| **Last closed** | **v1.00.0 `ClayConnectedCorrDecay` → `ClayYangMillsPhysicalStrong` canonical-distance bridge** — structural bridge to the first non-vacuous Clay target, no bar movement. Added `connectedCorrDecayBundle_of_clayConnectedCorrDecay_siteDist` and `physicalStrong_of_clayConnectedCorrDecay_siteDist` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`. A ClayCore separated-distance witness now yields `ClayYangMillsPhysicalStrong (sunHaarProb N_c) (wilsonPlaquetteEnergy N_c) β F (fun L p q => siteLatticeDist p.site q.site)` once the local inputs are supplied explicitly: `0 < β`, `|F| ≤ 1`, probability of every tilted Gibbs measure, and integrability of `W_p`, `W_q`, and `W_p*W_q`. The bridge uses separated exponential decay for `1 ≤ siteLatticeDist` and the new local `≤ 2` bound otherwise, with prefactor `w.C + 2*exp w.m`. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-24) |
