...
| **Last closed** | **v0.81.0 restrict global F3 count data to local lattices** — pure-additive, no bar movement. Adds `ShiftedConnectingClusterCountBound.toAt` in `YangMills/ClayCore/ConnectingClusterCount.lean` and `ShiftedF3CountPackage.toAt` in `YangMills/ClayCore/ClusterRpowBridge.lean`, projecting a global uniform shifted count bound/package to any fixed finite plaquette lattice package `ShiftedF3CountPackageAt d L`. This does not prove the global lattice-animal estimate; it records the expected direction that once the true uniform count package is proved, every finite-volume local package follows mechanically. Builds `lake build YangMills.ClayCore.ConnectingClusterCount` and `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
