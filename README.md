...
| **Last closed** | **v1.97.0 physical finite-walk count bound from plaquette branching** — F3-count infrastructure, no bar movement. Closed the bridge from local branching to finite-walk enumeration in `YangMills/ClayCore/LatticeAnimalCount.lean`: dependent neighbor-choice codes are repackaged as global step codes `current → next → Fin D`, injective on each actual neighbor set, and `plaquetteWalkCodeOfStepCode_injective` reconstructs a walk from its step-code word by induction over `Fin (n+1)`. Main endpoint: `plaquetteWalk_card_le_physical_ternary {L} [NeZero L] (p : ConcretePlaquette physicalClayDimension L) (n : ℕ) : Fintype.card (PlaquetteWalk physicalClayDimension L n p) ≤ 1296 ^ n`. The remaining F3-count work is now the graph-theoretic packaging from connected finite plaquette sets to a rooted walk/tree encoding usable by `ShiftedConnectingClusterCountBoundExp`; local branching and finite-walk coding are closed. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
