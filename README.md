...
| **Last closed** | **v1.52.0 F3 count packages are monotone in profile dimension** — F3 count-side API/frontier normalization, no bar movement. Lifted shifted count-bound monotonicity to package objects: `ShiftedF3CountPackageAt.mono_dim`, `ShiftedF3CountPackageDim.mono_dim`, `PhysicalShiftedF3CountPackage.mono_dim`, and global `ShiftedF3CountPackage.mono_dim`, with simp lemmas pinning `C_conn` unchanged and `dim = old dim + k`. Future lattice-animal/combinatorial count proofs can now align polynomial exponents at finite-volume, fixed-dimension, physical `d = 4`, or global scope without rebuilding package records by hand. The live mathematical obligations remain unchanged: construct the volume-uniform shifted count package and the Mayer/Ursell package. Builds `lake build YangMills.ClayCore.ConnectingClusterCount` and `lake build YangMills.ClayCore.ClusterRpowBridge` green; all twelve new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
