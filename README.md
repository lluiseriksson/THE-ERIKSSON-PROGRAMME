...
| **Last closed** | **v1.22.0 uniform-rpow frontier gets direct applicator** — small-β frontier API cleanup, no bar movement. Added `WilsonUniformRpowBound.apply` in `YangMills/ClayCore/ZeroMeanCancellation.lean`, exposing the named uniform-rpow hypothesis in direct application form and matching the existing `ShiftedConnectingClusterCountBound.apply` / `ShiftedF3CountPackage.apply` style. Once the uniform rpow estimate is supplied, downstream code can consume it without unfolding the predicate. The mathematical frontier remains proving this bound for the nonabelian theory. Builds `lake build YangMills.ClayCore.ZeroMeanCancellation` and `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
