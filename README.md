...
| **Last closed** | **v0.50.0 HS audit — explicit normalized-Gibbs LSI axiom replaces hidden `sorry`** — audit-only, no bar movement. The unrestricted Holley--Stroock transfer `lsi_normalized_gibbs_from_haar` in `YangMills/P8_PhysicalGap/BalabanToLSI.lean` is now a named axiom instead of a theorem-shaped placeholder carrying the same universal integrability gap behind `sorry`. The vanilla terminal witness `sun_physical_mass_gap_vacuous` therefore visibly depends on `YangMills.lsi_normalized_gibbs_from_haar`; the Σ/MemLp-gated route `lsi_normalized_gibbs_from_haar_memLp` and `sun_physical_mass_gap_vacuous_memLp` still print only `[propext, Classical.choice, Quot.sound]`. Targeted builds green for `YangMills.P8_PhysicalGap.BalabanToLSI`, `PhysicalMassGap`, and `ClayViaLSI`. This is a transparency repair, not a mathematical closure; the live Clay-grade front remains `ClusterCorrelatorBound`. No `sorry`. (2026-04-24) |
