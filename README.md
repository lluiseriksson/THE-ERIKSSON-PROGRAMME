...
| **Last closed** | **v1.03.0 plaquette-observable measurability bridge** — structural bridge cleanup to the first non-vacuous Clay target, no bar movement. Added `measurable_plaquetteHolonomy` in `YangMills/L2_Balaban/Measurability.lean`: plaquette holonomy is measurable whenever the gauge group has measurable inversion and multiplication. Added `physicalStrong_of_clayConnectedCorrDecay_siteDist_of_boltzmannIntegrable_observableMeasurable` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`: the v1.02 `PhysicalStrong` bridge now derives local Wilson-observable measurability from `Measurable F` by composition with measurable plaquette holonomy, then obtains local integrability from unit-boundedness. Boltzmann-weight integrability still supplies Gibbs probability via `gibbsMeasure_isProbability`; F1/F2/F3 remain the active analytic front, and the concrete SU(N) measurable-multiplication instance is a remaining infrastructure item. Builds `lake build YangMills.L2_Balaban.Measurability` and `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-24) |
