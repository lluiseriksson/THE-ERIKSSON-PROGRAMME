...
| **Last closed** | **v1.12.0 uniform-rpow small-β frontier lands in `ClusterCorrelatorBound`** — API cleanup, no bar movement. Added `clusterCorrelatorBound_small_beta_of_uniformRpow` in `YangMills/ClayCore/ZeroMeanCancellation.lean`: `0 < β`, `β < 1`, `0 < C`, and `WilsonUniformRpowBound N_c β C` now yield `ClusterCorrelatorBound N_c β C` by the existing rpow-to-KP exponential conversion. `clayMassGap_small_beta_of_uniformRpow` now routes through this named hub bridge, so the small-β frontier explicitly lands in `ClusterCorrelatorBound` before projecting to mass gap, connected decay, theorem, or `PhysicalStrong`. The remaining analytic target is unchanged: prove `WilsonUniformRpowBound` for `N_c ≥ 2` via Mayer/Kotecky-Preiss. Build `lake build YangMills.ClayCore.ZeroMeanCancellation` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
