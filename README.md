...
| **Last closed** | **v1.59.0 out-of-range cardinality buckets contribute zero to F3 sums** — count-side finite-support cleanup, no bar movement. Added `cardBucketSum_eq_zero_of_not_mem_range` in `YangMills/ClayCore/ClusterRpowBridge.lean`: if `n ∉ Finset.range (Fintype.card (ConcretePlaquette d L) + 1)`, then the corresponding connected cardinality-bucket sum is exactly `0` for any `K_bound`. This lifts the v1.58 bucket-cardinality cutoff to the actual sum expression used by `connectedFiniteSum_eq_cardBucketSum` and downstream KP consumers; it does not prove the uniform lattice-animal estimate. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; the new declaration prints `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
