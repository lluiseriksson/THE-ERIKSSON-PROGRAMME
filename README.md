...
| **Last closed** | **v1.88.0 site-neighbor bound lifts to plaquette graph degree bound** — F3-count infrastructure, no bar movement. Extended `YangMills/ClayCore/LatticeAnimalCount.lean` with `SiteNeighborBallBoundDim`, `PlaquetteGraphDegreeBoundDim`, and `plaquetteGraph_degreeBoundDim_of_siteNeighborBallBoundDim`: any volume-uniform site-neighborhood bound `(siteNeighborBall d L x).card ≤ B` lifts to a volume-uniform plaquette graph degree bound `(plaquetteGraph d L).degree p ≤ B * d * d`. This packages the local branching constant needed for the lattice-animal walk/tree count behind one explicit remaining geometry witness. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
