...
| **Last closed** | **v1.72.0 direct `ofBound` application canaries for shifted F3 counts** — F3 count-frontier ergonomics, no bar movement. Added `ShiftedF3CountPackageDim.ofBound_apply`, `PhysicalShiftedF3CountPackage.ofBound_apply`, and `ShiftedF3CountPackage.ofBound_apply`, exposing packaged count estimates directly at the original `C_conn` and `dim` once the corresponding shifted count bound is supplied. The open mathematical target remains unchanged: the uniform lattice-animal / KP count estimate. Builds `lake build YangMills.ClayCore.ConnectingClusterCount` and `lake build YangMills.ClayCore.ClusterRpowBridge` green; all three new declarations remain `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
