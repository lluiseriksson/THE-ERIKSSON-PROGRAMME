...
| **Last closed** | **v0.85.0 small-beta uniform-rpow connected-decay endpoint** — pure-additive, no bar movement. The named frontier `WilsonUniformRpowBound N_c β C` now feeds both `clayMassGap_small_beta_of_uniformRpow : ClayYangMillsMassGap N_c` and `clayConnectedCorrDecay_small_beta_of_uniformRpow : ClayConnectedCorrDecay N_c` in `YangMills/ClayCore/ZeroMeanCancellation.lean`. Canaries pin the connected-decay projection constants `mass = kpParameter β` and `prefactor = C`. This does not prove the uniform rpow bound; it makes the small-β F3/Mayer terminal shape explicit through the non-vacuous mass-gap and connected-decay APIs, leaving the analytic task exactly where it belongs: proving `WilsonUniformRpowBound`. Build `lake build YangMills.ClayCore.ZeroMeanCancellation` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
