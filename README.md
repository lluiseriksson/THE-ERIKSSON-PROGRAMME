...
| **Last closed** | **v1.84.0 physical exponential F3 route reaches `ClayYangMillsPhysicalStrong`** — L8 terminal plumbing, no bar movement. Added `connectedCorrDecayBundle_of_expCountBound_mayerData_siteDist_measurableF`, `connectedCorrDecayBundle_of_physicalMayerData_expCount_siteDist_measurableF`, `physicalStrong_of_expCountBound_mayerData_siteDist_measurableF`, and `physicalStrong_of_physicalMayerData_expCount_siteDist_measurableF` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`. These compose the physical exponential F3 correlator endpoints with the existing `PhysicalClusterCorrelatorBound → ConnectedCorrDecayBundle → ClayYangMillsPhysicalStrong` path. The honest physical exponential route now lands at the non-vacuous physical target, given `PhysicalShiftedConnectingClusterCountBoundExp C_conn K`, Mayer/activity data, `K * r < 1`, `Measurable F`, `0 < β`, and `|F| ≤ 1`. Remaining mathematical work: prove physical/global Mayer-Ursell activity decay and the physical uniform exponential count estimate. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
