...
| **Last closed** | **v0.59.0 F3 terminal endpoint from count + global cardinality decay** — pure-additive, no bar movement. Adds `pointwiseBucketBound_of_card_decay`, `clusterCorrelatorBound_of_count_cardDecayBounds_ceil`, and `clay_theorem_of_count_cardDecayBounds_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The v0.58 endpoint needed a bucket-local pointwise estimate `K_bound Y ≤ A₀ * r^(n + ⌈dist⌉₊)`; v0.59 reduces this to the canonical global polymer-activity shape `K_bound Y ≤ A₀ * r^Y.card`, then reads the bucket equality `Y.card = n + ⌈dist⌉₊`. The current terminal F3 inputs are now Mayer/Ursell identity `h_mayer`, disconnected support cancellation `h_zero`, lattice-animal bucket count `h_count`, and global cardinality decay `h_card_decay`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
