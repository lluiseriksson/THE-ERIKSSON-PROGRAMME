...
| **Last closed** | **v0.84.0 small-beta uniform-rpow authentic mass-gap endpoint** — pure-additive, no bar movement. The named frontier `WilsonUniformRpowBound N_c β C` now feeds `clayMassGap_small_beta_of_uniformRpow : ClayYangMillsMassGap N_c` in `YangMills/ClayCore/ZeroMeanCancellation.lean`, with `clay_theorem_small_beta_of_uniformRpow` only the weak projection through `clayMassGap_implies_clayTheorem`. Canaries pin `mass = kpParameter β` and `prefactor = C`. This does not prove the uniform rpow bound; it ensures that when the F3-Mayer analytic bound is supplied in small-β form, the downstream object is the non-vacuous mass-gap structure, not merely `ClayYangMillsTheorem`. Build `lake build YangMills.ClayCore.ZeroMeanCancellation` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
