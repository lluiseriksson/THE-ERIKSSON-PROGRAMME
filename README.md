...
| **Last closed** | **v1.64.0 KP consumers route through the direct bucket-`tsum` API** — finite-volume F3 proof-term cleanup, no bar movement and no public theorem statement changes. Refactored `connectedFiniteSum_le_of_cardBucketBounds_kp` and `connectedFiniteSum_le_of_cardBucketBounds_kp_shifted` in `YangMills/ClayCore/ClusterRpowBridge.lean` so both delegate to the v1.63 direct `tsum` comparison lemmas instead of replaying the older finite-range partial-sum route. The public KP API is now aligned with the bucket-series spine: connected sum = bucket `tsum`, bucketwise bound plus zero outside range, termwise comparison to the KP `tsum`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; both declarations remain `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
