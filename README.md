...
| **Last closed** | **v1.13.0 finite-volume shifted count package made inspectable** — local F3-count bookkeeping cleanup, no bar movement. Added `ShiftedF3CountPackageAt.finite_C_conn`, `ShiftedF3CountPackageAt.finite_dim`, and `ShiftedF3CountPackageAt.finite_apply` in `YangMills/ClayCore/ConnectingClusterCount.lean`: the local finite-volume count package now exposes `C_conn = Fintype.card (Finset (ConcretePlaquette d L)) + 1`, `dim = 0`, and a direct application theorem for the shifted bucket bound. This does not prove the global uniform `ShiftedConnectingClusterCountBound`; it makes clear that the remaining F3-count gap is the uniform lattice-animal estimate, not local finite-volume plumbing. Build `lake build YangMills.ClayCore.ConnectingClusterCount` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
