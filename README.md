...
| **Last closed** | **v1.50.0 mixed physical F3 package restricts to preferred physical package** — ClayCore API/frontier normalization, no bar movement. Added `PhysicalShiftedF3MayerCountPackage.toPhysicalOnly` plus projection/constant simp lemmas in `YangMills/ClayCore/ClusterRpowBridge.lean`. A mixed package `pkg : PhysicalShiftedF3MayerCountPackage N_c wab` now converts to `pkg.toPhysicalOnly : PhysicalOnlyShiftedF3MayerCountPackage N_c wab`, applying `pkg.mayer.toPhysical` while preserving the already-physical count half and constants `A₀`, `C_conn`, and `dim`. The live mathematical obligations remain unchanged: construct the Mayer/Ursell package and the physical or global count package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
