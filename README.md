...
| **Last closed** | **v1.92.0 explicit plaquette-graph branching bound** — F3-count infrastructure, no bar movement. Packaged the v1.91 ternary site-neighborhood estimate into `PlaquetteGraphDegreeBoundDim`: `plaquetteGraph_degreeBoundDim_ternary` gives the generic fixed-dimension bound `(3 ^ d) * Fintype.card (Fin d) * Fintype.card (Fin d)`, while `plaquetteGraph_degreeBoundDim_physical_ternary` and `plaquetteGraph_degree_le_physical_ternary` specialize it to `physicalClayDimension = 4`, proving every finite-volume physical plaquette graph has degree `≤ 1296`. This is the volume-uniform local branching constant needed for the next lattice-animal walk/tree count. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
