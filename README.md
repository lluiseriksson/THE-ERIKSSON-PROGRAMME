...
| **Last closed** | **v2.08.0 physical-only exponential F3 package** — F3 route infrastructure, no bar movement. Added `PhysicalOnlyShiftedF3MayerCountPackageExp` and `physicalClusterCorrelatorBound_of_physicalOnlyShiftedF3MayerCountPackageExp` in `YangMills/ClayCore/ClusterRpowBridge.lean`, combining the physical Mayer half, a physical exponential count package, and the KP smallness condition `count.K * wab.r < 1`. This aligns the v2.07 physical `1296` count package with a fully physical exponential endpoint: `PhysicalConnectingClusterBaselineExtraWordDecoderCovers1296` + `PhysicalShiftedF3MayerPackage N_c wab` + `1296 * wab.r < 1` feeds `PhysicalClusterCorrelatorBound`. The remaining mathematical work is still constructing the baseline-plus-word decoder and the physical Mayer/activity package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
