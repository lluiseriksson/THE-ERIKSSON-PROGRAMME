...
| **Last closed** | **v1.65.0 Mayer finite connecting sum equals the bucket `tsum`** — finite-volume F3 infrastructure, no bar movement. Added `finiteConnectingSum_eq_cardBucketTsum` in `YangMills/ClayCore/ClusterRpowBridge.lean`: under the usual disconnected-support cancellation hypothesis, the finite connecting sum over all polymers containing `p` and `q` is exactly the `tsum` of the connected cardinality buckets. This composes v1.62's connected-sum bucket `tsum` identity with `finiteConnectingSum_eq_connectedFiniteSum`, so callers coming directly from the Mayer identity can enter the bucket-series API without manually inserting the connected-sum intermediate. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; the new declaration prints `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
