...
| **Last closed** | **v1.61.0 bucket `tsum` reduces to the finite plaquette range** — finite-volume F3 infrastructure, no bar movement. Added `cardBucketTsum_eq_cardBucketSum_range` in `YangMills/ClayCore/ClusterRpowBridge.lean`: for fixed finite volume, marked plaquettes `p q`, and arbitrary `K_bound`, the infinite cardinality-bucket series is exactly the finite sum over `Finset.range (Fintype.card (ConcretePlaquette d L) + 1)`. This packages the v1.59 zero-outside-range lemma and v1.60 summability wrapper into the exact finite/infinite conversion needed by later bucket-series manipulations; it does not prove the uniform lattice-animal estimate. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; the new declaration prints `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
