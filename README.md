...
| **Last closed** | **v1.51.0 shifted count bounds are monotone in profile dimension** — F3 count-side infrastructure, no bar movement. Added `real_pow_le_pow_add_of_one_le` plus `ShiftedConnectingClusterCountBound.mono_dim`, `ShiftedConnectingClusterCountBoundDim.mono_dim`, and `ShiftedConnectingClusterCountBoundAt.mono_dim` in `YangMills/ClayCore/ConnectingClusterCount.lean`. Any shifted count bound proved with polynomial profile dimension `dim` can now be weakened mechanically to `dim + k` at the global, fixed-dimension, or finite-volume scope. This lets future lattice-animal/combinatorial count proofs align polynomial exponents across F3 packages without reproving the bucket-count inequality. The live mathematical obligations remain unchanged: construct the volume-uniform physical/global shifted count package and the Mayer/Ursell package. Build `lake build YangMills.ClayCore.ConnectingClusterCount` green; all four new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
