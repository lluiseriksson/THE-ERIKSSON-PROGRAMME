...
| **Last closed** | **v1.04.0 concrete SU(N) measurable multiplication instance** — structural bridge cleanup to the first non-vacuous Clay target, no bar movement. Added `instSecondCountableTopologySUN`, `instMeasurableMulSUN`, and `sun_measurableMul₂` in `YangMills/P8_PhysicalGap/SUN_StateConstruction.lean`: SU(N) now exposes second-countability as a subtype of finite-dimensional complex matrix space, letting Mathlib synthesize `MeasurableMul₂` from the existing continuous multiplication/Borel structure. This closes the infrastructure item left by v1.03, so the `PhysicalStrong` bridge can derive local Wilson-observable measurability from `Measurable F` using the repository's concrete SU(N) measurable group structure. Boltzmann-weight integrability still supplies Gibbs probability; F1/F2/F3 remain the active analytic front. Builds `lake build YangMills.P8_PhysicalGap.SUN_StateConstruction` and `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-24) |
