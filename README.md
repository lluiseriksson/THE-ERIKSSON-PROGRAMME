...
| **Last closed** | **v0.74.0 F3 endpoints into `ClayWitnessHyp`** — pure-additive, no bar movement. Adds `clayWitnessHyp_of_shiftedCountBound_mayerData_ceil` and `clayWitnessHyp_of_shiftedCountBound_connectedCardDecayActivities_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The preferred shifted F3 packages now route through `ClusterCorrelatorBound` into the older analytic witness bundle `ClayWitnessHyp N_c`, covering both packaged `ConnectedCardDecayMayerData` and raw `K / hK_abs_le / h_mayer` entry styles. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
