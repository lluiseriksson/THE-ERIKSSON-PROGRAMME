...
| **Last closed** | **v0.57.0 F3 `ClusterCorrelatorBound` from count + pointwise bounds** — pure-additive, no bar movement. Adds `clusterCorrelatorBound_of_count_pointwiseBounds_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`, composing the v0.56 bucket estimate with the terminal bucket consumer. The F3 bridge now accepts the two natural lattice-animal inputs directly: bucket cardinality `# {Y ∋ p,q | connected Y ∧ |Y| = n + ⌈dist⌉₊} ≤ C_conn * n^dim` and pointwise activity `K_bound Y ≤ A₀ * r^(n + ⌈dist⌉₊)`. Together with `h_mayer` and disconnected support cancellation `h_zero`, these produce `ClusterCorrelatorBound`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
