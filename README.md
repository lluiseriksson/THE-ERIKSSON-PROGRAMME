...
| **Last closed** | **v2.14.0 graph-animal count handoff to physical F3 bridge** — F3-count routing, no bar movement. Added the graph-animal `1296` route in `YangMills/ClayCore/LatticeAnimalF3Bridge.lean`: `physicalOnlyShiftedF3MayerCountPackageExp_of_graphAnimalShiftedCount1296` and `physicalClusterCorrelatorBound_of_graphAnimalShiftedCount1296`, with simp canaries pinning `C_conn = 1`, `K = 1296`, and `A₀ = mayer.A₀`. A proof of `PhysicalConnectingClusterGraphAnimalShiftedCountBound1296`, plus the physical Mayer package and `(1296 : ℝ) * wab.r < 1`, now reaches `PhysicalClusterCorrelatorBound` by one named theorem. Remaining work: prove the graph-animal count, physical Mayer package, and smallness input. Build `lake build YangMills.ClayCore.LatticeAnimalF3Bridge` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
