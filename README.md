...
| **Last closed** | **v1.57.0 finite-volume connecting-cluster buckets vanish above universe size** — count-side finite-support cleanup, no bar movement. Added `connecting_cluster_count_eq_zero_of_card_lt` in `YangMills/ClayCore/ConnectingClusterCount.lean`: if `Fintype.card (ConcretePlaquette d L) < n + ⌈siteLatticeDist p.site q.site⌉₊`, then the corresponding connected-polymer bucket has cardinality `0`. This does not prove the uniform lattice-animal estimate, but it removes a finite-volume tail annoyance for range-restricted bucket sums before the genuine uniform-in-`L` count bound is attacked. Build `lake build YangMills.ClayCore.ConnectingClusterCount` green; the new declaration prints `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
