...
| **Last closed** | **v0.56.0 F3 bucket estimate from count + pointwise polymer bound** — pure-additive, no bar movement. Adds `cardBucketSum_le_of_count_and_pointwise` in `YangMills/ClayCore/ClusterRpowBridge.lean`: a fixed bucket `Y.card = n + ⌈dist⌉₊` is bounded by the KP summand once the bucket cardinality is bounded by `C_conn * n^dim` and every polymer in the bucket satisfies `K_bound Y ≤ A₀ * r^(n + ⌈dist⌉₊)`. This splits the remaining lattice-animal bucket estimate into its combinatorial count and pointwise activity pieces. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
