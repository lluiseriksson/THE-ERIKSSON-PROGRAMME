...
| **Last closed** | **v1.30.0 physical F3 package finite-volume consumers exposed** — proof-ergonomics infrastructure around the active F3 frontier, no bar movement. Added direct `PhysicalShiftedF3MayerCountPackage` consumers in `YangMills/ClayCore/ClusterRpowBridge.lean`: `toTruncatedActivities`, `toTruncatedActivities_K`, `toTruncatedActivities_K_bound_le_cardDecay`, `toTruncatedActivities_K_bound_eq_zero_of_not_connected`, `wilsonConnectedCorr_eq_toTruncatedActivities_connectingSum`, and `apply_count`. Future proofs can now use the single physical package directly at finite-volume `d = 4` sites without manually projecting through `pkg.mayer` and `pkg.count`. The mathematical obligations are unchanged: construct `ShiftedF3MayerPackage N_c wab` and `PhysicalShiftedF3CountPackage` for `N_c ≥ 2`. Builds `lake build YangMills.ClayCore.ClusterRpowBridge` and `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; new consumers print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
