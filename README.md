...
| **Last closed** | **v2.03.0 baseline-plus-extra decoder target** — F3-count infrastructure, no bar movement. Added `PhysicalConnectingClusterBaselineExtraDecoderCovers` plus bridges to `PhysicalConnectingClusterExtraWalkDecoderBound` and `PhysicalShiftedConnectingClusterCountBoundExp 1 1296` in `YangMills/ClayCore/LatticeAnimalCount.lean`. This is the corrected concrete target after the v2.02 obstruction: the decoded bucket has shape `baseline ∪ decodeExtra w`, where `baseline` accounts for the deterministic `p`-to-`q` distance part and the length-`n` walk/word accounts for the `n` extra plaquettes. The remaining proof is now the real BFS/Klarner construction of that decoder, not the impossible claim that the bucket is literally the range of a length-`n` walk. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
