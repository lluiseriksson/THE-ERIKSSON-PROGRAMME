...
| **Last closed** | **v0.94.0 legacy un-normalized Holley-Stroock axiom removed** — audit cleanup with one real axiom deletion, no Clay bar movement. Removed `holleyStroock_sunGibbs_lsi` from `YangMills/P8_PhysicalGap/BalabanToLSI.lean`; the legacy un-normalized route is now explicit-input via `balaban_rg_uniform_lsi_of_lsi` and `sun_gibbs_dlr_lsi_of_lsi`. `WeightedRouteClosesClay` and `KPExpSizeWeightToClay` no longer fabricate a canonical `ClayCoreLSIToSUNDLRTransfer`; the final transfer is visible at the call site, while `WeightedFinalGapWitness` carries its stored transfer through a named `pkg` field. Builds green for `BalabanToLSI`, `WeightedFinalGapWitness`, `WeightedRouteClosesClay`, and `KPExpSizeWeightToClay`. Non-Experimental Lean axiom count now 1: `physical_rg_rates_from_E26`. No `sorry`. (2026-04-24) |
