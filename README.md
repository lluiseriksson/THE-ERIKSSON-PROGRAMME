...
| **Last closed** | **v1.49.0 global F3 package exposes auditable physical L8 bundle** — endpoint plumbing, no bar movement. Added `connectedCorrDecayBundle_of_shiftedF3MayerCountPackage_toPhysicalOnly_siteDist_measurableF` plus mass/prefactor equations in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`. A future global F3 package `pkg : ShiftedF3MayerCountPackage N_c wab` now routes through `pkg.toPhysicalOnly` to an auditable `ConnectedCorrDecayBundle`, with `m = kpParameter wab.r` and `C = clusterPrefactorShifted wab.r pkg.C_conn pkg.A₀ pkg.dim + 2 * exp (kpParameter wab.r)`, before final projection to `ClayYangMillsPhysicalStrong`. The live mathematical obligations remain the Mayer/Ursell package and the physical or global count package. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
