...
| **Last closed** | **v2.20.0 total-size route to `ClusterCorrelatorBound`** — no bar movement, but the corrected F3 total-cardinality path now reaches the public Wilson-facing target. Added `clusterPrefactorExp_effective_rpow_ceil_le_exp` and `clusterCorrelatorBound_of_truncatedActivities_ceil_total_exp` in `YangMills/ClayCore/ClusterRpowBridge.lean`: a KP estimate `∑' n, C_conn * K^(n+⌈dist⌉₊) * A₀ * r^(n+⌈dist⌉₊)` yields `ClusterCorrelatorBound N_c (K*r) (clusterPrefactorExp r K C_conn A₀)` under `0 < r`, `0 < K`, `K*r < 1`, and positive constants. This is the standard total-size BFS/Klarner route: the distance factor is absorbed into the effective decay parameter `K*r`, avoiding the stronger shifted `K^n` target. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. Next F3 move: produce the physical total-size `connectingBound` KP estimate and discharge `K*r < 1` for the Wilson parameters. (2026-04-25) |
