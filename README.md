...
| **Last closed** | **Super-polynomial dominance lemma (P2a) — analytic core of `h_dominated`** — the analytic content of Balaban CMP 122 II Eq (1.98)–(1.100) / Paper [55] Theorem 8.5, `exp(−A · (log g⁻²)^p) ≤ E · g²` for `A > 0`, `p > 1`, `E > 0` at sufficiently small `g`, is formalized as a first-class Lean theorem `YangMills.superPoly_dominance` in `YangMills/ClayCore/LargeFieldDominance.lean`, with companion `YangMills.superPolyProfile : LargeFieldProfile` and specific-`g` corollary `YangMills.superPoly_dominance_at_specific`. Elementary proof (no `Asymptotics.IsLittleO`): four-step cascade via `u := log(g⁻²) = −2 log g`, threshold `M := ((1 + C)/A)^{1/(p−1)}`, and `g₀ := exp(−U/2)` closes via `Real.exp_le_exp`. Structural integration into `LargeFieldActivityBound.h_dominated` deferred pending refactor of the over-strong `∀ E₀ > 0` quantifier to a fixed `E₀`. Oracle: `[propext, Classical.choice, Quot.sound]`. (2026-04-23) |
