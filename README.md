...
| **Last closed** | **v1.31.0 physical F3 bundle certificate exposed** — terminal certificate sharpening, no bar movement. Added `connectedCorrDecayBundle_of_physicalShiftedF3MayerCountPackage_siteDist_measurableF` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`, sending `pkg : PhysicalShiftedF3MayerCountPackage N_c wab` to an auditable `ConnectedCorrDecayBundle` at `physicalClayDimension = 4` before the final `ClayYangMillsPhysicalStrong` projection. The constructor discharges Gibbs probability and local Wilson-observable integrability from the concrete Wilson plaquette energy and `Measurable F`. Audit lemmas pin the bundle rate as `kpParameter wab.r` and the prefactor as `clusterPrefactorShifted wab.r pkg.count.C_conn pkg.mayer.A₀ pkg.count.dim + 2 * exp (kpParameter wab.r)`, where the added term is the standard local-distance padding. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
