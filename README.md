...
| **Last closed** | **v2.16.0 graph-animal word-decoder count bridge** — F3-count constructive interface, no bar movement. Added `PhysicalConnectingClusterGraphAnimalShiftedWordDecoderBound K` and bridges in `YangMills/ClayCore/LatticeAnimalCount.lean`: a length-`n` word decoder covering each shifted graph-animal bucket gives `PhysicalConnectingClusterGraphAnimalShiftedCountBound K`; at `K = 1296` it packages directly as `PhysicalShiftedF3CountPackageExp`. Remaining mathematical work: construct the actual BFS/Klarner-style decoder, then combine it with the physical Mayer package and smallness regime. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
