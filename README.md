...
| **Last closed** | **v1.16.0 Mayer data converts directly to truncated activities** — F3 Mayer-interface cleanup, no bar movement. Added `ConnectedCardDecayMayerData.toTruncatedActivities` plus projection/bound lemmas in `YangMills/ClayCore/ClusterRpowBridge.lean`: `toTruncatedActivities_K`, `toTruncatedActivities_K_bound_le_cardDecay`, and `toTruncatedActivities_K_bound_eq_zero_of_not_connected`. The packaged bridge `clusterCorrelatorBound_of_shiftedCountBound_mayerData_ceil` now consumes this method instead of rebuilding `TruncatedActivities.ofConnectedCardDecay` inline. This gives a future nonabelian Mayer/Ursell construction a single named route from `ConnectedCardDecayMayerData` to the finite-volume `TruncatedActivities` consumer; the mathematical frontier remains constructing that Mayer package and the global shifted count package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
