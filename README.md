...
| **Last closed** | **v0.69.0 packaged `ClusterCorrelatorBound` endpoint for preferred F3** — pure-additive, no bar movement. Adds `clusterCorrelatorBound_of_shiftedCountBound_mayerData_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`, exposing the real analytic target `ClusterCorrelatorBound N_c r (clusterPrefactorShifted r C_conn A₀ dim)` directly from the two named packages: `ConnectedCardDecayMayerData N_c r A₀ hr_pos.le hA.le` for the raw Mayer/activity kernel, connected-cardinality decay, and Mayer/Ursell identity; and `ShiftedConnectingClusterCountBound C_conn dim` for the shifted lattice-animal count. The weak Clay wrapper `clay_theorem_of_shiftedCountBound_mayerData_ceil` now factors through this stronger theorem via `clay_theorem_from_wilson_activity`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
