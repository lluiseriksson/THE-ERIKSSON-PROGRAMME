...
| **Last closed** | **v0.49.0 F3 counting interface — canonical buckets + finite/connected `connectingBound`** — pure-additive, no bar movement. Adds `ceil_siteLatticeDist_le_polymer_card` (`PolymerDiameterBound.lean`), `connected_polymer_card_eq_extra_add_dist` (`ConnectingClusterCount.lean`), `TruncatedActivities.connectingBound_eq_finset_sum` (`MayerExpansion.lean`), plus finite and connected finite bridges in `ClusterRpowBridge.lean`: `clusterCorrelatorBound_of_finiteConnectingBounds_ceil`, `finiteConnectingSum_eq_connectedFiniteSum`, and `clusterCorrelatorBound_of_connectedFiniteBounds_ceil`. These expose the exact `n + ⌈siteLatticeDist p.site q.site⌉₊` buckets, rewrite Mayer `connectingBound` as a finite sum, restrict that sum to `PolymerConnected` under a zero-on-disconnected support hypothesis, and consume the resulting connected finite estimate directly to produce `ClusterCorrelatorBound`. The remaining KP task is now stated in connected finite lattice-animal form. Root `lake build YangMills` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
