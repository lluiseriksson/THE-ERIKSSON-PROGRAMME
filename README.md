...
| **Last closed** | **v1.02.0 local Wilson integrability from measurability + unit bound** — structural bridge cleanup to the first non-vacuous Clay target, no bar movement. Added `plaquetteWilsonObs_integrable_of_unitBound` and `plaquetteWilsonObs_mul_integrable_of_unitBound` in `YangMills/ClayCore/ClusterCorrelatorBound.lean`: on any finite measure space, a unit-bounded Wilson plaquette observable and the product of two such observables are integrable once the relevant `AEStronglyMeasurable` facts are supplied. Added `physicalStrong_of_clayConnectedCorrDecay_siteDist_of_boltzmannIntegrable_measurable` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`, so the v1.01 `PhysicalStrong` bridge now needs local measurability of `W_p` under Gibbs rather than separate integrability hypotheses for `W_p`, `W_q`, and `W_p*W_q`. Boltzmann-weight integrability still supplies Gibbs probability via `gibbsMeasure_isProbability`; F1/F2/F3 remain the active analytic front. Builds `lake build YangMills.ClayCore.ClusterCorrelatorBound` and `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-24) |
