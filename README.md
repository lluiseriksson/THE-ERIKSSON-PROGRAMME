...
| **Last closed** | **v1.56.0 L8 F3 subpackage routes expose mono-count-dimension constants** — terminal audit/endpoint normalization, no bar movement. Added six `ConnectedCorrDecayBundle` canaries for the physical shifted subpackage, fully physical subpackage, and global-Mayer/physical-count routes. Replacing `count` by `count.mono_dim k` keeps mass `kpParameter wab.r` and exposes prefactor `clusterPrefactorShifted wab.r count.C_conn mayer.A₀ (count.dim + k) + 2 * Real.exp (kpParameter wab.r)`. Future F3 scripts can enlarge count exponents directly at the subpackage endpoint without losing auditable L8 constants. The live mathematical obligations remain unchanged: construct the volume-uniform shifted count package and the Mayer/Ursell package. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; all six new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
