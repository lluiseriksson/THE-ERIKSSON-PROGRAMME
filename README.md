...
| **Last closed** | **v2.01.0 concrete walk-range decoder scaffold** — F3-count infrastructure, no bar movement. Added the concrete decoder candidate `plaquetteWalkRangeFinset w := Finset.univ.image w.1`, the visited plaquette set of a finite walk, and proved start membership, indexed-vertex membership, and `(plaquetteWalkRangeFinset w).card ≤ n+1`. Added the concrete coverage target `PhysicalConnectingClusterRangeDecoderCovers`: every shifted physical connecting-cluster bucket should be exactly the visited set of some length-`n` walk from `p`. Assuming that coverage theorem, `physicalConnectingClusterExtraWalkDecoderBound_of_rangeDecoderCovers` derives the abstract decoder target and `physicalShiftedConnectingClusterCountBoundExp_of_rangeDecoderCovers` discharges `PhysicalShiftedConnectingClusterCountBoundExp 1 1296`. The coverage theorem itself remains the open BFS/tree construction. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
