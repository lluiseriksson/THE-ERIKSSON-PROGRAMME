...
| **Last closed** | **v1.05.0 `PhysicalStrong` bridge with only `Measurable F` local regularity** — structural bridge cleanup to the first non-vacuous Clay target, no bar movement. Added `physicalStrong_of_clayConnectedCorrDecay_siteDist_of_boltzmannIntegrable_measurableF` in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean`: from a `ClayConnectedCorrDecay N_c` witness, `0 < β`, `|F| ≤ 1`, `Measurable F`, and Boltzmann-weight integrability, the theorem derives Gibbs probability, local Wilson-observable measurability, local Wilson integrability, and the canonical site-distance `ClayYangMillsPhysicalStrong` endpoint. This packages the v1.02-v1.04 local finite-volume cleanup into the simplest current bridge interface. F1/F2/F3 remain the active analytic front. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-24) |
