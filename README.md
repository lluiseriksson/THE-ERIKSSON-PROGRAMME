...
| **Last closed** | **v1.94.0 plaquette graph branching-bound interface** — F3-count infrastructure, no bar movement. Added `PlaquetteGraphBranchingBoundDim` to `YangMills/ClayCore/LatticeAnimalCount.lean`, exposing the BFS/tree-count input as the abstract neighbor-finset statement `∀ p, ((plaquetteGraph d L).neighborFinset p).card ≤ D`. The geometry from v1.92-v1.93 now packages as `plaquetteGraph_branchingBoundDim_physical_ternary : PlaquetteGraphBranchingBoundDim physicalClayDimension 1296`, with direct application theorem `plaquetteGraph_branching_le_physical_ternary`. This cleanly separates geometry (`D = 1296`) from the next finite walk/tree enumeration layer. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
