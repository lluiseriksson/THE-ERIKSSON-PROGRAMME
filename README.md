...
| **Last closed** | **v0.70.0 authentic mass-gap endpoint for preferred F3** — pure-additive, no bar movement. Adds `clayMassGap_of_shiftedCountBound_mayerData_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`, so the preferred shifted F3 packages now produce the non-vacuous `ClayYangMillsMassGap N_c` structure directly. Inputs remain exactly `ConnectedCardDecayMayerData N_c wab.r A₀ wab.hr_pos.le hA.le` for the raw Mayer/activity kernel, connected-cardinality decay, and Mayer/Ursell identity, plus `ShiftedConnectingClusterCountBound C_conn dim` for the shifted lattice-animal count. Internally it uses `clusterCorrelatorBound_of_shiftedCountBound_mayerData_ceil` and `clay_massGap_large_beta`; the weak `clay_theorem_of_shiftedCountBound_mayerData_ceil` remains downstream, not the primary audited endpoint. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
