...
| **Last closed** | **v0.68.0 packaged Mayer/activity data for preferred F3 endpoint** — pure-additive, no bar movement. Adds `ConnectedCardDecayMayerData` and `clay_theorem_of_shiftedCountBound_mayerData_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The preferred F3 terminal interface now consumes exactly two named packages: `ConnectedCardDecayMayerData N_c wab.r A₀ wab.hr_pos.le hA.le` for the raw truncated activity, connected-cardinality decay, and Mayer/Ursell identity; and `ShiftedConnectingClusterCountBound C_conn dim` for the lattice-animal count. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
