...
| **Last closed** | **v0.53.0 F3 bucket-bound consumer — connected finite sum from bucket estimates** — pure-additive, no bar movement. Adds `connectedFiniteSum_le_of_cardBucketBounds` in `YangMills/ClayCore/ClusterRpowBridge.lean`: after the v0.52 cardinal-bucket decomposition, it is enough to prove a bound for each bucket `Y.card = n + ⌈siteLatticeDist p.site q.site⌉₊` and compare the finite KP partial sum with the infinite KP `tsum`. This narrows the connected KP task to the lattice-animal bucket estimate plus the standard nonnegative partial-sum-to-`tsum` comparison. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
