...
| **Last closed** | **v0.87.0 unused legacy SZ-to-clustering axiom deleted** — audit cleanup with one real dead-axiom deletion, no Clay bar movement. Removed orphaned backward-compatibility axiom `legacy_sz_lsi_to_clustering` from `YangMills/P8_PhysicalGap/BalabanToLSI.lean`; source search before deletion found only the declaration itself. The active Stroock-Zegarlinski route remains in `StroockZegarlinski.lean`, where the live frontier is the separate `poincare_to_covariance_decay` bridge; normalized Gibbs LSI remains governed by `lsi_normalized_gibbs_from_haar`. Build `lake build YangMills.P8_PhysicalGap.BalabanToLSI` green. `git grep legacy_sz_lsi_to_clustering -- YangMills` returns no matches. No `sorry`. (2026-04-24) |
