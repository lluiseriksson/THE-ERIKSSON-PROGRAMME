...
| **Last closed** | **v2.07.0 package physical 1296 decoder count half** — F3-count infrastructure, no bar movement. Added `physicalShiftedF3CountPackageExp_of_baselineExtraWordDecoderCovers1296` in `YangMills/ClayCore/LatticeAnimalCount.lean`, packaging the live decoder target `PhysicalConnectingClusterBaselineExtraWordDecoderCovers1296` as a downstream `PhysicalShiftedF3CountPackageExp` with constants `C_conn = 1`, `K = 1296`. This means the count side now feeds the same package API used by the F3 Mayer/count route; the remaining mathematical work is still constructing the baseline-plus-word decoder and the Mayer/activity package. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
