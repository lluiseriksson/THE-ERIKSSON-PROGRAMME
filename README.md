...
| **Last closed** | **v1.33.0 physical F3 package projections exposed** — API/audit sharpening, no bar movement. Added `PhysicalShiftedF3MayerCountPackage.mayerPackage` and `.countPackage` in `YangMills/ClayCore/ClusterRpowBridge.lean`, plus simp/audit lemmas pinning `A₀`, `data`, `C_conn`, `dim`, and the inverse reconstruction `ofSubpackages (mayerPackage pkg) (countPackage pkg) = pkg` by definitional equality. This lets future physical F3 scripts treat `pkg : PhysicalShiftedF3MayerCountPackage N_c wab` as a reversible package of its Mayer/activity half and physical count half without reaching through raw fields. Remaining mathematical obligations are unchanged: construct `ShiftedF3MayerPackage N_c wab` and `PhysicalShiftedF3CountPackage` for `N_c ≥ 2`. Build `lake build YangMills.ClayCore.ClusterRpowBridge YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
