...
- **What's next.** `ClusterCorrelatorBound`: the F3 summability/factoring bridge now reaches the exact Wilson-facing target. The live analytic inputs are the Mayer/Ursell identity for `wilsonConnectedCorr` and the geometric comparison from discrete cluster distance to `siteLatticeDist`.
...
| **Last closed** | **v0.48.0 F3 bridge — truncated activities → `ClusterCorrelatorBound`** — pure-additive, no bar movement; proves `clusterCorrelatorBound_of_truncatedActivities` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The theorem composes `TruncatedActivities.abs_connectingSum_le_connectingBound` + `connecting_cluster_tsum_le` and lands the exact Wilson-facing target `ClusterCorrelatorBound N_c r (clusterPrefactor r C_conn A₀ dim)`, assuming precisely the remaining analytic inputs: a Mayer/Ursell identity identifying `wilsonConnectedCorr` with the connecting truncated-activity sum, a connecting-cluster series bound on `connectingBound`, and a geometric comparison from `distNat` to `siteLatticeDist`. This isolates the live F3 work to those inputs; the summability/factoring bridge is oracle-clean. Root `lake build YangMills` is also green after namespace-collision cleanup (`legacy_sz_lsi_to_clustering`, `IsSpatialLocalObservable`, `ClayConnectedCorrDecay`, `connecting_cluster_tsum_eq_factored`). Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
