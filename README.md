...
| **Last closed** | **v1.39.0 count-family package constants are simp-visible** — proof-script ergonomics, no bar movement. Added `ShiftedF3CountPackageDim.ofAtFamily_C_conn`, `ShiftedF3CountPackageDim.ofAtFamily_dim`, `PhysicalShiftedF3CountPackage.ofAtFamily_C_conn`, and `PhysicalShiftedF3CountPackage.ofAtFamily_dim` in `YangMills/ClayCore/ConnectingClusterCount.lean`. Future physical count proofs can assemble `PhysicalShiftedF3CountPackage.ofAtFamily C_conn hC dim h_at` and recover the packaged `C_conn` / `dim` by simp, without unfolding the uniformization constructor. Build `lake build YangMills.ClayCore.ConnectingClusterCount` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
