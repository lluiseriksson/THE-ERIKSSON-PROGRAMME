...
![Core build](https://img.shields.io/badge/lake_build_YangMillsCore-green_(8398_jobs)-success)
...
```bash
lake build YangMillsCore          # the verified core — green, 8398 jobs
...
checkpoint:** 2026-07-04, `origin/main` commit `f4fe7f1d`.  The verification
...
core build remains
`lake build YangMillsCore` green at **8398 jobs**.
...
* the latest recorded full core build is still the 8398-job core build, while
  `origin/main` has advanced to `f4fe7f1d`.
...
```mermaid
graph TD
    subgraph core["YangMillsCore  (8398 jobs, oracle-clean)"]
...
| Build the core | `lake build YangMillsCore` | `Build completed successfully (8398 jobs)` |
