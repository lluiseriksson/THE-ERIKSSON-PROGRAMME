...
| **Last closed** | **v0.52.0 F3 cardinal-bucket decomposition — connected finite sum by extra size** — pure-additive, no bar movement. Adds `connectedFiniteSum_eq_cardBucketSum` in `YangMills/ClayCore/ClusterRpowBridge.lean`, rewriting the finite connected-polymer sum as a finite sum over buckets `Y.card = n + ⌈siteLatticeDist p.site q.site⌉₊`. The proof uses `ceil_siteLatticeDist_le_polymer_card` to put every connected polymer containing `p,q` into a unique nonnegative extra-size bucket. This is the exact combinatorial form immediately before the lattice-animal/KP bucket estimate. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
