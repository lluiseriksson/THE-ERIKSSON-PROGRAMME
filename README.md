...
| **Last closed** | **v0.61.0 projection simp rules for card-decay activities** — pure-additive, no bar movement. Adds `[simp]` projection theorems `TruncatedActivities.ofCardDecay_K` and `TruncatedActivities.ofCardDecay_K_bound` in `YangMills/ClayCore/MayerExpansion.lean`, exposing `(ofCardDecay K r A₀ ...).K Y = K Y` and `(ofCardDecay K r A₀ ...).K_bound Y = A₀ * r^Y.card`. This polishes v0.60.0 so future finite-volume F3 producers can reduce concrete truncated activities by `simp` instead of manual unfolding. Build `lake build YangMills.ClayCore.MayerExpansion` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
