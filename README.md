...
| **Last closed** | **v0.82.0 audit normalized plaquette zero-mean layer** — Mayer-side audit, no bar movement. Adds `#print axioms` traces in `YangMills/ClayCore/ZeroMeanCancellation.lean` for `singlePlaquetteZ_pos`, `plaquetteFluctuationNorm_integrable`, `plaquetteFluctuationNorm_mean_zero`, and `plaquetteFluctuationNorm_zero_beta`. This pins the closed single-plaquette cancellation input for the normalized fluctuation `w̃(U) = plaquetteWeight N_c β U / singlePlaquetteZ N_c β - 1`: positive one-plaquette partition function, integrability, exact Haar mean zero, and the `β = 0` sanity check. It does **not** claim the full Mayer/product-measure factorisation theorem; it records the algebraic one-plaquette layer that F3-Mayer must lift to gauge configurations and polymers. Build `lake build YangMills.ClayCore.ZeroMeanCancellation` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
