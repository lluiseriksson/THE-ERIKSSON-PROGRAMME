...
| **Last closed** | **v1.98.0 connecting-cluster bucket to walk-count interface** — F3-count infrastructure, no bar movement. Added `ConnectingClusterBucket d L p q n`, the subtype version of the exact filtered bucket used by `ShiftedConnectingClusterCountBoundExp`, and proved `connectingClusterBucket_card_eq_filter`. Added the explicit remaining graph-theoretic target `PhysicalConnectingClusterWalkCodeBound`: every shifted physical connected bucket injects into finite plaquette walks of length `n + ⌈siteLatticeDist p.site q.site⌉₊` from `p`. Assuming that target, `physical_connectingCluster_filter_card_le_walk_exp_of_walkCode` now proves the frontier filter count is bounded by `1296 ^ (n + ⌈siteLatticeDist p.site q.site⌉₊)` using the v1.97 walk bound. This does not close the BFS/tree injection; it pins down the exact missing theorem and its composition point. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
