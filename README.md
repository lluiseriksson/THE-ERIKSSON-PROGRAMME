...
| **Last closed** | **v0.86.0 P91 weak-coupling-window axiom retired** — audit cleanup with one real axiom deletion, no Clay bar movement. Removed stale RG-branch axiom `p91_tight_weak_coupling_window` from `YangMills/ClayCore/BalabanRG/P91WeakCouplingWindow.lean`; the data-driven theorem `p91_tight_weak_coupling_window_theorem` in `P91WindowClosed.lean` already supplies the window from `P91RecursionData`. The remaining P91WeakCouplingWindow wrappers are algebraic (`denominator_pos_tight`, `denominator_pos_from_tight`) and consume the tight-window inequality explicitly. Builds `lake build YangMills.ClayCore.BalabanRG.P91WeakCouplingWindow` and `lake build YangMills.ClayCore.BalabanRG.P91WindowClosed` green. Oracle for the wrappers: `[propext, choice, Quot.sound]`. No `p91_tight_weak_coupling_window` axiom remains. No `sorry`. (2026-04-24) |
