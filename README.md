...
| **Last closed** | **v0.93.0 unrestricted normalized Gibbs LSI axiom removed** — audit cleanup with one real axiom deletion, no Clay bar movement. Removed `lsi_normalized_gibbs_from_haar` from `YangMills/P8_PhysicalGap/BalabanToLSI.lean`; the unrestricted normalized route is now explicit-input via `balaban_rg_uniform_lsi_norm_of_lsi` and `sun_gibbs_dlr_lsi_norm_of_lsi`. The automatic axiom-free normalized route remains the MemLp-gated Σ chain (`lsi_normalized_gibbs_from_haar_memLp → balaban_rg_uniform_lsi_norm_memLp → sun_gibbs_dlr_lsi_norm_memLp`). `sun_physical_mass_gap` now takes its DLR-LSI witness explicitly, and `sun_physical_mass_gap_vacuous` routes through Σ. Build `lake build YangMills.P8_PhysicalGap.PhysicalMassGap` green; pinned traces for the new wrappers and physical endpoints are `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count now 2: `physical_rg_rates_from_E26` and legacy unnormalized `holleyStroock_sunGibbs_lsi`. No `sorry`. (2026-04-24) |
