...
| **Last closed** | **v2.19.0 total-size exponential KP series bridge** — analytic companion to v2.18, no bar movement. Added `connecting_cluster_tsum_le_total_exp` in `YangMills/ClayCore/ClusterSeriesBound.lean` and `TruncatedActivities.two_point_decay_from_cluster_tsum_total_exp` in `YangMills/ClayCore/ClusterRpowBridge.lean`: a standard total-cardinality count produces the series `C_conn * K^(n+dist) * A₀ * r^(n+dist)`, which factors as `clusterPrefactorExp r K C_conn A₀ * (K*r)^dist`. This is the correct landing zone for BFS/Klarner total-size counting, with effective decay parameter `K*r`. Builds `lake build YangMills.ClayCore.ClusterSeriesBound` and `lake build YangMills.ClayCore.ClusterRpowBridge` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
