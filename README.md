...
| **Last closed** | **v1.48.0 global F3 package restricts to preferred physical F3 package** — ClayCore API/frontier sharpening, no bar movement. Added `ShiftedF3MayerCountPackage.toPhysicalOnly` plus projection/constant simp lemmas in `YangMills/ClayCore/ClusterRpowBridge.lean`. A future global F3 package `pkg : ShiftedF3MayerCountPackage N_c wab` can now be consumed as `pkg.toPhysicalOnly : PhysicalOnlyShiftedF3MayerCountPackage N_c wab`, composing `pkg.mayerPackage.toPhysical` and `pkg.countPackage.toPhysical` while preserving `A₀`, `C_conn`, and `dim`. The live mathematical obligations remain unchanged: construct the Mayer/Ursell package and the physical or global count package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
