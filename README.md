...
| **Last closed** | **v1.24.0 fixed-dimension count package exposed** — F3 count-interface sharpening, no bar movement. Added `ShiftedF3CountPackageDim` plus `ofBound`, `apply`, `toAt`, field lemmas, and `toAt_apply` in `YangMills/ClayCore/ConnectingClusterCount.lean`; also added `ShiftedF3CountPackage.toDim`, `toDim_C_conn`, `toDim_dim`, and `toDim_apply` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The physical `d = 4` count proof can now be delivered as a first-class `ShiftedF3CountPackageDim 4`, projected to finite volumes by `toAt`, or obtained from a fully global `ShiftedF3CountPackage` by `toDim`. Builds `lake build YangMills.ClayCore.ConnectingClusterCount` and `lake build YangMills.ClayCore.ClusterRpowBridge` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
