...
| **Last closed** | **v1.91.0 concrete ternary site-neighbor bound** — F3-count infrastructure, no bar movement. Closed the coordinate geometry witness in `YangMills/ClayCore/LatticeAnimalCount.lean`: `siteLatticeDist x y ≤ 1` now implies each integer coordinate of `siteDisplacement x y` lies in `{-1,0,1}` via `siteDisplacement_sq_le_one_of_siteLatticeDist_le_one` and `siteDisplacement_mem_unit_of_siteLatticeDist_le_one`. This discharges the v1.90 bridge and proves the concrete uniform estimate `siteNeighborBallBoundDim_ternary : SiteNeighborBallBoundDim d (3 ^ d)`. Combined with v1.88, this yields an explicit plaquette-graph local branching bound `(3 ^ d) * d * d`; the next F3-count step is packaging that constant into the lattice-animal walk/tree count. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
