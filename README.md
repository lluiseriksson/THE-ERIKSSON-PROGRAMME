...
| **Last closed** | **v1.79.0 exponential F3 count frontier interface** — additive F3 count-surface correction, no bar movement. Added `YangMills/ClayCore/ConnectingClusterCountExp.lean` with exponential KP/lattice-animal predicates and packages: `ShiftedConnectingClusterCountBoundExp`, fixed-dimension/finite-volume variants, global/dimension/finite packages, and physical `d = 4` wrappers. This records the natural count target `C_conn * K^n` beside the older polynomial profile, preparing the future smallness route `r * K < 1` without breaking existing consumers. Includes local finite-volume canary `shiftedConnectingClusterCountBoundAtExp_finite` / `ShiftedF3CountPackageAtExp_finite`; it is not the uniform F3 count theorem. Build `lake build YangMills.ClayCore.ConnectingClusterCountExp` green; pinned declarations remain `[propext, Classical.choice, Quot.sound]`. Full `lake build YangMills` exceeded the 15-minute local timeout without a Lean error, so module-level build is the pinned verification. No `sorry`. (2026-04-25) |
