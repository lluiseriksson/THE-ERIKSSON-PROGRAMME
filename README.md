...
| **Last closed** | **v1.23.0 fixed-dimension shifted count frontier exposed** — F3 count-frontier sharpening, no bar movement. Added `ShiftedConnectingClusterCountBoundDim`, `ShiftedConnectingClusterCountBoundDim.apply`, `ShiftedConnectingClusterCountBound.toDim`, and `ShiftedConnectingClusterCountBoundDim.toAt` in `YangMills/ClayCore/ConnectingClusterCount.lean`. This sits between the global count predicate, uniform over all dimensions, and the finite-volume `At` predicate whose constants may depend on `d,L`. Since the physical Clay target has fixed spacetime dimension, future lattice-animal work can now target a volume-uniform bound at fixed `d` (especially `d = 4`) while allowing constants to depend on that dimension. Builds `lake build YangMills.ClayCore.ConnectingClusterCount` and `lake build YangMills.ClayCore.ClusterRpowBridge` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
