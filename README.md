...
| **Last closed** | **v1.11.0 SU(1) inhabits the named uniform-rpow frontier** — singleton canary, no bar movement. Added `wilsonUniformRpowBound_su1` in `YangMills/ClayCore/ZeroMeanCancellation.lean`: for `β > 0` and `0 ≤ C`, it proves `WilsonUniformRpowBound 1 β C` from `wilsonConnectedCorr_su1_eq_zero` and nonnegativity of `C * β ^ dist`. Together with v1.10, the already-closed `SU(1)` route now passes through the same `WilsonUniformRpowBound → PhysicalStrong` terminal interface intended for the Mayer/Kotecky-Preiss route. This does not construct the `N_c ≥ 2` uniform-rpow bound; that remains the live analytic front. Build `lake build YangMills.ClayCore.ZeroMeanCancellation` green; pinned oracle `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
