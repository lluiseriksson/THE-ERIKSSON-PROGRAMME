...
| **Last closed** | **v1.43.0 `ofGlobalMayer` constants are simp-visible** — proof-script ergonomics, no bar movement. Added direct field-normalization lemmas for `PhysicalOnlyShiftedF3MayerCountPackage.ofGlobalMayer` in `YangMills/ClayCore/ClusterRpowBridge.lean`: the resulting physical package rewrites its stored Mayer `A₀`, Mayer data, count `C_conn`, and count `dim` back to the original all-dimensions Mayer package and physical count package. Future scripts can compare the global-route and preferred `physicalClayDimension = 4` prefactors by `simp`, without unfolding package constructors. This tightens the global-Mayer → physical-F3 path while the live obligations remain `PhysicalShiftedF3MayerPackage N_c wab` and `PhysicalShiftedF3CountPackage`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; new declarations print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
