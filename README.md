...
| **Last closed** | **v1.99.0 extra-walk code target discharges physical F3 count frontier** — F3-count infrastructure, no bar movement. Added the stronger shifted-count coding contract `PhysicalConnectingClusterExtraWalkCodeBound`: every physical connecting-cluster bucket with `X.card = n + ⌈siteLatticeDist p.site q.site⌉₊` must inject into `PlaquetteWalk physicalClayDimension L n p`, so the distance baseline is accounted for by the bucket and the walk word has length only `n`. Assuming exactly that BFS/tree injection, `physical_connectingCluster_filter_card_le_extra_walk_exp_of_walkCode` proves the natural bound `bucket.card ≤ 1296 ^ n`, and `physicalShiftedConnectingClusterCountBoundExp_of_extraWalkCode` discharges `PhysicalShiftedConnectingClusterCountBoundExp 1 1296`. The BFS/tree injection itself remains the open graph-theoretic construction; the downstream F3-count algebra is now closed. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
