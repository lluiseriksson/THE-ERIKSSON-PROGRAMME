...
| **Last closed** | **v1.77.0 `mono_count_dim` terminal canaries for the global F3 package** — F3 terminal wiring, no bar movement. Added `clayMassGap_of_shiftedF3MayerCountPackage_mono_count_dim`, `clayConnectedCorrDecay_of_shiftedF3MayerCountPackage_mono_count_dim`, `clay_theorem_of_shiftedF3MayerCountPackage_mono_count_dim`, plus mass/prefactor projection canaries in `YangMills/ClayCore/ClusterRpowBridge.lean`. After an F3 Mayer/count package is assembled, callers can raise only the count polynomial exponent by `k` and still reach the authentic mass-gap structure, connected-decay hub, or weak theorem endpoint directly. The terminal decay rate remains `kpParameter wab.r`; the prefactor exposes `dim + k`. The open mathematical target remains unchanged: the uniform lattice-animal / KP count estimate. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; all seven new declarations remain `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
