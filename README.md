...
| **Last closed** | **v1.26.0 physical d=4 terminal endpoints exposed** — terminal API sharpening, no bar movement. Added four L8 wrappers in `YangMills/L8_Terminal/ConnectedCorrDecayBundle.lean` fixing the final distance profile to `ConcretePlaquette physicalClayDimension L` with `physicalClayDimension = 4`: from `ClusterCorrelatorBound`, from `ShiftedF3MayerCountPackage`, from shifted F3 subpackages, and from `WilsonUniformRpowBound`. These route existing global analytic inputs directly into the non-vacuous `ClayYangMillsPhysicalStrong` target at physical spacetime dimension. This does not convert the new `PhysicalShiftedF3CountPackage` into an all-dimensions package; the four-dimensional count estimate remains an honest open F3 subtarget. Build `lake build YangMills.L8_Terminal.ConnectedCorrDecayBundle` green; all four new wrappers print `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
