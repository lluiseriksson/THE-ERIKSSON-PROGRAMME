...
| **Last closed** | **v1.67.0 card-bucket ClusterCorrelator bridge uses Mayer-facing KP consumer** — finite-volume F3 proof-spine cleanup, no bar movement and no public statement changes. Refactored `clusterCorrelatorBound_of_cardBucketBounds_ceil` in `YangMills/ClayCore/ClusterRpowBridge.lean` so its proof routes directly through `clusterCorrelatorBound_of_finiteConnectingBounds_ceil` plus the v1.66 Mayer-facing bucket-`tsum` consumer `finiteConnectingSum_le_of_cardBucketBounds_tsum`, instead of first converting to the connected finite-sum bridge. The closest public F3 bucket consumer is now aligned with the spine: Mayer identity, disconnected-support cancellation, finite connecting sum ≤ KP `tsum` via bucket bounds, then `ClusterCorrelatorBound`. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; the declaration remains `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
