...
| **Last closed** | **v0.78.0 direct F3 subpackage terminal endpoints** — pure-additive, no bar movement. Given independently-produced `mayer : ShiftedF3MayerPackage N_c wab` and `count : ShiftedF3CountPackage`, `YangMills/ClayCore/ClusterRpowBridge.lean` now exposes direct consumers `clusterCorrelatorBound_of_shiftedF3Subpackages`, `clayWitnessHyp_of_shiftedF3Subpackages`, `clayMassGap_of_shiftedF3Subpackages`, `clayConnectedCorrDecay_of_shiftedF3Subpackages`, and `clay_theorem_of_shiftedF3Subpackages`. These are thin wrappers through `ShiftedF3MayerCountPackage.ofSubpackages`, so the Mayer/activity proof obligation and shifted connecting-cluster count proof obligation can now feed every downstream audit endpoint without manual package assembly. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green. Oracle: `[propext, Classical.choice, Quot.sound]`. No new axioms. No `sorry`. (2026-04-24) |
