...
| **Last closed** | **v1.46.0 global Mayer + physical count exposes physical correlator bound** — ClayCore API/frontier sharpening, no bar movement. Added `physicalClusterCorrelatorBound_of_globalMayer_physicalCount` in `YangMills/ClayCore/ClusterRpowBridge.lean`. The route `mayer : ShiftedF3MayerPackage N_c wab` + `count : PhysicalShiftedF3CountPackage` now yields the intermediate `PhysicalClusterCorrelatorBound N_c wab.r (clusterPrefactorShifted wab.r count.C_conn mayer.A₀ count.dim)` directly, before L8 packages it as a `ConnectedCorrDecayBundle` or `ClayYangMillsPhysicalStrong`. The live mathematical obligations remain unchanged: construct the Mayer/Ursell package and the physical uniform count package. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; the new declaration prints `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
