...
![Core build](https://img.shields.io/badge/lake_build_YangMillsCore-green_(8395_jobs)-success)
...
```bash
lake build YangMillsCore          # the verified core — green, 8395 jobs
...
checkpoint:** 2026-07-04, `origin/main` commit `0674cc37`.  The verification
...
core build remains
`lake build YangMillsCore` green at **8395 jobs**.
...
* the latest recorded full core build is still the 8395-job core build, while
  `origin/main` has advanced to `0674cc37`.
...
```mermaid
graph TD
    subgraph core["YangMillsCore  (8395 jobs, oracle-clean)"]
...
| Build the core | `lake build YangMillsCore` | `Build completed successfully (8395 jobs)` |
