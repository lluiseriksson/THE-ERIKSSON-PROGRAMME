...
| **Last closed** | **v1.75.0 `toPhysicalOnly` count-application canaries for F3 packages** — F3 package ergonomics, no bar movement. Added `PhysicalShiftedF3MayerCountPackage.toPhysicalOnly_apply_count`, `PhysicalShiftedF3MayerCountPackage.toPhysicalOnly_mono_count_dim_apply_count`, `ShiftedF3MayerCountPackage.toPhysicalOnly_apply_count`, and `ShiftedF3MayerCountPackage.toPhysicalOnly_mono_count_dim_apply_count` in `YangMills/ClayCore/ClusterRpowBridge.lean`, making the Clay-physical restriction path directly usable without manual projection. After `toPhysicalOnly` (and after the commuted `mono_count_dim` restriction), the physical count estimate is available at the original count constant and expected exponent. The open mathematical target remains unchanged: the uniform lattice-animal / KP count estimate. Build `lake build YangMills.ClayCore.ClusterRpowBridge` green; all four new declarations remain `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
