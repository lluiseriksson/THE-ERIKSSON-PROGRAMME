...
| **Last closed** | **v1.89.0 site-neighbor finite-code interface** — F3-count infrastructure, no bar movement. Extended `YangMills/ClayCore/LatticeAnimalCount.lean` with `siteNeighborBall_card_le_of_injective_code` and `siteNeighborBallBoundDim_of_injective_code`: any volume-uniform injective code from `siteNeighborBall d L x` into a fixed finite type `α` with `Fintype.card α ≤ B` produces `SiteNeighborBallBoundDim d B`. This gives the concrete displacement-code witness a direct API into the local branching bound from v1.88; expected next code target is a finite displacement alphabet such as `{-1,0,1}^d` or a sharper axis-only code. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; new traces remain `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
