...
| **Last closed** | **v1.71.0 direct `ofAtFamily` application canaries for shifted F3 counts** — F3 count-frontier ergonomics, no bar movement. Added `ShiftedF3CountPackageDim.ofAtFamily_apply` and `PhysicalShiftedF3CountPackage.ofAtFamily_apply` in `YangMills/ClayCore/ConnectingClusterCount.lean`, exposing the finite-volume count estimate obtained from a volume-uniform family directly at the supplied `C_conn` and `dim`. This keeps the remaining mathematical target unchanged — the uniform lattice-animal / KP count estimate — while making the eventual handoff into `PhysicalShiftedF3CountPackage.ofAtFamily` immediate for downstream F3 scripts. Builds `lake build YangMills.ClayCore.ConnectingClusterCount` and `lake build YangMills.ClayCore.ClusterRpowBridge` green; both new declarations remain `[propext, Classical.choice, Quot.sound]`. Non-Experimental Lean axiom count remains 0. No `sorry`. (2026-04-25) |
