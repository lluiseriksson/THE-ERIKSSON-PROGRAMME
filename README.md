...
| **Last closed** | **v2.02.0 exact walk-range decoder obstruction** — F3-count audit infrastructure, no bar movement. Added `physicalConnectingClusterRangeDecoderCovers_forces_dist_ceiling_le_one` in `YangMills/ClayCore/LatticeAnimalCount.lean`. It proves that the exact range-decoder coverage target from v2.01 would force `⌈siteLatticeDist p.site q.site⌉₊ ≤ 1`: a length-`n` walk visits at most `n+1` plaquettes, while a shifted bucket has cardinality `n + ⌈siteLatticeDist p.site q.site⌉₊`. This is an honesty correction: `plaquetteWalkRangeFinset` remains useful local scaffolding, but exact equality with the range of a length-`n` walk is globally too strong. The viable F3-count target must be a genuine BFS/tree word decoder whose word length is `n` while the decoded cluster includes the deterministic distance baseline plus `n` extra plaquettes. Build `lake build YangMills.ClayCore.LatticeAnimalCount` green; trace remains `[propext, Classical.choice, Quot.sound]`. No `sorry`. (2026-04-25) |
