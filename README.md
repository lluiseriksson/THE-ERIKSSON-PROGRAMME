...
| **Last closed** | **v0.88.0 SUN Lieb-Robinson axioms converted to explicit inputs** — audit cleanup with two real axiom deletions, no Clay bar movement. Removed `sun_variance_decay` and `sun_lieb_robinson_bound` from `YangMills/P8_PhysicalGap/SUN_LiebRobin.lean`; the only consumer, `sun_locality_to_covariance`, now takes `hVar : HasVarianceDecay (sunMarkovSemigroup N_c)` and `hLR : LiebRobinsonBound (d := d) (sunMarkovSemigroup N_c)` explicitly before applying `locality_to_static_covariance_v2`. This does not prove those physical inputs; it stops declaring them as global axioms. Build `lake build YangMills.P8_PhysicalGap.SUN_LiebRobin` green. Pinned oracle for `sun_locality_to_covariance`: `[lieDerivReg_all, propext, sunGeneratorData, Classical.choice, Quot.sound, hille_yosida_semigroup, sunDirichletForm_contraction, Experimental.LieSUN.matExp_traceless_det_one]`. No `sun_variance_decay` / `sun_lieb_robinson_bound` axiom declarations remain. No `sorry`. (2026-04-24) |
